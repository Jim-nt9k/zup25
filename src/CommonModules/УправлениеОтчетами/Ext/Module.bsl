...
Функция СформироватьЗаголовокОсновнойФормы(ДатаНач, ДатаКон, НазваниеОтчета, РежимВводаПериода) Экспорт
	...
	Если ЗначениеЗаполнено(ОписаниеПериода) Тогда
...
Процедура ДобавитьВТекстСвойстваИКатегории(ТаблицаПолей, Текст, СтруктураПредставлениеПолей, мСоответствиеНазначений, 
	...
	Для Каждого Строка Из ТаблицаПолей Цикл
		...
		Если НЕ (Строка.НетКатегорий=Истина) Тогда
			...
			|	КОНЕЦ)"+?(Индекс = 1,".*","")+" КАК Категории"+Индекс+"Категория";
...
Процедура ДобавитьВТекстПостроителяИнтервальныеГруппировкиФизлиц(ТекстЗапроса, ТаблицаИнтервалов, СтруктураПараметры, СоответствиеТиповИнтервальныхПолей,СтруктураПредставлениеПолей,
	...
	// Начало ФИО
	ПолеФИО="ЕСТЬNULL(ФИОФизЛиц.Фамилия + "" "" + ФИОФизЛиц.Имя + "" "" + ФИОФизЛиц.Отчество," + ИмяПоляФизЛицо + ".Наименование)";
...
//	ТаблицаПолей.Колонки.Добавить("ТипЗначения");  // тип значения поля, для которого добавляются свойства и категории.
	...
	ТаблицаПолей.Индексы.Добавить("Назначение");
...
////////////////////////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ РАБОТЫ СО СХЕМОЙ КОМПОНОВКИ ДАННЫХ

Функция ПолучитьМассивПолейРасшифровки(Расшифровка, ДанныеРасшифровки, ТекущийОтчет = Неопределено) Экспорт
	...
	Если ТипЗнч(ДанныеРасшифровки.Элементы[Расшифровка]) = Тип("ЭлементРасшифровкиКомпоновкиДанныхПоля") 
		И ДанныеРасшифровки.Элементы[Расшифровка].ПолучитьРодителей() = 0 Тогда
		ПоляРасшифровки = ДанныеРасшифровки.Элементы[Расшифровка].ПолучитьПоля();
		Для каждого ПолеРасшифровки Из ПоляРасшифровки Цикл
			МассивПолейРасшифровки.Добавить(ПолеРасшифровки);
		КонецЦикла;
	КонецЕсли;
	
	Для каждого РодительПоля Из ДанныеРасшифровки.Элементы[Расшифровка].ПолучитьРодителей() Цикл
		Если ТипЗнч(РодительПоля) = Тип("ЭлементРасшифровкиКомпоновкиДанныхГруппировка") Тогда
			Прервать;
		КонецЕсли;
		ИД = РодительПоля.Идентификатор;
		ПоляРасшифровки = ДанныеРасшифровки.Элементы[ИД].ПолучитьПоля();
		Для каждого ПолеРасшифровки Из ПоляРасшифровки Цикл
			МассивПолейРасшифровки.Добавить(ПолеРасшифровки);
		КонецЦикла;
	КонецЦикла;
	...
	Для каждого ЭлементОтбора Из ТекущийОтчет.Настройки.Отбор.Элементы Цикл
		Если Не ЭлементОтбора.Использование Тогда
			Продолжить;
		КонецЕсли;
		СтруктураЭлементаОтбора = Новый Структура("Поле, Значение, Иерархия");
		СтруктураЭлементаОтбора.Поле = Строка(ЭлементОтбора.ЛевоеЗначение);
		СтруктураЭлементаОтбора.Значение = ЭлементОтбора.ПравоеЗначение;
		Если ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.ВИерархии 
		 ИЛИ ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии Тогда 
			СтруктураЭлементаОтбора.Иерархия = Истина;
		Иначе
			СтруктураЭлементаОтбора.Иерархия = Ложь;
		КонецЕсли;
		МассивПолейРасшифровки.Добавить(СтруктураЭлементаОтбора);
	КонецЦикла;
...
Функция CклонятьФИО(Компонента, Знач ФИО = "", Падеж = 1, Пол = 0, СообщениеОбОшибке = "", ПервоеСловоБольшимиБуквами = Ложь) Экспорт //Островская Татьяна 21.11.2013 № 16927
	Результат = "";
	Если ПросклонятьФИО(Компонента, ФИО, Падеж, Пол, Результат, СообщениеОбОшибке, ПервоеСловоБольшимиБуквами) Тогда
		Возврат Результат;
	Иначе
		Возврат ФИО;
	КонецЕсли;
...
Функция ПросклонятьФИО(Компонента, Знач ФИО = "", Падеж = 1, Пол = 0, Результат = "", СообщениеОбОшибке = "", ПервоеСловоБольшимиБуквами = Ложь) Экспорт  //Островская Татьяна 21.11.2013 № 16927
	Результат = ФИО;
	...
	Если СтрЧислоВхождений(ФИО, " ") > 3 Тогда
		...
		Возврат Ложь;
	КонецЕсли;
	Если ПервоеСловоБольшимиБуквами Тогда
		ФИО = СокрЛП(ФИО);
		ФИО = ВРег(Лев(ФИО, Найти(ФИО, " ") - 1)) + Сред(ФИО, Найти(ФИО, " "));
	КонецЕсли;
	Попытка
...
Функция ФИОНаписаноПоРусски(Знач СтрокаПараметр) //Островская Татьяна 21.11.2013 № 16927
	СтрокаПараметр = СокрЛП(СтрокаПараметр);
	СписокДопустимыхЗначений = Новый Соответствие;
	СписокДопустимыхЗначений.Вставить(КодСимвола(" ", 1), " ");
	СписокДопустимыхЗначений.Вставить(КодСимвола("-", 1), "-");
	СписокДопустимыхЗначений.Вставить(КодСимвола("ё", 1), "ё");
	СписокДопустимыхЗначений.Вставить(КодСимвола("Ё", 1), "Ё");
	КодА = КодСимвола("А", 1);
	КодЯ = КодСимвола("я", 1);
...
Функция Просклонять(Компонента, Знач Фраза = "", Падеж = 1, Результат, КоличествоСлов = 0, ПервоеСловоСМаленькойБуквы = Ложь, НеСклонятьАббревиатуры = Истина) Экспорт  //Островская Татьяна 21.11.2013 № 16927
	Результат = "";
	СоответствиеПадежей = Новый Соответствие;
	СоответствиеПадежей.Вставить(1, Перечисления.Падеж.Именительный);
	СоответствиеПадежей.Вставить(2, Перечисления.Падеж.Родительный);
	СоответствиеПадежей.Вставить(3, Перечисления.Падеж.Дательный);
	СоответствиеПадежей.Вставить(4, Перечисления.Падеж.Винительный);
	СоответствиеПадежей.Вставить(5, Перечисления.Падеж.Творительный);
	СоответствиеПадежей.Вставить(6, Перечисления.Падеж.Предложный);
	РезультатПолный = "";
	Если ТипЗнч(Фраза) = Тип("Массив") Тогда
		Массив = Фраза;
		Для каждого Фраза Из Массив Цикл
			Набор = РегистрыСведений.Склонение.Получить(Новый Структура("Слово, Падеж", Фраза, СоответствиеПадежей.Получить(Падеж)));
			Если ЗначениеЗаполнено(Набор.Значение) Тогда
				Результат = Набор.Значение;
				Если ПервоеСловоСМаленькойБуквы И Не (Результат = ВРег(Результат)) Тогда
					Результат = НРег(Лев(Результат, 1)) + Сред(Результат, 2);
				КонецЕсли;
				РезультатПолный = РезультатПолный + " " + Результат;
				Продолжить;
			КонецЕсли;
			Количество = 0;
			Фраза = СокрЛП(Фраза);
			...
			Для каждого СтрокаМассива Из МассивСтрок Цикл
				...
				Если ПервоеСловоСМаленькойБуквы И Количество = 1 И Не (СтрокаМассива = ВРег(СтрокаМассива)) Тогда
					СтрокаМассива = НРег(Лев(СтрокаМассива, 1)) + Сред(СтрокаМассива, 2);
				КонецЕсли;
				Если ПустаяСтрока(СтрокаМассива) Или Не ФИОНаписаноПоРусски(СтрокаМассива) Или НеСклонятьАббревиатуры И СтрокаМассива = ВРег(СтрокаМассива) Или КоличествоСлов И Количество > КоличествоСлов Тогда
	...
	Иначе
		...
		Запрос.УстановитьПараметр("Объект", Фраза);
		Запрос.УстановитьПараметр("Падеж", СоответствиеПадежей.Получить(Падеж));
		
		Набор = Запрос.Выполнить().Выгрузить();
		
		
		//Набор = РегистрыСведений.Склонение.Получить(Новый Структура("Объект, Падеж", Фраза, СоответствиеПадежей.Получить(Падеж)));
		Если Набор.Количество()>0 Тогда
			Результат = Набор[0].Значение;
			Если ПервоеСловоСМаленькойБуквы И Не (Результат = ВРег(Результат)) Тогда
				Результат = НРег(Лев(Результат, 1)) + Сред(Результат, 2);
			КонецЕсли;
			Возврат Истина;
		...
		Для каждого СтрокаМассива Из МассивСтрок Цикл
			...
			КонецПопытки;
		КонецЦикла;
		РезультатПолный = Результат;
	КонецЕсли;
	Результат = СокрЛП(РезультатПолный);
	Возврат Истина;
КонецФункции
