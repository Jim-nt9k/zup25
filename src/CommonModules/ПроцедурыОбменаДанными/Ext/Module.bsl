...
Процедура ПроверитьДоступностьРаботыСКаталогом(ИмяФайлаЧтенияИзменений, Знач НужноМенятьРасширениеНаZIP = Ложь)
	...
	Если НужноМенятьРасширениеНаZIP Тогда
		УниверсальныеМеханизмы.УстановитьРасширениеФайла(ИмяФайлаЧтенияИзменений, ".zip");
	КонецЕсли;
...
Процедура СформироватьПараметрыДляОбновленияКонфигурации(ИмяПользователяБД, ИмяПрограммыЗапуска, ПутьКБД, ФайловаяБД)
	...
	ИмяПрограммыЗапуска = УниверсальныеМеханизмы.ПолучитьИмяФайла(КаталогПрограммы(), "1cv8.exe");
...
Функция СформироватьЧастьВызоваОбновленияКонфигурации(Знач ИмяАдминистратораИБ = "", Знач ИмяФайлаИнформацииОбОшибках = "", 
	...
	ТекстФайлаПоУмолчанию = "
		...
		|start """" /wait %v8exe% ENTERPRISE %DataBase% %AdminUser% /CЗавершитьРаботуПользователей	" + СтрокаДляВыводаДанныхВФайл + " /DisableStartupMessages
        |
		|start """" /wait %v8exe% CONFIG     %DataBase% %AdminUser% /UpdateDBCfg	" + СтрокаДляВыводаДанныхВФайл + "
        |
		|rem Разрешение работы пользователей
		|start """" /wait %v8exe% ENTERPRISE %DataBase% %AdminUser% /CРазрешитьРаботуПользователей	" + СтрокаДляВыводаДанныхВФайл + " /DisableStartupMessages
        |
...
Процедура ПредложитьПерезагрузкуПрограммы(СтруктураПараметров = Неопределено, 
	...
	ИмяБАТФайла = УниверсальныеМеханизмы.ПолучитьИмяФайла(КаталогВременныхФайлов(), "Upd1C.bat");
...
Функция ПолучитьПутьККаталогуВременныхФайловОбмена(Знач НаименованиеНастройкиОбмена, УзелОбмена, Знач ПроверятьНаличиеКаталога = Ложь)
	...
	ИмяПланаОбмена = УниверсальныеМеханизмы.УдалитьЗапрещенныеСимволыИмени(ИмяПланаОбмена);
	
	ПутьКФайлуОбмена = УниверсальныеМеханизмы.ПолучитьИмяФайла(КаталогВременныхФайлов(), ИмяПланаОбмена);
	
	СтрокаКаталогаОбмена = УниверсальныеМеханизмы.УдалитьЗапрещенныеСимволыИмени(НаименованиеНастройкиОбмена);
	ПутьКФайлуОбмена = УниверсальныеМеханизмы.ПолучитьИмяФайла(ПутьКФайлуОбмена, СтрокаКаталогаОбмена);
...
Функция ВернутьПолныйПутьКФайлуОбменаМеждуУзлами(Знач ИмяКаталогаОбмена = "", Знач НаименованиеНастройкиОбмена, УзелОтправитель, УзелПолучатель, 
	...
	ИмяФайла = УниверсальныеМеханизмы.ПолучитьИмяФайла(ПутьКФайлуОбмена, СформироватьИмяФайлаОбменаМеждуУзлами(УзелОтправитель, УзелПолучатель, ".xml"));
...
Функция ВерутьИмяВременногоФайлаЧтенияДанных(СтруктураНастроекОбменаДанными, Знач ИзменитьРасширениеФайлаНаZip)
	...
	// для файла чтения данных важно, чтобы существовал каталог, откуда будем читать данные
...
Функция ВыбратьФайлДляОбмена(Знач НачальныйКаталог, Знач ИмяФайлаПоУмолчанию = "") Экспорт
	...
	ДиалогВыбора = УниверсальныеМеханизмы.ПолучитьДиалогВыбораФайлов(Ложь, НачальныйКаталог);
...
Функция ВернутьВыбранныйПользователемКаталогДляОбмена(КаталогОбмена = "")
	...
	РезультатВыбора = УниверсальныеМеханизмы.ВыбратьКаталог(КаталогОбмена, "Выбор каталога обмена данными");
...
Функция РазархивироватьФайл(СтруктураНастроекОбменаДанными) 
	...
	Попытка
		...
		Если Архиватор.Элементы.Количество() > 0 Тогда
			...
			ИмяРазархивированногоФайла = УниверсальныеМеханизмы.ПолучитьИмяФайла(КаталогДляРаспаковки, Архиватор.Элементы[0].Имя);
...
Процедура ВыполнитьЗагрузкуДанныхИзФайлаОбмена(СтруктураНастроекОбменаДанными)
	...
	РасширениеФайлаОбмена = ОбщегоНазначения.ПолучитьРасширениеФайла(СтруктураНастроекОбменаДанными.ИмяФайлаЧтенияИзменений);
...
Процедура ПодготовитьОбменЧерезFTPРесурс(СтруктураНастроекОбменаДанными)
	...
	Если СтруктураНастроекОбменаДанными.ПроизводитьЧтениеДанных Тогда
		...
		Для Каждого ФайлФТП Из МассивНайденныхФайлов Цикл
			...
			// проверим чтобы размер файла не превосходил ограничения по размеру
...
Процедура ПодготовитьОбменЧерезПочтовыйРесурс(СтруктураНастроекОбменаДанными)
	...
	СписокДоступныхЗаписей = УправлениеЭлектроннойПочтой.ПолучитьДоступныеУчетныеЗаписи(ПараметрыСеанса.ТекущийПользователь).Запись;
	...
	Если СтруктураНастроекОбменаДанными.ПроизводитьЧтениеДанных Тогда
		...
		Если СтруктураНастроекОбменаДанными.РучнойРежимЗапуска 
			...
			#Если Клиент Тогда
				...
				Попытка
					...
					УправлениеЭлектроннойПочтой.ПолучениеОтправкаПисем(СтруктураНастроекОбменаДанными.СоответствиеТекстовЭлектронныхПисем, ПараметрыСеанса.ТекущийПользователь, 
						МассивУчетныхЗаписей, , Ложь, Истина, Ложь, ТекстОшибок);
		...
		Если ФайлЗаписанНаДиск Тогда
			...
			Попытка
				...
				СтруктураНастроекОбменаДанными.ИмяФайлаЧтенияИзменений = ВерутьИмяВременногоФайлаЧтенияДанных(СтруктураНастроекОбменаДанными,
					(Врег(ОбщегоНазначения.ПолучитьРасширениеФайла(ИмяФайлаОбмена)) = "ZIP"));
...
Функция ОпределитьНужноОптравлятьОтветныйПакет(СтруктураНастроекОбменаДанными, НужноПроверитьЧерезПочту)
	...
	// нужно предварительно проанализировать на предмет того, чтобы размер отправляемого письма не превосходил ограничений
...
Процедура ДействияПослеОбменаЧерезФайловыйРесурс(СтруктураНастроекОбменаДанными)
	...
	Если ОпределитьБылаУдачнноПроизведенаЗаписьДанных(СтруктураНастроекОбменаДанными) Тогда
		...
		ИмяФайла = ОбщегоНазначения.ПолучитьИмяФайлаИзПолногоПути(СтруктураНастроекОбменаДанными.ИмяФайлаЗаписиИзменений);
...
Процедура ДействияПослеОбменаЧерезFTPРесурс(СтруктураНастроекОбменаДанными)
	...
	Если ОпределитьБылаУдачнноПроизведенаЗаписьДанных(СтруктураНастроекОбменаДанными) Тогда
		...
		Если НужноЗаписатьФайлНаFTP Тогда
			...
			ИмяФайлаFTPЗаписи = ИмяКаталогаСервера + ОбщегоНазначения.ПолучитьИмяФайлаИзПолногоПути(СтруктураНастроекОбменаДанными.ИмяФайлаЗаписиИзменений);
		...
		Если НужноОтправлятьОтветноеПисьмо Тогда
			...
			#Если Клиент Тогда
				...
				Если СтруктураПисем <> Неопределено Тогда				   
					...
					ТекстОшибок = "";
					УправлениеЭлектроннойПочтой.ОтправитьПисьма(СоответствиеПисем, СтруктураНастроекОбменаДанными.СоответствиеТекстовЭлектронныхПисем, ПараметрыСеанса.ТекущийПользователь, Ложь, ТекстОшибок);
...
Процедура ВыполнитьОбменДаннымиДляНастройкиОбмена(КодНастройки) Экспорт
	...
	ПроизвестиСписокОбменовДанными(МассивНастроекОбмена);
	...
	#Если Клиент Тогда
		...
		Если РучнойРежимЗапуска Тогда 		
			...
			Если КоличествоПроизведенныхОбменов > 0 Тогда
				Предупреждение("Обмен данными завершен.", 30, "Обмен данными");
			Иначе
