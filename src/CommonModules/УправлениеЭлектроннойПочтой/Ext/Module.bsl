...
Функция ПолучитьИнтернетПочтовыйПрофиль(УчетнаяЗапись) Экспорт
	...
	//О.Т. Аверсон 17.10.2018 № 
	Если УчетнаяЗапись.ИспользоватьЗащищенноеСоединениеДляИсходящейПочты Тогда 
		Профиль.ИспользоватьSSLSMTP = 1;
	КонецЕсли;
	Если УчетнаяЗапись.ИспользоватьЗащищенноеСоединениеДляВходящейПочты Тогда
		Профиль.ИспользоватьSSLPOP3 = 1;
	КонецЕсли;
...
Функция ПолучитьДоступныеУчетныеЗаписи(Пользователь) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Справочники.УчетныеЗаписиЭлектроннойПочты) Тогда
		...
		Если НЕ ЗначениеЗаполнено(Пользователь) ИЛИ Пользователь = ПараметрыСеанса.ТекущийПользователь Тогда
			РольПолныеПрава = РольДоступна("ПолныеПрава") или РольДоступна("ПолныеПраваБезЭкономики"); //О.Т. Аверсон 17.03.2017 № 335
		Иначе
