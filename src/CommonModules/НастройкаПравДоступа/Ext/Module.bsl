...
Функция ПолучитьВидОбъектаДоступа(ОбъектДоступа) Экспорт
	...
	ИначеЕсли ТипЗнч(ОбъектДоступа) = Тип("СправочникСсылка.ГрафикиРаботы") Тогда //О.Т. 27.08.2014 № 
		Возврат Перечисления.ВидыОбъектовДоступа.ГрафикиРаботы;
	ИначеЕсли ТипЗнч(ОбъектДоступа) = Тип("СправочникСсылка.Пользователи") Тогда // О.Т. 04.12.2014  
		Возврат Перечисления.ВидыОбъектовДоступа.Пользователи;
	Иначе
...
Процедура ПроверкаПериодаДокумента(ДокументОбъект, Отказ, РежимЗаписи) Экспорт
	//ПСА 03.12.2021 СВФР-008424 +++
	ОбновитьПараметрГраницыЗапретаИзмененияДанных();
	//+++ПСА 03.12.2021 СВФР-008424
...
Процедура ПроверитьВерсиюДокумента(ДокументОбъект, ПараметрыПроверкиДокумента, СоответствиеГраницЗапрета, Отказ, РежимЗаписи = Неопределено) Экспорт
	...
	//Аверсон 26.07.2021 КВВ СВФР-007889 (
	Попытка
		ПоДок = ДокументОбъект.Документ;
		ТипДок = 1;
	Исключение
		ПоДок = ДокументОбъект;
		ТипДок = 2;
	КонецПопытки;
	
	Если ТипЗнч(ПоДок) = ?(ТипДок = 1,Тип("ДокументСсылка.ВводСведенийОПлановыхНачисленияхРаботниковОрганизаций"),Тип("ДокументОбъект.ВводСведенийОПлановыхНачисленияхРаботниковОрганизаций")) Тогда
		ЕстьПособиеДоТрех = Ложь;
		Для Каждого СтрТЧ ИЗ ПоДок.ОсновныеНачисления Цикл
			Если СтрТЧ.ВидРасчета <> ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ПособиеНаРебенкаДо3Лет Тогда
				Продолжить;
			КонецЕсли;
			ЕстьПособиеДоТрех = Истина;
		КонецЦикла;
		Если ЕстьПособиеДоТрех Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	//Аверсон 26.07.2021 КВВ СВФР-007889 )
...
//толик граница запрета пользователь
		мГраница = РегистрыСведений.ГраницыЗапретаИзмененияДанных.Получить(Новый Структура("Организация, Пользователь", Неопределено, ПараметрыСеанса.ТекущийПользователь));
		Если мГраница.Количество() <> 0 Тогда
			Если ГраницаПоОрганизации <> Неопределено Тогда
				Если мГраница.ГраницаЗапретаИзменений > ГраницаПоОрганизации Тогда
					ГраницаПоОрганизации = мГраница.ГраницаЗапретаИзменений;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
//енд
...
//толик
		Если НЕ (ГраницаПоОрганизации = Неопределено) Тогда
			Если ДокументОбъект.Дата <= КонецДня(ГраницаПоОрганизации)	Тогда
				Отказ = Истина;			
			КонецЕсли;
			Если НЕ ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЗарплатаКВыплатеОрганизаций") Тогда //Аверсон 19.07.2021 КВВ СВФР-007861
				Попытка
					Если ДокументОбъект.ПериодРегистрации <= КонецДня(ГраницаПоОрганизации)	Тогда
						Отказ = Истина;			
					КонецЕсли;
				Исключение
				КонецПопытки;
			КонецЕсли;
			//проверка по ременным			
			Попытка
				Если ДокументОбъект.Документ.РаботникиОрганизации[0].Временно Тогда
					Отказ = Ложь;			
				КонецЕсли;
			Исключение
			КонецПопытки;
			Попытка
				Если ДокументОбъект.РаботникиОрганизации[0].Временно Тогда
					Отказ = Ложь;			
				КонецЕсли;
			Исключение
			КонецПопытки;
...
//енд
	...
	Если ПараметрыПроверкиДокумента.ПроверятьУправленческуюДатуЗапрета Тогда        
		...
		Если ГраницаПериода <> Неопределено Тогда
			...
			Если ДокументОбъект.Дата <= КонецДня(ГраницаПериода) Тогда
				Отказ = Истина;				
			КонецЕсли;         			
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьПараметрыПроверкиДокумента( ДокументОбъект ) Экспорт
	...
	//+ СВЭЙ 14.03.10 КДЯ СВФР-005125
	Если ПроверятьУправленческуюДатуЗапрета 
		 И ПараметрыПроверкиДокумента.ЕстьОрганизация 
		 И ТипЗнч( ДокументОбъект ) = Тип( "ДокументОбъект.ДоговорНаВыполнениеРаботСФизЛицом" ) 
		 И СокрЛП( ДокументОбъект.Организация.ИНН ) = "806000046" Тогда
			ПроверятьУправленческуюДатуЗапрета      = Ложь;
	КонецЕсли; 
	Если ПроверятьРегламентированнуюДатуЗапрета 
		 И ТипЗнч( ДокументОбъект ) = Тип( "ДокументОбъект.ДоговорНаВыполнениеРаботСФизЛицом" ) 
		 И СокрЛП( ДокументОбъект.Организация.ИНН ) = "806000046" Тогда
...
// Обновляет параметр сеанса ГраницыЗапретаИзмененияДанных
//  в зависимости от ряда условий: первый раз в течение сеанса - сразу,
//  затем с интервалом = ИнтервалОбновления (в 5 минут), если регистр сведений 
//  "ГраницыЗапретаИзмененияДанных" был отредактирован одним из пользователей.
Процедура ОбновитьПараметрГраницыЗапретаИзмененияДанных(ИнтервалОбновления = 12) Экспорт
	Перем ДатаОбновленияДЗР, ТекДата, ДатаРедактированияДЗР;
	
	УстановитьПривилегированныйРежим( Истина );
	ТекДата = ТекущаяДата();
	ДатаОбновленияДЗР = ПараметрыСеанса.ДатаОбновленияДЗР;
	ПрошлоСМоментаПоследнегоОбновления = ТекДата - ДатаОбновленияДЗР;
	Если ПрошлоСМоментаПоследнегоОбновления <= ИнтервалОбновления Тогда
		Возврат;	
	КонецЕсли; 
	ПараметрыСеанса.ДатаОбновленияДЗР = ТекДата;
	
	//прочитать дату редактирования ДЗР
	ДатаРедактированияДЗР = Константы.АВ_ДатаРедактированияДЗР.Получить();
	//сравнить дату редактирования ДЗР с параметром дата редактирования ДЗР
	Если ПараметрыСеанса.ДатаРедактированияДЗР = ДатаРедактированияДЗР Тогда
		Возврат;
	КонецЕсли;
	
	ПолныеПрава.УстановитьПараметрГраницыЗапретаИзмененияДанных();

	ПараметрыСеанса.ДатаРедактированияДЗР = ДатаРедактированияДЗР;
	УстановитьПривилегированныйРежим( Ложь );
	
КонецПроцедуры // ОбновитьПараметрГраницыЗапретаИзмененияДанных()
//- СВЭЙ КДЯ 04.03.20 СВФР-005274
...
Процедура ПередЗаписьюДокументаПроверкаДоступностиПериода(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	//Аверсон 14.07.2022 КВВ СВФР-009298 (пункт 7) ( 
	Если Отказ И ТипЗнч(Источник) = Тип("ДокументОбъект.УстановкаКоличестваДнейОтпуска") И Источник.Организация.ИНН = "806000046" Тогда
		Отказ = Ложь;
	КонецЕсли;
	//Аверсон 14.07.2022 КВВ СВФР-009298 (пункт 7) )
