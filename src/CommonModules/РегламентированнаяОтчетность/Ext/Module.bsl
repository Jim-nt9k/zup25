...
Функция СтрокаНаписанаРусскимиИлиЛатинскими(Знач СтрокаПараметр) Экспорт
	...
	Если  КоличествоСимволов > 0 Тогда
		ПервыйСимвол = КодСимвола(СтрокаПараметр); 
	...
	Для Сч = 1 По КоличествоСимволов Цикл
		...
		Если (Русские = 1) Тогда
			Если Код < 192 И Код <> 184 И Код <> 168 И Код <> 45  Тогда
				Возврат 0;
			КонецЕсли;
...
Функция ПолучитьСписокПраздниковРФ(КалендарныйГод) Экспорт
	...
	//	СписокПраздников.Добавить("1107", "День согласия и примерения");
	...
	СписокПраздников.Добавить("0101", "Новый год");
	СписокПраздников.Добавить("0102", "Новый год");
	СписокПраздников.Добавить("0107", "Рождество православное");
	СписокПраздников.Добавить("0308", "Международный женский день");
	СписокПраздников.Добавить("0501", "Праздник Весны и Труда");
	СписокПраздников.Добавить("0509", "День Победы");
	СписокПраздников.Добавить("0703", "День независимости");
	СписокПраздников.Добавить("1107", "День народного единства");
	СписокПраздников.Добавить("1225", "Рождество католическое");
...
Функция ИДКонфигурации() Экспорт
	...
	Если МетаданныеИмя = "БухгалтерияПредприятия" ИЛИ МетаданныеИмя = "БухгалтерияПредприятияБазовая" Тогда
		Возврат "БП";
	ИначеЕсли МетаданныеИмя = "ЗарплатаИУправлениеПерсоналом" ИЛИ МетаданныеИмя = "ЗарплатаИУправлениеПерсоналомБазовая" Тогда
		Возврат "ЗУП";
...
Функция ПолучитьСУчетомКэша(Кэш, Параметр, ФизЛицо = Неопределено)
	...
	ИначеЕсли Параметр = "ЮрАдресОрганизации" Тогда
		Данные = Новый Структура("Объект, Тип, Вид", Кэш.Организация.Ссылка, Перечисления.ТипыКонтактнойИнформации.Адрес, Справочники.ВидыКонтактнойИнформации.ЮрАдресОрганизации);
	...
	ИначеЕсли Параметр = "ТелефонОрганизации" Тогда
		Если Кэш.ПБОЮЛ И ((ИДКонфигурации() = "БП" И РедакцияКонфигурации() >= "1.5") ИЛИ (ИДКонфигурации() = "ЗУП" И РедакцияКонфигурации() >= "2.1") ИЛИ (ИДКонфигурации() = "УПП" И РедакцияКонфигурации() >= "1.2")) Тогда
			Данные = Новый Структура("Объект, Тип, Вид", Кэш.Организация.ИндивидуальныйПредприниматель, Перечисления.ТипыКонтактнойИнформации.Телефон, ?(СвойствоОпределено(Справочники.ВидыКонтактнойИнформации, "ТелефонФизЛицаДомашний"), Справочники.ВидыКонтактнойИнформации.ТелефонФизЛицаДомашний, Справочники.ВидыКонтактнойИнформации.ТелефонФизЛица));
			Результат = РегистрыСведений.КонтактнаяИнформация.Получить(Данные);
		Иначе
			Данные = Новый Структура("Объект, Тип, Вид", Кэш.Организация.Ссылка, Перечисления.ТипыКонтактнойИнформации.Телефон, Справочники.ВидыКонтактнойИнформации.ТелефонОрганизации);
			Результат = РегистрыСведений.КонтактнаяИнформация.Получить(Данные);
		КонецЕсли;
...
Функция ПолучитьСведенияОбОрганизации(Знач Организация, Знач ДатаЗначения = Неопределено, Знач СписокПоказателей = Неопределено) Экспорт
	...
	Для Каждого Показатель Из СписокПоказателей Цикл
		...
		ИначеЕсли ИмяПоказателя = "СтраховойТариф" Тогда
			...
			//Значение = Организация.СтраховойТариф;
			Результат = РегистрыСведений.СтавкаВзносаНаСтрахованиеОтНесчастныхСлучаев.СрезПоследних(ДатаЗначения, Новый Структура("Организация,ВидНалогообложения", Организация.Ссылка, Организация.ОсновнаяСтавкаНалогообложения));//О.Т. Аверсон 30.03.2016 № 950
		...
		ИначеЕсли ИмяПоказателя = "АдрПочт" Тогда
			...
			Данные = Новый Структура("Объект, Тип, Вид",Организация.Ссылка, Перечисления.ТипыКонтактнойИнформации.Адрес, Справочники.ВидыКонтактнойИнформации.АдресОрганизацииПочтовый);
				...
				Если Метаданные.Справочники.Организации.Реквизиты.Найти("ИндивидуальныйПредприниматель") <> Неопределено Тогда
					...
					Иначе
						...
						Если ДанныеФЛ.Количество() > 0 Тогда
							Значение = СокрЛП(ДанныеФЛ[0].Фамилия) + " " + СокрЛП(ДанныеФЛ[0].Имя) + " " + СокрЛП(ДанныеФЛ[0].Отчество);
						Иначе
							Значение = "";
						КонецЕсли;
					КонецЕсли;
		...
		ИначеЕсли ИмяПоказателя = "ГраждРук" Тогда
			...
			Если Результат <> Неопределено И Результат.Количество() > 0 Тогда
				Если Результат[0].ФизическоеЛицо = Справочники.ФизическиеЛица.ПустаяСсылка() Тогда
					Значение = "";
				Иначе
					Гражданство = ПолучитьСУчетомКэша(Кэш, "СведенияОРуководителеГражданство", Результат[0].ФизическоеЛицо);
					//Гражданство = ПолучитьГражданство(Результат[0].ФизическоеЛицо, ДатаЗначения);
					Если Гражданство = Неопределено Тогда
						Значение = "";
					Иначе
		...
		ИначеЕсли ИмяПоказателя = "ГраждБух" Тогда
			...
			Если Результат <> Неопределено И Результат.Количество() > 0 Тогда
				Если Результат[0].ФизическоеЛицо = Справочники.ФизическиеЛица.ПустаяСсылка() Тогда
					Значение = "";
				Иначе
					Гражданство = ПолучитьСУчетомКэша(Кэш, "СведенияОБухгалтереГражданство", Результат[0].ФизическоеЛицо);
					Если Гражданство = Неопределено Тогда
						Значение = "";
					Иначе
						Значение = Гражданство.Код;
		...
		ИначеЕсли ИмяПоказателя = "ДатаПереходаНаУСН" Тогда
			...
			Если Результат.Количество() > 0 Тогда
				Значение = Результат[0].ДатаПереходаНаУСН;
			Иначе
				Значение = ПустаяДата;
			КонецЕсли;
		...
		ИначеЕсли ИмяПоказателя = "ОбъектНалогообложенияУСН" Тогда
			...
			Если Результат.Количество() > 0 Тогда
				...
				ПромежуточноеЗначение = Результат[0].ОбъектНалогообложенияУСН;
...
Функция ИННСоответствуетТребованиям(Знач ИНН, ВладелецИНН) Экспорт
	...
	Если НЕ (ОбщегоНазначения.ТолькоЦифрыВСтроке(ИНН)) и ВладелецИНН = Перечисления.ЮрФизЛицо.ЮрЛицо  Тогда
		Возврат Ложь;
	ИначеЕсли ОбщегоНазначения.ТолькоЦифрыВСтроке(ИНН) и ВладелецИНН = Перечисления.ЮрФизЛицо.ФизЛицо  Тогда 
		Возврат Ложь;
	КонецЕсли;
...
Функция ОтветственныеЛицаОрганизаций(Организация, ДатаСреза, Исполнитель = Неопределено) Экспорт
	...
	Результат = Новый Структура("Руководитель, РуководительДолжность,СпециалистКадровойСлужбы,СпециалистКадровойСлужбыДолжность,НачальникУправленияЭкономики,НачальникУправленияЭкономикиДолжность,ГлавныйЮристконсульт,ГлавныйЮристконсультДолжность, ГлавныйБухгалтер,ГлавныйБухгалтерДолжность, Кассир, Исполнитель, ИсполнительДолжность, ОтветственныйЗаРегистры,ЗаместительДиректораПоУправлениюПерсоналом,ЗаместительДиректораПоУправлениюПерсоналомДолжность,РуководительКадровойСлужбы,РуководительКадровойСлужбыДолжность,ПредседательПрофсоюзногоКомитета,ПредседательПрофсоюзногоКомитетаДолжность"); //Аверсон 09.10.2019 КВВ СВФР-004868
	...
	Если Организация <> Неопределено тогда
		...
		Если ИДКонфигурации() = "ЗУП" Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				//О.Т. 08.04.2015 № 412
				ИначеЕсли Выборка.ОтветственноеЛицо = Перечисления.ОтветственныеЛицаОрганизаций.СпециалистКадровойСлужбы Тогда
					Результат.СпециалистКадровойСлужбы            = Выборка.ФИОПолное;
					Результат.СпециалистКадровойСлужбыДолжность   = Выборка.Должность;
				ИначеЕсли Выборка.ОтветственноеЛицо = Перечисления.ОтветственныеЛицаОрганизаций.НачальникУправленияЭкономики Тогда
					Результат.НачальникУправленияЭкономики            = Выборка.ФИОПолное;
					Результат.НачальникУправленияЭкономикиДолжность   = Выборка.Должность;
				ИначеЕсли Выборка.ОтветственноеЛицо = Перечисления.ОтветственныеЛицаОрганизаций.ГлавныйЮристконсульт Тогда
					Результат.ГлавныйЮристконсульт            = Выборка.ФИОПолное;
					Результат.ГлавныйЮристконсультДолжность   = Выборка.Должность;
				ИначеЕсли Выборка.ОтветственноеЛицо = Перечисления.ОтветственныеЛицаОрганизаций.ЗаместительДиректораПоУправлениюПерсоналом Тогда
					Результат.ЗаместительДиректораПоУправлениюПерсоналом            = Выборка.ФИОПолное;
					Результат.ЗаместительДиректораПоУправлениюПерсоналомДолжность   = Выборка.Должность;
				//О.Т. 08.04.2015 №	412	
				ИначеЕсли Выборка.ОтветственноеЛицо = Перечисления.ОтветственныеЛицаОрганизаций.РуководительКадровойСлужбы Тогда
				...
				ИначеЕсли Выборка.ОтветственноеЛицо = Перечисления.ОтветственныеЛицаОрганизаций.ОтветственныйЗаНалоговыеРегистры Тогда
					...
					//Аверсон 09.10.2019 КВВ СВФР-004868 (
				ИначеЕсли Выборка.ОтветственноеЛицо = Перечисления.ОтветственныеЛицаОрганизаций.ПредседательПрофсоюзногоКомитета Тогда
					Результат.ПредседательПрофсоюзногоКомитета            = Выборка.ФИОПолное;
					Результат.ПредседательПрофсоюзногоКомитетаДолжность   = Выборка.Должность;
					//Аверсон 09.10.2019 КВВ СВФР-004868 )
				КонецЕсли;
		...
		Иначе
			...
			Если (ИДКонфигурации() = "БП" И РедакцияКонфигурации() > "1.0") ИЛИ (ИДКонфигурации() = "УПП" И РедакцияКонфигурации() > "1.1") Тогда
				ЗапросПоЛицам = Новый Запрос();
				ЗапросПоЛицам.УстановитьПараметр("Организация", Организация);
				ЗапросПоЛицам.УстановитьПараметр("ДатаСреза",   ДатаСреза);
			...
			Иначе
				...
				|	ОрганизацииОтветственныеЛица.Сотрудник как ФИОПолное,
				...
				ЗапросПоЛицам.УстановитьПараметр("Ссылка", Организация);
			...
			Пока Выборка.Следующий() Цикл
				...
				ИначеЕсли Выборка.ОтветственноеЛицо = Перечисления.ОтветственныеЛицаОрганизаций.ГлавныйБухгалтер Тогда
					Результат.ГлавныйБухгалтер        = Выборка.ФИОПолное;
					Результат.ГлавныйБухгалтерДолжность  = Выборка.Должность; //Островская Татьяна 16.05.2013
				ИначеЕсли Выборка.ОтветственноеЛицо = Перечисления.ОтветственныеЛицаОрганизаций.Кассир Тогда
					Результат.Кассир                  = Выборка.ФИОПолное;
				//О.Т. 08.04.2015 № 412
				ИначеЕсли Выборка.ОтветственноеЛицо = Перечисления.ОтветственныеЛицаОрганизаций.СпециалистКадровойСлужбы Тогда
			...
			Если Исполнитель <> Неопределено тогда
				...
				|	ВЫБОР КОГДА (ФИОФизЛицСрезПоследних.ФизЛицо) ЕСТЬ NULL  ТОГДА РаботникиОрганизацииСрезПоследних.Сотрудник.Физлицо.Наименование ИНАЧЕ ФИОФизЛицСрезПоследних.Фамилия + ВЫБОР КОГДА ПОДСТРОКА(ФИОФизЛицСрезПоследних.Имя, 1, 1) <> """" ТОГДА "" "" + ПОДСТРОКА(ФИОФизЛицСрезПоследних.Имя, 1, 1) + ""."" ИНАЧЕ """" КОНЕЦ + ВЫБОР КОГДА ПОДСТРОКА(ФИОФизЛицСрезПоследних.Отчество, 1, 1) <> """" ТОГДА "" "" + ПОДСТРОКА(ФИОФизЛицСрезПоследних.Отчество, 1, 1) + ""."" ИНАЧЕ """" КОНЕЦ КОНЕЦ КАК ФИОПолное
				|ИЗ
				|	РегистрСведений.РаботникиОрганизаций.СрезПоследних(&ДатаСреза, Организация = &ГоловнаяОрганизация И Сотрудник.ФизЛицо = &Исполнитель И (Сотрудник.ВидЗанятости <> &ВнутреннееСовместительство ИЛИ (Сотрудник.ВидЗанятости) ЕСТЬ NULL )) КАК РаботникиОрганизацииСрезПоследних
				|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизЛиц.СрезПоследних(&ДатаСреза, ФизЛицо = &Исполнитель) КАК ФИОФизЛицСрезПоследних
				|		ПО РаботникиОрганизацииСрезПоследних.Сотрудник.Физлицо = ФИОФизЛицСрезПоследних.ФизЛицо
				|
				|ГДЕ
...
Функция ПолучитьТекстФайлаИзРегистра(ДокументСсылка) Экспорт //Островская Татьяна 20.01.2014 № 17051
...
Функция ПредставлениеМестаРождения(Знач СтрокаМестоРождения, Знач ФлагПроизвольногоАдреса) Экспорт
	
	СтруктураМестоРождения = РазложитьМестоРождения(СтрокаМестоРождения, Ложь);
	
	Если ФлагПроизвольногоАдреса = 0 Тогда
		...
		КонецЕсли; 
	Иначе 
		Представление	=	СтрокаМестоРождения;
	КонецЕсли;	
...
Процедура ПоказатьФайлДокумента(ДокументОбъект, ЗаголовокОкна) Экспорт
	...
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументСсылка.АнкетаЗастрахованногоЛица") или ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.АнкетаЗастрахованногоЛица") Тогда //О.Т. Аверсон 21.04.2016 № 868
		ПутьКФайлу = РегистрыСведений.УчетнаяПолитикаПоПерсоналуОрганизаций.Получить(Новый Структура("Организация",ДокументОбъект.Организация)).ПутьДляВыгрузкиПУ12;
		если не ЗначениеЗаполнено(ПутьКФайлу) Тогда 
			ТекстовыйДокумент = Новый ТекстовыйДокумент;
			ТекстовыйДокумент.УстановитьТекст(ТекстФайла); 
			ТекстовыйДокумент.Показать(ЗаголовокОкна, Прав(ДокументОбъект.Номер,5));
		КонецЕсли;
	иначе
		ТекстовыйДокумент = Новый ТекстовыйДокумент;
		ТекстовыйДокумент.УстановитьТекст(ТекстФайла); 
...
Функция ПолучитьСчетчикВыгруженныхДокументов(Организация, ДатаДокумента = Неопределено) Экспорт
	
	Если Организация = Неопределено Тогда
		Возврат 0;
	КонецЕсли;
	
	Отбор = Новый Структура("Тип, Организация", Перечисления.ТипыСчетчиковВыгрузки.СчетчикДокументовВФормате300, Организация);
	РезД = РегистрыСведений.СчетчикиВыгрузок.СрезПоследних(КонецГода(?(ДатаДокумента = Неопределено, РабочаяДата, ДатаДокумента)), Отбор);
	Для Каждого Срез Из РезД Цикл
		Если Год(Срез.Период) = Год(?(ДатаДокумента = Неопределено, РабочаяДата, ДатаДокумента)) Тогда
			Возврат Срез.Значение + 1;
		КонецЕсли;
	КонецЦикла;
	Возврат 1;
...
Процедура УстановитьСчетчикВыгруженныхДокументов(Организация, СчДок, ДатаДокумента = Неопределено) Экспорт
	МенЗап = РегистрыСведений.СчетчикиВыгрузок.СоздатьМенеджерЗаписи();
...
Функция ВывестиСведенияОбОтправителе30(Текст, Организация = Неопределено, Знач ОргНазв = Неопределено, Знач ОргТелефон = Неопределено, Знач ОргБухгалтер = Неопределено, ВыгрузкаДекларацииПоАлкоголю = Ложь) Экспорт
	...
	Если КатегорияНП = 1 ИЛИ ВыгрузкаДекларацииПоАлкоголю Тогда
		// отправитель - ЮЛ
		Текст.ДобавитьСтроку("НаимОтпрЮл:" + ВРЕГ(ОргНазв));
	...
	Иначе                  
		// отправитель - ФЛ 
		Текст.ДобавитьСтроку("ФИООтпрФЛ:" + СтрЗаменитьПробелы(ПолучитьСведенияОбОрганизации(Организация, РабочаяДата, "ФИО").ФИО));
		Текст.ДобавитьСтроку("ТелОтпр:"   + СокрЛП(ВРЕГ(Лев(ОргТелефон, 20)))); 
		Текст.ДобавитьСтроку("@@@"); 
	КонецЕсли;
...
Функция ВывестиОбщиеСведенияИнформационнойЧасти30(Текст, Организация = Неопределено, Знач ОргИНН = Неопределено, Знач ОргКПП = Неопределено, Знач ДатаДок = Неопределено, КодИФНС = Неопределено) Экспорт
	...
	Если Организация <> Неопределено Тогда
		...
		Если ОргКПП = Неопределено Тогда
...
//Выводит общие сведения информационной части (формат 3.00)
Функция ВывестиОбщиеСведенияИнформационнойЧасти201(Текст, Организация = Неопределено, Знач ОргИНН = Неопределено, Знач ОргКПП = Неопределено, Знач ДатаДок = Неопределено) Экспорт
	...
	Если ПустаяСтрока(ОргКПП) = 0 Тогда
		// Если КПП указан
		ИДОтпр = ОргИНН + "**" + ОргКПП;
	Иначе
		// Если КПП не указан
...
Процедура ПроставитьНомераЛистов(Форма, ВидПечати, ПризнакНумеруемыхФорм = Истина) Экспорт;
	...
	Если НЕ НаложитьДвухмерныйШтрихКод(Форма) Тогда
		Сообщить("Не удалось сгенерировать двухмерный штрихкод.", СтатусСообщения.Внимание);
		Возврат;
	КонецЕсли;
...
Функция роОчистить(Форма, ЗадаватьВопросПередОчисткой = Истина) Экспорт
	...
	Если ЗадаватьВопросПередОчисткой Тогда
		...
		Ответ = Вопрос("Внимание! Будут очищены все разделы (листы) отчета." + Символы.ПС
		...
		Если ТипЗнч(ПервоначальноеЗначение) = Тип("Строка") Тогда
			...
			Пока Вводить Цикл
				...
				ИначеЕсли МеханизмВводаИНН Тогда
					...
					Или (((РазмерностьД = 10) И (ИННСоответствуетТребованиям(ПервоначальноеЗначение, Перечисления.ЮрФизЛицо.ЮрЛицо))) Или ПустаяСтрока(ПервоначальноеЗначение))
					Или (((РазмерностьД = 12) И (ИННСоответствуетТребованиям(ПервоначальноеЗначение, Перечисления.ЮрФизЛицо.ФизЛицо))) Или ПустаяСтрока(ПервоначальноеЗначение)) Тогда
						Вводить = Ложь;
...
//
Процедура ПроставитьИННКППНаВсехЛистахРегОтчета(Форма, НачальноеЗаполнение = Истина) Экспорт
...
Процедура СформироватьСоставКнопкиОчиститьДляРегОтчетов(Форма) Экспорт
	...
	Если ЭтоФормаАлкогольнойОтчетности(Форма) Тогда
...
Процедура МеханизмыОчисткиРегламентированныхОтчетов(Форма, Кнопка) Экспорт
	...
	Если ОчисткаПроизошла Тогда
		...
		Иначе
			...
			// будет указано, что нужно перерасчитывать.
			Форма.Расчет("");
		КонецЕсли;
...
//	
Функция ДокументыСУчетомКритериевОтбораКалендарь(ИсточникОтчета, ДатаНачалаПериодаОтчета, ДатаКонцаПериодаОтчета, ОрганизацияОтбор, КодИФНС) Экспорт
...
// Функция, используя внешнюю библиотеку, создает изображения с двухмерным штрихкодом
...
//
// Возвращаемое значение:
//	Булево - Истина, если изображения штрихкода успешно выведены на печатные формы или если
//				признак печати отчета с двухмерным штрихкодом не установлен.
//		   - Ложь, если изображения с двухмерным штрихкодом успешно выведены в печатные формы отчета.
//
Функция НаложитьДвухмерныйШтрихКод(Форма)
	
	// Если для отчета не предусмотрена печать с двухмерным штрихкодом или этот режим отключен, то
	// возвращаем Истина.
	Попытка
	...
	// пытаемся инициализировать объект для генерации изображений штрихкода
	Попытка
		VAztecBarCode = Новый COMОбъект("VAztecBarCode.BarCodeService");
	Исключение
		Сообщить("Не установлена компонента формирования двухмерного штрихкода!", СтатусСообщения.Внимание);
		Возврат Ложь;
	КонецПопытки;
	
	// создаем два подкаталога во временном каталоге Windows:
	// - для вертикальных штрихкодов;
	// - для горизонтальных штрихкодов.
	...
	Исключение
		Сообщить("Во время формирования произошла ошибка: " + Символы.ПС + ОписаниеОшибки(), СтатусСообщения.Внимание);
		Возврат Ложь;
	КонецПопытки;
	VAztecBarCode = Неопределено;
		
	ЭтоФормаБаланса = ЭтоФормаБаланса(Форма);
	Если ЭтоФормаБаланса Тогда
		Если ТипЗнч(Форма.мПечатныеФормы.Получить(0).Значение) = Тип("ТабличныйДокумент") Тогда
			ТаблДок = Форма.мПечатныеФормы.Получить(0).Значение;
		Иначе
			ТаблДок = Форма.мПечатныеФормы.Получить(0).Значение.Значение;
		КонецЕсли;
		ШиринаТаблДокБаланса = 0;
		Для Инд = 1 По ТаблДок.ШиринаТаблицы Цикл
			ШиринаТаблДокБаланса = ШиринаТаблДокБаланса + ТаблДок.Область(0, Инд, 0, Инд).ШиринаКолонки;
		КонецЦикла;
	КонецЕсли;
	...
	Инд = 1;
	КоличествоБлоковНаЛисте = ?(ЭтоФормаБаланса, 6, 4);
	КоличествоБлоков = 0;
	Для Инд = 1 По Форма.мПечатныеФормы.Количество()*КоличествоБлоковНаЛисте Цикл
		...
		Если ТаблДок.ОриентацияСтраницы = ОриентацияСтраницы.Ландшафт Тогда
			ИмяФайла = ВременныйКаталогГоризонтальныхШтрихКодов + "1C2D" + Формат(Инд, "ЧЦ=4; ЧН=; ЧВН=; ЧГ=") + ".bmp";
		Иначе
			ИмяФайла = ВременныйКаталогВертикальныхШтрихКодов + "1C2D" + Формат(Инд, "ЧЦ=4; ЧН=; ЧВН=; ЧГ=") + ".bmp";
		КонецЕсли;
		МассивДвумерныхШтрихКодов.Добавить(ИмяФайла);
		ФайлСБлоком = Новый Файл(ИмяФайла);
		КоличествоБлоков = КоличествоБлоков + Число(ФайлСБлоком.Существует());
		
	КонецЦикла;
	
	НумероватьБлоки = Истина; // нумеруем блоки для всех видов очтетов
	
	ИндексПечатнойФормы = 0;
	ИндексОбходаЛистов = 1;
	НомерБлокаШтрихКода = 0;
	КоличествоПечатныхФорм = Форма.мПечатныеФормы.Количество();
	Для Каждого ФайлШтрихКода Из МассивДвумерныхШтрихКодов Цикл
		
		НомерБлокаШтрихКода = НомерБлокаШтрихКода + 1;
	
		// инициализируем переменную - текущий табличный документ, на который будет наноситься штрихкод
		Если ТипЗнч(Форма.мПечатныеФормы.Получить(ИндексПечатнойФормы).Значение) = Тип("ТабличныйДокумент") Тогда
			ТаблДок = Форма.мПечатныеФормы.Получить(ИндексПечатнойФормы).Значение;
		Иначе
			ТаблДок = Форма.мПечатныеФормы.Получить(ИндексПечатнойФормы).Значение.Значение;
		...
		Если ОграничивающиеКвадраты.Количество() = 0 И НЕ ЭтоФормаБаланса Тогда
			Сообщить("Не удалось поместить двухмерный штрихкод на печатные формы отчета!", СтатусСообщения.Внимание);
			Возврат Ложь;
		КонецЕсли;
		
		// определяем координаты рисунка
		Если НЕ ЭтоФормаБаланса Тогда
			Если ТаблДок.ОриентацияСтраницы = ОриентацияСтраницы.Ландшафт Тогда
				...
				ЛевоРисунка = ОграничивающиеКвадраты[0].Право - СтандартнаяВысотаРисунка - (10 + 73*(ИндексОбходаЛистов - 1));
			Иначе
				ВерхРисунка = 10 + 73*(ИндексОбходаЛистов - 1);
				ОграничивающиеКвадраты.Сортировать("Право Убыв");
			...
			КонецЕсли;
		Иначе
			ВерхРисунка = 5 + 73*((ИндексОбходаЛистов  - 1) % 3);
			ЛевоРисунка = ШиринаТаблДокБаланса * 1.865 + Цел((ИндексОбходаЛистов  - 1)/3) * (СтандартнаяШиринаРисунка + 4);
			ШиринаТаблДок = ТаблДок.ШиринаТаблицы;
		КонецЕсли;
		...
		Попытка
			...
			Если НумероватьБлоки Тогда
				РисунокНадпись = ТаблДок.Рисунки.Добавить(ТипРисункаТабличногоДокумента.Текст);
				РисунокНадпись.Текст = Формат(НомерБлокаШтрихКода, "ЧГ=") + " / " + КоличествоБлоков;
				РисунокНадпись.Шрифт = Новый Шрифт("Arial", 7);
				РисунокНадпись.ЦветФона = Новый Цвет();
				РисунокНадпись.Линия = Новый Линия(ТипЛинииРисункаТабличногоДокумента.НетЛинии);
				РисунокНадпись.ГоризонтальноеПоложение = ГоризонтальноеПоложение.Центр;
				РисунокНадпись.ВертикальноеПоложение = ВертикальноеПоложение.Низ;
				РисунокНадпись.Отступ = 0;
			КонецЕсли;
		Исключение
			Прервать;// если файл не найден - значит все файлы выведены
		Конецпопытки;
		
		Если ТаблДок.ОриентацияСтраницы = ОриентацияСтраницы.Портрет И ИндексОбходаЛистов = 1 И НЕ ЭтоФормаБаланса Тогда
		...
		Если ТаблДок.ОриентацияСтраницы = ОриентацияСтраницы.Ландшафт Тогда
			...
			Если НумероватьБлоки Тогда
				РисунокНадпись.Верх = Рисунок.Верх;
				РисунокНадпись.Лево = Рисунок.Лево + Рисунок.Ширина;
				РисунокНадпись.Ширина = 4.5;
				РисунокНадпись.Высота = СтандартнаяШиринаРисунка;
				РисунокНадпись.ОриентацияТекста = 270;
				РисунокНадпись.ГоризонтальноеПоложение = ГоризонтальноеПоложение.Лево;
				РисунокНадпись.ВертикальноеПоложение = ВертикальноеПоложение.Низ;
			КонецЕсли;
		Иначе
			...
			Если НумероватьБлоки Тогда
				РисунокНадпись.Верх = Рисунок.Верх - 4;
				РисунокНадпись.Лево = Рисунок.Лево;
				РисунокНадпись.Высота = 4.5;
				РисунокНадпись.Ширина = СтандартнаяШиринаРисунка;			
				РисунокНадпись.ОриентацияТекста = 0;
				РисунокНадпись.ГоризонтальноеПоложение = ГоризонтальноеПоложение.Право;
				РисунокНадпись.ВертикальноеПоложение = ВертикальноеПоложение.Низ;
			КонецЕсли;
		КонецЕсли;
	...
	// удаляем временные каталоги, содержащие файлы с изображениями штрихкодов
	Попытка
...
Функция ПечататьРаздел(Форма, СтрокаРаздела, ЧтоИскать = Неопределено) Экспорт
	...
	Если НеВыбранРаздел Тогда
		Сообщить("В режиме печати с двухмерным штрихкодом не выбран обязательный для печати раздел """ + СтрокаРаздела.Представление + """. Печатная форма раздела будет сформирована принудительно.", СтатусСообщения.Информация);
		Форма.мПечатаемыеРазделы.Добавить(СтрокаРаздела.ИмяСтраницы);
		Возврат 1;
	КонецЕсли;
	...
	Если Форма.мПечатьСДвухмернымШтрихКодом = Истина И ВРЕГ(СтрокаРаздела.ИмяСтраницы) = "ФИЗЛИЦА" Тогда
		...
		Для Каждого ФизЛицо Из КлючФизЛица Цикл
			Если ФизЛицо <> Неопределено И ФизЛицо.Данные <> Неопределено И 
				...
				Сообщить("В режиме печати с двухмерным штрихкодом не выбран обязательный для печати раздел ""Физические лица"". Печатная форма раздела будет сформирована принудительно.", СтатусСообщения.Информация);
				Форма.мПечатаемыеРазделы.Добавить(СтрокаРаздела.ИмяСтраницы);
				Возврат 1;
			КонецЕсли;
		КонецЦикла;
...
// Функция определяет установлена ли компонента формирования двухмерного штрихкода.
//
// Параметры:
//	Нет.
//
...
Функция НапечататьОтчеты(Док, ВидПечати = "ПоказатьБланк", ПечататьДвухмерныйШтрихКод = Неопределено, Форма = Неопределено, ПроверятьСоотношенияПриПечатиИВыгрузки = Неопределено) Экспорт
	...
	Для Каждого Стр Из Док Цикл	
		...
		Попытка
			...
			КонецЕсли;
			Отчет.Инициализация(Истина);
			Если ПечататьДвухмерныйШтрихКод <> Неопределено И ПечататьДвухмерныйШтрихКод <> 2 Тогда
				Попытка
					Отчет.мПечатьСДвухмернымШтрихКодом = ПечататьДвухмерныйШтрихКод;
				Исключение
				КонецПопытки;
			КонецЕсли;
