...
Процедура УстановитьИзменяемыеПараметрыСеансаПользователя() Экспорт
	...
	ПараметрыСеанса.ДатаРедактированияДЗР = Константы.АВ_ДатаРедактированияДЗР.Получить();
	ПараметрыСеанса.ДатаОбновленияДЗР = Дата( 1, 1, 1 );
	//- СВЭЙ КДЯ 04.03.20 СВФР-005274
...
Процедура ЗаписатьПраваДоступаПользователей(ТаблицаНабораПрав, СтруктураОтбора, Отказ = Ложь, ШапкаОшибки = "") Экспорт
	...
	Если Не (РольДоступна(Метаданные.Роли.ПолныеПрава) или РольДоступна(Метаданные.Роли.ПолныеПраваБезЭкономики)) Тогда //О.Т. Аверсон 17.03.2017 № 335
