...
Процедура СохранитьВСохраненныеНастройки(Ссылка) Экспорт
	
	Если ЗначениеЗаполнено(Ссылка) Тогда
	
		ФормаВыбора = Справочники.СохраненныеНастройки.ПолучитьФормуВыбораГруппы("ФормаСохраненияВСохраненныеНастройки");
		ФормаВыбора.НоваяСсылка = Ссылка;
		ФормаВыбора.ОткрытьМодально();
...
Процедура ПрименитьНастройкуОтбораСписка(СтруктураНастроек, КлючЗначения, Отбор) Экспорт
	...
	Для Каждого ЭлементОтбора Из Отбор Цикл
		...
		УстановитьНовоеЗначение(ЭлементОтбора.Использование, СтрокаТаблицы.Использование);
		Если ЭлементОтбора.ТипЗначения.СодержитТип(Тип("Строка")) И ЭлементОтбора.ТипЗначения.КвалификаторыСтроки.Длина = 0 Тогда
			Если СтрокаТаблицы.ВидСравнения = ВидСравнения.НеРавно или СтрокаТаблицы.ВидСравнения = ВидСравнения.НеСодержит Тогда
				УстановитьНовоеЗначение(ЭлементОтбора.ВидСравнения,  ВидСравнения.НеСодержит);
			Иначе
				УстановитьНовоеЗначение(ЭлементОтбора.ВидСравнения,  ВидСравнения.Содержит);
			КонецЕсли;
		Иначе
			УстановитьНовоеЗначение(ЭлементОтбора.ВидСравнения,  СтрокаТаблицы.ВидСравнения);
		КонецЕсли;
		...
		Если ТипЗнч(ЭлементОтбора.Значение) = Тип("СписокЗначений") Тогда
			...
			КонецЕсли;
		Иначе
			УстановитьНовоеЗначение(ЭлементОтбора.Значение, ЭлементОтбора.ТипЗначения.ПривестиЗначение(СтрокаТаблицы.Значение));
		КонецЕсли;
		
		УстановитьНовоеЗначение(ЭлементОтбора.ЗначениеС,  ЭлементОтбора.ТипЗначения.ПривестиЗначение(СтрокаТаблицы.ЗначениеС));
		УстановитьНовоеЗначение(ЭлементОтбора.ЗначениеПо, ЭлементОтбора.ТипЗначения.ПривестиЗначение(СтрокаТаблицы.ЗначениеПо));
	КонецЦикла;
...
//
Процедура ПрименитьЭлементСохраненнойНастройки(СтруктураНастроек, КлючЗначения, ПриемникЗначения, ТипЗначения) Экспорт
	...
	// Значение
	//Если ТипЗнч(ТипЗначения) = Тип("ОписаниеТипов") И ТипЗначения.СодержитТип(ТипЗнч(Значение))
	// ИЛИ ТипЗнч(Значение) = Тип(ТипЗначения) Тогда
		ПриемникЗначения = Значение;
	//КонецЕсли;
	
КонецПроцедуры

// Откывает форму по настройке формы.
//
// Параметры:
//  СохраненнаяНастройка - СправочникСсылка.СохраненныеНастройки- открываемая настройка.
//
Процедура ОткрытьНастройкуФормы(СохраненнаяНастройка) Экспорт
	
	НастраиваемыйОбъект = СохраненнаяНастройка.НастраиваемыйОбъект;
	
	Если СохраненнаяНастройка.ЭтоГруппа Тогда
		Возврат;
	                                        
	Иначе
		
		ОткрытьФормуЗначения(НастраиваемыйОбъект);
		Возврат;
		
	КонецЕсли;
	
	//Форма.СохраненнаяНастройка = СохраненнаяНастройка;
	//
	//Форма.ПрименитьНастройку();
	//
	//Форма.Открыть();
	
КонецПроцедуры

Функция ОткрытьФормуЗначения(ГуидИлиСсылка) Экспорт

	Попытка
		Если ТипЗнч(ГуидИлиСсылка) = Тип("Строка") Тогда
			Ссылка = ЗначениеИзСтрокиВнутр(ГуидИлиСсылка);
		Иначе
			Ссылка = ГуидИлиСсылка;
		КонецЕсли;
...
Процедура НастроитьВидСписка(ТабличноеПоле, Пользователь) Экспорт
	...
	ТабличноеПоле.РежимВыделения         = РежимВыделенияТабличногоПоля.Множественный;
	
	//ТабличноеПоле.РежимВыделенияСтроки   = РежимВыделенияСтрокиТабличногоПоля[?(Пользователь.ВСпискахВыделятьВсюСтрокуЦеликом, "Строка", "Ячейка")];
	//
	//ТабличноеПоле.ЧередованиеЦветовСтрок = Пользователь.ВСпискахЧередоватьЦветаСтрок;
...
Процедура СохранитьОбщиеПараметрыОтчета(ОтчетОбъект, Раздел = "") Экспорт
	
	ОбщиеПараметры = ОтчетОбъект.ПолучитьСтруктуруПараметровОтчета(Ложь, Истина, Ложь);
	
	ВосстановленноеЗначение = ВосстановитьЗначение("ОбщиеНастройки" + Раздел);
	
	Если ТипЗнч(ВосстановленноеЗначение) <> Тип("Структура") Тогда
		ВосстановленноеЗначение = Новый Структура;
	КонецЕсли;
	Для каждого ОбщийПараметр Из ОбщиеПараметры Цикл
		ВосстановленноеЗначение.Вставить(ОбщийПараметр.Ключ, ОбщийПараметр.Значение);
	КонецЦикла;
	
	СохранитьЗначение("ОбщиеНастройки" + Раздел, ВосстановленноеЗначение);
	
КонецПроцедуры

Процедура ЗаполнитьОбщиеНастройкиОтчетов(ЭтотОбъект, Пользователь, Раздел = "") Экспорт
	
	СтруктураПараметров = Новый Структура;
	
	// 1. Заполнить настройки периода
	ДатаНач    = УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(Пользователь, "ОтчетыДатаНачала");
	ДатаКон    = УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(Пользователь, "ОтчетыДатаОкончания");
	СтруктураПараметров.Вставить("ДатаНач", ДатаНач);
	СтруктураПараметров.Вставить("ДатаКон", ДатаКон);
	
	// 2. Заполнить общие настройки
	ВосстановленноеЗначение = ВосстановитьЗначение("ОбщиеНастройки" + Раздел);
	Если ТипЗнч(ВосстановленноеЗначение) = Тип("Структура") Тогда
		Для каждого Элемент Из ВосстановленноеЗначение Цикл
			СтруктураПараметров.Вставить(Элемент.Ключ, Элемент.Значение);
		КонецЦикла;
	КонецЕсли;
	
	// 3. Применить настройки периода и общие настройки
	ЭтотОбъект.ПрименитьСтруктуруПараметровОтчета(СтруктураПараметров, Истина, Истина, Ложь);
	
КонецПроцедуры

Процедура СохранитьНастройкиПериодаОтчета(ОтчетОбъект, Пользователь) Экспорт
	
	Форма = ПолучитьОбщуюФорму("ФормаСохраненияПараметров");
	ЗаполнитьЗначенияСвойств(Форма, ОтчетОбъект, "ДатаНач, ДатаКон");
	Форма.Пользователь = Пользователь;
	Форма.Открыть();
	
КонецПроцедуры

Процедура СохранитьНастройкиПериодаДляОтчетов() Экспорт
	
	Форма = ПолучитьОбщуюФорму("ФормаСохраненияПараметров");
	Форма.Пользователь = ПараметрыСеанса.ТекущийПользователь;
	Форма.Открыть();
	
КонецПроцедуры

Процедура ВосстановитьОбщиеПараметры(Объект, СтруктураОбщихПараметров, Префикс) Экспорт
	
	Для каждого Параметр Из СтруктураОбщихПараметров Цикл
		
		Значение = ВосстановитьЗначение(Префикс + "_" + Параметр.Ключ);
		Если ЗначениеЗаполнено(Значение) Тогда
			СтруктураОбщихПараметров.Вставить(Параметр.Ключ, Значение);
			Объект[Параметр.Ключ] = Значение;
		КонецЕсли;
...
Процедура ОткрытьПриЗапуске() Экспорт
