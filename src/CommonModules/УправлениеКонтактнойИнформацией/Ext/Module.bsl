...
//
Функция ПолучитьАдресныйЭлемент(НазваниеЭлемента, ТипЭлемента, КодЭлементаРодителя = 0, НазвГородаПоле10 = Неопределено) Экспорт //Аверсон 31.10.2022 КВВ СВФР-009754 (добавлено НазвГородаПоле10)
	...
	//Аверсон 31.10.2022 КВВ СВФР-009754 (
	Если ЗначениеЗаполнено(НазвГородаПоле10) Тогда
		Запрос.Текст = Запрос.Текст + "
		|	АдресныйКлассификатор.Сокращение = &Сокращение И";
	КонецЕсли;
	Запрос.Текст = Запрос.Текст + "
	//Аверсон 31.10.2022 КВВ СВФР-009754 )
	...
	Запрос.УстановитьПараметр("Сокращение",НазвГородаПоле10); //Аверсон 31.10.2022 КВВ СВФР-009754
...
Процедура РазложитькодАдресногоЭлементаНаСоставляющие(Знач Код, КодРегиона, КодРайона, КодГорода, КодНаселенногоПункта, КодУлицы, КодДома)
    ...
    //Островская Татьяна 29.11.2013 № 16953
