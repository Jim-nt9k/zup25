...
//+ СВЭЙ КДЯ 20.09.19 СВФР-004805
Функция НомерКвартала( ОтчетныйПериод ) Экспорт
	
	НомерМесяца = Месяц( ОтчетныйПериод );
...
//возвращает имя файла формы ПУ в соотв. с постановлением Министерства труда
// и социальной защиты Республики Беларусь № 4 от 03.05.19 в формате:
//  PREF_UNPF_UNP_T_YEARKV_yyyymmddhhmmss.RRR
Функция ПолучитьИмяФайлаФормыПУ( НомерФормыПУ, ТипДПУ, ОтчетныйПериод, Организация ) Экспорт
	Перем ИмяФайлаФормыПУ;
	
	ВремяФормированияФайлаПачки = ТекущаяДата();
	
	PREF = "PU" + НомерФормыПУ;	
	UNPF = СокрЛП( Организация.РегистрационныйНомерПФР );	
	UNP = СокрЛП( Организация.ИНН );	
	T = "" + ТипДПУ;
	
	Если НомерФормыПУ = 1 Тогда
		YEAR = "0000";
	Иначе	
		YEAR = Строка( Год( ОтчетныйПериод ) );
		YEAR = СтрЗаменить( YEAR, Символы.НПП, "" );
	КонецЕсли; 
	
	Если ( НомерФормыПУ = 1 ИЛИ НомерФормыПУ = 3 ) И ОтчетныйПериод < Дата( 2019, 1, 1 ) Тогда
		KV = "00";
	Иначе	
		KV = "0" + НомерКвартала( ОтчетныйПериод );
	КонецЕсли;
	
	yyyy = Строка( Год( ВремяФормированияФайлаПачки ) );
	yyyy = СтрЗаменить( yyyy, Символы.НПП, "" );
	mm = Прав( "0" + Месяц( ВремяФормированияФайлаПачки ), 2 );
	dd = Прав( "0" + День( ВремяФормированияФайлаПачки ), 2 );
	hh = Прав( "0" + Час( ВремяФормированияФайлаПачки ), 2 );
	mm_ = Прав( "0" + Минута( ВремяФормированияФайлаПачки ), 2 );
	ss = Прав( "0" + Секунда( ВремяФормированияФайлаПачки ), 2 );
	
	RRR = "TXT";
	
	// PREF_UNPF_UNP_T_YEARKV_yyyymmddhhmmss.RRR
	ИмяФайлаФормыПУ = PREF + "_"+ UNPF + "_" + UNP + "_" + T + "_" 
			+ YEAR + KV + "_" + yyyy + mm + dd + hh + mm_ + ss + "." + RRR;
	
	Возврат ИмяФайлаФормыПУ;
КонецФункции // ПолучитьИмяФайлаФормыПУ()
...
Процедура ПоказатьФайлДокумента(ДокументОбъект, ЗаголовокОкна) Экспорт
	...
	//Если ТипЗнч(ТекстФайла) = Тип("Строка") Тогда
		...
		Если не(ТипЗнч(ДокументОбъект) = Тип("ДокументСсылка.СведенияОИндивидуальномПенсионномСтрахованииРаботниковДПУ") //или ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.СведенияОИндивидуальномПенсионномСтрахованииРаботниковДПУ")
			или ТипЗнч(ДокументОбъект) = Тип("ДокументСсылка.СведенияОДвиженииРаботниковДПУ") или ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.СведенияОДвиженииРаботниковДПУ") //О.Т. Аверсон 27.06.2016 № 1278
			или ТипЗнч(ДокументОбъект) = Тип("ДокументСсылка.СведенияОДвиженииРаботниковПУ2") или ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.СведенияОДвиженииРаботниковПУ2")) Тогда  //Аверсон 27.06.2019 КВВ СВФР-004420
	...
	//КонецЕсли; 
	//ПСА 29.01.2021 СВФР-007125+++
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.СведенияОИндивидуальномПенсионномСтрахованииРаботниковДПУ") Тогда
		...
		ИмяФайлаПачки = "" + ДокументОбъект.Номер + ".txt";
		если ЗначениеЗаполнено(ПутьКФайлу) Тогда 
			ПолныйПутьКФайлу = ПутьКФайлу + ?(Прав(ПутьКФайлу,1) = "\","","\") + ИмяФайлаПачки;
			Попытка 		
				ТекстовыйДокумент.Записать(ПолныйПутьКФайлу, КодировкаТекста.ANSI );
				Сообщить( "Файл сохранен: " + ПолныйПутьКФайлу);
			Исключение
