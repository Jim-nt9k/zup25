...
Функция РазрешитьРедактированиеКИвСписке(Пользователь = Неопределено) Экспорт
	...
	Разрешено = ПолучитьЗначениеПраваДляТекущегоПользователя(ПланыВидовХарактеристик.ПраваПользователей.РазрешитьРедактированиеКИвСписке, Ложь);
	Если (Разрешено.Количество() = 0)
	 ИЛИ (Разрешено.Количество() > 1) Тогда
		Возврат Истина;
	Иначе
...
Функция РазрешитьИзменениеПользователяВКалендареПользователя(Пользователь = Неопределено) Экспорт
	...
	Разрешено = ПолучитьЗначениеПраваДляТекущегоПользователя(ПланыВидовХарактеристик.ПраваПользователей.ИзменениеПользователяВКалендареПользователя, Ложь);
	Если (Разрешено.Количество() = 0)
	 ИЛИ (Разрешено.Количество() > 1) Тогда
		Возврат Истина;
	Иначе
...
Процедура СинхронизироватьПользователейИПользователейИБ() Экспорт
	...
	Для Каждого ПользовательИБ Из МассивПользователейИБ Цикл
		...
		ИмяПользователяВСправочнике = УправлениеПользователями.СформироватьИмяПользователяВСправочнике(ПользовательИБ.Имя);
		...
		Если ЗначениеЗаполнено(ПользовательСправочника) Тогда
			...
			// нельзя чтобы имя пользователя ИБ совпадало с именем группы
			...
			Попытка
				// обходим для того, чтобы можно было установить пометку на удаление
				ПользовательОбъект.ОбменДанными.Загрузка = Истина;
...
Функция СкопироватьПользователяИБ(ИмяПользователяИБ) Экспорт
	...
	НовыйПользовательИБ = УправлениеПользователями.ДобавитьНовогоПользователяИБ(ИмяПользователяИБ, ПользовательИБ.ПолноеИмя, Ложь, Ложь);
