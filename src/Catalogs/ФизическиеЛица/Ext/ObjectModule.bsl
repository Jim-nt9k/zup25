...
Функция Печать(ТабДокумент = Неопределено, ДатаАктуальности = Неопределено) Экспорт
	...
	Если Подробно Тогда
		//Островская Татьяна 12.03.2013 МестоРожденияПроизвольныйФормат
		Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|	ФизическиеЛица.МестоРожденияПроизвольныйФормат
		|ИЗ
	...
	Для Каждого СекцияДанных Из СтруктураДанных Цикл
		...
		ИначеЕсли Макет.Области.Найти(ИмяСекции + "_Заголовок") <> Неопределено и Выборка.Следующий() Тогда
			...
			Если ИмяСекции = "ФизическиеЛица" Тогда
				//Островская Татьяна 12.03.2013 МестоРожденияПроизвольныйФормат
				ОбластьМакета.Параметры.МестоРожденияРаботника = РегламентированнаяОтчетность.ПредставлениеМестаРождения(Выборка.МестоРождения,Выборка.МестоРожденияПроизвольныйФормат);
				//Островская Татьяна 12.03.2013 МестоРожденияПроизвольныйФормат
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	Если Не ЭтоГруппа Тогда
		...
		КодИМНС = "";
		СтраховойНомерПФР = "";
		ЛичныйНомер = ""; //Аверсон 02.01.2023 КВВ СВФР-010087
...
Процедура ПередЗаписью(Отказ)
	// заявка №1592 Дмитрук О.Г.
	// это условие должно выполняться только на элементах
	Если НЕ ЭтотОбъект.ЭтоНовый() и Не ЭтотОбъект.ЭтоГруппа Тогда
		мСотрудник = РегистрыСведений.ФИОФизЛиц.ПолучитьПоследнее(ТекущаяДата(),
				Новый Структура ("ФизЛицо", ЭтотОбъект.Ссылка));
		...
		Если СтрЧислоВхождений(Наименование, мФИО)=0 Тогда
			ТекстИсключения = "Несоответствие имени сотрудника и физического лица !" + Символы.ПС + "Укажите правильно ФИО сотрудника!";
		КонецЕсли;
		
		Если НЕ ПустаяСтрока(ТекстИсключения) Тогда
			//ВызватьИсключение ТекстИсключения; // потом вернуть обратно
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПриЗаписи(Отказ)
	Если Не Отказ Тогда
		...
		Если Орг.ИНН = "100194961" Тогда
			НаборЗаписей = РегистрыСведений.ОбменЕРПФизЛица.СоздатьНаборЗаписей();
			...
			Исключение
				ОбщегоНазначения.СообщитьОбОшибке("Не удалось записать данные для обмена с ЕРП о физ. лице : " + ОписаниеОшибки());
				Отказ = Истина;
			КонецПопытки;
		КонецЕсли;
	КонецЕсли;
