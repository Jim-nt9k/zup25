...
Функция ПроверитьЗаполнениеОбязательныхПолей() Экспорт
	...
	//	ТекстОшибок = ТекстОшибок + Символы.ПС + "Не указано имя POP3 сервера.";//О.Т. Аверсон 25.04.2019 № 4292 убрать проверку
	//КонецЕсли;

	//Если НЕ ЗначениеЗаполнено(ПортPOP3) Тогда
	//	ТекстОшибок = ТекстОшибок + Символы.ПС + "Не указан порт POP3 сервера.";//О.Т. Аверсон 25.04.2019 № 4292 убрать проверку
	//КонецЕсли;

	//Если НЕ ЗначениеЗаполнено(Логин) Тогда
	//	ТекстОшибок = ТекстОшибок + Символы.ПС + "Не указано имя пользователя для доступа к почтовому серверу.";//О.Т. Аверсон 25.04.2019 № 4292 убрать проверку
	//КонецЕсли;
...
Процедура ПередЗаписью(Отказ)
	...
	Если ЭтоНовый() Тогда
		...
		ЭтотОбъект.ГруппаНежелательные = Справочники.ГруппыПисемЭлектроннойПочты.ПолучитьСсылку(Новый УникальныйИдентификатор);
	...
	Если ЗначениеЗаполнено(ГруппаЧерновики) Тогда
		...
		НоваяСтрока.Количество = 1;
	КонецЕсли;
	Если ЗначениеЗаполнено(ГруппаНежелательные) Тогда
		НоваяСтрока = ТаблицаПроверкиГрупп.Добавить();
		НоваяСтрока.Группа     = ГруппаНежелательные;
		НоваяСтрока.Количество = 1;
	КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	Если мНовыеГруппы = Истина Тогда
		...
		// Нежелательные
		НовыйОбъект = Справочники.ГруппыПисемЭлектроннойПочты.СоздатьЭлемент();
		НовыйОбъект.УстановитьСсылкуНового(ГруппаНежелательные);
		НовыйОбъект.Владелец						= Ссылка;
		НовыйОбъект.Наименование					= "Нежелательные";
		НовыйОбъект.ИспользоватьПредметыПисем		= ИспользоватьКлассификациюПисемПоПредметам;
		Попытка
			НовыйОбъект.Записать();
		Исключение
