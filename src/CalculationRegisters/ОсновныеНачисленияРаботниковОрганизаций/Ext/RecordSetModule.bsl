...
//толик
		мРаботникиОрганизаций = РегистрыСведений.РаботникиОрганизаций.ПолучитьПоследнее(СтрокаНабора.ПериодДействияНачало,
			Новый Структура("Организация, Сотрудник", СтрокаНабора.Организация, СтрокаНабора.Сотрудник));
			
			
		мРаботникиОрганизацийОкончание = РегистрыСведений.РаботникиОрганизаций.ПолучитьПоследнее(СтрокаНабора.ПериодДействияКонец,
...
Процедура ПриЗаписи(Отказ, Замещение, ТолькоЗапись)
	...
	Если НеВыполнятьРасчетПериодаНачисленияСотрудника(Ложь) Тогда
		//Аверсон 01.06.2022 КВВ СВФР-009135 (
		Если ТипЗнч(Регистратор) = Тип("ДокументСсылка.НачислениеЗарплатыРаботникамОрганизаций")
			И Регистратор.Начисления.Количество() > 0  Тогда
			З = Новый Запрос;
			З.Текст = "ВЫБРАТЬ
			|ОснНач.Регистратор
			|ИЗ РегистрРасчета.ОсновныеНачисленияРаботниковОрганизаций КАК ОснНач
			|ГДЕ ОснНач.Регистратор = &Регистратор
			|СГРУППИРОВАТЬ ПО 
			|ОснНач.Регистратор
			|";
			З.УстановитьПараметр("Регистратор",Регистратор);
			РезЗ = З.Выполнить().Выгрузить();
			Если РезЗ.Количество() = 0 Тогда
				Отказ = Истина;
				Сообщить("Документ не проведен, не внесены данные в регистр расчета основные начисления!");
			КонецЕсли;
		КонецЕсли;
		//Аверсон 01.06.2022 КВВ СВФР-009135 )
        Возврат;
