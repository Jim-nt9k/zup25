...
//толик сортировка по фамилии
	...
	|					И (НЕ Временно)) КАК РаботникиОрганизацииСрезПоследних
	|		ПО (ВЫБОР
	|				КОГДА РаботникиОрганизацииСрезПоследних.ПериодЗавершения <= &ДатаАктуальности
	|						И РаботникиОрганизацииСрезПоследних.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
...
//толик	ДокументРезультат.Очистить();
...
//толик	изменил (ошибка переполнение даты в SQL)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|			ВЫБОР
	|				КОГДА УдержанияРаботниковОрганизации.Период <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|					ТОГДА ДОБАВИТЬКДАТЕ(УдержанияРаботниковОрганизации.Период, ДЕНЬ, -1)
	|				ИНАЧЕ УдержанияРаботниковОрганизации.Период
	|			КОНЕЦ КАК Период,
	|			УдержанияРаботниковОрганизации.ВидРасчета КАК ВидРасчета,
	|			УдержанияРаботниковОрганизации.ДокументОснование КАК ДокументОснование
	|		ИЗ
	...
	|		ВЫБРАТЬ
	|			ВЫБОР
	|				КОГДА УдержанияРаботниковОрганизации.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|					ТОГДА ДОБАВИТЬКДАТЕ(УдержанияРаботниковОрганизации.ПериодЗавершения, ДЕНЬ, -1)
	|				ИНАЧЕ УдержанияРаботниковОрганизации.ПериодЗавершения
	|			КОНЕЦ,
	|			УдержанияРаботниковОрганизации.ВидРасчета,
	|			УдержанияРаботниковОрганизации.ДокументОснование
	|		ИЗ
	...
	Пока ВыборкаРезультата.Следующий()	Цикл
		...
		Если ВыборкаРезультата.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.Процентом
			или ВыборкаРезультата.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПроцентомОтТарифаИБазы Тогда
...
//толик краткий отчет по пасчетным	
	ТиповыеОтчеты.УстановитьПараметр(ОтчетРасчетныеЛистки.КомпоновщикНастроек, "ВидРасчетногоЛистка",  "Кратко");
