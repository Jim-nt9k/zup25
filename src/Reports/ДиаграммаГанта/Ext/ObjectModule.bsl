...
Функция ПолучитьТекстОсновнойВыборкиДляВидаДиаграммы(КодОтчета ,ТекстОграничениеПериода)
	...
	ИначеЕсли КодОтчета = мФактическиеОтпускаОрганизации Тогда
		...
		|ГДЕ
		|	СостояниеРаботниковОрганизации.Состояние В (&РаботникОрганизацииВОтпуске)" + ?(ТекстОграничениеПериода <> ""," И " + ТекстОграничениеПериода,"");
...
Процедура СформироватьОтчет(ДиаграммаГанта) Экспорт
	...
	ПостроительОтчета.Параметры.Вставить("РаботникОрганизацииВОтпуске", СписокОтпусков);
...
Функция  Печать(Регистратор = "") Экспорт
	...
	//Возврат УниверсальныеМеханизмы.НапечататьДокумент(ДокументРезультат, ,, "Диаграмма: " + ВидОтчета);
	
	ФормаПечати = ПолучитьОбщуюФорму("ПечатьДокументовЛандшафт",, ДокументРезультат);
	ФормаПечати.ПечатныйДокумент = ДокументРезультат;
	ФормаПечати.Открыть();
