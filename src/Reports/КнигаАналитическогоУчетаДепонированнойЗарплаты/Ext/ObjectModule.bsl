...
Процедура СформироватьОтчет(ДокументРезультат) Экспорт
	...
	|			И (ВЫБОР
	|				КОГДА РаботникиОрганизацийСрезПоследних.ПериодЗавершения <= &ДатаКон
	...
	// заполним подписи в подвале
	Запрос = Новый Запрос();
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ФИОФизЛицСрезПоследних.Фамилия + "" "" + (ВЫРАЗИТЬ(ФИОФизЛицСрезПоследних.Имя КАК СТРОКА(1))) + "". "" + (ВЫРАЗИТЬ(ФИОФизЛицСрезПоследних.Отчество КАК СТРОКА(1))) + "". "" КАК ФИОГлавногоБухгалтера
	|ИЗ
	...
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("Период", ДатаКон);
	ВыборкаОтветственного = Запрос.Выполнить().Выбрать();
	Если ВыборкаОтветственного.Следующий() тогда
		ОбластьМакетаПодвал.Параметры.Заполнить(ВыборкаОтветственного);
	КонецЕсли;
