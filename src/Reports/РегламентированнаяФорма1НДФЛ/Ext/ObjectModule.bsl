...
Процедура СформироватьОтчет(ДокументРезультат) Экспорт
	...
	Пока ВыборкаСтандартныхВычетовФизЛица.Следующий() Цикл
		...
		Если ВыборкаСтандартныхВычетовФизЛица.КодВычетаЛичный <> Null Тогда
			...
			//Островская Татьяна 14.03.2013 № 16317
			Если НомерВычета <=7 и НомерВычета <> 0 Тогда
	...
	Пока ВыборкаНалогиПомесяцамПоСтавкам.Следующий() Цикл
		...
		Для М = 1 По ПоследнийМесяцДоходовИНалогов Цикл
			...
			НалогИсчисленныйПоБазе = ОбщегоНазначения.ОкруглитьЗначение2016(ПараметрВЧисло(ОбластьРаздела.Параметры["НБаза_" + М]) * СтавкаНалога/100,ДатаКонцаНП);//О.Т. Аверсон 18.03.2016 № 949
