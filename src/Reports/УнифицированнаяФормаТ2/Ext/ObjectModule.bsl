...
Процедура СформироватьОтчет(ДокументРезультат, ЕстьОшибки) Экспорт
	...
	//Аверсон 27.10.2021 КВВ СВФР-008084 (пункт 6) (
	...
	|		ПО СотрудникиОрганизаций.Физлицо = УченыеСтепениЗванияСрезПоследних.ФизЛицо
	//Аверсон 27.10.2021 КВВ СВФР-008084 (пункт 6) )
	...
	//ПСА 16.09.2021 СВФР-008138 +++
	Если ОтчетНа2Страницы Тогда
		СФормироватьОтчетНа2Страницы(ВыборкаРезультата, ДокументРезультат, ЕстьОшибки);
	Иначе	
	//+++ ПСА 16.09.2021 СВФР-008138 
	...
	ДопСведения   =   Макет.ПолучитьОбласть("ДопСведения"); //Островская Татьяна 27.11.2013 № 16936
	ПроверкаДоп   =   Макет.ПолучитьОбласть("ПроверкаДоп"); //Островская Татьяна 27.11.2013 № 16936
	Если ВыборкаРезультата.Следующий() тогда
		...
		|	ФизическиеЛицаОбразование.ВидОбразования КАК ВидВидОбразования, //Аверсон 25.08.2021 КВВ СВФР-008031
		...
		Пока ВыборкаОбразование.Следующий() цикл
			Если ВыборкаОбразование.ВидОбразования = 1 тогда 
				...
				Если НомерОбразованияВВУЗЕ < 4  тогда
					ПерваяСтраница.Параметры["НазваниеВысш" + НомерОбразованияВВУЗЕ] = СокрЛП(ВыборкаОбразование.ВидВидОбразования) + ", " + СокрЛП(ВыборкаОбразование.УчебноеЗаведение);  //Аверсон 25.08.2021 КВВ СВФР-008031 (Вид образ)                  
					ПерваяСтраница.Параметры["ДатаОкончанияВысш" + НомерОбразованияВВУЗЕ] = Формат(ВыборкаОбразование.ГодОкончания,"ЧГ=0") + " г. выпуска";
					ПерваяСтраница.Параметры["Специальность" + НомерОбразованияВВУЗЕ] = ВыборкаОбразование.Специальность;
					ПерваяСтраница.Параметры["Квалификация" + НомерОбразованияВВУЗЕ] = ВыборкаОбразование.Квалификация1; //О.Т. 01.08.2014 № 154
...
//					ПерваяСтраница.Параметры.ОКСО = ВыборкаОбразование.Специальность.Код;
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			ПерваяСтраница.Параметры.Заполнить(ВыборкаДетальныеЗаписи);
		КонецЦикла;
	...
	ДокументРезультат.Вывести(ДопСведения);
...
//ПСА 16.09.2021 СВФР-008138 +++
Процедура СформироватьОтчетНа2Страницы(ВыборкаРезультата, ДокументРезультат, ЕстьОшибки)
