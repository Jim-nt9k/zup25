			...
			Если ЗначениеЗаполнено(ТекСтрокаТЗ.Должность) Тогда
				...
				если Факт Тогда 
					Для Каждого СтрТЗ из ТЗ_ТаблицаПовышенийОр Цикл
						...
						Если НайденнаяСтр.Количество() = 0 Тогда
							НайденнаяСтр = ТаблицаВакантноКоэф.НайтиСтроки(Новый Структура("ПодразделениеОрганизации,Должность,МесячнаяТарифнаяСтавка, Коэффициент", ТекСтрокаТЗ.Подразделение, ТекСтрокаТЗ.Должность, ТекСтрокаТЗ.МТСтавка, СтрТЗ.Коэффициент));
							если НайденнаяСтр.Количество()>0 Тогда 
								если ВторойОтчет Тогда
