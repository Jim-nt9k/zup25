...
Перем ФондЗПИтог, СтавокИтог, ТарифнаяСтавкаИтог, СтавкаИтог, ЗаСложностьИтог;
Перем ФондЗПИтогПоПредпр, СтавокИтогПоПредпр, ТарифнаяСтавкаИтогПоПредпр, СтавкаИтогПоПредпр, ЗаСложностьИтогПоПредпр;
...
Перем ТаблицаВакантноКоэф, ВторойОтчет;

#Если Клиент Тогда	
	Функция СформироватьЗапросПоВакантноПБХ(СписокВидовРасчета)
		
		//Аверсон 17.06.2020 КВВ СВФР-006124 (
		ДатаАктуальностиДляВакансий = ДатаАктуальности;
		З = Новый Запрос;
		З.Текст ="ВЫБРАТЬ *
		|ИЗ Документ.ПлановоеШтатноеРасписаниеОрганизаций  КАК ДокПланШР
		|ГДЕ НАЧАЛОПЕРИОДА(ДокПланШР.Дата,ДЕНЬ) = НАЧАЛОПЕРИОДА(&Дата)";
		З.УстановитьПараметр("Дата",ДатаАктуальности);
		РезЗ = З.Выполнить().Выбрать();
		Если РезЗ.Следующий() Тогда
			ДатаАктуальностиДляВакансий = КонецДня(ДатаАктуальностиДляВакансий) + 1;
		КонецЕсли;
		//Аверсон 17.06.2020 КВВ СВФР-006124 )
		
		//Аверсон 13.04.2020 КВВ СВФР-005765 ( 
		Категория = Справочники.КатегорииРаботников.ПустаяСсылка();
		Запрос = Новый Запрос;
		...
		Запрос.УстановитьПараметр("Период", ДатаАктуальностиДляВакансий); //Аверсон 17.06.2020 КВВ СВФР-006124
		...
		|	ВЫБОР
		|	КОГДА ВакантныеСтавки.ПодразделениеОрганизации.Приоритет > 10
		|			И ВакантныеСтавки.ПодразделениеОрганизации.Приоритет < 100
		|			И ВакантныеСтавки.ПодразделениеОрганизации.Родитель <> ЗНАЧЕНИЕ(справочник.ПодразделенияОрганизаций.пустаяСсылка)
		|		ТОГДА ВЫРАЗИТЬ(ВакантныеСтавки.ПодразделениеОрганизации.Приоритет / 10 КАК ЧИСЛО(10, 1))
		|	КОГДА ВакантныеСтавки.ПодразделениеОрганизации.Приоритет > 100
		|			И ВакантныеСтавки.ПодразделениеОрганизации.Приоритет < 1000
		|			И ВакантныеСтавки.ПодразделениеОрганизации.Родитель <> ЗНАЧЕНИЕ(справочник.ПодразделенияОрганизаций.пустаяСсылка)
		|		ТОГДА ВЫРАЗИТЬ(ВакантныеСтавки.ПодразделениеОрганизации.Приоритет / 100 КАК ЧИСЛО(10, 2))
		|	КОГДА ВакантныеСтавки.ПодразделениеОрганизации.Приоритет > 1000
		|			И ВакантныеСтавки.ПодразделениеОрганизации.Приоритет < 10000
		|			И ВакантныеСтавки.ПодразделениеОрганизации.Родитель <> ЗНАЧЕНИЕ(справочник.ПодразделенияОрганизаций.пустаяСсылка)
		|		ТОГДА ВЫРАЗИТЬ(ВакантныеСтавки.ПодразделениеОрганизации.Приоритет / 1000 КАК ЧИСЛО(10, 3))
		|	ИНАЧЕ ВакантныеСтавки.ПодразделениеОрганизации.Приоритет
		|	КОНЕЦ КАК ПодразделениеОрганизацииПриоритет,    	
		|	ВакантныеСтавки.Должность КАК Должность,
		|	ВЫБОР
		|	КОГДА ВакантныеСтавки.Должность.Приоритет > 10
		|			И ВакантныеСтавки.Должность.Приоритет < 100
		|			И ВакантныеСтавки.Должность.Родитель <> ЗНАЧЕНИЕ(справочник.ДолжностиОрганизаций.пустаяСсылка)
		|		ТОГДА ВЫРАЗИТЬ(ВакантныеСтавки.Должность.Приоритет / 10 КАК ЧИСЛО(10, 1))
		|	КОГДА ВакантныеСтавки.Должность.Приоритет > 100
		|			И ВакантныеСтавки.Должность.Приоритет < 1000
		|			И ВакантныеСтавки.Должность.Родитель <> ЗНАЧЕНИЕ(справочник.ДолжностиОрганизаций.пустаяСсылка)
		|		ТОГДА ВЫРАЗИТЬ(ВакантныеСтавки.Должность.Приоритет / 100 КАК ЧИСЛО(10, 2))
		|	КОГДА ВакантныеСтавки.Должность.Приоритет > 1000
		|			И ВакантныеСтавки.Должность.Приоритет < 10000
		|			И ВакантныеСтавки.Должность.Родитель <> ЗНАЧЕНИЕ(справочник.ДолжностиОрганизаций.пустаяСсылка)
		|		ТОГДА ВЫРАЗИТЬ(ВакантныеСтавки.Должность.Приоритет / 1000 КАК ЧИСЛО(10, 3))
		|	ИНАЧЕ ВакантныеСтавки.Должность.Приоритет
		...
		//|   ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ТарифнаяСтавкаПервогоРазряда  КАК РазмерСтавки1Разряда,
		...
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ //О.Т. Аверсон 27.02.2018 № 1651(условие на ИНН)
		|			ТОГДА ВЫРАЗИТЬ(ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.МесячнаяТарифнаяСтавка / ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ЗанимаемыхСтавок  КАК ЧИСЛО(10, 2)) //- 0.005
		...
		|			ВложенныйЗапрос.ИтоговоеПовышениеПоИнструкции КАК ИтоговоеПовышениеПоИнструкции
		|		ИЗ
		|			(ВЫБРАТЬ
		...
		|				ШтатноеРасписание.ИтоговоеПовышениеПоИнструкции КАК ИтоговоеПовышениеПоИнструкции
		|			ИЗ
		|				(ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|									ИНАЧЕ ИСТИНА
		|								КОНЕЦ
		|							ТОГДА ВЫРАЗИТЬ(ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.МесячнаяТарифнаяСтавка / ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ЗанимаемыхСтавок КАК ЧИСЛО(10, 2))
		|						ИНАЧЕ ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.МесячнаяТарифнаяСтавка
		|					КОНЕЦ КАК МесячнаяТарифнаяСтавка,
		|					ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ПроцентПоКонтракту КАК ПроцентПоКонтракту,
		|					ВЫБОР
		...
		|					ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ГрафикРаботы КАК ГрафикРаботы,
		|					ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ИтоговоеПовышениеПоИнструкции КАК ИтоговоеПовышениеПоИнструкции
		|				ИЗ
		...
		|					И ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ВидРасчета В(&ВидРасчета)";
		Если ЗначениеЗаполнено(Категория) Тогда 
			ТекстЗапроса = ТекстЗапроса +"
			|		И ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.Должность.Категория В(&Категория)";
		КонецЕсли;
		ТекстЗапроса = ТекстЗапроса +"
		|				
		|				СГРУППИРОВАТЬ ПО
		...
		|						ИНАЧЕ ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.МесячнаяТарифнаяСтавка
		|					КОНЕЦ,
		|					ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ГрафикРаботы,
		|					ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ИтоговоеПовышениеПоИнструкции) КАК ШтатноеРасписание
		...
		|											ТОГДА ВЫРАЗИТЬ(ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.МесячнаяТарифнаяСтавка / ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.ЗанимаемыхСтавок КАК ЧИСЛО(10, 2))
		...
		|										КОГДА РаботникиОрганизацийСрезПоследних.ЗанимаемыхСтавок < 1
		|												И РаботникиОрганизацийСрезПоследних.ПодразделениеОрганизации.Владелец.ИНН <> ""190288027"" //О.Т. Аверсон 01.02.2018 № 1651 новое условие ИНН
		|											ТОГДА ВЫРАЗИТЬ(ПриемНаРаботуВОрганизациюРаботникиОрганизации.МесячнаяТарифнаяСтавка / РаботникиОрганизацийСрезПоследних.ЗанимаемыхСтавок КАК ЧИСЛО(10, 2))
		...
		|										КОГДА РаботникиОрганизацийСрезПоследних.ЗанимаемыхСтавок < 1
		|												И РаботникиОрганизацийСрезПоследних.ПодразделениеОрганизации.Владелец.ИНН <> ""190288027"" //О.Т. Аверсон 01.02.2018 № 1651 новое условие ИНН
		|											ТОГДА ВЫРАЗИТЬ(ТарифныеСтавкиРаботниковОрганизации.Ставка / РаботникиОрганизацийСрезПоследних.ЗанимаемыхСтавок КАК ЧИСЛО(10, 2))
		...
		|								ТОГДА ВЫБОР
		|										КОГДА РаботникиОрганизацийСрезПоследних.ЗанимаемыхСтавок < 1
		|											ТОГДА (ВЫРАЗИТЬ((ВЫРАЗИТЬ(ПриемНаРаботуВОрганизациюРаботникиОрганизации.РазмерСтавки1Разряда * ПриемНаРаботуВОрганизациюРаботникиОрганизации.КоэффициентЕТС КАК ЧИСЛО(10, 2))) * ПриемНаРаботуВОрганизациюРаботникиОрганизации.ПовышенныйКоэффициентЕТС КАК ЧИСЛО(10, 2))) +
		|												 (ВЫРАЗИТЬ((ВЫРАЗИТЬ(ПриемНаРаботуВОрганизациюРаботникиОрганизации.РазмерСтавки1Разряда * ПриемНаРаботуВОрганизациюРаботникиОрганизации.КоэффициентЕТС КАК ЧИСЛО(10, 2))) * ПриемНаРаботуВОрганизациюРаботникиОрганизации.ИтоговоеПовышениеПоИнструкции / 100 КАК ЧИСЛО(10, 2))) +
		|												 (ВЫРАЗИТЬ((ВЫРАЗИТЬ(ПриемНаРаботуВОрганизациюРаботникиОрганизации.РазмерСтавки1Разряда * ПриемНаРаботуВОрганизациюРаботникиОрганизации.КоэффициентЕТС КАК ЧИСЛО(10, 2))) * ПриемНаРаботуВОрганизациюРаботникиОрганизации.ПроцентПоКонтракту / 100 КАК ЧИСЛО(10, 2)))
		|													ИНАЧЕ ПриемНаРаботуВОрганизациюРаботникиОрганизации.МесячнаяТарифнаяСтавка
		|									КОНЕЦ
		|							КОГДА РаботникиОрганизацийСрезПоследних.Регистратор ССЫЛКА Документ.ИзменениеПериодическихРеквизитов       
		|								ТОГДА ВЫБОР
		|										КОГДА РаботникиОрганизацийСрезПоследних.ЗанимаемыхСтавок < 1
		|											ТОГДА (ВЫРАЗИТЬ((ВЫРАЗИТЬ(ИзменениеПериодическихРеквизитовРаботникиОрганизации.РазмерСтавки1Разряда * ИзменениеПериодическихРеквизитовРаботникиОрганизации.КоэффициентЕТС КАК ЧИСЛО(10, 2))) * ИзменениеПериодическихРеквизитовРаботникиОрганизации.ПовышенныйКоэффициентЕТС КАК ЧИСЛО(10, 2))) +
		|												 (ВЫРАЗИТЬ((ВЫРАЗИТЬ(ИзменениеПериодическихРеквизитовРаботникиОрганизации.РазмерСтавки1Разряда * ИзменениеПериодическихРеквизитовРаботникиОрганизации.КоэффициентЕТС КАК ЧИСЛО(10, 2))) * ИзменениеПериодическихРеквизитовРаботникиОрганизации.ИтоговоеПовышениеПоИнструкции / 100 КАК ЧИСЛО(10, 2))) +
		|												 (ВЫРАЗИТЬ((ВЫРАЗИТЬ(ИзменениеПериодическихРеквизитовРаботникиОрганизации.РазмерСтавки1Разряда * ИзменениеПериодическихРеквизитовРаботникиОрганизации.КоэффициентЕТС КАК ЧИСЛО(10, 2))) * ИзменениеПериодическихРеквизитовРаботникиОрганизации.ПроцентПоКонтракту / 100 КАК ЧИСЛО(10, 2)))
		|													ИНАЧЕ ТарифныеСтавкиРаботниковОрганизации.Ставка
		|									КОНЕЦ
		|							КОГДА РаботникиОрганизацийСрезПоследних.Регистратор ССЫЛКА Документ.КадровоеПеремещениеОрганизаций
		|								ТОГДА ВЫБОР
		|										КОГДА РаботникиОрганизацийСрезПоследних.ЗанимаемыхСтавок < 1
		|											ТОГДА (ВЫРАЗИТЬ((ВЫРАЗИТЬ(КадровоеПеремещениеОрганизацийРаботникиОрганизации.РазмерСтавки1Разряда * КадровоеПеремещениеОрганизацийРаботникиОрганизации.КоэффициентЕТС КАК ЧИСЛО(10, 2))) * КадровоеПеремещениеОрганизацийРаботникиОрганизации.ПовышенныйКоэффициентЕТС КАК ЧИСЛО(10, 2))) +
		|												 (ВЫРАЗИТЬ((ВЫРАЗИТЬ(КадровоеПеремещениеОрганизацийРаботникиОрганизации.РазмерСтавки1Разряда * КадровоеПеремещениеОрганизацийРаботникиОрганизации.КоэффициентЕТС КАК ЧИСЛО(10, 2))) * КадровоеПеремещениеОрганизацийРаботникиОрганизации.ИтоговоеПовышениеПоИнструкции / 100 КАК ЧИСЛО(10, 2))) +
		|												 (ВЫРАЗИТЬ((ВЫРАЗИТЬ(КадровоеПеремещениеОрганизацийРаботникиОрганизации.РазмерСтавки1Разряда * КадровоеПеремещениеОрганизацийРаботникиОрганизации.КоэффициентЕТС КАК ЧИСЛО(10, 2))) * КадровоеПеремещениеОрганизацийРаботникиОрганизации.ПроцентПоКонтракту / 100 КАК ЧИСЛО(10, 2)))
		|													ИНАЧЕ ТарифныеСтавкиРаботниковОрганизации.Ставка
		|									КОНЕЦ
		|						КОНЕЦ КАК МесячнаяТарифнаяСтавка1,
		|						ЗаменяемыеСотрудникиСрезПоследних.Сотрудник КАК Сотрудник,
		|										ВЫБОР
		|											КОГДА ЕСТЬNULL(ЗаменяемыеСотрудникиСрезПоследних.СотрудникЗаменяемый, 0) <> 0
		|													ИЛИ ЕСТЬNULL(ЗаменяемыеСотрудникиСрезПоследних.СотрудникЗаменяемый2, 0) <> 0
		|													ИЛИ ЕСТЬNULL(ЗаменяемыеСотрудникиСрезПоследних.СотрудникЗаменяемый3, 0) <> 0
		|													ИЛИ ЕСТЬNULL(ЗаменяемыеСотрудникиСрезПоследних.СотрудникЗаменяемый4, 0) <> 0
		|												ТОГДА 1
		|											ИНАЧЕ 0
		|						КОНЕЦ КАК ЕстьЗаменяемый,
		...
		|								И &Период МЕЖДУ СостояниеРаботниковОрганизаций.Период И СостояниеРаботниковОрганизаций.ПериодЗавершения) КАК СостояниеРаботников
		...
		|								ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗаменяемыеСотрудники.СрезПоследних КАК ЗаменяемыеСотрудникиСрезПоследних
		|								ПО РаботникиОрганизацийСрезПоследних.Сотрудник = ЗаменяемыеСотрудникиСрезПоследних.Сотрудник
		|					ГДЕ
		...
		|								ТОГДА ТарифныеСтавкиРаботниковОрганизации.ПроцентПоКонтракту
		|						КОНЕЦ,
		|						ЗаменяемыеСотрудникиСрезПоследних.Сотрудник,
		|										ВЫБОР
		|											КОГДА ЕСТЬNULL(ЗаменяемыеСотрудникиСрезПоследних.СотрудникЗаменяемый, 0) <> 0
		|													ИЛИ ЕСТЬNULL(ЗаменяемыеСотрудникиСрезПоследних.СотрудникЗаменяемый2, 0) <> 0
		|													ИЛИ ЕСТЬNULL(ЗаменяемыеСотрудникиСрезПоследних.СотрудникЗаменяемый3, 0) <> 0
		...
		|											ИНАЧЕ 0
		|						КОНЕЦ,
		|						РаботникиОрганизацийСрезПоследних.ГрафикРаботы) КАК РаботникиФакт
		...
		|						И (ШтатноеРасписание.МесячнаяТарифнаяСтавка = РаботникиФакт.МесячнаяТарифнаяСтавка
		|							ИЛИ ШтатноеРасписание.МесячнаяТарифнаяСтавка = РаботникиФакт.МесячнаяТарифнаяСтавка1)
		|						И (ВЫБОР
		|											КОГДА ЕСТЬNULL(РаботникиФакт.Сотрудник, 0) <> 0
		|													И РаботникиФакт.ЕстьЗаменяемый = 1
		|												ТОГДА ИСТИНА
		|											ИНАЧЕ ШтатноеРасписание.ПроцентПоКонтракту = РаботникиФакт.ПроцентПоКонтракту
		|										КОНЕЦ)
		|						И ШтатноеРасписание.ГрафикРаботы = РаботникиФакт.ГрафикРаботы
		|			ГДЕ
		...
		|				ШтатноеРасписание.ЗанимаемыхСтавок,
		|				ШтатноеРасписание.ИтоговоеПовышениеПоИнструкции) КАК ВложенныйЗапрос
		|		
		|		СГРУППИРОВАТЬ ПО
		...
		|			ВложенныйЗапрос.МесячнаяТарифнаяСтавка,
		|			ВложенныйЗапрос.ИтоговоеПовышениеПоИнструкции) КАК ВакантныеСтавки
		...
		|				ИНАЧЕ ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.МесячнаяТарифнаяСтавка
		|			КОНЕЦ)
		|			И ВакантныеСтавки.ИтоговоеПовышениеПоИнструкции = ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ИтоговоеПовышениеПоИнструкции
		|ГДЕ
		...
		|	ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ТарифнаяСтавкаПервогоРазряда,
		|	ВЫБОР
		|		КОГДА ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ЗанимаемыхСтавок > 0 и ВЫБОР
		|			КОГДА ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ЗанимаемыхСтавок < 1
		|				ТОГДА ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ПодразделениеОрганизации.Владелец.ИНН <> ""190288027""
		...
		|	ВТВакантные.ПодразделениеОрганизацииПриоритет,
		|	ВТВакантные.Должность,
		|	ВТВакантные.ДолжностьПриоритет,
		...
		//Островская Татьяна 04.03.2013 № 16283
	...
	Функция СформироватьЗапросПоВакантно(СписокВидовРасчета)
		...
		//Аверсон ЕАЕ 26.01.2024 СВФР-012191
		|" + ?(Организация.ИНН = "101542412", " ВакантныеСтавки.ГрафикРаботы КАК ГрафикРаботы,", "") + "
		//Аверсон СВФР-012191
		
		|	ВакантныеСтавки.КоэффициентЕТС КАК КоэффициентЕТС,
		|	ВакантныеСтавки.ПовышенныйКоэффициентЕТС КАК ПовышенныйКоэффициентЕТС,
		//+ Ништ 28.01.2026 свфр-017928: дублируются строки в отчете если вводятся две плановые ставки и одна закрыта не полностью
		|	ВакантныеСтавки.КодСтроки КАК КодСтроки,
		//- Ништ 28.01.2026 свфр-017928
		...
		//Аверсон ЕАЕ 09.10.2025 СВФР-016997
		|		ТОГДА //ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ТарифнаяСтавка / ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ЗанимаемыхСтавок
		|       	ВЫБОР
		|           КОГДА ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ЗанимаемыхСтавок < 1 И %%УсловиеИНН%%
		|           ТОГДА ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ТарифнаяСтавка
		|           ИНАЧЕ ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ТарифнаяСтавка / ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ЗанимаемыхСтавок
		|           КОНЕЦ
		//Аверсон СВФР-016997
		
		//|		ИНАЧЕ ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ТарифнаяСтавка	
		//Аверсон 31.03.2020 КВВ СВФР-005748 (
		|       ИНАЧЕ 
		...
		|       	ВЫБОР
		|           КОГДА ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ЗанимаемыхСтавок < 1 И %%УсловиеИНН%%
		|           ТОГДА ВЫРАЗИТЬ(ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ТарифныйСПовышениемПоИнструкции КАК ЧИСЛО(15, 3))
		|           ИНАЧЕ ВЫРАЗИТЬ(ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ТарифныйСПовышениемПоИнструкции / ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ЗанимаемыхСтавок КАК ЧИСЛО(15, 3))
		|           КОНЕЦ
		//Аверсон СВФР-016997
		...
		|       	ВЫБОР
		|           КОГДА ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ЗанимаемыхСтавок < 1 И %%УсловиеИНН%%
		|           ТОГДА ВЫРАЗИТЬ(ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.МесячнаяТарифнаяСтавка КАК ЧИСЛО(15, 2))
		|           ИНАЧЕ ВЫРАЗИТЬ(ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.МесячнаяТарифнаяСтавка / ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ЗанимаемыхСтавок КАК ЧИСЛО(15, 2))
		|           КОНЕЦ
		//Аверсон СВФР-016997
		...
		|				ШтатноеРасписание.МесячнаяТарифнаяСтавка КАК МесячнаяТарифнаяСтавка,
		//+ Ништ 28.01.2026 свфр-017928: дублируются строки в отчете если вводятся две плановые ставки и одна закрыта не полностью
		|				ШтатноеРасписание.КодСтроки КАК КодСтроки
		//- Ништ 28.01.2026 свфр-017928
		...
		|					КОНЕЦ КАК МесячнаяТарифнаяСтавка1,
		//+ Ништ 28.01.2026 свфр-017928: дублируются строки в отчете если вводятся две плановые ставки и одна закрыта не полностью
		|					ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.КодСтроки КАК КодСтроки
		//- Ништ 28.01.2026 свфр-017928
		|				ИЗ
		|					РегистрСведений.ПлановоеШтатноеРасписаниеОрганизаций.СрезПоследних(&Период, ) КАК ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних
		|						ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		...
		|					КОНЕЦ,
		//+ Ништ 28.01.2026 свфр-017928: дублируются строки в отчете если вводятся две плановые ставки и одна закрыта не полностью
		|					ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.КодСтроки) КАК ШтатноеРасписание
		//- Ништ 28.01.2026 свфр-017928
		|					ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		...
		//Аверсон 18.05.2021 КВВ СВФР-007550 (
		Если Организация.ИНН = "101542412" Тогда //Минск-Новости
			...
			//Аверсон 28.06.2023 КВВ СВФР-010981 (
			|ВЫБОР
			|КОГДА РаботникиОрганизацийСрезПоследних.Регистратор ССЫЛКА Документ.ПлановоеШтатноеРасписаниеОрганизаций
			|ТОГДА ВЫБОР
			|КОГДА РаботникиОрганизацийСрезПоследних.ЗанимаемыхСтавок < 1
			...
			|
			//Аверсон 29.11.2021 КВВ СВФР-007998 (
			|ВЫБОР
			|КОГДА РаботникиОрганизацийСрезПоследних.Регистратор ССЫЛКА Документ.ПлановоеШтатноеРасписаниеОрганизаций
			|ТОГДА ВЫБОР
			|КОГДА РаботникиОрганизацийСрезПоследних.ЗанимаемыхСтавок < 1
		...
		|						И ";
		//Аверсон 09.03.2022 КВВ СВФР-008804 (
		Если Факт И ВторойОтчет Тогда
			ТекстЗапроса = ТекстЗапроса + "	
			|(";
		КонецЕсли;
		ТекстЗапроса = ТекстЗапроса + "
		...
		Если Факт И ВторойОтчет Тогда
			...
			//Аверсон 05.04.2022 КВВ СВФР-008903 (пункт 1) (
			|ИЛИ
			| (
		...
		|				ШтатноеРасписание.ЗанимаемыхСтавок,
		//+ Ништ 28.01.2026 свфр-017928: дублируются строки в отчете если вводятся две плановые ставки и одна закрыта не полностью
		|				ШтатноеРасписание.КодСтроки) КАК ВложенныйЗапрос
		//- Ништ 28.01.2026 свфр-017928
		|		
		|		СГРУППИРОВАТЬ ПО
		|			ВложенныйЗапрос.ПодразделениеОрганизации,
		...
		|			ВложенныйЗапрос.МесячнаяТарифнаяСтавка,
		//+ Ништ 28.01.2026 свфр-017928: дублируются строки в отчете если вводятся две плановые ставки и одна закрыта не полностью
		|			ВложенныйЗапрос.КодСтроки) КАК ВакантныеСтавки
		//- Ништ 28.01.2026 свфр-017928
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПлановоеШтатноеРасписаниеОрганизаций.СрезПоследних(&Период, ) КАК ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|				МАКСИМУМ(ПлановоеШтатноеРасписаниеОрганизаций.Период) КАК Период,
		...
		|			И ВакантныеСтавки.РазрядЕТС = ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.РазрядЕТС
		//+ Ништ 28.01.2026 свфр-017928: дублируются строки в отчете если вводятся две плановые ставки и одна закрыта не полностью
		|			И ВакантныеСтавки.КодСтроки = ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.КодСтроки
		//- Ништ 28.01.2026 свфр-017928
		//Аверсон ЕАЕ 26.01.2024 СВФР-012191
		...
		//Аверсон ЕАЕ 10.10.2025 СВФР-016997
		Если Организация.ИНН = "691159723" ИЛИ Организация.ИНН = "101542412" Тогда
			УсловиеИНН = "Истина";
		Иначе
			УсловиеИНН = "Ложь";
		КонецЕсли;
		...
		|	ВТВакантные КАК ВТВакантные
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			Вложенный_ПШР.Коэффициент КАК Коэффициент,
		|			Вложенный_ПШР.Размер КАК Размер,
		|			Вложенный_ПШР.КодСтроки КАК Сотрудник,
		|			ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.ПодразделениеОрганизации КАК ПодразделениеОрганизации,
		|			ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.Должность КАК Должность,
		|			ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.МесячнаяТарифнаяСтавка КАК МесячнаяТарифнаяСтавка,
		|			ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.Ссылка КАК Ссылка
		|		ИЗ
		|			(ВЫБРАТЬ
		|				ПлановоеШтатноеРасписаниеОрганизацийКоэфПовышенияТО.Ссылка КАК Ссылка,
		|				ПлановоеШтатноеРасписаниеОрганизацийКоэфПовышенияТО.НомерСтроки КАК НомерСтроки,
		|				ПлановоеШтатноеРасписаниеОрганизацийКоэфПовышенияТО.Коэффициент КАК Коэффициент,
		|				ПлановоеШтатноеРасписаниеОрганизацийКоэфПовышенияТО.Размер КАК Размер,
		|				ПлановоеШтатноеРасписаниеОрганизацийКоэфПовышенияТО.КодСтроки КАК КодСтроки
		|			ИЗ
		|				Документ.ПлановоеШтатноеРасписаниеОрганизаций.КоэфПовышенияТО КАК ПлановоеШтатноеРасписаниеОрганизацийКоэфПовышенияТО
		|			
		|			ОБЪЕДИНИТЬ ВСЕ
		|			
		|			ВЫБРАТЬ
		|				ПлановоеШтатноеРасписаниеОрганизацийПовышенияПоИнструкции.Ссылка,
		|				ПлановоеШтатноеРасписаниеОрганизацийПовышенияПоИнструкции.НомерСтроки,
		|				ПлановоеШтатноеРасписаниеОрганизацийПовышенияПоИнструкции.Коэффициент,
		|				ПлановоеШтатноеРасписаниеОрганизацийПовышенияПоИнструкции.Размер,
		|				ПлановоеШтатноеРасписаниеОрганизацийПовышенияПоИнструкции.КодСтроки
		|			ИЗ
		|				Документ.ПлановоеШтатноеРасписаниеОрганизаций.ПовышенияПоИнструкции КАК ПлановоеШтатноеРасписаниеОрганизацийПовышенияПоИнструкции) КАК Вложенный_ПШР
		|				ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПлановоеШтатноеРасписаниеОрганизаций.ШтатныеЕдиницы КАК ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы
		//+ Ништ 28.01.2026 свфр-017928: дублируются строки в отчете если вводятся две плановые ставки и одна закрыта не полностью
		|				ПО Вложенный_ПШР.КодСтроки = ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.КодСтроки
		//- Ништ 28.01.2026 свфр-017928
		|					И Вложенный_ПШР.Ссылка = ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.Ссылка) КАК ВложенныйЗапрос
		|		ПО ВТВакантные.ПодразделениеОрганизации = ВложенныйЗапрос.ПодразделениеОрганизации
		|			И ВТВакантные.Должность = ВложенныйЗапрос.Должность
		//|			И ВТВакантные.МесячнаяТарифнаяСтавка = ВложенныйЗапрос.МесячнаяТарифнаяСтавка
		|			И ВТВакантные.Регистратор = ВложенныйЗапрос.Ссылка
		|			И ВТВакантные.КодСтроки = ВложенныйЗапрос.Сотрудник
		|ГДЕ
		|	ВложенныйЗапрос.Размер > 0
		|";
		...
		Возврат Результ;
	КонецФункции // СформироватьЗапросПоВакантно()
	
	//запрос по декретницам
	//
	Функция СформироватьЗапросПоДекретницам(СписокВидовРасчета)
		Запрос = Новый Запрос;
		...
		|	РаботникиОрганизацийСрезПоследних.ИтоговоеПовышениеПоИнструкции
		|ИЗ
		|	РегистрСведений.РаботникиОрганизаций.СрезПоследних(&Период, ) КАК РаботникиОрганизацийСрезПоследних
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СостояниеРаботниковОрганизаций КАК СостояниеРаботниковОрганизаций
		|		ПО РаботникиОрганизацийСрезПоследних.Организация = СостояниеРаботниковОрганизаций.Организация
		|			И РаботникиОрганизацийСрезПоследних.Сотрудник = СостояниеРаботниковОрганизаций.Сотрудник
		...
		|		ПО РаботникиОрганизацийСрезПоследних.Сотрудник = ПлановыеНачисленияРаботниковОрганизаций.Сотрудник
		|ГДЕ
		|	СостояниеРаботниковОрганизаций.Состояние В(&Состояние)
		|	И &Период МЕЖДУ СостояниеРаботниковОрганизаций.Период И ДобавитьКДате(СостояниеРаботниковОрганизаций.ПериодЗавершения,День,-1) //О.Т. Аверсон 15.05.2019 № 4345 (-1)
		|	И РаботникиОрганизацийСрезПоследних.ПодразделениеОрганизации.Владелец = &Организация
		...
		//О.Т. Аверсон 28.08.2017 № 977
		|	И ВЫБОР
		|	КОГДА СостояниеРаботниковОрганизаций.Регистратор ССЫЛКА Документ.ОтпускПоУходуЗаРебенком
		|		ТОГДА ВЫБОР
		|				КОГДА СостояниеРаботниковОрганизаций.Регистратор.ОсвобождатьСтавку
		|					ТОГДА ЛОЖЬ
		|				ИНАЧЕ ИСТИНА
		|			КОНЕЦ
		|	ИНАЧЕ ИСТИНА
	...
	Процедура ВывестиСтрокиПодразделения(ВременнаяТЗ, ДокументРезультат,ТаблицаВакантно,ВторойОтчет,ФормаВыборОтчета)
		...
		Для Каждого ТекСтрокаТЗ Из ВременнаяТЗ Цикл 
			Если (ЗначениеЗаполнено(ТекПодр)) и (ТекПодр <> ТекСтрокаТЗ.Подразделение) Тогда
				Если ФондЗПИтог <> 0 или СтавокИтог <> 0 или ТарифнаяСтавкаИтог <> 0 или СтавкаИтог <> 0 или ЗаСложностьИтог <> 0 Тогда 
					...
					ОбластьДетальныхЗаписей.Параметры.ТарифнаяСтавкаИтог = ТарифнаяСтавкаИтог;
					...
					ОбластьДетальныхЗаписей.Параметры.СтавкаИтог = СтавкаИтог;
					ДокументРезультат.Присоединить(ОбластьДетальныхЗаписей);
					...
					Если ВидРасчета.Количество() > 0 Тогда
						...
						Для Каждого ТекСтрока из ТЗ_ПоПодразделению Цикл
							...
							ОбластьДетальныхЗаписей.Параметры.ЗаСложностьИтог = ТекСтрока.Сумма;
							ДокументРезультат.Присоединить(ОбластьДетальныхЗаписей);
						КонецЦикла;
					КонецЕсли;
					
					ОбластьДетальныхЗаписей = мМакет.ПолучитьОбласть("Итого|Область5");
					ОбластьДетальныхЗаписей.Параметры.ФондЗПИтог = ФондЗПИтог;				
					ДокументРезультат.Присоединить(ОбластьДетальныхЗаписей);
			...
			Если ТекПодр <> ТекСтрокаТЗ.Подразделение Тогда
				...
				Иначе
					...
					Если СтавкаПервогоНеОсн <> СтавкаПервогоОсновная Тогда
						стрТС = "Тарифная ставка первого разряда - " + Строка(СтавкаПервогоНеОсн) + " руб.";
					КонецЕсли;
			...
			Если ЗначениеЗаполнено(ТекСтрокаТЗ.Должность) Тогда
				...
				Если Факт Тогда
					Если ТипЗнч(ТекСтрокаТЗ.РазмерСтавки1Разряда) = Тип("СправочникСсылка.ТарифныеСтавкиПервогоРазряда") Тогда
						...
						//Островская Татьяна 13.01.2014 
						СтавкаПервого = ОбщегоНазначения.ОкруглитьЗначение2016(ПараметрыСтавки.Размер,ДатаАктуальности);
					Иначе
						//Островская Татьяна 13.01.2014 
						СтавкаПервого = ОбщегоНазначения.ОкруглитьЗначение2016(ТекСтрокаТЗ.РазмерСтавки1Разряда,ДатаАктуальности);//Островская Татьяна 19.12.2013 № 16976;
						//Островская Татьяна 13.01.2014
					КонецЕсли;
				Иначе
					//Островская Татьяна 13.01.2014 
				...
				мТарифнаяСтавкаРазряд = ОбщегоНазначения.ОкруглитьЗначение2016(ТекСтрокаТЗ.КоэффициентЕТС * СтавкаПервого,ДатаАктуальности);//О.Т. Аверсон 18.03.2016 № 949
				
				ОбластьДетальныхЗаписей.Параметры.Должность = ТекСтрокаТЗ.Должность;
				если ФормаВыборОтчета = "Форма2" Тогда
					ОбластьДетальныхЗаписей.Параметры.Категория = ТекСтрокаТЗ.Должность.Категория;//Островская Татьяна 22.01.2014
				КонецЕсли;
				...
				//Островская Татьяна 18.12.2013 № 16976
				если ВторойОтчет Тогда 
					ОбластьДетальныхЗаписей.Параметры.СтавкаПервого = СтавкаПервого;
				КонецЕсли;
				//Островская Татьяна 18.12.2013 № 16976
				//ОбластьДетальныхЗаписей.Параметры.ТарифнаяСтавкаРазряд = мТарифнаяСтавкаРазряд*ТекСтрокаТЗ.ЗанимаемыхСтавок;
				
				//Аверсон ЕАЕ 08.02.2024 СВФР-012191
				Если ВторойОтчет И Факт И ЗначениеЗаполнено(ТекСтрокаТЗ.Сотрудник) И Организация.ИНН = "101542412" Тогда
					ОбластьДетальныхЗаписей.Параметры.ТарифнаяСтавкаРазряд = ТекСтрокаТЗ.ТарифнаяСтавкаРазряд;
				Иначе
				//Аверсон СВФР-012191
					ОбластьДетальныхЗаписей.Параметры.ТарифнаяСтавкаРазряд = мТарифнаяСтавкаРазряд*?(ВторойОтчет И НЕ Факт,1,ТекСтрокаТЗ.ЗанимаемыхСтавок); //Аверсон 09.04.2021 КВВ СВФР-007390
				КонецЕсли;
				...
				//Островская Татьяна 07.03.2013 № 16283
				если Факт Тогда 
					Для Каждого СтрТЗ из ТЗ_ТаблицаПовышенийОр Цикл
						ПроцентПровышения = 0; 
						НайденнаяСтр = ТаблицаПовышений.НайтиСтроки(
							Новый Структура(
								"Сотрудник, Коэффициент", 
								?(
									НЕ ЗначениеЗаполнено(ТекСтрокаТЗ.Декретница), 
									ТекСтрокаТЗ.Сотрудник, 
									ТекСтрокаТЗ.Декретница
								), 
								СтрТЗ.Коэффициент)
						);
						Если НайденнаяСтр.Количество() = 0 Тогда
							НайденнаяСтр = ТаблицаВакантноКоэф.НайтиСтроки(
								Новый Структура(
									//+ Ништ 28.01.2026 свфр-017928: дублируются строки в отчете если вводятся две плановые ставки и одна закрыта не полностью
									//"ПодразделениеОрганизации, Должность, МесячнаяТарифнаяСтавка, Коэффициент", 
									//ТекСтрокаТЗ.Подразделение, ТекСтрокаТЗ.Должность, ТекСтрокаТЗ.МТСтавка, СтрТЗ.Коэффициент
									"ПодразделениеОрганизации, Должность, Сотрудник, Коэффициент", 
									ТекСтрокаТЗ.Подразделение, ТекСтрокаТЗ.Должность, ТекСтрокаТЗ.КодСтроки, СтрТЗ.Коэффициент
									//- Ништ 28.01.2026 свфр-017928
								)
							);
							если НайденнаяСтр.Количество()>0 Тогда 
								если ВторойОтчет Тогда
									//Аверсон 27.05.2021 КВВ СВФР-007550 (
									Если Организация.ИНН = "101542412" Тогда  //Минск-Новости
										//Аверсон 11.11.2021 КВВ СВФР-008363 (
										//ПроцентПровышения = НайденнаяСтр[0].РазмерПовышения;
										//ПроцентПовышенияИтог = ПроцентПовышенияИтог + НайденнаяСтр[0].РазмерПовышения;
										Если НайденнаяСтр.Количество() = 1 Тогда
											ПроцентПровышения = НайденнаяСтр[0].РазмерПовышения;
											ПроцентПовышенияИтог = ПроцентПовышенияИтог + НайденнаяСтр[0].РазмерПовышения;
										Иначе
											Для Каждого СтрНайденнаяСтр ИЗ НайденнаяСтр Цикл
												ЗРП = Новый Запрос;
												ЗРП.Текст = "
												|ВЫБРАТЬ
												|ПлановоеШтатноеРасписаниеОрганизацийПовышенияПоИнструкции.Коэффициент,
												...
												|ПО ПлановоеШтатноеРасписаниеОрганизацийПовышенияПоИнструкции.КодСтроки = ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.КодСтроки
												|ГДЕ
												|ПлановоеШтатноеРасписаниеОрганизацийПовышенияПоИнструкции.Ссылка = &Док
												|И ПлановоеШтатноеРасписаниеОрганизацийПовышенияПоИнструкции.Коэффициент = &Коэффициент
												|И ПлановоеШтатноеРасписаниеОрганизацийПовышенияПоИнструкции.Размер = &РазмерПовышения
												|И ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.Должность = &Должность
												|И ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.ПодразделениеОрганизации = &ПодразделениеОрганизации";
												ЗРП.УстановитьПараметр("Док",СтрНайденнаяСтр.Регистратор);
												ЗРП.УстановитьПараметр("Коэффициент",СтрНайденнаяСтр.Коэффициент);
												ЗРП.УстановитьПараметр("Должность",СтрНайденнаяСтр.Должность);
												ЗРП.УстановитьПараметр("РазмерПовышения",СтрНайденнаяСтр.РазмерПовышения);
												ЗРП.УстановитьПараметр("ПодразделениеОрганизации",СтрНайденнаяСтр.ПодразделениеОрганизации);
												РезЗРП = ЗРП.Выполнить().Выгрузить();
												Если РезЗРП.Количество() > 0 Тогда
													ПроцентПровышения = СтрНайденнаяСтр.РазмерПовышения;
													ПроцентПовышенияИтог = ПроцентПовышенияИтог + СтрНайденнаяСтр.РазмерПовышения;
													Прервать;
												КонецЕсли;
											КонецЦикла;
										КонецЕсли;
									...
									Иначе
										//Аверсон 27.05.2021 КВВ СВФР-007550 )
										//+ Ништ 28.01.2026 свфр-017928: дублируются строки в отчете если вводятся две плановые ставки и одна закрыта не полностью
										//ПроцентПровышения = (НайденнаяСтр[0].РазмерПовышения-1)*100;//Островская Татьяна 18.12.2013 № 16976
										//ПроцентПовышенияИтог = (ПроцентПовышенияИтог + НайденнаяСтр[0].РазмерПовышения-1)*100; //Островская Татьяна 18.12.2013 № 16976
										ПроцентПровышения = НайденнаяСтр[0].РазмерПовышения;
										ПроцентПовышенияИтог = ПроцентПовышенияИтог + ПроцентПровышения;
										//- Ништ 28.01.2026 свфр-017928
									КонецЕсли;
								иначе
								...
								//Аверсон 13.07.2023 КВВ СВФР-011103 (
									Иначе 
								Если Организация.ИНН = "101542412" Тогда//Минск-Новости
									ЗНов = Новый Запрос;
									ЗНов.Текст = "ВЫБРАТЬ   
									|ПлановоеШтатноеРасписаниеОрганизацийПовышенияПоИнструкции.Ссылка.Дата  КАК Период,
									|ВЫБОР КОГДА 
									|ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.ЗанимаемыхСтавок > 0
									|ТОГДА ВЫРАЗИТЬ( ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.МесячнаяТарифнаяСтавка/ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.ЗанимаемыхСтавок КАК ЧИСЛО(15,2))
									|ИНАЧЕ 0
									|КОНЕЦ КАК МТС,
									|ПлановоеШтатноеРасписаниеОрганизацийПовышенияПоИнструкции.Ссылка как Рег,
									|ПлановоеШтатноеРасписаниеОрганизацийПовышенияПоИнструкции.Коэффициент,
									|ПлановоеШтатноеРасписаниеОрганизацийПовышенияПоИнструкции.Размер
									|ПОМЕСТИТЬ ВтКоэф
									|ИЗ
									...
									//Аверсон ЕАЕ 26.01.2024 СВФР-012191
									|И ПлановоеШтатноеРасписаниеОрганизацийПовышенияПоИнструкции.Ссылка = ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.Ссылка
									//Аверсон СВФР-012191
									
									|ГДЕ
									|ПлановоеШтатноеРасписаниеОрганизацийПовышенияПоИнструкции.Коэффициент = &Коэффициент
									|И ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.Должность = &Должность
									|И ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.ПодразделениеОрганизации = &ПодразделениеОрганизации
									
									//Аверсон ЕАЕ 26.01.2024 СВФР-012191
									|И ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.ССылка = &Регистратор
									//Аверсон СВФР-012191
									
									|;
									|ВЫБРАТЬ
									|МАКСИМУМ(ВтКоэф.Период) КАК Период
									|ПОМЕСТИТЬ ВтКоэфПериод
									|ИЗ ВтКоэф
									|;
									|ВЫБРАТЬ
									|ВтКоэф.Период,
									...
									|ВтКоэф.Рег
									|ИЗ ВтКоэф
									|ЛЕВОЕ СОЕДИНЕНИЕ ВтКоэфПериод
									|ПО ВтКоэф.Период = ВтКоэфПериод.Период
									|ГДЕ ВтКоэф.МТС = &МТС
									|И ВтКоэф.Период = ВтКоэфПериод.Период";
									ЗНов.УстановитьПараметр("Должность",ТекСтрокаТЗ.Должность);
									ЗНов.УстановитьПараметр("ПодразделениеОрганизации",ТекСтрокаТЗ.Подразделение);
									ЗНов.УстановитьПараметр("Коэффициент",СтрТЗ.Коэффициент);
									ЗНов.УстановитьПараметр("МТС",ТекСтрокаТЗ.МТСтавка);
									
									//Аверсон ЕАЕ 26.01.2024 СВФР-012191
									ЗНов.УстановитьПараметр("Регистратор", ТекСтрокаТЗ.Регистратор);
									//Аверсон СВФР-012191
									
									РезЗНов = ЗНов.Выполнить().Выгрузить();
									Если РезЗНов.Количество() > 0 Тогда
										ПроцентПровышения = РезЗНов[0].Размер;
										ПроцентПовышенияИтог = ПроцентПовышенияИтог + РезЗНов[0].Размер;
									//	Прервать;
									КонецЕсли;
								КонецЕсли;
								//Аверсон 13.07.2023 КВВ СВФР-011103 )
						...
						Иначе
							...
							//	Иначе
							//		Аверсон 27.05.2021 КВВ СВФР-007550 )
							//		ПроцентПровышения = (НайденнаяСтр[0].Размер-1)*100;//Островская Татьяна 18.12.2013 № 16976
							//		ПроцентПовышенияИтог = (ПроцентПовышенияИтог + НайденнаяСтр[0].Размер-1)*100; //Островская Татьяна 18.12.2013 № 16976
							//	КонецЕсли;
							//иначе  повышения вводится процентами а не коэффи
								
								ПроцентПровышения = НайденнаяСтр[0].Размер;
								ПроцентПовышенияИтог = ПроцентПовышенияИтог + НайденнаяСтр[0].Размер;
							//КонецЕсли;	
						КонецЕсли;
						если не ВторойОтчет Тогда 
							ОбластьДетальныхЗаписей.Параметры.ПроцентПровышения = ПроцентПровышения;
						иначе
							ОбластьДетальныхЗаписей.Параметры.ПроцентПровышения = ?(ЗначениеЗаполнено(ОбластьДетальныхЗаписей.Параметры.ПроцентПровышения),ОбластьДетальныхЗаписей.Параметры.ПроцентПровышения,0)+ПроцентПровышения;
						КонецЕсли;
						//Островская Татьяна 18.12.2013 № 16976
						если ВторойОтчет Тогда
							если ЗначениеЗаполнено(ПроцентПровышения) Тогда 
								ОбластьДетальныхЗаписей.Параметры.ПроцентПровышенияСумма = ?(ЗначениеЗаполнено(ОбластьДетальныхЗаписей.Параметры.ПроцентПровышенияСумма),ОбластьДетальныхЗаписей.Параметры.ПроцентПровышенияСумма,0)+
								ОбщегоНазначения.ОкруглитьЗначение2016(мТарифнаяСтавкаРазряд*ПроцентПровышения/100,ДатаАктуальности)*?(ТекСтрокаТЗ.ЗанимаемыхСтавок<1,ТекСтрокаТЗ.ЗанимаемыхСтавок,1); //(округление)   человечее
							иначе
								ОбластьДетальныхЗаписей.Параметры.ПроцентПровышенияСумма = ?(ЗначениеЗаполнено(ОбластьДетальныхЗаписей.Параметры.ПроцентПровышенияСумма),ОбластьДетальныхЗаписей.Параметры.ПроцентПровышенияСумма,0)+0*?(ТекСтрокаТЗ.ЗанимаемыхСтавок<1,ТекСтрокаТЗ.ЗанимаемыхСтавок,1);
							КонецЕсли;
						...
						КонецЕсли;
					КонецЦикла;	
					если ВторойОтчет и ТЗ_ТаблицаПовышенийОр.Количество()>0 Тогда 
						ДокументРезультат.Присоединить(ОбластьДетальныхЗаписей);
					КонецЕсли;
				...
				иначе
					Для Каждого СтрТЗ из ТЗ_ТаблицаПовышенийОр Цикл
						ПроцентПровышения = 0;
						//Аверсон 09.04.2021 КВВ СВФР-007390 (
						Если ВторойОтчет и НЕ факт Тогда
							
							//Аверсон ЕАЕ 28.10.2025 СВФР-016913
							Если Организация.ИНН = "101542412" Тогда
								НайденнаяСтр = ТаблицаПовышений.НайтиСтроки(Новый Структура("ПодразделениеОрганизации,Должность,МесячнаяТарифнаяСтавка, Коэффициент, ГрафикРаботы", ТекСтрокаТЗ.Подразделение, ТекСтрокаТЗ.Должность, ТекСтрокаТЗ.МТСтавка, СтрТЗ.Коэффициент, ТекСтрокаТЗ.ГрафикРаботы));
							Иначе
							//Аверсон СВФР-016913
							
								НайденнаяСтр = ТаблицаПовышений.НайтиСтроки(Новый Структура("ПодразделениеОрганизации,Должность,МесячнаяТарифнаяСтавка, Коэффициент", ТекСтрокаТЗ.Подразделение, ТекСтрокаТЗ.Должность, ТекСтрокаТЗ.МТСтавка, СтрТЗ.Коэффициент));
							КонецЕсли;
						Иначе
							//Аверсон 09.04.2021 КВВ СВФР-007390 )
							НайденнаяСтр = ТаблицаПовышений.НайтиСтроки(Новый Структура("ПодразделениеОрганизации,Должность,МесячнаяТарифнаяСтавка, Коэффициент", ТекСтрокаТЗ.Подразделение, ТекСтрокаТЗ.Должность, ТекСтрокаТЗ.Ставка, СтрТЗ.Коэффициент));
						КонецЕсли;
						Если НайденнаяСтр.Количество() = 0 Тогда
						Иначе
							Если Не ВторойОтчет Тогда //Аверсон 09.04.2021 КВВ СВФР-007390
								ПроцентПровышения = НайденнаяСтр[0].Размер;
								ПроцентПовышенияИтог = ПроцентПовышенияИтог + НайденнаяСтр[0].Размер;
								//Аверсон 09.04.2021 КВВ СВФР-007390 (
							Иначе
								Для Каждого СтрНС ИЗ НайденнаяСтр Цикл
									ПроцентПровышения = ПроцентПровышения + СтрНС.Размер;
									ПроцентПовышенияИтог = ПроцентПовышенияИтог + СтрНС.Размер;
								КонецЦикла;
							КонецЕсли;
							//Аверсон 09.04.2021 КВВ СВФР-007390 )
						КонецЕсли;
						...
						если ВторойОтчет Тогда
							...
							//Аверсон ЕАЕ 10.10.2025 СВФР-016913
							//ОбластьДетальныхЗаписей.Параметры.ПроцентПровышенияСумма = ?(ЗначениеЗаполнено(ОбластьДетальныхЗаписей.Параметры.ПроцентПровышенияСумма),ОбластьДетальныхЗаписей.Параметры.ПроцентПровышенияСумма,0)+Окр(мТарифнаяСтавкаРазряд*ПроцентПровышения/100)*?(ТекСтрокаТЗ.ЗанимаемыхСтавок<1,ТекСтрокаТЗ.ЗанимаемыхСтавок,1); 
							ОбластьДетальныхЗаписей.Параметры.ПроцентПровышенияСумма = ?(ЗначениеЗаполнено(ОбластьДетальныхЗаписей.Параметры.ПроцентПровышенияСумма),ОбластьДетальныхЗаписей.Параметры.ПроцентПровышенияСумма,0)+Окр(мТарифнаяСтавкаРазряд*ПроцентПровышения/100,2)*?(ТекСтрокаТЗ.ЗанимаемыхСтавок<1,ТекСтрокаТЗ.ЗанимаемыхСтавок,1); 
							//Аверсон СВФР-016913
				...
				//Аверсон 13.07.2023 КВВ СВФР-011103 (
				Если Организация.ИНН = "101542412" И НЕ ЗначениеЗаполнено(ТекСтрокаТЗ.Сотрудник)   //Минск-Новости , вакансия  
					И Факт Тогда //СВЭЙ 19.02.2024 КВВ СВФР-012329
					ОбластьДетальныхЗаписей.Параметры.ПроцентПоКонтракту = "";
				КонецЕсли;
				//Аверсон 13.07.2023 КВВ СВФР-011103 )
				
				если ВторойОтчет Тогда 
					ОбластьДетальныхЗаписей.Параметры.СтавкаПовышение = ОбщегоНазначения.ОкруглитьЗначение2016(ТекСтрокаТЗ.ФондЗП,ДатаАктуальности);//О.Т. Аверсон 18.03.2016 № 949
					//ОбластьДетальныхЗаписей.Параметры.ПроцентПоКонтрактуСумма = ОбщегоНазначения.ОкруглитьЗначение2016(ТекСтрокаТЗ.ФондЗП *?(ЗначениеЗаполнено(ТекСтрокаТЗ.ПроцентПоКонтракту),ТекСтрокаТЗ.ПроцентПоКонтракту,0)/100,ДатаАктуальности);//О.Т. Аверсон 18.03.2016 № 949
					//ОбластьДетальныхЗаписей.Параметры.ПроцентПоКонтрактуСумма = ОбщегоНазначения.ОкруглитьЗначение2016(?(НЕ Факт,мТарифнаяСтавкаРазряд,ТекСтрокаТЗ.ФондЗП) *?(ЗначениеЗаполнено(ТекСтрокаТЗ.ПроцентПоКонтракту),ТекСтрокаТЗ.ПроцентПоКонтракту,0)/100,ДатаАктуальности); //Аверсон 09.04.2021 КВВ СВФР-007390
					ОбластьДетальныхЗаписей.Параметры.ПроцентПоКонтрактуСумма = ОбщегоНазначения.ОкруглитьЗначение2016(мТарифнаяСтавкаРазряд *?(ЗначениеЗаполнено(ТекСтрокаТЗ.ПроцентПоКонтракту),ТекСтрокаТЗ.ПроцентПоКонтракту,0)/100,ДатаАктуальности); //Аверсон 27.05.2021 КВВ СВФР-007550
					
					//СВЭЙ 14.03.2024 КВВ СВФР-012329 (
					//СВЭЙ 23.02.2024 КВВ СВФР-012329 (
					//Если мТарифнаяСтавкаРазряд = 0 И Факт И ЗначениеЗаполнено(ТекСтрокаТЗ.Сотрудник) И ТекСтрокаТЗ.ПроцентПоКонтракту <> 0 Тогда
					//	СотрБезЕТС = РегистрыСведений.РаботникиОрганизаций.ПолучитьПоследнее(ДатаАктуальности,Новый Структура("Сотрудник",ТекСтрокаТЗ.Сотрудник)).БезЕТС;
					//	Если СотрБезЕТС Тогда
					//		ОбластьДетальныхЗаписей.Параметры.ПроцентПоКонтрактуСумма = ОбщегоНазначения.ОкруглитьЗначение2016(ТекСтрокаТЗ.ФондЗП * ТекСтрокаТЗ.ПроцентПоКонтракту/100,ДатаАктуальности);
					//	КонецЕсли;
					//КонецЕсли;
					//СВЭЙ 23.02.2024 КВВ СВФР-012329 )
					
					Если мТарифнаяСтавкаРазряд = 0 И Факт И ТекСтрокаТЗ.ПроцентПоКонтракту <> 0 Тогда
						ОбластьДетальныхЗаписей.Параметры.ПроцентПоКонтрактуСумма = ОбщегоНазначения.ОкруглитьЗначение2016(ТекСтрокаТЗ.ФондЗП * ТекСтрокаТЗ.ПроцентПоКонтракту/100,ДатаАктуальности);
					КонецЕсли;
				...
				КонецЕсли;
				//Островская Татьяна 18.12.2013 № 16976
				ОбластьДетальныхЗаписей.Параметры.Ставка = ТекСтрокаТЗ.Ставка;
				
				//Аверсон 13.07.2023 КВВ СВФР-011103 (
				СтавкаРасчетная = ТекСтрокаТЗ.Ставка;
				
				//Аверсон ЕАЕ 20.10.2025 СВФР-016913
				СтавкаПовышение = 0; 
				//Аверсон СВФР-016913
				
				Если Организация.ИНН = "101542412" И НЕ ЗначениеЗаполнено(ТекСтрокаТЗ.Сотрудник) Тогда //Минск-Новости
					
					//Аверсон ЕАЕ 20.10.2025 СВФР-016913
					СтавкаПовышение = ОбластьДетальныхЗаписей.Параметры.СтавкаПовышение; 
					Если ТекСтрокаТЗ.ЗанимаемыхСтавок < 1 Тогда 
						СтавкаРасчетная = СтавкаПовышение;	
					Иначе
					//Аверсон СВФР-016913
						
						СтавкаРасчетная = Окр(ТекСтрокаТЗ.Ставка*ТекСтрокаТЗ.ЗанимаемыхСтавок,2);
					КонецЕсли;
				
					ОбластьДетальныхЗаписей.Параметры.Ставка = СтавкаРасчетная;
				КонецЕсли;
				//Аверсон 13.07.2023 КВВ СВФР-011103 )
				...
				Если ВидРасчета.Количество() > 0 Тогда
					Для Каждого ТекСтрока из ВидРасчета Цикл
						...
						//Островская Татьяна 19.12.2013 № 16976
						если Факт Тогда 
						...
						иначе
							мСтрокаТЧ = ТаблицаНадбавок.НайтиСтроки(Новый Структура("ВидРасчета", ТекСтрока.Значение));
						КонецЕсли;
						//Островская Татьяна 19.12.2013 № 16976
						Если мСтрокаТЧ.Количество() <> 0 Тогда
							Надб1 = мСтрокаТЧ[0].Показатель;
							СуммаНадб1 = ОбщегоНазначения.ОкруглитьЗначение2016(ТекСтрокаТЗ.Ставка * мСтрокаТЧ[0].Показатель / 100,ДатаАктуальности);//О.Т. Аверсон 18.03.2016 № 949
						Иначе
							Надб1 = 0;
							СуммаНадб1 = 0;
						КонецЕсли;
						ОбластьДетальныхЗаписей.Параметры.Надб1 = Надб1;
						ОбластьДетальныхЗаписей.Параметры.СуммаНадб1 = СуммаНадб1;
				...
				//Аверсон 09.04.2021 КВВ СВФР-007390 (
				Если ВторойОтчет и НЕ факт Тогда
					ФондЗП = (ТекСтрокаТЗ.МТСтавка + СуммаНадбПоСотруднику);
				Иначе
					//Аверсон 09.04.2021 КВВ СВФР-007390 )
					
					//Аверсон 27.05.2021 КВВ СВФР-007550 (
					Если НЕ ЗначениеЗаполнено(ТекСтрокаТЗ.Сотрудник) И ТекСтрокаТЗ.ЗанимаемыхСтавок < 1 И Организация.ИНН = "101542412" Тогда //ИЛИ Организация.ИНН = "691159723") Тогда  //Минск-Новости (09.10.2025 ЕАЕ СВФР-016997)

						//Аверсон ЕАЕ 20.10.2025 СВФР-016913
						//ФондЗП = (ТекСтрокаТЗ.Ставка + СуммаНадбПоСотруднику)*ТекСтрокаТЗ.ЗанимаемыхСтавок;
						ФондЗП = СтавкаПовышение + СуммаНадбПоСотруднику * ТекСтрокаТЗ.ЗанимаемыхСтавок;
						//Аверсон СВФР-016913
					...
					Иначе
						...
						//Аверсон ЕАЕ 10.10.2025 СВФР-016997
						Если ВторойОтчет И Факт И ЗначениеЗаполнено(ТекСтрокаТЗ.Сотрудник) И ТекСтрокаТЗ.ЗанимаемыхСтавок < 1 И Организация.ИНН = "691159723" Тогда
							ФондЗП = (ТекСтрокаТЗ.Ставка + СуммаНадбПоСотруднику) * ТекСтрокаТЗ.ЗанимаемыхСтавок;
						Иначе
						//Аверсон СВФР-016997
						
							ФондЗП = (ТекСтрокаТЗ.Ставка + СуммаНадбПоСотруднику)*?(ТекСтрокаТЗ.ЗанимаемыхСтавок>1,ТекСтрокаТЗ.ЗанимаемыхСтавок,1);
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
				
				ОбластьДетальныхЗаписей.Параметры.ФондЗП = ФондЗП;//*ТекСтрокаТЗ.ЗанимаемыхСтавок;				
				...
				ТарифнаяСтавкаИтог = ТарифнаяСтавкаИтог + (мТарифнаяСтавкаРазряд*?(ВторойОтчет И НЕ Факт,1,ТекСтрокаТЗ.ЗанимаемыхСтавок)); //Аверсон 09.04.2021 КВВ СВФР-007390
				//СтавкаИтог = СтавкаИтог + ТекСтрокаТЗ.Ставка;
				СтавкаИтог = СтавкаИтог + СтавкаРасчетная; //Аверсон 13.07.2023 КВВ СВФР-011103
				ФондЗПИтог = ФондЗПИтог + ФондЗП;
				ЗаСложностьИтог = ЗаСложностьИтог + СуммаНадб1;
				
				СтавокИтогПоПредпр = СтавокИтогПоПредпр + ТекСтрокаТЗ.ЗанимаемыхСтавок;
				ТарифнаяСтавкаИтогПоПредпр = ТарифнаяСтавкаИтогПоПредпр + (мТарифнаяСтавкаРазряд*?(ВторойОтчет И НЕ Факт,1,ТекСтрокаТЗ.ЗанимаемыхСтавок)); //Аверсон 09.04.2021 КВВ СВФР-007390
				//СтавкаИтогПоПредпр = СтавкаИтогПоПредпр + ТекСтрокаТЗ.Ставка;
				СтавкаИтогПоПредпр = СтавкаИтогПоПредпр + СтавкаРасчетная; //Аверсон 13.07.2023 КВВ СВФР-011103
				ФондЗПИтогПоПредпр = ФондЗПИтогПоПредпр + ФондЗП;
				ЗаСложностьИтогПоПредпр = ЗаСложностьИтогПоПредпр + СуммаНадб1;
			КонецЕсли;
	...
	Процедура ЗаполнимВременнуюТаблицу(ВременнаяТЗ, ВыборкаСтрок, ТаблицаВакантно)
		...
		//Аверсон ЕАЕ 28.10.2025 СВФР-016913
		Если Организация.ИНН = "101542412" И ВторойОтчет И Не Факт Тогда
			ВременнаяТЗ.Колонки.Добавить("ГрафикРаботы", Новый ОписаниеТипов("СправочникСсылка.ГрафикиРаботы"));	
		КонецЕсли;
		//Аверсон СВФР-016913
		...
		Пока ВыборкаСтрок.Следующий() Цикл 
			...
			Если ЗначениеЗаполнено(ВыборкаСтрок.Должность) Тогда
				...
				НоваяСтрока.РазрядЕТС              = ВыборкаСтрок.РазрядЕТС;
				...
				КонецЕсли;
				мТарифнаяСтавкаРазряд = ОбщегоНазначения.ОкруглитьЗначение2016(ВыборкаСтрок.КоэффициентЕТС * СтавкаПервого * ВыборкаСтрок.ЗанимаемыхСтавок,ДатаАктуальности);//О.Т. Аверсон 18.03.2016 № 949
				...
				НоваяСтрока.Ставка                 = ?(ВторойОтчет И НЕ Факт,Окр(ВыборкаСтрок.Ставка/ВыборкаСтрок.ЗанимаемыхСтавок,2),ВыборкаСтрок.Ставка); //Аверсон 09.04.2021 КВВ СВФР-007390
				НоваяСтрока.МТСтавка                 = ВыборкаСтрок.Ставка;//Островская Татьяна 24.03.2014
				
				СуммаНадбПоСотруднику = 0;
				//Островская Татьяна 09.06.2014 № 17120
				//Если ВидРасчета.Количество() > 0 Тогда
				//	Для Каждого ТекСтрока из ВидРасчета Цикл
				//		СуммаНадб = 0; 
				//		мСтрокаТЧ = ТаблицаНадбавок.НайтиСтроки(Новый Структура("Сотрудник, ВидРасчета", ВыборкаСтрок.Сотрудник, ТекСтрока.Значение));
				//		Если мСтрокаТЧ.Количество() <> 0 Тогда
				//			СуммаНадб = Окр(ВыборкаСтрок.Ставка * мСтрокаТЧ[0].Показатель / 100);
				//		Иначе
				//			СуммаНадб = 0;
				//		КонецЕсли;
				...
				//НоваяСтрока.ФондЗП                 = ВыборкаСтрок.ТарифныйСПовышениемПоИнструкции + СуммаНадбПоСотруднику;
				НоваяСтрока.ФондЗП = ?(ВторойОтчет И НЕ Факт,?(ВыборкаСтрок.ЗанимаемыхСтавок=0,0,ВыборкаСтрок.Ставка),ВыборкаСтрок.ТарифныйСПовышениемПоИнструкции) + СуммаНадбПоСотруднику; //Аверсон 09.04.2021 КВВ СВФР-007390
				НоваяСтрока.Сотрудник              = ВыборкаСтрок.Сотрудник;
				//Если (РольДоступна("ОК")) и
				Если Факт Тогда
				...
				//Аверсон ЕАЕ 28.10.2025 СВФР-016913
				Если Организация.ИНН = "101542412" И ВторойОтчет И Не Факт Тогда
					НоваяСтрока.ГрафикРаботы = ВыборкаСтрок.ГрафикРаботы;	
				КонецЕсли;
				//Аверсон СВФР-016913
		...
		//Аверсон ЕАЕ 26.01.2024 СВФР-012191
		Если Организация.ИНН = "101542412" Тогда
			ВременнаяТЗ.Колонки.Добавить("Регистратор", Новый ОписаниеТипов("ДокументСсылка.ПлановоеШтатноеРасписаниеОрганизаций"));
		КонецЕсли;
		//Аверсон СВФР-012191
		...
		Для Каждого ТекСтрокаВакантно Из ТаблицаВакантно Цикл
			...
			//Островская Татьяна 01.04.2014 Добавление декретниц не нужно - ВЕРНУЛИ ОБРАТНО!!!
			
			//Аверсон ЕАЕ 17.11.2025 СВФР-017223
			//НайтиСтроки = ТЗ_Декретницы.НайтиСтроки(Новый Структура("ПодразделениеОрганизации, Должность, РазрядЕТС,ИтоговоеПовышениеПоИнструкции,ПроцентПоКонтракту", ТекСтрокаВакантно.ПодразделениеОрганизации, ТекСтрокаВакантно.Должность, РазрядЕТС,ТекСтрокаВакантно.ИтоговоеПовышениеПоИнструкции,ТекСтрокаВакантно.ПроцентПоКонтракту));
			НайтиСтроки = ТЗ_Декретницы.НайтиСтроки(Новый Структура("ПодразделениеОрганизации, Должность, РазрядЕТС,ИтоговоеПовышениеПоИнструкции,ПроцентПоКонтракту,Ставка", ТекСтрокаВакантно.ПодразделениеОрганизации, ТекСтрокаВакантно.Должность, РазрядЕТС,ТекСтрокаВакантно.ИтоговоеПовышениеПоИнструкции,ТекСтрокаВакантно.ПроцентПоКонтракту,ТекСтрокаВакантно.МесячнаяТарифнаяСтавка));
			//Аверсон ЕАЕ СВФР-017223
			
			Если НайтиСтроки = Неопределено Тогда	
			Иначе
				Для Каждого ТекСтрокаД Из НайтиСтроки Цикл
					
					//проверим, может эти декретницы есть в списке
					СтрокиПроверки = ВременнаяТЗ.НайтиСтроки(Новый Структура("СотрудникЗаменяемый", ТекСтрокаД.Сотрудник));
					...
					//Аверсон ЕАЕ 26.01.2024 СВФР-012191
					Если Организация.ИНН = "101542412" Тогда
						НоваяСтрока.Регистратор        = ТекСтрокаВакантно.Регистратор;
					КонецЕсли;
					//Аверсон СВФР-012191
					
					//НоваяСтрока.ИтоговоеПовышениеПоИнструкции     = ТекСтрокаВакантно.ИтоговоеПовышениеПоИнструкции; //Островская Татьяна 07.03.2013 № ВИС-00000283
					ПроцентПовышенияИтог = 0;
					Для Каждого СтрТЗ из ТЗ_ТаблицаПовышенийОр Цикл
						ПроцентПровышения = 0;
						...
						КонецЕсли;
					КонецЦикла;	
					мТарифнаяСтавкаРазряд = ОбщегоНазначения.ОкруглитьЗначение2016(ТекСтрокаВакантно.КоэффициентЕТС * ТекСтрокаВакантно.РазмерСтавки1Разряда * ТекСтрокаД.ЗанимаемыхСтавок,ДатаАктуальности);//О.Т. Аверсон 18.03.2016 № 949
					
					//НоваяСтрока.Ставка                 = ТекСтрокаД.Ставка;
					
					//Аверсон 13.10.2022 КВВ СВФР-009643 (
					МТС = ТекСтрокаВакантно.МесячнаяТарифнаяСтавка;
					Если Организация.ИНН = "190288027" Тогда
						Если ТекСтрокаВакантно.МесячнаяТарифнаяСтавка > ТекСтрокаД.Ставка 
							И ТекСтрокаД.ЗанимаемыхСтавок < 1 Тогда
							//Сообщить(Сокрлп(ТекСтрокаД.Сотрудник) + " " + сокрлп(ТекСтрокаВакантно.МесячнаяТарифнаяСтавка) + " - > " + Сокрлп(ТекСтрокаД.Ставка));
							МТС = ТекСтрокаД.Ставка;
						КонецЕсли;
					КонецЕсли;
					
					//НоваяСтрока.Ставка                 = ОбщегоНазначения.ОкруглитьЗначение2016(ТекСтрокаВакантно.МесячнаяТарифнаяСтавка * (1 + (ПроцентПовышенияИтог + ТекСтрокаД.ПроцентПоКонтракту) / 100 ),ДатаАктуальности);//Островская Татьяна 04.03.2013 № 16283 949
					//НоваяСтрока.Ставка                 = ТекСтрокаВакантно.МесячнаяТарифнаяСтавка;//Островская Татьяна 24.03.2014
					НоваяСтрока.Ставка = МТС;			
					//Аверсон 13.10.2022 КВВ СВФР-009643 )
					
					НоваяСтрока.ПроцентПоКонтракту     = ТекСтрокаД.ПроцентПоКонтракту;
					НоваяСтрока.Декретница             = ТекСтрокаД.Сотрудник;
					НоваяСтрока.ФондЗП                 = ТекСтрокаВакантно.ТарифныйСПовышениемПоИнструкции;//Островская Татьяна 04.03.2013 № 16283
			...
			НоваяСтрока.Должность              = ТекСтрокаВакантно.Должность;
			НоваяСтрока.ПриоритетДолжность     = ТекСтрокаВакантно.Должность.Приоритет;
			НоваяСтрока.РазмерСтавки1Разряда   = ТекСтрокаВакантно.РазмерСтавки1Разряда;//Островская Татьяна 06.03.2013 № 16283
			НоваяСтрока.РазрядЕТС              = ТекСтрокаВакантно.РазрядЕТС;
			НоваяСтрока.ЗанимаемыхСтавок       = ТекСтрокаВакантно.Свободно - ЗанимаемыхСтавокДекрет;
			НоваяСтрока.КоэффициентЕТС         = ТекСтрокаВакантно.КоэффициентЕТС;
			НоваяСтрока.ПроцентПоКонтракту     = ТекСтрокаВакантно.ПроцентПоКонтракту;//Островская Татьяна 14.01.2014
			//НоваяСтрока.ИтоговоеПовышениеПоИнструкции     = ТекСтрокаВакантно.ИтоговоеПовышениеПоИнструкции; //Островская Татьяна 07.03.2013 № ВИС-00000283
			мТарифнаяСтавкаРазряд = ОбщегоНазначения.ОкруглитьЗначение2016(ТекСтрокаВакантно.КоэффициентЕТС * ТекСтрокаВакантно.РазмерСтавки1Разряда * ТекСтрокаВакантно.Свободно,ДатаАктуальности);//О.Т. Аверсон 18.03.2016 № 949
			//Островская Татьяна 04.03.2013 № 16283
			НоваяСтрока.Ставка                 = ОбщегоНазначения.ОкруглитьЗначение2016(ТекСтрокаВакантно.МесячнаяТарифнаяСтавка,ДатаАктуальности);//О.Т. 15.07.2014 № 124 (была убрана ставок. изначально умножалось. заявка. 16976) 949
			НоваяСтрока.МТСтавка               = ТекСтрокаВакантно.МесячнаяТарифнаяСтавка; //Островская Татьяна 24.03.2014
			НоваяСтрока.ФондЗП                 = ОбщегоНазначения.ОкруглитьЗначение2016(ТекСтрокаВакантно.ТарифныйСПовышениемПоИнструкции,ДатаАктуальности);//Островская Татьяна 19.12.2013 № 16976;  949
			
			//Аверсон 26.10.2022 КВВ СВФР-009711 (
			Если Организация.ИНН = "100194961" И НЕ ВторойОтчет Тогда
				НоваяСтрока.Ставка = Окр(НоваяСтрока.Ставка * НоваяСтрока.ЗанимаемыхСтавок,2);
				НоваяСтрока.ФондЗП = Окр(НоваяСтрока.ФондЗП * НоваяСтрока.ЗанимаемыхСтавок,2);
			КонецЕсли;
			//Аверсон 26.10.2022 КВВ СВФР-009711 )
			
			НоваяСтрока.ЗанимаемыхСтавокПоШтатному = ТекСтрокаВакантно.ЗанимаемыхСтавокПоШтатному;//Островская Татьяна 13.01.2014
			//Островская Татьяна 04.03.2013 № 16283
			...
			//Аверсон СВФР-012191
			//+ Ништ 28.01.2026 свфр-017928: дублируются строки в отчете если вводятся две плановые ставки и одна закрыта не полностью
			НоваяСтрока.КодСтроки              = ТекСтрокаВакантно.КодСтроки;
			//- Ништ 28.01.2026 свфр-017928
		КонецЦикла;	
	...
	//
	Процедура СформироватьОтчет(ДокументРезультат, ФормаВыборОтчета, Дописать = Ложь) Экспорт
		//Островская Татьяна 18.12.2013 № 16976
		ВторойОтчет = Ложь;
		если ФормаВыборОтчета = "Форма2" Тогда 
			мМакет = ПолучитьМакет("МакетРабочиеВариантВторой");
			ВторойОтчет = Истина;
		иначе
			мМакет = ПолучитьМакет("МакетРабочие");
		КонецЕсли;
		//Островская Татьяна 18.12.2013 № 16976
		ФондЗПИтог = 0; СтавокИтог = 0; ТарифнаяСтавкаИтог = 0; СтавкаИтог = 0; ЗаСложностьИтог = 0;
		ФондЗПИтогПоПредпр = 0; СтавокИтогПоПредпр = 0; ТарифнаяСтавкаИтогПоПредпр = 0; СтавкаИтогПоПредпр = 0; ЗаСложностьИтогПоПредпр = 0;
		ТекПодр = "";
		...
		Если Факт Тогда
			...
			|	ТарифныеСтавкиРаботниковОрганизации.ТарифнаяСПовышениемПоИнструкции КАК ТарифныйСПовышениемПоИнструкции,
			...
			|	И ВидТС.ВидРасчета В(&ВидРасчета)
			|	И ВЫБОР
			|			КОГДА РаботникиОрганизации.ПериодЗавершения <= &Период
			|					И РаботникиОрганизации.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
			|				ТОГДА РаботникиОрганизации.ЗанимаемыхСтавокЗавершения
			...
			|					ИЛИ СостояниеРаботниковОрганизаций.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.ОтпускЕжегодный) //Аверсон 23.08.2020 КВВ СВФР-006429
			|					)
			|				И &Период МЕЖДУ СостояниеРаботниковОрганизаций.Период И ДобавитьКДате(СостояниеРаботниковОрганизаций.ПериодЗавершения,День,-1) //О.Т. Аверсон 15.05.2019 № 4345 (-1)
			|				И ВЫБОР
			...
			//ПСА 11.02.2021 СВФР-007153+++
			|					Иначе ВЫБОР
			|					КОГДА СостояниеРаботниковОрганизаций.Регистратор ССЫЛКА Документ.ОтзывИзОтпуска
			|						ТОГДА Ложь	
			//+++ПСА 11.02.2021 СВФР-007153
			|					ИНАЧЕ ИСТИНА
			|					Конец
		...
		Иначе
			//Аверсон 05.04.2022 КВВ СВФР-008903 (пункт 2) (
			ТекстЗапроса = "";
			Если Организация.ИНН = "101542412" И ВторойОтчет Тогда  //Минск-Новости
				ТекстЗапроса = "ВЫБРАТЬ
				|ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ПодразделениеОрганизации КАК ПодразделениеОрганизации,
				|ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.Должность КАК Должность,
				|ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ПроцентПоКонтракту КАК ПроцентПоКонтракту,
				|МАКСИМУМ(ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.Период) КАК Период//,
				|ПОМЕСТИТЬ ВтПоследнихСтавок
				|ИЗ
				|РегистрСведений.ПлановоеШтатноеРасписаниеОрганизаций КАК ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних
				|ГДЕ
				|ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.Период <= &Период
				|СГРУППИРОВАТЬ ПО
				|ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ПодразделениеОрганизации,
				|ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ПроцентПоКонтракту,
				|ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.Должность
				...
				|ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.Должность КАК Должность,
				|ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ПроцентПоКонтракту КАК ПроцентПоКонтракту,
				|ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.Период КАК Период,
				|СУММА(ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ЗанимаемыхСтавок) КАК ЗанимаемыхСтавок
				|ПОМЕСТИТЬ ВтВывелиСтавки0
				|ИЗ
				|РегистрСведений.ПлановоеШтатноеРасписаниеОрганизаций КАК ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних
				|
				|ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтПоследнихСтавок КАК ВтПоследнихСтавок
				|ПО ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ПодразделениеОрганизации = ВтПоследнихСтавок.ПодразделениеОрганизации
				|И ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.Должность = ВтПоследнихСтавок.Должность
				|И ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ПроцентПоКонтракту = ВтПоследнихСтавок.ПроцентПоКонтракту
				|И ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.Период = ВтПоследнихСтавок.Период
				|СГРУППИРОВАТЬ ПО
				|ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ПодразделениеОрганизации,
				|ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ПроцентПоКонтракту,
				|ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.Должность,
				|ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.Период
				|
				|;
				|ВЫБРАТЬ *
				|ПОМЕСТИТЬ ВтВывелиСтавки
				|ИЗ ВтВывелиСтавки0 КАК ВтВывелиСтавки0
				|ГДЕ  ВтВывелиСтавки0.ЗанимаемыхСтавок = 0
				|;";
			КонецЕсли;
			//Аверсон 05.04.2022 КВВ СВФР-008903 (пункт 2) )
			...
			//Аверсон ЕАЕ 28.10.2025 СВФР-016913
			Если Организация.ИНН = "101542412" И ВторойОтчет Тогда
				ТекстЗапроса = ТекстЗапроса + "	
				|   ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ГрафикРаботы,";
			КонецЕсли;
			//Аверсон СВФР-016913
			...
			//Аверсон 01.03.2022 КВВ СВФР-008804 (
			Если Организация.ИНН = "101542412" И НЕ ВторойОтчет Тогда  //Минск-Новости
			Иначе
				ТекстЗапроса = ТекстЗапроса + "
			...
			|		СГРУППИРОВАТЬ ПО
			|			ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ПодразделениеОрганизации,
			|			ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.Должность) КАК ВложенныйЗапрос
			|		ПО ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ПодразделениеОрганизации = ВложенныйЗапрос.ПодразделениеОрганизации
			|			И ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.Должность = ВложенныйЗапрос.Должность
			|			И ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.Период = ВложенныйЗапрос.Период
			|ГДЕ
			...
			Если Организация.ИНН = "101542412" И ВторойОтчет Тогда  //Минск-Новости
				ТекстЗапроса = ТекстЗапроса + "
				|И НЕ (ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ПодразделениеОрганизации, ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.Должность,ПлановоеШтатноеРасписаниеОрганизацийСрезПоследних.ПроцентПоКонтракту) В (ВЫБРАТЬ ВтВывелиСтавки.ПодразделениеОрганизации, ВтВывелиСтавки.Должность,ВтВывелиСтавки.ПроцентПоКонтракту ИЗ ВтВывелиСтавки)";
			КонецЕсли;
			ТекстЗапроса = ТекстЗапроса + "
			//Аверсон 05.04.2022 КВВ СВФР-008903 (пункт 2) )
			|
		...
		СписокВидовОклад.Добавить(ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ТарифДневной);//Островская Татьяна 22.01.2014
		СписокВидовОклад.Добавить(ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ТарифЧасовой);//Островская Татьяна 22.01.2014
		...
		//Аверсон ЕАЕ 08.10.2025 СВФР-016913
		Если ВторойОтчет И Не Факт Тогда
			ТЗРезультатЗапросаДляРасчетаПовышений = РезультатЗапроса.Выгрузить();
		КонецЕсли;
		//Аверсон СВФР-016913
		...
		Если Факт Тогда
			//Аверсон 13.04.2020 КВВ СВФР-005765 (
			Если Организация.ИНН = "190288027" Тогда
				ТаблицаВакантно = СформироватьЗапросПоВакантноПБХ(СписокВидовОклад).Выгрузить();
			Иначе
				//Аверсон 13.04.2020 КВВ СВФР-005765 )
				ТаблицаВакантно = СформироватьЗапросПоВакантно(СписокВидовОклад).Выгрузить();
			КонецЕсли;
		иначе
			ТаблицаВакантно = Новый ТаблицаЗначений;
		КонецЕсли;
		...
		Иначе
			...
			Если ВторойОтчет Тогда
				...
				//Аверсон ЕАЕ 28.10.2025 СВФР-016913
				|   ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.ГрафикРаботы
				//Аверсон СВФР-016913
				...
				|И ПлановоеШтатноеРасписаниеОрганизацийКоэфПовышенияТО.Ссылка = ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.Ссылка //СВЭЙ 19.02.2024 КВВ СВФР-012329
				
				|ГДЕ
				|	(ПлановоеШтатноеРасписаниеОрганизацийКоэфПовышенияТО.Ссылка,
				|    ПлановоеШтатноеРасписаниеОрганизацийКоэфПовышенияТО.КодСтроки,
				|    ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.ПодразделениеОрганизации,
				|    ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.Должность) В(ВЫБРАТЬ
				|                                                                    ВТТЗ.Документ,
				|                                                                    ВТТЗ.Сотрудник,
				|                                                                    ВТТЗ.ПодразделениеОрганизации,
				|                                                                    ВТТЗ.Должность
				|                                                                    ИЗ ВТТЗ)
				|
				|ОБЪЕДИНИТЬ ВСЕ
				...
				|И ПлановоеШтатноеРасписаниеОрганизацийПовышенияПоИнструкции.Ссылка = ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.Ссылка //СВЭЙ 19.02.2024 КВВ СВФР-012329
				...
				ЗапросП.УстановитьПараметр("ТЗ", ТЗРезультатЗапросаДляРасчетаПовышений);
			Иначе
			//Аверсон СВФР-016913	
				...
				//Аверсон 01.03.2022 КВВ СВФР-008804 (
				Если Организация.ИНН = "101542412" И НЕ ВторойОтчет Тогда  //Минск-Новости
					ЗапросП.Текст = ЗапросП.Текст + "
					| И ПлановоеШтатноеРасписаниеОрганизацийШтатныеЕдиницы.Ссылка В(&Документ)";
				КонецЕсли;
				//Аверсон 01.03.2022 КВВ СВФР-008804 )
		...
		КонецЦикла;
		если Факт Тогда 
			для Каждого стрВак из ТаблицаВакантноКоэф Цикл 
				СтрТЗОр = ТЗ_ТаблицаПовышенийОр.Добавить();
				СтрТЗОр.Коэффициент = стрВак.Коэффициент;
			КонецЦикла;
		КонецЕсли;
		...
		ТЗ_Декретницы = СформироватьЗапросПоДекретницам(СписокВидовОклад).Выгрузить();
		Для Каждого СтрТЗ ИЗ ТЗ_Декретницы Цикл
			СтрТЗ.РазрядЕТС = СокрЛП(СтрТЗ.РазрядЕТС);
		...
		ВременнаяТЗ.Колонки.Добавить("РазрядЕТС", Новый ОписаниеТипов("Число"));
		ВременнаяТЗ.Колонки.Добавить("ЗанимаемыхСтавок", Новый ОписаниеТипов("Число"));
		ВременнаяТЗ.Колонки.Добавить("ЗанимаемыхСтавокПоШтатному", Новый ОписаниеТипов("Число"));//Островская Татьяна 13.01.2014
		...
		ВременнаяТЗ.Колонки.Добавить("МТСтавка");//Островская Татьяна 24.03.2014
		...
		//КонецЕсли;
		//+ Ништ 28.01.2026 свфр-017928: дублируются строки в отчете если вводятся две плановые ставки и одна закрыта не полностью
		ВременнаяТЗ.Колонки.Добавить("КодСтроки", Новый ОписаниеТипов("Число"));
		//- Ништ 28.01.2026 свфр-017928
		
		ЗаполнимВременнуюТаблицу(ВременнаяТЗ, РезультатЗапроса.Выбрать(), ТаблицаВакантно);
		
		Если НеВыводитьФИО Тогда 
			Для Каждого стр из ВременнаяТЗ Цикл 
				стр.Сотрудник = "";
				стр.СотрудникЗаменяемый = "";
				стр.Декретница = "";
			КонецЦикла;
			
			//Аверсон ЕАЕ 08.02.2024 СВФР-012191
			Если Организация.ИНН = "101542412" Тогда
				ВременнаяТЗ.Свернуть("Подразделение,ПриоритетПодразделение,Должность, ПриоритетДолжность,РазмерСтавки1Разряда,РазрядЕТС,КоэффициентЕТС,ТарифнаяСтавкаРазряд,ПроцентПоКонтракту,Ставка,МТСтавка,Сотрудник,СотрудникЗаменяемый,Декретница,Регистратор","ЗанимаемыхСтавок,ЗанимаемыхСтавокПоШтатному,ФондЗП"); 
			Иначе
				ВременнаяТЗ.Свернуть("Подразделение,ПриоритетПодразделение,Должность, ПриоритетДолжность,РазмерСтавки1Разряда,РазрядЕТС,КоэффициентЕТС,ТарифнаяСтавкаРазряд,ПроцентПоКонтракту,Ставка,МТСтавка,Сотрудник,СотрудникЗаменяемый,Декретница","ЗанимаемыхСтавок,ЗанимаемыхСтавокПоШтатному,ФондЗП"); 
			КонецЕсли;
			//Аверсон СВФР-012191
		...
		Область.Параметры.ФондЗП = ВременнаяТЗ.Итог("ФондЗП");
		Область.Параметры.ФондЗППрописью = РаботаСДиалогами.СформироватьСуммуПрописью(ВременнаяТЗ.Итог("ФондЗП"), Константы.ВалютаРегламентированногоУчета.Получить());
		...
		н = 0;  к = Окр(ТЗ_ТаблицаПовышенийОр.Количество() / 2);
		Для Каждого стрТЗ из ТЗ_ТаблицаПовышенийОр Цикл
			н = н + 1;
			
			мТекст = "";
			Если н = к Тогда
				мТекст = "Повышения, предусмотренные колдоговором";
			КонецЕсли;
			//Островская Татьяна 19.12.2013 № 16976
			если не ВторойОтчет Тогда 
			...
			//Островская Татьяна 19.12.2013 № 16976
			если ВторойОтчет Тогда 
				Область.Параметры.н = 8 + н;
				Область.Параметры.н1 = Область.Параметры.н+1;
				н = н+1;
			иначе
				Область.Параметры.н = 7 + н;
			КонецЕсли;
			//Островская Татьяна 19.12.2013 № 16976
			если не ВторойОтчет Тогда
			...
			//Островская Татьяна 18.12.2013 № 16976
			//если ВторойОтчет Тогда 
			//	Область.Параметры.ПовышениеНаименованиеСумма = СокрЛП("Сумма");
			//КонецЕсли;
			//Островская Татьяна 18.12.2013 № 16976
			ДокументРезультат.Присоединить(Область);
			если ВторойОтчет Тогда
		...
		если ВторойОтчет Тогда
			м01 = 8 + н + 1;
			Область.Параметры.м01 = м01;
			м0 = м01 + 1;
			Область.Параметры.м0 = м0;
			//Островская Татьяна 19.12.2013 № 16976
			м = м0 + 1;
		иначе
			м = 7 + н + 1;
		...
		Если ВидРасчета.Количество() > 0 Тогда
			Для Каждого ТекСтрока Из ВидРасчета Цикл
				...
				//Островская Татьяна 19.12.2013 № 16976
				если не ВторойОтчет Тогда 
					Область.Параметры.ВидРасчета = ТекСтрока.Значение.Наименование;
				КонецЕсли;
				//Островская Татьяна 19.12.2013 № 16976
				м2 = м + х;
		...
		мНомер = 0;
		ВывестиСтрокиПодразделения(ВременнаяТЗ, ДокументРезультат,ТаблицаВакантно,ВторойОтчет,ФормаВыборОтчета);//Островская Татьяна 07.03.2013 № 16283
		
		Область = мМакет.ПолучитьОбласть("Итого|Область1");
		Область.Параметры.СтавокИтог = СтавокИтог;
		Область.Параметры.ТарифнаяСтавкаИтог = ТарифнаяСтавкаИтог;
		ДокументРезультат.Вывести(Область);
		...
		Область.Параметры.СтавкаИтог = СтавкаИтог;
		ДокументРезультат.Присоединить(Область);
		...
		Если ВидРасчета.Количество() > 0 Тогда
			...
			Для Каждого ТекСтрока Из ТЗ_ПоПодразделению Цикл
				...
				Область.Параметры.ЗаСложностьИтог = ТекСтрока.Сумма;
				ДокументРезультат.Присоединить(Область);
			КонецЦикла;
		КонецЕсли;
		
		Область = мМакет.ПолучитьОбласть("Итого|Область5");
		Область.Параметры.ФондЗПИтог = ФондЗПИтог;
		ДокументРезультат.Присоединить(Область);
		...
		Область.Параметры.ТарифнаяСтавкаИтогПоПредпр = ТарифнаяСтавкаИтогПоПредпр;
		ДокументРезультат.Вывести(Область);
		...
		Область.Параметры.ФондЗПИтогПоПредпр = ФондЗПИтогПоПредпр;
		ДокументРезультат.Присоединить(Область);
		
		//Аверсон 27.05.2021 КВВ СВФР-007550 (
		Если ФормаВыборОтчета = "Форма2" Тогда
			ДокументРезультат.Область("ФондЗПЗаголовок").Текст = "Фонд заработной платы " + СокрЛП(Окр(ФондЗПИтогПоПредпр,2));
			
			ПарПредмета = "рубль,рубля,рублей,м,копейка,копейки,копеек,ж,2";
			ФормСтрока = "Л = ru_RU; ДП = Истина";
			ДокументРезультат.Область("ФондЗПЗаголовокПрописью").Текст = ЧислоПрописью(Окр(ФондЗПИтогПоПредпр,2),ФормСтрока,ПарПредмета);
		КонецЕсли;
		//Аверсон 27.05.2021 КВВ СВФР-007550 )
