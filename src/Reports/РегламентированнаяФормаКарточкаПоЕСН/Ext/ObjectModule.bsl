...
Процедура СформироватьОтчет(ДокументРезультат) Экспорт
	...
	|	СУММА(Доходы.Результат) КАК НачисленоВсего,
	|	СУММА(0) КАК ВыплатыЗаСчетПрибыли,
	|	СУММА(ВЫБОР КОГДА Доходы.КодДоходаЕСН = &КодДоходаНеОблагаетсяЦеликом ТОГДА Доходы.Результат ИНАЧЕ Доходы.Скидка КОНЕЦ) КАК НеОблагаетсяПоСт238КромеДоговоров,
	|	СУММА(0) КАК ВыплатыПоДоговорам,
	|	СУММА( 0 ) КАК ПособияЗаСчетФСС
	|	ИЗ 
	...
	|	ЕСНИсчисленный.ПримененнаяЛьготаФССОборот * Ставки.ФССвПроцентах / 100 КАК НачисленоФСС,
	|	ЕСНИсчисленный.ПримененнаяЛьготаФОМСОборот * Ставки.ФФОМСвПроцентах / 100 КАК НачисленоФФОМС,
	|	ЕСНИсчисленный.ПримененнаяЛьготаФОМСОборот * Ставки.ТФОМСвПроцентах / 100 КАК НачисленоТФОМС,
	...
	//Островская Татьяна 14.03.2013 № 16317
	МесячныеПоказатели = Новый Структура("НачисленоВсего, ВыплатыЗаСчетПрибыли, НеОблагаетсяПоСт238КромеДоговоров, ВыплатыПоДоговорам, НалоговаяБазаФБ, НалоговаяБазаФСС, БазаПФР, ПримененнаяЛьготаИнвалида, ПособияЗаСчетФСС, НачисленоФБ, НачисленоФФОМС, НачисленоТФОМС, НачисленоФСС, ИсчисленоФФОМС, ИсчисленоТФОМС, НеПодлежитФБ, НеПодлежитФФОМС, НеПодлежитТФОМС, НеПодлежитФСС, НачисленоПФРНакопительная, НачисленоПФРСтраховая,НачисленоПФР");
