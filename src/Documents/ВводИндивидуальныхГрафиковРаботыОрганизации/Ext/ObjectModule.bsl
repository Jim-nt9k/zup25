...
Процедура Автозаполнение(Запрос = Неопределено, Сотрудник = Неопределено, ТекущаяСтрокаГрафикаРаботы = Неопределено) Экспорт
	...
	Если Запрос = Неопределено Тогда
		...
		Если Сотрудник = Неопределено Тогда
			...
			|	РаботникиОрганизаций.Сотрудник.Код КАК ТабНомер // О.Т. 28.11.2014 
	...
	// О.Т. 28.11.2014 
	Если Запрос = Неопределено Тогда
		ЗапросАвтоЗаполнения.Текст = ЗапросАвтоЗаполнения.Текст + " СотрудникиОрганизаций.ТабНомер,";
	иначе
		ЗапросАвтоЗаполнения.Текст = ЗапросАвтоЗаполнения.Текст + " Код,";
	КонецЕсли;
	
	ЗапросАвтоЗаполнения.Текст = ЗапросАвтоЗаполнения.Текст + " Сотрудник,
	|	ВидУчетаВремени,
	|	День";
	// О.Т. 28.11.2014 
	...
	Иначе 	
		...
		Пока ВыборкаРезультата.СледующийПоЗначениюПоля("Сотрудник") Цикл
			// О.Т. 30.10.2014 261
			если ВыборкаРезультата.ГрафикРаботы.НеЗаполнятьВИндГрафиках Тогда 
				Продолжить;
			КонецЕсли;
			// О.Т. 30.10.2014 261
			//О.Т. 08.09.2014 № 214
			...
			|	И ПлановыеНачисленияРаботниковОрганизацийСрезПоследних.ПериодЗавершения > &КонецМесяца
			|	И ПлановыеНачисленияРаботниковОрганизацийСрезПоследних.Сотрудник = &Сотрудник";
			
			ЗапросОтпуск.УстановитьПараметр("ВидРасчета", ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ОтпускПоУходуЗаРебенкомБезОплаты);
			ЗапросОтпуск.УстановитьПараметр("ДействиеЗавершения", Перечисления.ВидыДействияСНачислением.Прекратить);
			ЗапросОтпуск.УстановитьПараметр("КонецМесяца", КонецМесяца(ПериодРегистрации));
			ЗапросОтпуск.УстановитьПараметр("Сотрудник", ВыборкаРезультата.Сотрудник);
			РезультатОтпуск = ЗапросОтпуск.Выполнить().Выгрузить();
			
			Если РезультатОтпуск.Количество()>0 Тогда
				
				Сообщить("Сотрудник " + ВыборкаРезультата.Сотрудник + " находится в отпуске по уходу, в документ не попадает.");
				Продолжить;
...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если ИмяМакета = "Т13" или ИмяМакета = "Т13Н" Тогда
		...
		|	И ГрафикРаботы.Ссылка.Проведен";
		...
		ТекстОшибки = "";
//толик		Отчет.СформироватьОтчет(ТабДокумент, "Табель", ТекстОшибки);
		Отчет.СформироватьОтчет(ТабДокумент, "График", ТекстОшибки);
		...
		КонецЕсли;
	//О.Т. 18.08.2014 № 194	
	ИначеЕсли ИмяМакета = "МакетГрафик" или ИмяМакета = "МакетГрафикН" Тогда
		Если Не Проведен Тогда
			РаботаСДиалогами.ВывестиПредупреждение("Документ можно распечатать только после его проведения!");
			Возврат Неопределено;
		КонецЕсли;
		...
		если ИмяМакета = "МакетГрафикН" Тогда 
			если СпособСортировкиПечФормы Тогда 
				Отчет.СформироватьОтчет(ТабДокумент, "ГрафикНовыйНТ", ТекстОшибки,,БезВредности);
			иначе
				Отчет.СформироватьОтчет(ТабДокумент, "ГрафикНовыйНИ", ТекстОшибки,,БезВредности);
			КонецЕсли;
		иначе
			если СпособСортировкиПечФормы Тогда 
				Отчет.СформироватьОтчет(ТабДокумент, "ГрафикНовыйТ", ТекстОшибки,,БезВредности);
			иначе
				Отчет.СформироватьОтчет(ТабДокумент, "ГрафикНовыйИ", ТекстОшибки,,БезВредности);
			КонецЕсли;
...
//толик		
	ИначеЕсли ИмяМакета = "Т13П" или ИмяМакета = "Т13ПН" Тогда
		...
		Запрос = Новый Запрос;
		ПериодНеВведен = Истина;
		НачДата = Дата;
		КонДата = Дата;
		Если ВвестиДату(НачДата, "Дата начала печати графиков") Тогда
			Запрос.УстановитьПараметр("НачДата", НачДата);
		Иначе
			ПериодНеВведен = Ложь;
		КонецЕсли;
		Если ВвестиДату(КонДата, "Дата окончания печати графиков") Тогда
			Запрос.УстановитьПараметр("КонДата", КонДата);
		Иначе
			ПериодНеВведен = Ложь;
		КонецЕсли;
		Если ПериодНеВведен Тогда
			Запрос.Текст =
				"ВЫБРАТЬ РАЗЛИЧНЫЕ
				|	ГрафикРаботы.Ссылка КАК График
				|ИЗ
				|	Документ.ВводИндивидуальныхГрафиковРаботыОрганизации.ГрафикРаботы КАК ГрафикРаботы
				|ГДЕ
				|	ГрафикРаботы.Ссылка.Дата МЕЖДУ &НачДата И &КонДата
				|	И ГрафикРаботы.Ссылка.Проведен";
		Иначе
			Возврат Неопределено;
		КонецЕсли;
		...
		Пока Результат.Следующий() Цикл
			Запрос.УстановитьПараметр("ДокументСсылка", Результат.График);
			Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	ГрафикРаботы.Сотрудник КАК Сотрудник
			|ИЗ
			|	Документ.ВводИндивидуальныхГрафиковРаботыОрганизации.ГрафикРаботы КАК ГрафикРаботы
			...
			ФильтрСоответствия.Вставить("Организация", Результат.График.Организация);
			ФильтрСоответствия.Вставить("Подразделение", Результат.График.ПодразделениеОрганизации);
			
			ФильтрСоответствия.Вставить("ОтборОрганизации", ЗначениеЗаполнено(Результат.График.Организация));
			ФильтрСоответствия.Вставить("ОтборПодразделения", ЗначениеЗаполнено(Результат.График.ПодразделениеОрганизации));
			
			ФильтрСоответствия.Вставить("ВидСравненияОрганизации", ВидСравнения.Равно);
			ФильтрСоответствия.Вставить("ВидСравненияПодразделения", ВидСравнения.Равно);
			
			ФильтрСоответствия.Вставить("ПериодРегистрации", Результат.График.ПериодРегистрации);
			...
			Отчет.УстановитьФильтр(ФильтрСоответствия);
			ТекстОшибки = "";
			Отчет.СформироватьОтчет(ТабДокумент, "График", ТекстОшибки, Ложь);
		КонецЦикла;
...
//толик	СтруктураМакетов.Вставить("Т13",	"Форма Т-13");
	СтруктураМакетов.Вставить("Т13",	"График");
	СтруктураМакетов.Вставить("Т13П",	"Графики за период");
    СтруктураМакетов.Вставить("МакетГрафик",	"График (новый)");//О.Т. 18.08.2014 № 194
...
//толик для поределение вида учета времени
			Если Движение.ВидУчетаВремени = Перечисления.ВидыУчетаВремени.ПоЧасам Тогда
				Движение.ВидВремени = Справочники.КлассификаторИспользованияРабочегоВремени.Работа;
			ИначеЕсли Движение.ВидУчетаВремени = Перечисления.ВидыУчетаВремени.ПоНочнымЧасам Тогда
				Движение.ВидВремени = Справочники.КлассификаторИспользованияРабочегоВремени.РаботаНочныеЧасы;
			ИначеЕсли Движение.ВидУчетаВремени = Перечисления.ВидыУчетаВремени.ПоВечернимЧасам Тогда
				Движение.ВидВремени = Справочники.КлассификаторИспользованияРабочегоВремени.РаботаВечерниеЧасы;
			КонецЕсли;
//енд
