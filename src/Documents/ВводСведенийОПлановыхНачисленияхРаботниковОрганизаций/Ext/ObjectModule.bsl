...
Процедура ДобавитьСтрокиНачисленийПоРаботнику(Сотрудник, ДатаАктуальности = Неопределено) Экспорт
	...
	//Островская Татьяна 15.11.2012 № 15992
	...
	|	КОНЕЦ КАК Валюта6,
	|	Начисления.ИспользуетсяВНаряде,
	|	Начисления.ПериодЗавершения
	...
	Пока Выборка.Следующий() Цикл
		...
		//Островская Татьяна 15.11.2012 № 15992
		НоваяСтрокаТЧ.ДатаДействияКонец	= Выборка.ПериодЗавершения - 86400; //О.Т. Аверсон 18.11.2016
        //Островская Татьяна 15.11.2012 № 15992
	КонецЦикла;
...
Функция СформироватьЗапросПоРаботникиОрганизации(ВыборкаПоШапкеДокумента, ПроверкаШтатногоРасписания)
    ...
    Если ПроверкаШтатногоРасписания Тогда
		...
		|	Начисления.СчетВОтражение,
		|	Начисления.НомГруппаВОтражение, 
		|	Начисления.ПодразделениеВОтражение,
		//Аверсон 02.08.2023 КВВ СВФР-010889 )
		...
		|		ТЧОсновныеНачисления.ВидРасчета КАК ВидРасчета,
		|		ТЧОсновныеНачисления.ИспользуетсяВНаряде КАК ИспользуетсяВНаряде,
		
		//Аверсон 02.08.2023 КВВ СВФР-010889 (
		|	ТЧОсновныеНачисления.СчетВОтражение КАК СчетВОтражение, 
		|	ТЧОсновныеНачисления.НомГруппаВОтражение КАК НомГруппаВОтражение, 
		|	ТЧОсновныеНачисления.ПодразделениеВОтражение КАК ПодразделениеВОтражение,
		//Аверсон 02.08.2023 КВВ СВФР-010889 )
		...
		|		ТЧОсновныеНачисления.ВидРасчета,
		|		ТЧОсновныеНачисления.ИспользуетсяВНаряде,
		
		//Аверсон 02.08.2023 КВВ СВФР-010889 (
		|	ТЧОсновныеНачисления.СчетВОтражение, 
		|	ТЧОсновныеНачисления.НомГруппаВОтражение, 
		|	ТЧОсновныеНачисления.ПодразделениеВОтражение,
		//Аверсон 02.08.2023 КВВ СВФР-010889 )
	...
	Если ВыборкаПоСтрокамДокумента.Действие <> Перечисления.ВидыДействияСНачислением.НеИзменять Тогда
		...
		ИначеЕсли (ВыборкаПоСтрокамДокумента.Действие = Перечисления.ВидыДействияСНачислением.Начать ИЛИ ВыборкаПоСтрокамДокумента.Действие = Перечисления.ВидыДействияСНачислением.Изменить) Тогда
			...
			Если ПроверкаШтатногоРасписания И Не ВыборкаПоСтрокамДокумента.РазмерСоответствуетШТР И Не ВыборкаПоСтрокамДокумента.ТребуетВводаТарифногоРазряда Тогда
				//ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "размер основного начисления не соответствует штатному расписанию!", Отказ, Заголовок); //Пока убрать проверку для деноминации
			КонецЕсли;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.КонфликтнаяСтрокаНомер) Тогда
			...
			Иначе
				СтрокаСообщениеОбОшибке = "работнику " + ВыборкаПоСтрокамДокумента.СотрудникНаименование + " назначено одно и тоже начисление дважды (см. строку " + ВыборкаПоСтрокамДокумента.КонфликтнаяСтрокаНомер + ")! Проверьте правильность внесенных данных!"; 
				ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке  + СтрокаСообщениеОбОшибке, Отказ, Заголовок);
				Отказ = Ложь;
			КонецЕсли;
...
//  Нет.
//
Процедура ДобавитьСтрокуВДвиженияПоРегистрамСведений(ВыборкаПоШапкеДокумента, ВыборкаПоРаботникиОрганизации,Сек)
	...
	Если ВыборкаПоРаботникиОрганизации.Действие <> Перечисления.ВидыДействияСНачислением.НеИзменять тогда
		...
		// Свойства
		Движение.Период						= ВыборкаПоРаботникиОрганизации.ДатаДействия+Сек;
		Если  ВыборкаПоРаботникиОрганизации.Действие = Перечисления.ВидыДействияСНачислением.Прекратить  Тогда
			Движение.Период						= ВыборкаПоРаботникиОрганизации.ДатаДействия+мДлинаСуток;
		КОнецЕсли;
		// Измерения
		...
		Движение.ИспользуетсяВНаряде		= ВыборкаПоРаботникиОрганизации.ИспользуетсяВНаряде;
		
		//Аверсон 02.08.2023 КВВ СВФР-010889 (
		Движение.СчетВОтражение	= ВыборкаПоРаботникиОрганизации.СчетВОтражение; 
		Движение.НомГруппаВОтражение	= ВыборкаПоРаботникиОрганизации.НомГруппаВОтражение; 
		Движение.ПодразделениеВОтражение = ВыборкаПоРаботникиОрганизации.ПодразделениеВОтражение;
		//Аверсон 02.08.2023 КВВ СВФР-010889 )
...
//Толик
//добавил ещё одну строку дя того что-бы прекращать действие
		...
		//	
		//	// Свойства
		//	Движение.Период						= ВыборкаПоРаботникиОрганизации.ДатаДействияКонец + мДлинаСуток;
		//	
		//	// Измерения
		//	Движение.Сотрудник					= ВыборкаПоРаботникиОрганизации.Сотрудник;
		//	Движение.Организация				= ВыборкаПоШапкеДокумента.ГоловнаяОрганизация;
...
// ТОЛИК проверим перемещения 
	Для Каждого мСтрока Из ОсновныеНачисления Цикл
		Отказ = ПрочиеПроцедуры.ПроверитьИОтрегулироватьПеремещение(мСтрока.ДатаДействия, мСтрока.Сотрудник, Организация, ЭтотОбъект, мСтрока, мСтрока.ВидРасчета);
		Если Отказ Тогда
			Возврат
		КонецЕсли;
	КонецЦикла;
		...
		Если НЕ Отказ Тогда
			...
			Пока ВыборкаПоРаботникиОрганизации.Следующий() Цикл
				...
				Если НЕ Отказ Тогда
					...
					Если ВидРасчетаПрошл <> ВыборкаПоРаботникиОрганизации.ВидРасчета или СотрудникПрошл <> ВыборкаПоРаботникиОрганизации.Сотрудник
						
						//Аверсон ЕАЕ 10.12.2021 СВФР-008482
						или ДатаДействияПрошл <> ВыборкаПоРаботникиОрганизации.ДатаДействия
						//Аверсон СВФР-008482
						
						Тогда 
						Сек = 0;
					КонецЕсли;
					ДобавитьСтрокуВДвиженияПоРегистрамСведений(ВыборкаПоШапкеДокумента, ВыборкаПоРаботникиОрганизации,Сек);
					Сек = Сек+1;
					ВидРасчетаПрошл = ВыборкаПоРаботникиОрганизации.ВидРасчета;
					СотрудникПрошл = ВыборкаПоРаботникиОрганизации.Сотрудник;
					
					//Аверсон ЕАЕ 10.12.2021 СВФР-008482
					ДатаДействияПрошл = ВыборкаПоРаботникиОрганизации.ДатаДействия;
					//Аверсон СВФР-008482
