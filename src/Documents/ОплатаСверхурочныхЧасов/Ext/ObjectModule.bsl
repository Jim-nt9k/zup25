...
Процедура ЗаполнитьРаботавшимиПоНарядам(Запрос = Неопределено, Реквизиты = Неопределено, мВыборКнопки = "РаботавшимиСверхурочноПоНарядам") Экспорт   //Островская 15.03.2014 № //Аверсон ЕАЕ 24.05.2024 СВФР-012937
	...
	|
	|ПОМЕСТИТЬ ВтДатыИСотр //Аверсон 08.06.2022 КВВ СВФР-009178
	|
	|ИЗ
	...
	//Аверсон 08.06.2022 КВВ СВФР-009178 (
	|;
	|
	|ВЫБРАТЬ
	|ВтДатыИСотр.Период КАК Дата,
	|ВтДатыИСотр.Сотрудник КАК Сотрудник
	|ПОМЕСТИТЬ ВтТолькоДатыПоСотр
	|ИЗ ВтДатыИСотр КАК ВтДатыИСотр
	|СГРУППИРОВАТЬ ПО
	|ВтДатыИСотр.Период,
	|ВтДатыИСотр.Сотрудник
	|;
	|
	|ВЫБРАТЬ
	|ВтТолькоДатыПоСотр.Дата КАК Дата,
	...
	|И ВтТолькоДатыПоСотр.Дата >= РО.Период
	|СГРУППИРОВАТЬ ПО
	|ВтТолькоДатыПоСотр.Дата,
	...
	|ВтДатыИСотр.Период,
	|ВтДатыИСотр.Сотрудник,
	|ВтДатыИСотр.ВидИспользованияРабочегоВремени,
	|ВтДатыИСотр.Часов,
	|ВтДатыИСотр.Организация,
	|ВтДатыИСотр.Регистратор,
	|ВтДатыИСотр.ДатаНачала,
	|ВтПодразделения.ПодразделениеОрганизации КАК ПодразделениеОрганизации
	|ПОМЕСТИТЬ ВтДоПроверкиВЫбранныхПодразделений
	|ИЗ ВтДатыИСотр КАК ВтДатыИСотр
	|ЛЕВОЕ СОЕДИНЕНИЕ ВтПодразделения КАК ВтПодразделения
	|ПО ВтДатыИСотр.Сотрудник = ВтПодразделения.Сотрудник
	|И  ВтДатыИСотр.ДатаНачала = ВтПодразделения.Дата
	|;
	|
	|ВЫБРАТЬ *
	|ИЗ ВтДоПроверкиВЫбранныхПодразделений КАК ВтДоПроверкиВЫбранныхПодразделений";
	
	Если ЗначениеЗаполнено(ПодразделениеОрганизации) Тогда
		Запрос.Текст = Запрос.Текст + "
		|ГДЕ ВтДоПроверкиВЫбранныхПодразделений.ПодразделениеОрганизации В ИЕРАРХИИ (&ПодразделениеОрганизации)";
	КонецЕсли;
	...
	//Если Выборка.Количество() > 0 тогда   //Островская 15.03.2014 №
	Пока Выборка.Следующий() цикл
		
		//Аверсон ЕАЕ 24.05.2024 СВФР-012937
		Если мВыборКнопки = "РаботавшимиСверхурочноБезНарядов" Тогда
		...
		График = РегистрыСведений.РаботникиОрганизаций.ПолучитьПоследнее(ПериодРегистрации,Новый Структура("Сотрудник",Выборка.Сотрудник)).ГрафикРаботы; 
		//Островская Татьяна 15.04.2014 № 17281
		если ОплатаВДвойномРазмере  Тогда //Островская Татьяна 14.02.2013 № 16232
			...
			КонецЕсли;
			Строка.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ОплатаСверхурочных;
			Строка.ЧасовДвойных = Выборка.Часов; 		
			если ФлагОплатаПоЧасам Тогда
				Если  Выборка.Период < Дата(2016,07,1) Тогда //О.Т. Аверсон 16.03.2016 № 949
					Строка.Размер = Окр(ПрочиеПроцедуры.ПолучитьЧасовуюТарифнуюСтавку(Организация, Выборка.Сотрудник, Строка.ДатаВыхода, Выборка.Сотрудник, Ложь, Ложь,Истина,,,,,,,,РасчетОтСреднегодовойНормы,Курс), 3);
				иначе
					Строка.Размер = ОбщегоНазначения.ОкруглитьЗначение2016(ПрочиеПроцедуры.ПолучитьЧасовуюТарифнуюСтавку(Организация, Выборка.Сотрудник, Строка.ДатаВыхода, Выборка.Сотрудник, Ложь, Ложь,Истина,,,,,,,,РасчетОтСреднегодовойНормы,Курс), Выборка.Период);//О.Т. Аверсон 17.03.2016 № 949
				КонецЕсли; 
			иначе
				Если  Выборка.Период < Дата(2016,07,1) Тогда //О.Т. Аверсон 16.03.2016 № 949
					Строка.Размер = Окр(ПрочиеПроцедуры.ПолучитьЧасовуюТарифнуюСтавку(Организация, Выборка.Сотрудник, Строка.ДатаВыхода, Выборка.Сотрудник, Истина, Истина,,,,,,,,,РасчетОтСреднегодовойНормы,Курс), 3);
				иначе
					Строка.Размер = ПрочиеПроцедуры.ПолучитьЧасовуюТарифнуюСтавку(Организация, Выборка.Сотрудник, Строка.ДатаВыхода, Выборка.Сотрудник, Истина, Истина,,,,,,,,,РасчетОтСреднегодовойНормы,Курс);//О.Т. Аверсон 17.03.2016 № 949
				КонецЕсли; 
			КонецЕсли;
			...
			Если РасчетСПовышениями и Справочники.ГруппыРасчетов.ПовышениеДляСверхурочныхИПраздников.Основные.Количество()>0 Тогда 
				...
				пока РезультатЗапроса.Следующий() Цикл 
					...
					Иначе      						
						//СуммаПовышений = СуммаПовышений + ОбщегоНазначения.ОкруглитьЗначение2016(Строка.Размер * РезультатЗапроса.Показатель1/100, Выборка.Период, 1);
				...
				// 016024 23.06.2025
				Если Организация.ИНН = "190304936" тогда
					ВидыПовышенийФ = Новый СписокЗначений;
					Для Каждого стр из Справочники.ГруппыРасчетов.ПовышениеДляСверхурочныхИПраздников.Основные Цикл  
						Если стр.ВидРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ФиксированнойСуммой Тогда 
							ВидыПовышенийФ.Добавить(стр.ВидРасчета);
						КонецЕсли;
					КонецЦикла; 
					График = РегистрыСведений.РаботникиОрганизаций.ПолучитьПоследнее(ПериодРегистрации,Новый Структура("Сотрудник",Выборка.Сотрудник)).ГрафикРаботы; 
					...
					ЗапросПлановыеС = Новый Запрос;
					ЗапросПлановыеС.Текст = 
					...
					|	И ОсновныеНачисленияРаботниковОрганизаций.ВидРасчета В(&ВидыРасчетаПовышения)
					|	И ОсновныеНачисленияРаботниковОрганизаций.ПериодРегистрации = &Период
					|
					|СГРУППИРОВАТЬ ПО
					|	ОсновныеНачисленияРаботниковОрганизаций.Сотрудник";
					ЗапросПлановыеС.УстановитьПараметр("Период", ?(ЗначениеЗаполнено(Строка.ДатаВыхода),НачалоМесяца(Строка.ДатаВыхода),ПериодРегистрации));
					ЗапросПлановыеС.УстановитьПараметр("ВидыРасчетаПовышения", ВидыПовышенийФ);   
					ЗапросПлановыеС.УстановитьПараметр("Сотрудник", Строка.Сотрудник);
					РезультатЗапросаС = ЗапросПлановыеС.Выполнить().Выбрать();
					пока РезультатЗапросаС.Следующий() Цикл
						
						//Аверсон ЕАЕ 25.11.2025 СВФР-017393
						//СуммаПовышений = СуммаПовышений + РезультатЗапросаС.Результат/Норма;
						СуммаПовышений = СуммаПовышений + ?(ОкруглениеКаждогоПовышения, ОбщегоНазначения.ОкруглитьПоВалюте(РезультатЗапросаС.Результат/Норма, Константы.ВалютаРегламентированногоУчета.Получить()), 
															РезультатЗапросаС.Результат/Норма);
						//Аверсон СВФР-017393
				...
				Если Справочники.ГруппыРасчетов.ПовышениеДляСверхурочныхИПраздников.РасчетПоСуммам Тогда
					...
					Строка.Результат = ОбщегоНазначения.ОкруглитьЗначение2016((СуммаПовышений) * Выборка.Часов, Выборка.Период, 1);//О.Т. Аверсон 17.03.2016 № 949
				Иначе
					Строка.Результат = ОбщегоНазначения.ОкруглитьЗначение2016((Строка.Размер+СуммаПовышений) * Выборка.Часов, Выборка.Период, 1);
				КонецЕсли;
				//СВЭЙ 22.02.2024 КВВ СВФР-012096 )   				
			иначе        			
				Строка.Результат = ОбщегоНазначения.ОкруглитьЗначение2016(Строка.Размер * Выборка.Часов, Выборка.Период, 1);//О.Т. Аверсон 17.03.2016 № 949
			КонецЕсли;			
			//О.Т. Аверсон 19.03.2019 № 4089
			
			//О.Т. Аверсон 27.01.2017 № 227
			...
			МесячнаяТСВВалюте = РегистрыСведений.ТарифныеСтавкиРаботниковОрганизации.ПолучитьПоследнее(ПериодРегистрации,Новый Структура("Сотрудник",Выборка.Сотрудник)).МесячнаяТарифнаяСтавкаВВалюте;
			Норма = ПрочиеПроцедуры.ПолучитьНормуВремениПоГрафику(График,Перечисления.ВидыУчетаВремени.ПоЧасам,ПериодРегистрации,Организация);//О.Т. Аверсон 16.10.2017 № 1120
			Если РасчетОтСреднегодовойНормы Тогда 
				Строка.ВалютныйЧасовойТариф = ?(ЗначениеЗаполнено(СреднегодоваяНорма),МесячнаяТСВВалюте / СреднегодоваяНорма,0);
			иначе
				Строка.ВалютныйЧасовойТариф = ?(ЗначениеЗаполнено(Норма),МесячнаяТСВВалюте / Норма,0);  
			КонецЕсли;
			//О.Т. Аверсон 27.01.2017 № 227 
		КонецЕсли;
		//Островская Татьяна 15.04.2014 № 17281
		...
		Если Доплата Тогда
			Строка.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ОплатаСверхурочных;
		Иначе
			Строка.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ДоплатаЗаСверхурочные;
		КонецЕслИ;     		
		Строка.ЧасовДвойных = Выборка.Часов;
		если ФлагОплатаПоЧасам Тогда   
			Если  Выборка.Период < Дата(2016,07,1) Тогда //О.Т. Аверсон 16.03.2016 № 949
		...
		Если РасчетСПовышениями и Справочники.ГруппыРасчетов.ПовышениеДляСверхурочныхИПраздников.Основные.Количество()>0 Тогда 
			...
			Если Справочники.ГруппыРасчетов.ПовышениеДляСверхурочныхИПраздников.РасчетПоСуммам Тогда
				...
				КонецЕсли;
				Строка.Размер =СуммаПовышений;
				Строка.Результат = ОбщегоНазначения.ОкруглитьЗначение2016((СуммаПовышений) * Выборка.Часов, Выборка.Период, 1);
			Иначе
				Строка.Результат = ОбщегоНазначения.ОкруглитьЗначение2016((Строка.Размер+СуммаПовышений) * Выборка.Часов, Выборка.Период, 1);//О.Т. Аверсон 17.03.2016 № 949
			КонецЕсли;
			//СВЭЙ 22.02.2024 КВВ СВФР-012096 )
		иначе
			Строка.Результат = ОбщегоНазначения.ОкруглитьЗначение2016(Строка.Размер * Выборка.Часов, Выборка.Период, 1);//О.Т. Аверсон 17.03.2016 № 949
...
Функция БылНаряд(Сотр, Дат)
	...
	Запрос.Текст = 
		...
		|	И ОсновныеНачисленияРаботниковОрганизаций.Регистратор.Сверхурочно";
	
	Запрос.УстановитьПараметр("Дат", НачалоДня(Дат));
	Запрос.УстановитьПараметр("Сотрудник", Сотр);
	СписокВидовРасчета = Новый Массив;
	СписокВидовРасчета.Добавить(ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ОплатаСверхурочных);
	СписокВидовРасчета.Добавить(ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ДоплатаЗаСверхурочные);
...
Процедура Автозаполнение(Запрос = Неопределено, Реквизиты = Неопределено) Экспорт
	...
	|	КОНЕЦ КАК Размер,
	|	ВЫБОР
	|	КОГДА НормаВремениПоОбщимГрафикам.ЧасовЗаМесяц = 0
	|			ИЛИ НормаВремениПоОбщимГрафикам.ЧасовЗаМесяц = NULL
	|		ТОГДА 0
	|	ИНАЧЕ Выразить(РаботникиИНачисления.МесячнаяТарифнаяСтавкаВВалюте / НормаВремениПоОбщимГрафикам.ЧасовЗаМесяц как Число(15,2))
	|	КОНЕЦ КАК МесячнаяТарифнаяСтавкаВВалюте
	...
	|		ТарифныеСтавкиРаботниковОрганизацииСрезПоследних.МесячнаяТарифнаяСтавкаВВалюте КАК МесячнаяТарифнаяСтавкаВВалюте
	|	ИЗ
	|		ВТСписокРаботников КАК СписокРаботников
	...
	|						КОНЕЦ
	|				КОНЕЦ)
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТарифныеСтавкиРаботниковОрганизации.СрезПоследних(
	|							&ДатаВыхода,
	|							Сотрудник В
	|								(ВЫБРАТЬ
	|									СписокРаботников.Сотрудник
	|								ИЗ
	|									ВТСписокРаботников КАК СписокРаботников)) КАК ТарифныеСтавкиРаботниковОрганизацииСрезПоследних
	|					ПО СписокРаботников.Сотрудник = ТарифныеСтавкиРаботниковОрганизацииСрезПоследних.Сотрудник) КАК РаботникиИНачисления
	...
	|	ВЫРАЗИТЬ(СписокРаботников.ЧасовДвойных * СписокРаботников.Размер КАК ЧИСЛО(15, 2))  КАК Результат,
	|	ЗНАЧЕНИЕ(ПланВидовРасчета.ОсновныеНачисленияОрганизаций.ДоплатаЗаСверхурочные) КАК ВидРасчета,
	|	СписокРаботников.МесячнаяТарифнаяСтавкаВВалюте как ВалютныйЧасовойТариф
	|ИЗ
	...
	|	ВЫРАЗИТЬ(СписокРаботников.ЧасовДвойных * СписокРаботников.Размер КАК ЧИСЛО(15, 2)),
	|	ЗНАЧЕНИЕ(ПланВидовРасчета.ОсновныеНачисленияОрганизаций.ОплатаСверхурочных),
	|	СписокРаботников.МесячнаяТарифнаяСтавкаВВалюте
	|ИЗ
	|	ВТСотрудникиРазмерыНачислений КАК СписокРаботников
	|
	|УПОРЯДОЧИТЬ ПО
...
Процедура ДобавитьСтрокуОсновныхНачислений(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента)
	...
	//толик не надо	Движение.ОтработаноЧасов			= ВыборкаПоСтрокамДокумента.ОтработаноЧасов;
...
Процедура Печать(Макет) Экспорт
	//{{_КОНСТРУКТОР_ПЕЧАТИ_ЭЛЕМЕНТ(Печать)
	// Данный фрагмент построен конструктором.
	// При повторном использовании конструктора, внесенные вручную изменения будут утеряны!!!
	ТабДок = Новый ТабличныйДокумент;
	Макет = Документы.ОплатаСверхурочныхЧасов.ПолучитьМакет("Печать");
	//// Заголовок
	//Область = Макет.ПолучитьОбласть("Заголовок");
	//ТабДок.Вывести(Область);
	// Шапка
	Шапка = Макет.ПолучитьОбласть("Шапка");
	ЧасыВсего = Начисления.Итог("ЧасовДвойных");  
	Шапка.Параметры.Заполнить(ЭтотОбъект);
	Шапка.Параметры.ЧасовВсего = ЧасыВсего;
	Шапка.Параметры.Курс = Курс;
	
	
	Шапка.Параметры.ПериодРегистрации = Формат(ПериодРегистрации,"ДФ = 'MMMM гггг'")+" г.";
	ТабДок.Вывести(Шапка);                                                                    
	// Начисления
	Область = Макет.ПолучитьОбласть("НачисленияШапка");	
	ТабДок.Вывести(Область);
	ОбластьНачисления = Макет.ПолучитьОбласть("Начисления");
	ТЗНач = Начисления.Выгрузить();
	ТЗНач.Свернуть("Сотрудник,ДатаВыхода,ЧасовДвойных,Размер,ВидРасчета","Результат");
	СотрудникПрошл = "";
	ДатаПрошл = "";
	СуммаВсего = 0;
	СуммаВсего2 = 0;
	Для Каждого ТекСтрокаНачисления Из ТЗНач Цикл
		Если СотрудникПрошл <> "" и (СотрудникПрошл <> ТекСтрокаНачисления.Сотрудник или ДатаПрошл <> ТекСтрокаНачисления.ДатаВыхода) Тогда 
			ТабДок.Вывести(ОбластьНачисления);
		КонецЕсли;
		ОбластьНачисления.Параметры.Заполнить(ТекСтрокаНачисления); 
		
		//О.Т. Аверсон 11.04.2017 № 509
		Если ТекСтрокаНачисления.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ДоплатаЗаСверхурочные Тогда 
			ОбластьНачисления.Параметры.Результат2 = ТекСтрокаНачисления.Результат;
			СуммаВсего2 = СуммаВсего2 + ТекСтрокаНачисления.Результат;
		КонецЕсли;
		Если ТекСтрокаНачисления.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ОплатаСверхурочных Тогда 
			ОбластьНачисления.Параметры.Результат1 = ТекСтрокаНачисления.Результат;
			СуммаВсего = СуммаВсего + ТекСтрокаНачисления.Результат;
		КонецЕсли;
		//О.Т. Аверсон 11.04.2017 № 509
		
		СотрудникПрошл = ТекСтрокаНачисления.Сотрудник;
		ДатаПрошл = ТекСтрокаНачисления.ДатаВыхода;
	КонецЦикла;
	ТабДок.Вывести(ОбластьНачисления);
	
	
	ОбластьИтого = Макет.ПолучитьОбласть("Итого");
	ОбластьИтого.Параметры.ЧасовДвойныхВсего = ЧасыВсего;
	
	ОбластьИтого.Параметры.РезультатВсего = СуммаВсего;
	ОбластьИтого.Параметры.РезультатВсего2 = СуммаВсего2;
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Подвал.Параметры.ДолжностьОтветственный = ВыборкаДетальныеЗаписи.Должность;
		Подвал.Параметры.ФИООтветственный = ОбщегоНазначения.ФамилияИнициалыФизЛица(ВыборкаДетальныеЗаписи.Сотрудник);
	КонецЦикла;
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Возврат Новый Структура("Печать","Расчет сверхурочных");
...
Процедура СформироватьДвиженияДанныеДляСвода()
	...
	Для Каждого Стр Из Начисления Цикл
		...
		НоваяЗапись.Часов = Стр.ЧасовДвойных + Стр.ЧасовПолуторных;
