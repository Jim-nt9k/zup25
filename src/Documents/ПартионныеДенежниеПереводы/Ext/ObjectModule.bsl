Процедура Автозаполнение() Экспорт
	...
	//Аверсон 22.06.2020 КВВ СВФР-006173 (
	...
	УдержанияПоИЛ = Новый СписокЗначений;
	Для Каждого Стр из Справочники.ГруппыРасчетов.ДляПереводов.Удержания Цикл 
		УдержанияПоИЛ.Добавить(Стр.ВидРасчета);
	КонецЦикла;
	//Аверсон 22.06.2020 КВВ СВФР-006173 )
	...
	|		И УдержанияРаботниковОрганизаций1.ДокументОснование = УдержанияРаботниковОрганизаций.ДокументОснование
	...
	|   И НЕ УдержанияРаботниковОрганизаций1.ДокументОснование.НаКартСчет
	|   И УдержанияРаботниковОрганизаций1.ДокументОснование.ПроцентПочтовогоСбора = 0";
	
	
	
	Запрос.Текст = ЗапросТекст;
	ВременнаяТаблица = (Запрос.Выполнить().Выгрузить());
	ВременнаяТаблица.Свернуть("Адрес, НаименованиеПолное, Плательщик, Пол, Получатель, Страна, ТипУлицы","СуммаПеревода, ПочтовыйСбор");
	...
	Для Каждого СтрТЗ ИЗ ВременнаяТаблица Цикл
		Отчество = СтрТЗ.Отчество;
		Если ЗначениеЗаполнено(Отчество) Тогда
			СтрТЗ.Пол = ПроцедурыУправленияПерсоналом.ПолучитьПол(Отчество);
		КонецЕсли;
	КонецЦикла;
	//Аверсон 22.06.2020 КВВ СВФР-006173 )
...
Функция СформироватьВыходнойФайл(Отказ) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	СоответствиеПлатежнногоПорученияСПартионнымДенежнымПереводом.ПлатежноеПоручениеИсходящее.Номер КАК Номер,
		|	СоответствиеПлатежнногоПорученияСПартионнымДенежнымПереводом.ПлатежноеПоручениеИсходящее.Дата КАК Дата,
		|	СоответствиеПлатежнногоПорученияСПартионнымДенежнымПереводом.ПлатежноеПоручениеИсходящее.СуммаПоДокументу КАК СуммаПоДокументу
		|ИЗ
		|	РегистрСведений.СоответствиеПлатежнногоПорученияСПартионнымДенежнымПереводом КАК СоответствиеПлатежнногоПорученияСПартионнымДенежнымПереводом
		|ГДЕ
		|	СоответствиеПлатежнногоПорученияСПартионнымДенежнымПереводом.ПартионныйДенежныйПеревод = &ПартионныйДенежныйПеревод";
	
	Запрос.УстановитьПараметр("ПартионныйДенежныйПеревод", Ссылка);
	...
	Иначе	
		...
		Пока Выборка.Следующий() Цикл
			ТекстФайла.ДобавитьСтроку("*|"+ Выборка.Номер +"|"+Формат(Выборка.Дата,"ДФ=dd.MM.yyyy")+"|"+ Формат(Выборка.СуммаПоДокументу,"ЧГ=0") +"|"+НомерСчета+"|"+МФО+"|"+КодВалюты);
		КонецЦикла;
	КонецЕсли;
	// +++ПСА 11-09-2020 СВФР-006182 
	...
	Пока ВыборкаПоРаботникиОрганизации.Следующий()	Цикл
		...
		ПереводСумма = СокрЛП(Строка(Формат(ВыборкаПоРаботникиОрганизации.СуммаПеревода, "ЧДЦ=2; ЧРД=.; ЧГ=0")));
		СуммаПочтовыйСбор = СокрЛП(Строка(Формат(ВыборкаПоРаботникиОрганизации.ПочтовыйСбор, "ЧДЦ=2; ЧРД=.; ЧГ=0")));
		// +++ПСА 11-09-2020 СВФР-006182 
		...
		Если Алименты Тогда
			АлиментыВФайл = "1";
			ТестовоеСообщение = "//Алименты " + Формат(ПериодРегистрации,"ДФ=ММММ");	
			ЗаСчетФизлица = "0";
		Иначе
			АлиментыВФайл = "0";
			ТестовоеСообщение = "";
		...
		КонецЦикла;
		ТекстФайла.ДобавитьСтроку("" + НомерБезНулей + "|"+ПереводСумма+"|"+СуммаПочтовыйСбор+"|"+КодНаправления+
								  "|"+КатегорияПеревода+"|"+ВидУведомления+"|"+ДоставкаНаДом+"|"+Пенсия+"|"+АлиментыВФайл+"|"+ДоговорнойТариф+"|"+ТестовоеСообщение+
								  "|"+Фамилия+"|"+Имя+"|"+Отчество+"|"+Пол+"|"+СтруктураАдрес.Индекс+"|"+КодСтраны+
								  "|"+?(СтруктураАдрес.Регион="","",Врег(Область)+" ОБЛ")+?(СтруктураАдрес.Район="","",","+Врег(СтруктураАдрес.Район)+" Р-Н")+"|"+Врег(Город)+"|"+КодУлицы+"|"+Врег(Улица)+
								  "|"+СтруктураАдрес.Дом+"|"+СтруктураАдрес.Корпус+"|"+СтруктураАдрес.Квартира+"||"+ЗаСчетФизлица);
		// +++ПСА 11-09-2020 СВФР-006182
	КонецЦикла;
...
// ПСА 11-09-2020 СВФР-006182 +++
Функция РассчитатьПочтовыйСбор(СуммаПеревода) Экспорт
	Результат = 0;
	Если СуммаПеревода <> 0 Тогда
		Сбор = 0;
		...
		ЗапросШкала.УстановитьПараметр("НижнийПредел",СуммаПеревода);
		...
		Результат = ОбщегоНазначения.ОкруглитьРезультат(ПроцентПочтовогоСбора / 100 * СуммаПеревода,Константы.ОкруглениеПочтовогоСбора.Получить()); //О.Т. Аверсон 25.11.2016 № 50
		
		//О.Т. Аверсон 24.02.2017 № 339
		МинимальноеЗначениеПочтовогоСбора = Константы.МинимальноеЗначениеПочтовогоСбора.Получить();
		Если не (Результат = 0 и СуммаПеревода = 0) Тогда //О.Т. Аверсон 21.09.2017 № 1052 
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	Если ТипЗНЧ(ДанныеЗаполнения) = Тип("ДокументСсылка.РегистрацияРазовыхНачисленийРаботниковОрганизаций") Тогда 
		ЗаполнитьПоРегистрацияРазовыхНачисленийРаботниковОрганизаций(ДанныеЗаполнения);
	КонецЕсли;	
КонецПроцедуры

Процедура  ЗаполнитьПоРегистрацияРазовыхНачисленийРаботниковОрганизаций(ДанныеЗаполнения)
	
	Организация = ДанныеЗаполнения.Организация;
	ПериодРегистрации = ДанныеЗаполнения.ПериодРегистрации;
	ПолучательРаботник = Истина;
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	РегистрацияРазовыхНачисленийРаботниковОрганизацийОсновныеНачисления.Физлицо КАК Плательщик,
		|	РегистрацияРазовыхНачисленийРаботниковОрганизацийОсновныеНачисления.Результат КАК СуммаПеревода
		|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		НоваяСтрока = ДенежныеПереводы.Добавить();
		НоваяСтрока.Плательщик = Выборка.Плательщик;
		ЗаполнитьСтрокуПоФизЛицу(НоваяСтрока);
		НоваяСтрока.СуммаПеревода = Выборка.СуммаПеревода;
		НоваяСтрока.ПочтовыйСбор = РассчитатьПочтовыйСбор(НоваяСтрока.СуммаПеревода);
	КонецЦикла;
	
КонецПроцедуры	

Процедура ЗаполнитьСтрокуПоФизЛицу(СтрокаТЧ) Экспорт
	СтрокаТЧ.Пол        = СтрокаТЧ.Плательщик.Пол;
	...
	Запрос.УстановитьПараметр("ФизЛицо", СтрокаТЧ.Плательщик);
	РезультатЗапроса = Запрос.Выполнить();
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.Следующий() Цикл
	...
	КонецЦикла;
	СтрокаТЧ.Фамилия    = Фамилия;
	СтрокаТЧ.Имя        = Имя;
	СтрокаТЧ.Отчество   = Отчество;
	...
	|	"","" + АдресаРегистрации.Поле1 + "","" + АдресаРегистрации.Поле2 + "","" + АдресаРегистрации.Поле3 + "","" + АдресаРегистрации.Поле4 + "","" + АдресаРегистрации.Поле5 + "","" + АдресаРегистрации.Поле6 + "","" + АдресаРегистрации.Поле7 + "","" + АдресаРегистрации.Поле8 + "","" + АдресаРегистрации.Поле9 КАК Адрес,
	|	АдресаРегистрации.Поле11 КАК ТипУлицы,
	|	ISNULL(КлассификаторСтранМира.Ссылка,&СтранаПоУмолчанию) КАК Страна
	|ИЗ
	...
	|		ПО АдресаРегистрации.Поле1 = КлассификаторСтранМира.Наименование
	|ГДЕ
	|	АдресаРегистрации.Объект = &ФизЛицо
	|	И АдресаРегистрации.Тип = &Адрес
	|	И АдресаРегистрации.Вид = &ФактическийАдрес";
	Запрос.УстановитьПараметр("Адрес", Перечисления.ТипыКонтактнойИнформации.Адрес);
	//ПСА 19.07.2021 СВФР-007824+++
	//Запрос.УстановитьПараметр("ФактическийАдрес", Справочники.ВидыКонтактнойИнформации.ФактАдресФизЛица);
	Запрос.УстановитьПараметр("ФактическийАдрес", Справочники.ВидыКонтактнойИнформации.ЮрАдресФизЛица);
	//+++ПСА 19.07.2021 СВФР-007824
	Запрос.УстановитьПараметр("ФизЛицо",  СтрокаТЧ.Плательщик);
	Запрос.УстановитьПараметр("СтранаПоУмолчанию",  Справочники.КлассификаторСтранМира.НайтиПоКоду(112));
	...
	Пока Выборка.Следующий() Цикл
		...
		ТипУлицы = Выборка.ТипУлицы;
	КонецЦикла;		
	СтрокаТЧ.Адрес      = Адрес;
	СтрокаТЧ.Страна     = Страна;
	СтрокаТЧ.ТипУлицы   = ТипУлицы;
