...
Процедура РассчитатьДивиденды() Экспорт
	...
	Иначе
		...
		МассивСуммКНачислению = ОбщегоНазначения.РаспределитьПропорционально(ДивидендыНачисляемые,ЗаписиКРасчету.ВыгрузитьКолонку("КоличествоАкций"),,ПериодРегистрации); //О.Т. Аверсон 15.03.2016 № 949
...
Процедура РассчитатьНДФЛ() Экспорт
	...
	Если ДивидендыПолученные > 0 Тогда
		Если ВыборкаПоШапкеДокумента.ДатаРегистрации >= '20080101' Тогда  
			...
			МассивСуммКНачислению = ОбщегоНазначения.РаспределитьПропорционально(ДивидендыПолученные,ВременнаяТаблица.ВыгрузитьКолонку("Результат"),,ВыборкаПоШапкеДокумента.ДатаРегистрации);//О.Т. Аверсон 15.03.2016 № 949
		Иначе
			МассивСуммКНачислению = ОбщегоНазначения.РаспределитьПропорционально(ДивидендыПолученные,ВременнаяТаблица.ВыгрузитьКолонку("КоличествоАкцийРезидентов"),,ВыборкаПоШапкеДокумента.ДатаРегистрации);//О.Т. Аверсон 15.03.2016 № 949
		КонецЕсли;
	КонецЕсли;
	...
	Для каждого СтрокаТЗ Из ВременнаяТаблица Цикл
		...
		СтрокаТЗ.НДФЛ = ОбщегоНазначения.ОкруглитьЗначение2016(Макс(СтрокаТЗ.Результат - СтрокаТЗ.ВычетПоНДФЛ,0) * СтрокаТЗ.СтавкаНДФЛ / 100,ВыборкаПоШапкеДокумента.ДатаРегистрации);//О.Т. Аверсон 15.03.2016 № 949
...
Функция СформироватьЗапросПоНачисления(ВыборкаПоШапкеДокумента)
	...
	//|	ВЫБОР
	//|		КОГДА РаботникиОрганизацииСрезПоследних.Физлицо ЕСТЬ NULL 
	//|				ИЛИ РаботникиОрганизацииСрезПоследних.ЕстьРаботающие = 0
	//|			ТОГДА ИСТИНА
	//|		ИНАЧЕ ЛОЖЬ
	...
	|	ISNULL( РаботникиОрганизацииСрезПоследних.ЕстьРаботающие, 0) = 0 
	|			И Подрядчики.Физлицо ЕСТЬ NULL
	|			И НЕ ТЧНачисления.ПроводитьВзаиморасчеты
	|	КАК ФизлицоАкционер,
	...
	|			СУММА(ВЫБОР
	|					КОГДА НЕ РаботникиОрганизацииСрезПоследних.Регистратор ССЫЛКА Документ.УвольнениеИзОрганизаций
	|						ТОГДА 1
	|					ИНАЧЕ 0
	|				КОНЕЦ) КАК ЕстьРаботающие //О.Т. Аверсон 25.07.2018 № 2208
	|		ИЗ
	...
	|		
	|		СГРУППИРОВАТЬ ПО
	|			РаботникиОрганизацииСрезПоследних.Период,
	|			ВЫБОР
	|				КОГДА РаботникиОрганизацииСрезПоследних.ПериодЗавершения <= &ДатаНачисления
	|						И РаботникиОрганизацииСрезПоследних.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|					ТОГДА РаботникиОрганизацииСрезПоследних.ПодразделениеОрганизацииЗавершения
	...
	|			КОНЕЦ,
	|			РаботникиОрганизацииСрезПоследних.Сотрудник,
	|			РаботникиОрганизацииСрезПоследних.Сотрудник.Физлицо) КАК РаботникиОрганизацииСрезПоследних
	...
	|		ГДЕ
	|			ДоговорНаВыполнениеРаботСФизЛицом.ДатаНачала <= КОНЕЦПЕРИОДА( &ДатаНачисления, МЕСЯЦ )
	|			И ДоговорНаВыполнениеРаботСФизЛицом.ДатаОкончания >= НАЧАЛОПЕРИОДА( &ДатаНачисления, МЕСЯЦ )
	...
	|		ПО ТЧНачисления.Физлицо = Подрядчики.Физлицо
	//(конец) добавлено Кухарчуком Д.Я. 16.08.18, задача СВФР-002337
	|ГДЕ
...
Процедура ДобавитьСтрокуДополнительныхНачислений(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента)
	...
	Движение.БазовыйПериодНачало = НачалоМесяца(ВыборкаПоШапкеДокумента.ПериодРегистрацииДополнительныхНачислений); 
	Движение.БазовыйПериодКонец = КонецМесяца(ВыборкаПоШапкеДокумента.ПериодРегистрацииДополнительныхНачислений);
...
Процедура ДобавитьСтрокуВДвиженияПоРегистрамНакопления(ВыборкаПоШапкеДокумента, ВыборкаПоТЧ, 
	...
	Если СтруктураПроведенияПоРегистрамНакопления.Свойство(ИмяРегистра) Тогда
		...
		Движение.КодДохода              = ПланыВидовРасчета.ДополнительныеНачисленияОрганизаций.ДивидендыРаботников.КодДоходаНДФЛ;//О.Т. Аверсон 27.04.2018 № 1898//Справочники.ДоходыНДФЛ.Код1842;
	...
	Если СтруктураПроведенияПоРегистрамНакопления.Свойство(ИмяРегистра) Тогда
		...
		Если ВыборкаПоТЧ.НДФЛ <> 0 Тогда
			...
			Движение.Организация					= ВыборкаПоШапкеДокумента.ГоловнаяОрганизация;
			Движение.ФизЛицо               	 		= ВыборкаПоТЧ.ФизЛицо;
			Движение.СтавкаНалогообложенияРезидента = ПланыВидовРасчета.ДополнительныеНачисленияОрганизаций.ДивидендыРаботников.КодДоходаНДФЛ.СтавкаНалогообложенияРезидента; //О.Т. Аверсон 27.04.2018 № 1898//Перечисления.НДФЛСтавкиНалогообложенияРезидента.Ставка09;
