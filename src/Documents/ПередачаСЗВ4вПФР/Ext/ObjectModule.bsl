...
Процедура АвтозаполнениеСтрокЗадолженности() Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		СтрокаНачислено.НакопительнаяЧасть = ОбщегоНазначения.ОкруглитьЗначение2016(Выборка.НачисленоНаНакопительнуюЧастьПФР,ДатаДействия);//О.Т. Аверсон 17.03.2016 № 949
		СтрокаНачислено.СтраховаяЧасть = ОбщегоНазначения.ОкруглитьЗначение2016(Выборка.НачисленоНаСтраховуюЧастьПФР,ДатаДействия);//О.Т. Аверсон 17.03.2016 № 949
		СтрокаНачислено = СведенияОЗадолженности.Добавить();
		СтрокаНачислено.Год = Год;
		СтрокаНачислено.ТипСтроки = "Уплачено";
		СтрокаНачислено.НакопительнаяЧасть = ОбщегоНазначения.ОкруглитьЗначение2016(Выборка.НачисленоНаНакопительнуюЧастьПФР,ДатаДействия) - СальдоТекущееНакопительнаяЧасть;//О.Т. Аверсон 17.03.2016 № 949
		СтрокаНачислено.СтраховаяЧасть = ОбщегоНазначения.ОкруглитьЗначение2016(Выборка.НачисленоНаСтраховуюЧастьПФР,ДатаДействия) - СальдоТекущееСтраховаяЧасть;//О.Т. Аверсон 17.03.2016 № 949
	КонецЕсли;
