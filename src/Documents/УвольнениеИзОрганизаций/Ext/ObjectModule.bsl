...
#Если Клиент Тогда
	...
	Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
		...
		ИначеЕсли ИмяМакета = "ПриказОбУвольнении" Тогда
		
		ТабДокумент = ПечатьПриказОбУвольнении("ВсеСотрудники");
		// СВЭЙ СВФР-015593 СОА 24.04.2025)
		КонецЕсли;
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	// СВЭЙ СВФР-015593 СОА 24.04.2025 (
	СтруктураМакетов.Вставить("ПриказОбУвольнении", "Приказ об увольнении");
	// СВЭЙ СВФР-015593 СОА 24.04.2025)
...
Функция СформироватьЗапросДляПечати(Режим)
	...
	ИначеЕсли Режим = "ПоТабличнойЧастиДокумента" Тогда
		...
		|	УвольнениеИзОрганизацииРаботники.Физлицо
		|ИЗ
		...
		|			РаботникиОрганизации.ДолжностьЗавершения КАК ДолжностьЗавершения,
...
//  Табличный документ - печатная форма
//
Функция ПечатьПриказОбУвольнении(ВидПечатнойФормы)
	...
	ТабДокумент = Новый ТабличныйДокумент;
	ТабДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ПечатьПриказОбУвольнении";
	...
	// запоминаем области макета
	Макет = ПолучитьМакет("ПриказОбУвольнении");
	ОбластьМакетаШапка = Макет.ПолучитьОбласть("Шапка"); // Шапка документа
	...
	Если ВидПечатнойФормы = "ОдинСотрудник" Тогда
		...
		НомерДокДляПечати = ?(ЗначениеЗаполнено(НомерП),НомерП,ВыборкаДляШапки.НомерДок);
	Иначе
		НомерДокДляПечати = НомерД;
	КонецЕсли;
	...
	Если ВыборкаРаботники.Количество() > 1 Тогда
		ОбластьМакетаШапка.Параметры.ТекстоваяСтрока = "Об увольнении с работы.";
		ТабДокумент.Вывести(ОбластьМакетаШапка);
	Иначе 
		ОбластьМакетаШапка.Параметры.ТекстоваяСтрока = "Об увольнении с работы";
	КонецЕсли;	
	...
	Пока ВыборкаРаботники.Следующий() Цикл
        ...
        ФразаСклоненнаяДолжность = ""; 
			ЗапросСклонения.УстановитьПараметр("Объект", ВыборкаРаботники.Должность);
			РезультатЗапросаСклонения = ЗапросСклонения.Выполнить();
			Если РезультатЗапросаСклонения.Пустой() Тогда
				УправлениеОтчетами.Просклонять(глСклонение,ВыборкаРаботники.Должность,2,ФразаСклоненнаяДолжность,,ложь);
			Иначе	
		...
		ОбластьМакета.Параметры.Подразделение       = НРег(ВыборкаРаботники.Подразделение); 
		ОбластьМакета.Параметры.ДатаУвольнения      = НРег(ВыборкаРаботники.ДатаУвольнения); 
		ОбластьМакета.Параметры.ОснованиеУвольнения = ВыборкаРаботники.ОснованиеУвольнения; 
		
		ОбластьМакета.Параметры.ФизлицоСокращено = ФИОСклон;  
		 
		ОбластьМакета.Параметры.ДатаУвольнения = Формат(ВыборкаРаботники.ДатаУвольнения, "ДФ= 'дд.ММ.гггг'"); 
		
		Если КоличествоСтрок = 1 Тогда
			
			ОбластьМакетаШапка.Параметры.ФизлицоСокращено = ФИОСклон; 
			ТабДокумент.Вывести(ОбластьМакетаШапка);
...
Функция СформироватьЗапросПоРаботникиОрганизации(ВыборкаПоШапкеДокумента, Режим)
	...
	//толик добавил категорию
	
	//Аверсон 24.07.2023 КВВ СВФР-011154 (
	ТекстЗапроса = "ВЫБРАТЬ 
	|ПланНач.Сотрудник,
	|ПланНач.ВидРасчета,
	|ПланНач.ДействиеЗавершения,
	|ПланНач.ПериодЗавершения
	|ПОМЕСТИТЬ ВтУжеПрекращены
	|ИЗ РегистрСведений.ПлановыеНачисленияРаботниковОрганизаций КАК ПланНач
	|ЛЕВОЕ СОЕДИНЕНИЕ Документ.УвольнениеИзОрганизаций.РаботникиОрганизации КАК ДокУв
	|ПО ПланНач.Сотрудник = ДокУв.Сотрудник 
	|ГДЕ ПланНач.Сотрудник В (&МассивСотрудников)
	|И ПланНач.Регистратор <> &ДокументСсылка
	|И ПланНач.ДействиеЗавершения = ЗНАЧЕНИЕ(Перечисление.ВидыДействияСНачислением.Прекратить)
	|И ПланНач.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|И НАЧАЛОПЕРИОДА(ПланНач.ПериодЗавершения,ДЕНЬ) <= ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(ДокУв.ДатаУвольнения,ДЕНЬ),ДЕНЬ,1)
	|И ДокУв.Ссылка = &ДокументСсылка
	|;
	|";
	//Аверсон 24.07.2023 КВВ СВФР-011154 )
	...
	|	ТЧРаботникиОрганизации.Сотрудник.ТипРаботника КАК ТипРаботника,
	|	ВЫБОР
	...
	|	ДанныеПоРаботникуДоНазначения.Категория,
	|	ДанныеПоРаботникуДоНазначения.СотрудникЗаменяемый,
	
	//Аверсон ЕАЕ 02.03.2022 СВФР-008808
	|	ДанныеПоРаботникуДоНазначения.ауМестоРаботы КАК МестоРаботы
	//Аверсон СВФР-008808
	...
	//Аверсон ЕАЕ 30.10.2025 СВФР-016948
	Если Организация.ИНН = "700103211" Тогда
		ТекстЗапроса = ТекстЗапроса + "	
		|           ГДЕ НЕ СостояниеВнутри.ВременныйПереводКлимовичи";
	КонецЕсли;
	ТекстЗапроса = ТекстЗапроса + "
	//Аверсон СВФР-016948
	...
	|	ТЧРаботникиОрганизации.Сотрудник.ТипРаботника,	
	|	NULL,
	|	NULL,
	|	NULL,
	|	NULL,
	...
	//Аверсон 24.07.2023 КВВ СВФР-011154 (
	|ЛЕВОЕ СОЕДИНЕНИЕ ВтУжеПрекращены
	|ПО НачисленияРаботниковОрганизации.ВидРасчета = ВтУжеПрекращены.ВидРасчета
	|И НачисленияРаботниковОрганизации.Сотрудник = ВтУжеПрекращены.Сотрудник
	//Аверсон 24.07.2023 КВВ СВФР-011154 )
	...
	//Аверсон 24.07.2023 КВВ СВФР-011154 (
	|ГДЕ 
	|ВтУжеПрекращены.ВидРасчета ЕСТЬ NULL 
	//Аверсон 24.07.2023 КВВ СВФР-011154 )
	
	|) КАК СписокВидовРасчета
	...
	|	ДанныеПоРаботникуДоУвольнения.Сотрудник.ТипРаботника,	
	|	NULL,
	|	NULL,
	|	NULL,
	|	NULL,
	...
	//Аверсон ЕАЕ 02.03.2022 СВФР-008808
	|	ДанныеПоРаботникуДоУвольнения.ауМестоРаботы
	//Аверсон СВФР-008808
...
Процедура ПроверитьЗаполнениеСтрокиРаботникаОрганизации(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	ИначеЕсли ВыборкаПоСтрокамДокумента.ВидСтрокиЗапроса = "КонфликтныйДокумент" Тогда
		Если НЕ (ТипЗнч(ВыборкаПоСтрокамДокумента.КонфликтныйДокумент)=Тип("ДокументСсылка.ВозвратНаРаботуОрганизаций")  //Аверсон 13.01.2022 КВВ СВФР-008592
			ИЛИ ТипЗнч(ВыборкаПоСтрокамДокумента.КонфликтныйДокумент)=Тип("ДокументСсылка.ОтсутствиеНаРаботеОрганизаций")) Тогда //Аверсон 20.01.2022 КВВ СВФР-008629
...
Процедура ДобавитьСтрокуВДвиженияПоРегистрамСведений(ВыборкаПоШапкеДокумента, ВыборкаПоРаботникиОрганизации, 
	...
	Если ВыборкаПоРаботникиОрганизации.ВидСтрокиЗапроса = "ДанныеДляДвиженийКадров" Тогда
		...
		Если СтруктураПроведенияПоРегистрамСведений.Свойство(ИмяРегистра) Тогда
			...
			//Аверсон ЕАЕ 02.03.2022 СВФР-008808
			Движение.ауМестоРаботы              = ВыборкаПоРаботникиОрганизации.МестоРаботы;
			//Аверсон СВФР-008808
		КонецЕсли;
	...
	ИначеЕсли ВыборкаПоРаботникиОрганизации.ВидСтрокиЗапроса = "УдержанияРаботниковОрганизации" Тогда
		...
		Если СтруктураПроведенияПоРегистрамСведений.Свойство(ИмяРегистра) Тогда
			Если Не (НеПрекращатьАлименты И ТипЗнч(ВыборкаПоРаботникиОрганизации.ДокументОснованиеРасчета) = Тип("ДокументСсылка.ИсполнительныйЛист")) Тогда //СВФР-015822 СОА

			Движение = Движения[ИмяРегистра].Добавить();
			
			// Свойства
			Движение.Период						= ВыборкаПоРаботникиОрганизации.ДатаУвольнения + мДлинаСуток;//О.Т. 21.01.2015
...
Процедура ПрекратитьДопПенсиюИГражданство()
	//СВЭЙ 16.04.2024 КВВ СВФР-012875 (
	Для Каждого Стр ИЗ РаботникиОрганизации Цикл
		ДатаУвСледМес = КонецМесяца(Стр.ДатаУвольнения)+1;
		
		ДопПенсия = РегистрыСведений.ДополнительнаяПенсияРаботодательРаботник.ПолучитьПоследнее(ДатаУвСледМес,Новый Структура("Сотрудник",Стр.Сотрудник));
		Если ДопПенсия.ПроцентРаботника <> 0 ИЛИ ДопПенсия.ПроцентРаботодателя <> 0 Тогда      
			...
			НоваяЗапись.ФизЛицо = Стр.Сотрудник.Физлицо; 
			НоваяЗапись.Период = ДатаУвСледМес; 
			НоваяЗапись.ПроцентРаботника = 0;
			НоваяЗапись.ПроцентРаботодателя = 0;
			
			НаборЗаписейДопПенсия.Записать(Ложь);
		КонецЕсли;
		
		Гражданство = РегистрыСведений.ГражданствоФизЛиц.ПолучитьПоследнее(ДатаУвСледМес,Новый Структура("ФизЛицо",Стр.Сотрудник.Физлицо));
		Если Гражданство.ИнаяСтавка <> 0 Тогда      
			...
			НоваяЗапись.ФизЛицо = Стр.Сотрудник.Физлицо; 
			НоваяЗапись.Период = ДатаУвСледМес; 
			НоваяЗапись.ИнаяСтавка = 0;  
			НоваяЗапись.Страна = Гражданство.Страна;
			НоваяЗапись.НеИмеетПравоНаПенсию = Гражданство.НеИмеетПравоНаПенсию;
			НоваяЗапись.НеЯвляетсяНалоговымРезидентомРФ = Гражданство.НеЯвляетсяНалоговымРезидентомРФ;
			
			НаборЗаписейГражданство.Записать(Ложь);
		КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Для Каждого мСтрока Из РаботникиОрганизации Цикл
		Отказ = ПрочиеПроцедуры.ПроверитьИОтрегулироватьПеремещение(мСтрока.ДатаУвольнения+мДлинаСуток, мСтрока.Сотрудник, Организация, ЭтотОбъект, мСтрока);
	КонецЦикла;
	...
	Если ВыборкаПоШапкеДокумента.Следующий() Тогда
		...
		Если НЕ Отказ Тогда
			...
			КонецЦикла;
			//Островская Татьяна 11.04.2014 № 17272
			
			//Аверсон ЕАЕ 04.03.2021 СВФР-007286
			Если РасчетДнейКомпенсацииПоВидамОтпусков Тогда
				ДвиженияПоРегиструОтпускаПоТрудовымДоговорамОстатки();
			Иначе
				//Аверсон
				
				Пока ВыборкаПоРаботникиОрганизацииОтпуска.Следующий() Цикл
					ДобавитьСтрокуВДвиженияПоРегиструОтпускаПоТрудовымДоговорам(ВыборкаПоРаботникиОрганизацииОтпуска,ВыборкаПоШапкеДокумента);				
				КонецЦикла;
			КонецЕсли;
			//Островская Татьяна 11.04.2014 № 17272
			...
			Если НЕ Отказ Тогда
				...
				ПрекратитьДопПенсиюИГражданство(); //СВЭЙ 16.04.2024 КВВ СВФР-012875
			КонецЕсли;
		КонецЕсли;
		
	КонецЕсли;
	
	//Островская Татьяна 19.03.2014 № 17206
	Если Организация.ИНН = "100765738" Тогда 
		Для Каждого СтрРаботники из РаботникиОрганизации Цикл 
			СотудникИзменить = СтрРаботники.Сотрудник.ПолучитьОбъект();
			СотудникИзменить.Родитель = Справочники.СотрудникиОрганизаций.НайтиПоНаименованию("Уволенные");
			СотудникИзменить.Записать();
		КонецЦикла;
	КонецЕсли;
	//Островская Татьяна 19.03.2014 № 17206
	...
	//О.Т. Аверсон 13.05.2019 № 4308
	Для Каждого СтрРаботники из РаботникиОрганизации Цикл 
		мНаборЗаписейФИО = РегистрыСведений.ПаспортныеДанныеФизЛиц.СоздатьНаборЗаписей();
		мНаборЗаписейФИО.Отбор.ФизЛицо.Значение = СтрРаботники.Физлицо;
		мНаборЗаписейФИО.Отбор.ФизЛицо.Использование = Истина;
		мНаборЗаписейФИО.Прочитать();
		Колич = мНаборЗаписейФИО.Количество();
		ЗначениеПрошл = Неопределено;
		ЗначНеНапоминатьОСобытии = Ложь;
		Если Колич>0 Тогда 
			ЗначениеПрошл = мНаборЗаписейФИО[Колич-1];
			ЗначНеНапоминатьОСобытии = мНаборЗаписейФИО[Колич-1].НеНапоминатьОСобытии;
		КонецЕсли;
		Если ЗначНеНапоминатьОСобытии <> СтрРаботники.НеНапоминатьОСобытии Тогда 
			мНаборЗаписейФИО = РегистрыСведений.ПаспортныеДанныеФизЛиц.СоздатьНаборЗаписей();
			мНаборЗаписейФИО.Отбор.ФизЛицо.Значение = СтрРаботники.Физлицо;
			мНаборЗаписейФИО.Отбор.ФизЛицо.Использование = Истина;
			мНаборЗаписейФИО.Отбор.Период.Использование		= Истина;
			мНаборЗаписейФИО.Отбор.Период.Значение			= СтрРаботники.ДатаУвольнения;
			мНаборЗаписейФИО.Прочитать();
			Если мНаборЗаписейФИО.Количество()>0 Тогда 
				ЗаписьНабора1 = мНаборЗаписейФИО[0];
			иначе   				
				ЗаписьНабора1 = мНаборЗаписейФИО.Добавить();
			КонецЕсли;
			Если ЗначениеПрошл = Неопределено Тогда 
				ЗаписьНабора1.ФизЛицо = СтрРаботники.Физлицо;
			иначе
				ЗаполнитьЗначенияСвойств(ЗаписьНабора1,ЗначениеПрошл);
			КонецЕсли;
			ЗаписьНабора1.Период		= СтрРаботники.ДатаУвольнения;
			
			ЗаписьНабора1.НеНапоминатьОСобытии	= СтрРаботники.НеНапоминатьОСобытии;
			Если мНаборЗаписейФИО.Количество() > 0 И ЗначениеЗаполнено(мНаборЗаписейФИО[мНаборЗаписейФИО.Количество()-1].Период) Тогда
				мНаборЗаписейФИО[0].Физлицо = СтрРаботники.Физлицо;
				мНаборЗаписейФИО.Отбор.Физлицо.Использование	= Истина;
				мНаборЗаписейФИО.Отбор.Физлицо.Значение			= мНаборЗаписейФИО[мНаборЗаписейФИО.Количество()-1].Физлицо;
				мНаборЗаписейФИО.Отбор.Период.Использование		= Истина;
				мНаборЗаписейФИО.Отбор.Период.Значение			= мНаборЗаписейФИО[мНаборЗаписейФИО.Количество()-1].Период;
				мНаборЗаписейФИО.Записать(Истина);
			КонецЕсли;
		КонецЕсли;
		
		//Аверсон 06.09.2021 КВВ СВФР-008022 (
		Дата_ИНПС = НачалоМесяца(ДобавитьМесяц(СтрРаботники.ДатаУвольнения,1));
		ВидНалогооб_ИНПС = ?(ЗначениеЗаполнено(СтрРаботники.Сотрудник.ВидНалогообложенияОсновной),СтрРаботники.Сотрудник.ВидНалогообложенияОсновной,Организация.ОсновнаяСтавкаНалогообложения);
		...
		ЗапросШкала.УстановитьПараметр("КонецПериода", Дата_ИНПС);
		ЗапросШкала.УстановитьПараметр("Организация", Организация);
		ЗапросШкала.УстановитьПараметр("ВидНалогообложения", ВидНалогооб_ИНПС); 
		Шкала = ЗапросШкала.Выполнить().Выгрузить();
		
		Если Шкала.Количество() = 0 тогда
			Сообщить("Не заполнена шкала подоходного налога на "+ Формат(Дата_ИНПС,"ДЛФ=Д") + " с видом налогообложения """ + СокрЛП(ВидНалогооб_ИНПС) + """");
		Иначе
			Если не СтрРаботники.Сотрудник.ВидЗанятости = Перечисления.ВидыЗанятостиВОрганизации.ВнутреннееСовместительство Тогда   //для всех кроме внутренних
				НаборЗаписей_ИНПС = РегистрыСведений.ИноеНалогообложениеПоСотрудникам.СоздатьНаборЗаписей();  			
				НаборЗаписей_ИНПС.Отбор.Организация.Установить(Организация);
				НаборЗаписей_ИНПС.Отбор.Сотрудник.Установить(СтрРаботники.Сотрудник);
				НаборЗаписей_ИНПС.Отбор.ФизЛицо.Установить(СтрРаботники.Сотрудник.Физлицо);
				
				НаборЗаписей_ИНПС.Прочитать();
				Если НаборЗаписей_ИНПС.Количество() = 0 Тогда
					Новый_ИНПС = НаборЗаписей_ИНПС.Добавить();
				Иначе
					Новый_ИНПС = НаборЗаписей_ИНПС[0];
				КонецЕсли;
				Новый_ИНПС.Организация = Организация;
				Новый_ИНПС.Период = Дата_ИНПС;
				Новый_ИНПС.Сотрудник = СтрРаботники.Сотрудник; 
				Новый_ИНПС.ФизЛицо = СтрРаботники.Сотрудник.Физлицо; 
				Новый_ИНПС.ВидНалогообложения = ВидНалогооб_ИНПС;
				Новый_ИНПС.НДФЛ = Шкала[0].Налог;
				НаборЗаписей_ИНПС.Записать();
			КонецЕсли;
		КонецЕсли;
		//Аверсон 06.09.2021 КВВ СВФР-008022 )
		
		//Аверсон 11.07.2022 СВФР-009298 (пункт 5) (
		Если Организация.ИНН = "806000046" Тогда
			Сообщить(Формат(СтрРаботники.ДатаУвольнения,"ДЛФ=Д") + " по сотруднику " + СокрЛП(СтрРаботники.Сотрудник) + " необходимо подать Сведения о движении работников ПУ-2!");
		КонецЕсли;
		//Аверсон 11.07.2022 СВФР-009298 (пункт 5) )
	...
	//Аверсон 26.10.2022 КВВ СВФР-009310 (
	#Если Клиент Тогда
	Если Организация.ИНН = "700067572" Тогда 
		Для Каждого Стр ИЗ РаботникиОрганизации Цикл
			...
			|И РО.Период > &ДатаПроверки";
			З.УстановитьПараметр("Сотрудник",Стр.Сотрудник);
			З.УстановитьПараметр("ДатаПроверки",КонецДня(КонецДня(Стр.ДатаУвольнения)+1));
			РезЗ = З.Выполнить().Выгрузить();
			Если РезЗ.Количество() > 0 Тогда
				ТекстВопр = "";
				Для Каждого Стр1 Из РезЗ Цикл
					ТекстВопр = ?(ТекстВопр="","","," + Символы.ПС) + ТипЗнч(Стр1.Регистратор) + " № " + СокрЛП(Стр1.Регистратор.Номер) + " от " + Формат(Стр1.Регистратор.Дата,"ДЛФ=Д") + " (дата записи в регистр " +  Формат(Стр1.Период,"ДЛФ=Д") + ")";
				КонецЦикла;
				
				ОтветВопр = Вопрос("Удалить записи из регистра сведений ""Работники организации""" + Символы.ПС + "по сотруднику " + СокрЛП(Стр.Сотрудник) + " из документ" + ?(РезЗ.Количество()=1,"а","ов") + ": " + Символы.ПС + ТекстВопр, РежимДиалогаВопрос.ДаНетОтмена, 0);
				
				Если ОтветВопр = КодВозвратаДиалога.Да Тогда
					Для Каждого Стр1 Из РезЗ Цикл						
						НаборРО = РегистрыСведений.РаботникиОрганизаций.СоздатьНаборЗаписей();
						НаборРО.Отбор.Регистратор.Установить(Стр1.Регистратор);
						НаборРО.Прочитать();
						Для ИндексРО = 1 - НаборРО.Количество() По 0 Цикл
							ЗаписьРО = НаборРО[- ИндексРО];
							Если ЗаписьРО.Сотрудник = Стр.Сотрудник Тогда
								НаборРО.Удалить(- ИндексРО);
							КонецЕсли;
						КонецЦикла;
						НаборРО.Записать();
					КонецЦикла;
	...
	//СВЭЙ 04.06.2024 КВВ СВФР-012926 (
	#Если Клиент Тогда
	Если Организация.ИНН = "700103211" Тогда 
		Для Каждого Стр ИЗ РаботникиОрганизации Цикл
			ТекстИспытСрока = "Окончание испытательного срока " + СокрЛП(Стр.Сотрудник);
			З = Новый Запрос;
			З.Текст = "ВЫБРАТЬ 
			|РО.Пользователь,
			|РО.Задача,
			|РО.Период
			|ИЗ РегистрСведений.ТекущиеЗадачиПользователей КАК РО
			|ГДЕ РО.Задача = &Задача
			|И РО.Период > &ДатаПроверки";
			З.УстановитьПараметр("Задача",ТекстИспытСрока);
			З.УстановитьПараметр("ДатаПроверки",КонецДня(КонецДня(Стр.ДатаУвольнения)+1));
			РезЗ = З.Выполнить().Выгрузить();
			Если РезЗ.Количество() > 0 Тогда
				ТекстВопр = "";
				Для Каждого Стр1 Из РезЗ Цикл
					ТекстВопр = ?(ТекстВопр="","","," + Символы.ПС) + Формат(Стр1.Период,"ДЛФ=Д") + " - " + ТекстИспытСрока;
				КонецЦикла;
				
				ОтветВопр = Вопрос("Удалить записи из регистра сведений ""Текущие задачи""" + Символы.ПС + "по сотруднику " + СокрЛП(Стр.Сотрудник) + ": " + Символы.ПС + Символы.ПС + ТекстВопр, РежимДиалогаВопрос.ДаНетОтмена, 0);
				
				Если ОтветВопр = КодВозвратаДиалога.Да Тогда
					Для Каждого Стр1 Из РезЗ Цикл	
						
						НаборЗаписей=РегистрыСведений.ТекущиеЗадачиПользователей.СоздатьМенеджерЗаписи();
						НаборЗаписей.Период = Стр1.Период;
						НаборЗаписей.Пользователь = Стр1.Пользователь;
						НаборЗаписей.Задача = Стр1.Задача;
			...
			//Аверсон ЕАЕ 30.10.2025 СВФР-016948
			//Месячные тарифные ставки работников
			З = Новый Запрос;
			З.Текст = "ВЫБРАТЬ 
			|РО.Сотрудник,
			|РО.Регистратор,
			|РО.Период
			|ИЗ РегистрСведений.ТарифныеСтавкиРаботниковОрганизации КАК РО
			|ГДЕ РО.Сотрудник = &Сотрудник
			|И РО.Период > &ДатаПроверки";
			З.УстановитьПараметр("Сотрудник",Стр.Сотрудник);
			З.УстановитьПараметр("ДатаПроверки",КонецДня(КонецДня(Стр.ДатаУвольнения)+1));
			...
			Если РезЗ.Количество() > 0 Тогда
				...
				ОтветВопр = Вопрос("Удалить записи из регистра сведений ""Месячные тарифные ставки работников""" + Символы.ПС + "по сотруднику " + СокрЛП(Стр.Сотрудник) + " из документ" + ?(РезЗ.Количество()=1,"а","ов") + ": " + Символы.ПС + ТекстВопр, РежимДиалогаВопрос.ДаНетОтмена, 0);
				
				Если ОтветВопр = КодВозвратаДиалога.Да Тогда
					Для Каждого Стр1 Из РезЗ Цикл						
						НаборРО = РегистрыСведений.ТарифныеСтавкиРаботниковОрганизации.СоздатьНаборЗаписей();
						НаборРО.Отбор.Регистратор.Установить(Стр1.Регистратор);
						НаборРО.Прочитать();
						Для ИндексРО = 1 - НаборРО.Количество() По 0 Цикл
							ЗаписьРО = НаборРО[- ИндексРО];
							Если ЗаписьРО.Сотрудник = Стр.Сотрудник И ЗаписьРО.Период > КонецДня(КонецДня(Стр.ДатаУвольнения)+1) Тогда
								НаборРО.Удалить(- ИндексРО);
							КонецЕсли;
						КонецЦикла;
						НаборРО.Записать();
...
Функция СформироватьЗапросПоРаботникиОрганизацииОтпуска(ВыборкаПоШапкеДокумента)//Островская Татьяна 11.04.2014 № 17272
	...
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	УвольнениеИзОрганизацийОтпуска.ВидОтпуска,
	|	УвольнениеИзОрганизацийОтпуска.КоличествоДней,
	|	УвольнениеИзОрганизацийОтпуска.Сотрудник,
	|	УвольнениеИзОрганизацийОтпуска.РабочийГодС,
	|	УвольнениеИзОрганизацийОтпуска.РабочийГодПо
	|ИЗ
	|	Документ.УвольнениеИзОрганизаций.Отпуска КАК УвольнениеИзОрганизацийОтпуска
	|ГДЕ
	|	УвольнениеИзОрганизацийОтпуска.Ссылка = &ДокументСсылка";
	Запрос.Текст = ТекстЗапроса;
...
Процедура ДобавитьСтрокуВДвиженияПоРегиструОтпускаПоТрудовымДоговорам(ВыборкаПоДокументу,ВыборкаПоШапкеДокумента)//Островская Татьяна 11.04.2014 № 17272
	
	Движение = Движения.ОтпускаПоТрудовымДоговорам.Добавить();
	
	//Измерения
	Движение.ВидДвижения    = ВидДвиженияНакопления.Расход;
	Движение.Период         = ВыборкаПоДокументу.РабочийГодС;
	Движение.ДатаОкончания	= ВыборкаПоДокументу.РабочийГодПо;
...
КонецПроцедуры // ДобавитьСтрокуВДвиженияПоРегиструОтпускаПоТрудовымДоговорам()

//Аверсон ЕАЕ 04.03.2021 СВФР-007286
Процедура ДвиженияПоРегиструОтпускаПоТрудовымДоговорамОстатки()
	Запрос = Новый Запрос;
	Запрос.Текст = 
	...
	//Аверсон ЕАЕ 17.09.2025 СВФР-016376
	//|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	//|	ОтпускаПоТрудовымДоговорамОстатки.Сотрудник,
	//|	ОтпускаПоТрудовымДоговорамОстатки.ВидОтпуска,
	//|	СУММА(ОтпускаПоТрудовымДоговорамОстатки.КоличествоДнейОстаток) КАК КоличествоДней
	//|ИЗ
	//|	РегистрНакопления.ОтпускаПоТрудовымДоговорам.Остатки(
	//|			&МоментВремени,
	//|			Сотрудник В
	//|				(ВЫБРАТЬ
	//|					ВТСотрудники.Сотрудник
	//|				ИЗ
	//|					ВТСотрудники)) КАК ОтпускаПоТрудовымДоговорамОстатки
	//|
	//|СГРУППИРОВАТЬ ПО
	//|	ОтпускаПоТрудовымДоговорамОстатки.Сотрудник,
	//|	ОтпускаПоТрудовымДоговорамОстатки.ВидОтпуска
	...
	|	ВложенныйЗапрос.ВидОтпуска КАК ВидОтпуска,
	|	ВложенныйЗапрос.РабочийГодС КАК РабочийГодС,
	|	ВложенныйЗапрос.РабочийГодПо,
	|	СУММА(ВложенныйЗапрос.Дней) КАК КоличествоДней
	|ИЗ
	|	(ВЫБРАТЬ
	|		ОтпускаПоТрудовымДоговорам.Сотрудник КАК Сотрудник,
	|		ОтпускаПоТрудовымДоговорам.ВидОтпуска КАК ВидОтпуска,
	|		ОтпускаПоТрудовымДоговорам.Период КАК РабочийГодС,
	|		ОтпускаПоТрудовымДоговорам.ДатаОкончания КАК РабочийГодПо,
	|		ВЫБОР
	|			КОГДА ОтпускаПоТрудовымДоговорам.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	...
	|	ГДЕ
	|		ОтпускаПоТрудовымДоговорам.Сотрудник В (ВЫБРАТЬ ВТСотрудники.Сотрудник ИЗ ВТСотрудники)) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.Сотрудник,
	|	ВложенныйЗапрос.РабочийГодС,
	|	ВложенныйЗапрос.ВидОтпуска,
	|	ВложенныйЗапрос.РабочийГодПо
	|ИМЕЮЩИЕ
	|	СУММА(ВложенныйЗапрос.Дней) > 0
	|
	|УПОРЯДОЧИТЬ ПО
	|	РабочийГодС,
	|	ВидОтпуска
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ВидОтпуска = ВыборкаДетальныеЗаписи.ВидОтпуска;
		Сотрудник = ВыборкаДетальныеЗаписи.Сотрудник;
		
		//Аверсон ЕАЕ 17.09.2025 СВФР-016685
		//Отбор = Новый Структура;
		//Отбор.Вставить("Сотрудник", Сотрудник);
		//Отбор.Вставить("ВидОтпуска", ВидОтпуска);
		//НужныеСтроки = Отпуска.НайтиСтроки(Отбор);
		//Если НужныеСтроки.Количество() > 0 Тогда
		//	РабочийГодС = НужныеСтроки[0].РабочийГодС;
		//	РабочийГодПо = НужныеСтроки[0].РабочийГодПо;
		//КонецЕсли;
			РабочийГодС  = ВыборкаДетальныеЗаписи.РабочийГодС;
			РабочийГодПо = ВыборкаДетальныеЗаписи.РабочийГодПо;
		//Аверсон СВФР-016685
		
		Движение.Период         = РабочийГодС;
		Движение.ДатаОкончания	= РабочийГодПо;
		Движение.Сотрудник      = Сотрудник;
		Движение.ВидОтпуска     = ВидОтпуска;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ЗначениеЗаполнено(Организация) И Организация.ИНН = "700103211" Тогда
		КраткийСоставДокДатаУв = ПроцедурыУправленияПерсоналом.ЗаполнитьКраткийСоставДокументаДопКолонокПоОдномуВДоке(РаботникиОрганизации,"ДатаУвольнения");
		КраткийСоставДокСтатьяТК = ПроцедурыУправленияПерсоналом.ЗаполнитьКраткийСоставДокументаДопКолонокПоОдномуВДоке(РаботникиОрганизации,"СтатьяТКРФ");
		Если РаботникиОрганизации.Количество() = 1 Тогда
			ПервыйСотр = РаботникиОрганизации[0].Сотрудник;
			КраткийСоставДокДолжность = СокрЛП(РегистрыСведений.РаботникиОрганизаций.ПолучитьПоследнее(НачалоДня(РаботникиОрганизации[0].ДатаУвольнения),Новый Структура("Сотрудник",РаботникиОрганизации[0].Сотрудник)).Должность);
		КонецЕсли;
	КонецЕсли;
	//СВЭЙ 22.05.2024 КВВ СВФР-012926 )
