...
Процедура СформироватьДвиженияПоНачислениям(ВыборкаПоШапкеДокумента, НаборОсновныеНачисления) Экспорт 
	...
	//Аверсон ЕАЕ 25.07.2025 СВФР-016333
	//Виток
	Если Организация.ИНН = "100194961" И ТекущаяДатаНачала = НачалоМесяца(ТекущаяДатаНачала) И КонецДня(ТекущаяДатаОкончания) = КонецМесяца(ТекущаяДатаОкончания) Тогда
		ТЗПериодыПростоев = ПолучитьПериодыПростоев(Сотрудник,ТекущаяДатаНачала,ТекущаяДатаОкончания);
		Для Каждого Стр Из ТЗПериодыПростоев Цикл
			// строка движений
			Движение = НаборОсновныеНачисления.Добавить();
			
			// Свойства
			Движение.ПериодРегистрации			= ПериодРегистрации;
			Движение.ПериодДействияНачало		= НачалоДня(Стр.ДатаС);
			Движение.ПериодДействияКонец		= КонецДня(Стр.ДатаПо);
			...
			СтруктураГрафики = ПолучитьДанныеСотрудникаГрафики(ВыборкаПоШапкеДокумента.Сотрудник, НачалоДня(Стр.ДатаС));
			ГрафикиСотрудникаПоМесяцам = ПолучитьГрафикРаботыСотрудникаПоПараметрам(СтруктураГрафики, НачалоДня(Стр.ДатаС));
			ДанныеОГрафиках = ГрафикиСотрудникаПоМесяцам.Найти(НачалоДня(Стр.ДатаС),"Месяц");
			Если ДанныеОГрафиках <> Неопределено Тогда
			...
			Движение.ДатаНачалаСобытия			= Стр.ДатаС;
...
Функция СформироватьЗапросПоШапке(Режим = "")
	...
	//О.Т. Аверсон 04.01.2016 № 778
	Запрос.УстановитьПараметр("ФлагБезКонтракта",		БезКонтракта);
	График = РегистрыСведений.РаботникиОрганизаций.ПолучитьПоследнее(ДатаОкончания,
	Новый Структура("Организация, Сотрудник", Организация, Сотрудник)).ГрафикРаботы;
	ТарифныеСтавки = РегистрыСведений.ТарифныеСтавкиРаботниковОрганизации.ПолучитьПоследнее(ДатаОкончания,
	Новый Структура("Организация,Сотрудник", Организация, Сотрудник));
	Запрос.УстановитьПараметр("ТарифнаяСтавка",		ТарифныеСтавки.ТарифнаяСтавка);
	//О.Т. Аверсон 04.01.2016 № 778
	
	НаДату = ?(ЗначениеЗаполнено(Ссылка.ПерерассчитываемыйДокумент) И ЗначениеЗаполнено(ДатаОкончания),ДатаОкончания,ПериодРегистрации); //Аверсон 15.04.2020 КВВ СВФР-005864
	
	//О.Т. Аверсон 09.06.2017 № 706
	Норма = ПрочиеПроцедуры.ПолучитьНормуВремениПоГрафику(График,Перечисления.ВидыУчетаВремени.ПоЧасам,НаДату,Организация);//О.Т. Аверсон 16.10.2017 № 1120  //Аверсон 15.04.2020 КВВ СВФР-005864 (ПериодРегистрации изменено на НаДату)
	ДанныеИндивидуальногоГрафика = ПрочиеПроцедуры.ПолучитьДанныеПоИндивидуальному(НаДату,Сотрудник); //Аверсон 15.04.2020 КВВ СВФР-005864 (ПериодРегистрации изменено на НаДату)
	Если ДанныеИндивидуальногоГрафика.КоличествоЧасов = 0 Тогда 
		Запрос.УстановитьПараметр("НормаПоГрафику",		Норма);
	иначе
		Запрос.УстановитьПараметр("НормаПоГрафику",		ДанныеИндивидуальногоГрафика.КоличествоЧасов);
	КонецЕсли;
	//О.Т. Аверсон 09.06.2017 № 706	
	
	ЧТСтавка = ПрочиеПроцедуры.ПолучитьЧасовуюТарифнуюСтавку(Организация, Сотрудник, НачалоМесяца(НаДату), График,Истина,,,,,,,,); //Аверсон 15.04.2020 КВВ СВФР-005864 (ПериодРегистрации изменено на НаДату)
	
	//Аверсон 23.03.2020 КВВ СВФР-005719 (
	Если РасчетОтЧасовогоТарифаСПовышениями Тогда
		ЧТСтавка = ПрочиеПроцедуры.ПолучитьЧасовуюТарифнуюСтавку(Организация, Сотрудник, НачалоМесяца(НаДату), График,Ложь,Истина,,,,,,,Истина);  //Аверсон 15.04.2020 КВВ СВФР-005864 (ПериодРегистрации изменено на НаДату)
	КонецЕсли;
	//Аверсон 23.03.2020 КВВ СВФР-005719 )
	
	Запрос.УстановитьПараметр("ЧТС",		ЧТСтавка);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВЫБОР
	...
	Если Режим = "Расчет" И ВидПростоя <> Перечисления.ВидыПростояРаботника.ПоВинеРаботодателя Тогда
		...
		"ВЫБРАТЬ
		|	ВЫБОР
		|	КОГДА &ФлагБезКонтракта //О.Т. Аверсон 04.01.2016 № 778
		|		ТОГДА &ТарифнаяСтавка
		|	ИНАЧЕ
		|	ВЫБОР
	...
	|	Выбор когда РегистрацияПростоевВОрганизации.РасчетОтЧасовогоТарифа ИЛИ РегистрацияПростоевВОрганизации.РасчетОтЧасовогоТарифаСПовышениями Тогда  //Аверсон 23.03.2020 КВВ СВФР-005719 (+РасчетОтЧасовогоТарифаСПовышениями)
	|	   &ЧТС
	|	иначе
	|	ВЫБОР
	|		КОГДА РегистрацияПростоевВОрганизации.ВидПростоя = ЗНАЧЕНИЕ(Перечисление.ВидыПростояРаботника.ПоВинеРаботодателя)
	|			ТОГДА 100 * 2 / 3
	|		КОГДА РегистрацияПростоевВОрганизации.ВидПростоя = ЗНАЧЕНИЕ(Перечисление.ВидыПростояРаботника.ОплатаПростояОтТарифнойСтавки) //О.Т. Аверсон 09.06.2017 № 706
	|			ТОГДА ВЫРАЗИТЬ(&ТарифнаяСтавка / &НормаПоГрафику * 2 / 3 КАК ЧИСЛО(15, 2))
...
Функция СформироватьЗапросПоНачислениям(ВыборкаПоШапкеДокумента)
	...
	|	СтрокиДокумента.ОплаченоДнейЧасов как ОплаченоДней  //Островская Татьяна 03.05.2014 № 17331(Вместо ОплаченоДней использовать ОплаченоДнейЧасов)
...
//Аверсон ЕАЕ 25.07.2025 СВФР-016333
Функция ПолучитьПериодыПростоев(Сотр,ДатаНач,ДатаКон)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	СостоянияСрез.Сотрудник,
	|	&НачПериода КАК Период,
	|	СостоянияСрез.Состояние
	|ПОМЕСТИТЬ ВТСостояния
	|ИЗ РегистрСведений.СостояниеРаботниковОрганизаций.СрезПоследних(&НачПериода, Сотрудник = &Сотр) КАК СостоянияСрез
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ГДЕ Состояния.Период МЕЖДУ &НачПериода И &КонПериода
	|	И Состояния.Сотрудник = &Сотр
	|;
	|///////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Состояния.Сотрудник КАК Сотрудник,
	|	Состояния.Состояние КАК Состояние,
	|	Состояния.Период КАК ДатаС,
	|МИНИМУМ(ЕСТЬNULL(ДОБАВИТЬКДАТЕ(СостоянияБольше.Период,СЕКУНДА,-1),&КонПериода)) КАК ДатаПо
	|ПОМЕСТИТЬ ВТПериодыСостояний
	|ИЗ ВТСостояния КАК Состояния
	|ЛЕВОЕ СОЕДИНЕНИЕ ВТСостояния КАК СостоянияБольше
	|ПО Состояния.Сотрудник = СостоянияБольше.Сотрудник
	|	И СостоянияБольше.Период > Состояния.Период
	|
	|СГРУППИРОВАТЬ ПО
	|	Состояния.Сотрудник,
	|	Состояния.Состояние,
	...
	|ВЫБРАТЬ
	|	*
	|ИЗ ВТПериодыСостояний
	|ГДЕ ВТПериодыСостояний.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.Простой)";
...
Функция ПолучитьДанныеСотрудникаГрафики(Сотр, НаДату)
	СтруктураГрафики = Новый Структура;
	СтруктураГрафики.Вставить("Сотрудник", Справочники.СотрудникиОрганизаций.ПустаяСсылка());
	СтруктураГрафики.Вставить("ГрафикРаботы", Справочники.ГрафикиРаботы.ПустаяСсылка());
	СтруктураГрафики.Вставить("ГрафикПолногоРабочегоВремени", Справочники.ГрафикиРаботы.ПустаяСсылка());
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|   РаботникиОрганизацииСрезПоследних.Сотрудник,
	|	ВЫБОР
	|		КОГДА РаботникиОрганизацииСрезПоследних.ПериодЗавершения <= &НаДату
	|				И РаботникиОрганизацииСрезПоследних.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|			ТОГДА РаботникиОрганизацииСрезПоследних.ГрафикРаботыЗавершения
	|		ИНАЧЕ РаботникиОрганизацииСрезПоследних.ГрафикРаботы
	|	КОНЕЦ КАК ГрафикРаботы,
	...
	|	КОНЕЦ КАК ГрафикПолногоРабочегоВремени
	|ИЗ
	|	РегистрСведений.РаботникиОрганизаций.СрезПоследних(&НаДату, Сотрудник = &Сотрудник) КАК РаботникиОрганизацииСрезПоследних";
	...
	Если Не Рез.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			СтруктураГрафики.Сотрудник = Выборка.Сотрудник;
			СтруктураГрафики.ГрафикРаботы = Выборка.ГрафикРаботы;
			СтруктураГрафики.ГрафикПолногоРабочегоВремени = Выборка.ГрафикПолногоРабочегоВремени;
		КонецЦикла;
	КонецЕсли;
	
	Возврат СтруктураГрафики;
КонецФункции
	
// Формирует соответствие в котором находятся действующие графики сотрудника 
// на указанные даты
Функция ПолучитьГрафикРаботыСотрудникаПоПараметрам(СтруктураГрафики,Дат)
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	
	Запрос.УстановитьПараметр("Сотрудник", СтруктураГрафики.Сотрудник);
	Запрос.УстановитьПараметр("ГрафикРаботы", СтруктураГрафики.ГрафикРаботы);
	Запрос.УстановитьПараметр("ГрафикПолногоРабочегоВремени", СтруктураГрафики.ГрафикПолногоРабочегоВремени);
	
	ДатыПоМесяцамТекст = "ВЫБРАТЬ ДАТАВРЕМЯ(" + Формат(Дат,"ДФ=гггг,М,д,Ч,м,с") + ") КАК Период
	|ПОМЕСТИТЬ ВТДатыПоМесяцам";
	ДатыПоМесяцамТекст = ДатыПоМесяцамТекст + "
	|ИНДЕКСИРОВАТЬ ПО Период";
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикиРаботыПоВидамВремени КАК ГрафикиРаботыПоВидамВремени
	|		ПО (ГрафикиРаботыПоВидамВремени.ГрафикРаботы = &Сотрудник)
	|			И ПериодыДействия.Период = ГрафикиРаботыПоВидамВремени.Дата
	|ГДЕ
	|	ГрафикиРаботыПоВидамВремени.План ЕСТЬ НЕ NULL 
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СводныеИндивидуальныеГрафики КАК СводныеИндивидуальныеГрафики
	|		ПО (СводныеИндивидуальныеГрафики.Сотрудник = &Сотрудник)
	|			И НАЧАЛОПЕРИОДА(ПериодыДействия.Период,МЕСЯЦ) = СводныеИндивидуальныеГрафики.Месяц
	|ГДЕ
	|	СводныеИндивидуальныеГрафики.Сотрудник ЕСТЬ НЕ NULL 
	|
	|ИНДЕКСИРОВАТЬ ПО
