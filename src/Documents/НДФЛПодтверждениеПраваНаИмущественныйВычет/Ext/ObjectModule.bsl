...
Процедура ЗаполнитьДокумент(Основание)
	//Аверсон 28.08.2019 КВВ СВФР-004686 (
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.РегистрацияРазовыхУдержанийРаботниковОрганизаций") Тогда
		Режим = РежимДиалогаВопрос.ДаНетОтмена;
		Ответ = Вопрос("Какой вычет будет в документе ""Подтверждение права на имущественный вычет""?" + Символы.ПС + 
		"""Да"" - 630 - вычет социальный;" + Символы.ПС + 
		"""Нет"" - 640 - вычет на страхование;" + Символы.ПС +
		"""Отмена"" - 650 - вычет имущественный.", Режим, 0);
		КодВычетаВДоке = ?(Ответ = КодВозвратаДиалога.Да,Справочники.ВычетыНДФЛ.Код640,?(Ответ = КодВозвратаДиалога.Нет,Справочники.ВычетыНДФЛ.Код640страх,Справочники.ВычетыНДФЛ.Код650));
		
		ПодразделениеОрганизации = Основание.ПодразделениеОрганизации;
		Дата = Основание.Дата;
		КоличествоСтрок = Основание.Удержания.Количество();
		Для СчЦ=0 по КоличествоСтрок-1  Цикл
			Если Основание.Удержания[СчЦ].ЛьготаПН Тогда 
				СтрокаОснования = Основание.Удержания[СчЦ];
				СтрокаДока = РаботникиОрганизации.Добавить();
				СтрокаДока.ФизЛицо = СтрокаОснования.ФизЛицо;
				СтрокаДока.КодВычета = КодВычетаВДоке;
				СтрокаДока.Расходы = СтрокаОснования.Результат;
				СтрокаДока.ДатаСобытия = Основание.Дата;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	//Аверсон 28.08.2019 КВВ СВФР-004686 )
...
Процедура ОбработкаЗаполнения(Основание)
	ЗаполнитьДокумент(Основание);  //Аверсон 28.08.2019 КВВ СВФР-004686
КонецПроцедуры
