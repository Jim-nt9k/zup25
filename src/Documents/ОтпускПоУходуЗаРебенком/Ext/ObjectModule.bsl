...
Процедура Рассчитать(РассчитываемыеТаблицы, РежимПерерасчета = Ложь) Экспорт
	...
	Если РассчитываемыеТаблицы.Начисления Тогда
		...
		Иначе 
			Если ДатаНачала < Дата(2016,07,1) Тогда //О.Т. Аверсон 16.03.2016 № 949
				СреднедневнойЗаработок = Окр(ИсходныеДанные.СреднийЗаработокПособий / ИсходныеДанные.ОтработаноДнейПособий,2);
			иначе
				СреднедневнойЗаработок = ОбщегоНазначения.ОкруглитьЗначение2016(ИсходныеДанные.СреднийЗаработокПособий / ИсходныеДанные.ОтработаноДнейПособий,ДатаНачала);//О.Т. Аверсон 17.03.2016 № 949
			КонецЕсли;
		КонецЕсли;	
...
Процедура ЗаполнитьПлановыеНачисленияУдержания() Экспорт
	...
	//О.Т. Аверсон 29.09.2017 № 1093
	ОтпускаПоУходуСпособы = Новый Массив;
	ОтпускаПоУходуСпособы.Добавить(Перечисления.СпособыРасчетаОплатыТруда.ПособиеМатериОдиночкеДо1_5лет);
	ОтпускаПоУходуСпособы.Добавить(Перечисления.СпособыРасчетаОплатыТруда.ПособиеМатериОдиночкеДо3Лет);     	
	ОтпускаПоУходуСпособы.Добавить(Перечисления.СпособыРасчетаОплатыТруда.ПособиеНаРебенкаВИЧ);
	ОтпускаПоУходуСпособы.Добавить(Перечисления.СпособыРасчетаОплатыТруда.ПособиеНаРебенкаДо18ЛетНадб40);
	ОтпускаПоУходуСпособы.Добавить(Перечисления.СпособыРасчетаОплатыТруда.ПособиеНаРебенкаДо3Лет);
	ОтпускаПоУходуСпособы.Добавить(Перечисления.СпособыРасчетаОплатыТруда.ПособиеНаРебенкаИнвалида);
	ОтпускаПоУходуСпособы.Добавить(Перечисления.СпособыРасчетаОплатыТруда.ПособиеНаРебенкаСНадбавкой40До3Лет);
	ОтпускаПоУходуСпособы.Добавить(Перечисления.СпособыРасчетаОплатыТруда.ПособиеНаРебенкаСтарше3Лет);
	ОтпускаПоУходуСпособы.Добавить(Перечисления.СпособыРасчетаОплатыТруда.ПособиеНеРабМатериНадб40До3Лет);
	ОтпускаПоУходуСпособы.Добавить(Перечисления.СпособыРасчетаОплатыТруда.ПособиеНеРабМатериОдиночкеДо1_5Лет);
	ОтпускаПоУходуСпособы.Добавить(Перечисления.СпособыРасчетаОплатыТруда.ПособиеНеРабМатериОдиночкеДо3Лет);
	ОтпускаПоУходуСпособы.Добавить(Перечисления.СпособыРасчетаОплатыТруда.ПособиеНеРаботающейМатериДо3Лет);
	ОтпускаПоУходуСпособы.Добавить(Перечисления.СпособыРасчетаОплатыТруда.ПособиеНаДетейВВозрастеОт3До18);
	ОтпускаПоУходуСпособы.Добавить(Перечисления.СпособыРасчетаОплатыТруда.ПособиеНаВторогоИПоследующихДо3Лет);
	ОтпускаПоУходуСпособы.Добавить(Перечисления.СпособыРасчетаОплатыТруда.ПособиеСтарше3ЛетИзОтдельныхКатегорийСемейИнвалид);
	ОтпускаПоУходуСпособы.Добавить(Перечисления.СпособыРасчетаОплатыТруда.ПособиеСтарше3ЛетИзОтдельныхКатегорийСемей);
	
	Запрос.УстановитьПараметр("ОтпускаПоУходуСпособы",ОтпускаПоУходуСпособы);
	//О.Т. Аверсон 29.09.2017 № 1093
	...
	|				КОГДА СрезПлановыхНачислений.ПериодЗавершения <= &ДатаАктуальности
	|						И СрезПлановыхНачислений.ПериодЗавершения <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|					ТОГДА СрезПлановыхНачислений.ВидРасчетаЗавершения.СпособРасчета
	|				ИНАЧЕ СрезПлановыхНачислений.ВидРасчета.СпособРасчета
	|			КОНЕЦ В (&ОтпускаПоУходуСпособы))";
...
Функция СформироватьЗапросПоШапке()
	...
	|	ОтпускПоУходуЗаРебенком.ДатаНачалаФакт,
	|	ОтпускПоУходуЗаРебенком.ДатаОкончанияФакт,
	|	ВЫБОР
	...
	|	ОтпускПоУходуЗаРебенком.НапомнитьПоЗавершении,
	|	ОтпускПоУходуЗаРебенком.НеНапоминатьОСобытии
	|ИЗ
	|	Документ.ОтпускПоУходуЗаРебенком КАК ОтпускПоУходуЗаРебенком
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ, Заголовок, ПриПроведении = Истина)
	...
	Если ВыборкаПоШапкеДокумента.ВнутреннийСовместитель Тогда // на всех может быть, как в табеле отражать его
	...
	Если ПриПроведении Тогда
		Если ВыборкаПоШапкеДокумента.ВыплачиватьПособиеДоПолутораЛет Тогда
			Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ДатаОкончанияПособияДоПолутораЛет) Тогда
				ОбщегоНазначения.СообщитьОбОшибке("Не указана дата окончания выплаты пособия за счет ФСС до полутора лет!", Отказ, Заголовок);
			ИначеЕсли ВыборкаПоШапкеДокумента.ДатаОкончанияПособияДоПолутораЛет > ВыборкаПоШапкеДокумента.ДатаОкончания Тогда
				ОбщегоНазначения.СообщитьОбОшибке("Дата окончания выплаты пособия за счет ФСС до полутора лет превышает дату окончания отпуска, будет использована дата окончания отпуска.");
			ИначеЕсли ВыборкаПоШапкеДокумента.ДатаОкончанияПособияДоПолутораЛет < ВыборкаПоШапкеДокумента.ДатаНачала Тогда
				ОбщегоНазначения.СообщитьОбОшибке("Дата окончания выплаты пособия за счет ФСС до полутора лет не должна быть меньше даты начала отпуска!", Отказ, Заголовок);
			КонецЕсли;
...
Процедура ДобавитьСтрокуОтпуска(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента)
	...
	//О.Т. 18.12.2014 № 17730
	Если ВыборкаПоШапкеДокумента.ДатаОкончанияФакт = Дата(1,1,1) Тогда
		Движение.ПериодЗавершения		= ВыборкаПоШапкеДокумента.ДатаОкончания + мДлинаСуток;
	Иначе
		Движение.ПериодЗавершения		= ВыборкаПоШапкеДокумента.ДатаОкончанияФакт + мДлинаСуток;
	КонецЕсли;
	//О.Т. 18.12.2014 № 17730
	Движение.ВидРасчетаЗавершения	= Движение.ВидРасчета;
	Движение.ДействиеЗавершения			= Перечисления.ВидыДействияСНачислением.Прекратить;//О.Т. Аверсон 22.06.2016
	
	Движение = Движения.СостояниеРаботниковОрганизаций.Добавить();
	
	// Свойства
	Если ВыборкаПоШапкеДокумента.ДатаНачалаФакт = Дата(1,1,1) Тогда
		Движение.Период					= ВыборкаПоШапкеДокумента.ДатаНачала;
	Иначе
		Движение.Период					= ВыборкаПоШапкеДокумента.ДатаНачалаФакт;
	КонецЕсли;
	...
	// Ресурсы
	Движение.Состояние = Перечисления.ПричиныОтсутствияНаРаботеВОрганизации.ОтпускПоУходуЗаРебенком;
	Если ВыборкаПоШапкеДокумента.ДатаНачалаФакт = Дата(1,1,1) Тогда
		Движение.НачалоОтпуска	= ВыборкаПоШапкеДокумента.ДатаНачала;
	Иначе
		Движение.НачалоОтпуска	= ВыборкаПоШапкеДокумента.ДатаНачалаФакт;
	КонецЕсли;
	Если ВыборкаПоШапкеДокумента.ДатаОкончанияФакт = Дата(1,1,1) Тогда
		Движение.ОкончаниеОтпуска	= ВыборкаПоШапкеДокумента.ДатаОкончания;
	Иначе
		Движение.ОкончаниеОтпуска	= ВыборкаПоШапкеДокумента.ДатаОкончанияФакт;
	КонецЕсли;	
	...
	Если ВыборкаПоШапкеДокумента.ОсвобождатьСтавку Тогда
		...
		Если ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ДатаОкончания) Тогда
			...
			Иначе
				Движение.Период				= ВыборкаПоШапкеДокумента.ДатаОкончанияФакт + мДлинаСуток;
			КонецЕсли;	
	...
	Если ВыборкаПоШапкеДокумента.НапомнитьПоЗавершении Тогда
		...
		Если ВыборкаПоШапкеДокумента.ДатаОкончанияФакт = Дата(1,1,1) Тогда
			Движение.ДатаИзменения	= ВыборкаПоШапкеДокумента.ДатаОкончания + мДлинаСуток;
		Иначе
			Движение.ДатаИзменения	= ВыборкаПоШапкеДокумента.ДатаОкончанияФакт + мДлинаСуток;
		КонецЕсли;	
		...
		Движение.ПланируемоеСобытие	= Перечисления.НамеченныеСобытияПоПерсоналу.ОтпускПоУходуЗаРебенком;
		
		// Реквизиты
		Движение.ЗаниматьСтавку		= ВыборкаПоШапкеДокумента.ОсвобождатьСтавку;
		Движение.НеНапоминатьОСобытии		= ВыборкаПоШапкеДокумента.НеНапоминатьОСобытии;
...
Процедура ДобавитьСтрокуОсновныеНачисления(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента)
	...
	Если ТипЗнч(ВыборкаПоШапкеДокумента.Ссылка) = Тип("ДокументСсылка.ОтпускПоУходуЗаРебенком") Тогда
		Если не ВыборкаПоШапкеДокумента.Ссылка.НеВозобновлятьПлановыеНачисления
			и ВыборкаПоСтрокамДокумента.ДействиеЗавершения = Перечисления.ВидыДействияСНачислением.Начать Тогда   
			ЗаполнитьЗавершение = Истина;
		КонецЕсли;	
	КонецЕсли;
	
	Если ТипЗнч(ВыборкаПоШапкеДокумента.Ссылка) = Тип("ДокументСсылка.ОтпускПоУходуЗаРебенком") Тогда
		Если ВыборкаПоСтрокамДокумента.ДействиеЗавершения = Перечисления.ВидыДействияСНачислением.Начать 
			и ЗаполнитьЗавершение Тогда 
			Движение = Движения.ПлановыеНачисленияРаботниковОрганизаций.Добавить();
			Движение.Организация = ВыборкаПоШапкеДокумента.ГоловнаяОрганизация;   
			...
			КонецЕсли;
			Движение.Период = ВыборкаПоСтрокамДокумента.ПериодЗавершения; 
			Движение.Сотрудник = ВыборкаПоСтрокамДокумента.Сотрудник;
			Движение.Действие = ВыборкаПоСтрокамДокумента.ДействиеЗавершения;
			Движение.ВидРасчета = ВыборкаПоСтрокамДокумента.ВидРасчетаЗавершения;
				...
				Для Сч = 1 По 6 Цикл
					Если ВыборкаПоСтрокамДокумента.ПроизвольнаяФормулаРасчета Тогда
						Если Сч <= ВыборкаПоСтрокамДокумента.КоличествоПоказателей Тогда
							Если ВыборкаПоСтрокамДокумента["Показатель" + Сч + "ЗапрашиватьПриКадровыхПеремещениях"]
								...
								Движение["Показатель"+ Сч] = ВыборкаПоСтрокамДокумента["Показатель"+ Сч + "Завершения"];
								Движение["Валюта"+ Сч] = ВыборкаПоСтрокамДокумента["Валюта"+ Сч + "Завершения"];
								Движение["ТарифныйРазряд"+ Сч] = ВыборкаПоСтрокамДокумента["ТарифныйРазряд"+ Сч + "Завершения"];
							КонецЕсли;
						КонецЕсли;
					Иначе 
						Движение["Показатель"+ Сч] = ВыборкаПоСтрокамДокумента["Показатель"+ Сч + "Завершения"];
					...
					КонецЕсли;
				КонецЦикла;
			Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.Ссылка.ДатаОкончанияФакт) Тогда
				Движение.Период = КонецДня(ВыборкаПоСтрокамДокумента.Ссылка.ДатаОкончанияФакт)+ 1;
			КонецЕсли;   
...
Процедура ДобавитьСтрокуПлановыхУдержаний(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента)
	...
	Движение.ПериодЗавершения = ?(ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ДатаОкончанияФакт) И Движение.ДействиеЗавершения = Перечисления.ВидыДействияСНачислением.Начать,ВыборкаПоШапкеДокумента.ДатаОкончанияФакт,ВыборкаПоШапкеДокумента.ДатаОкончания) + мДлинаСуток;  //Аверсон 17.09.2021 КВВ СВФР-008080
	...
	Если ВыборкаПоШапкеДокумента.Следующий() Тогда
		...
		Если НЕ Отказ Тогда
			...
			Если ВыборкаПоШапкеДокумента.ВыплачиватьПособиеДоПолутораЛет Тогда
				...
				Движение.Сотрудник                  = ВыборкаПоШапкеДокумента.Сотрудник;
				Движение.Организация				= ВыборкаПоШапкеДокумента.ГоловнаяОрганизация;
				Движение.ВидРасчетаИзмерение    	= ВыборкаПоШапкеДокумента.ВидРасчетаДоПолутораЛет;
				
				// Ресурсы
				Движение.Действие					= Перечисления.ВидыДействияСНачислением.Начать;
				Движение.ВидРасчета           		= ВыборкаПоШапкеДокумента.ВидРасчетаДоПолутораЛет;

				Движение.Показатель1				= ВыборкаПоШапкеДокумента.ПроцентПособияДоПолутораЛет;
				Движение.Показатель2				= ВыборкаПоШапкеДокумента.КоличествоДетей;
				Движение.Показатель3				= ВыборкаПоШапкеДокумента.КоличествоДетей;
				...
				Иначе
					Движение.ПериодЗавершения		= Мин(ВыборкаПоШапкеДокумента.ДатаОкончанияФакт,ВыборкаПоШапкеДокумента.ДатаОкончанияПособияДоПолутораЛет) + 86400;
				КонецЕсли;
				//О.Т. 18.12.2014 № 17730
				Движение.ДействиеЗавершения			= Перечисления.ВидыДействияСНачислением.Прекратить;
			 	Движение.ВидРасчетаЗавершения		= Движение.ВидРасчета;
			...
			Если ВыборкаПоШапкеДокумента.ВыплачиватьПособиеДоТрехЛет Тогда
				...
				Иначе
					Движение.ПериодЗавершения		= Мин(ВыборкаПоШапкеДокумента.ДатаОкончанияФакт,ВыборкаПоШапкеДокумента.ДатаОкончанияПособияДоТрехЛет) + 86400;
				КонецЕсли;
				//О.Т. 18.12.2014 № 17730 
				Движение.ДействиеЗавершения			= Перечисления.ВидыДействияСНачислением.Прекратить;
				Движение.ВидРасчетаЗавершения		= Движение.ВидРасчета;
