	...
	Пока ДанныеДляРасчетаЕСН.Следующий() Цикл
		...
		иначе
			...
			Если ЗначениеИнойСтавки > 0 Тогда 
				...
				Если ДанныеДляРасчетаЕСН.БазаДоходФСЗН - ДанныеДляРасчетаЕСН.БазаДоходФСЗН_Прошл > 0  // ДанныеДляРасчетаЕСН.БазаДоходФСЗН  > 0   проверим больше ли за минусом прошлого месяца
		...
		Если ФондФСЗН < 0 Тогда
			...
			РазмерСреднейЗарплатыПред = РегистрыСведений.РазмерыПараметровДляРасчетаРБ.ПолучитьПоследнее(ПредМес,Новый Структура("ВидПараметра",Перечисления.ВидыПараметровДляРасчетаЗарплаты.СредняяЗПРаботников)).Размер;
