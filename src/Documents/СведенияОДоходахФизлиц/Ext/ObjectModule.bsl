...
//	МассивКодовДоходовСтандартные.Добавить(Справочники.ВычетыНДФЛ.Код630);
	...
	ДоходыПоВидамЗаГод =
	"ВЫБРАТЬ
	|	ВЫБОР КОГДА КодДохода.СтавкаНалогообложенияРезидента = &Ставка13 ТОГДА
	|		ВЫБОР КОГДА КодДохода В (&МассивКодовДоходовРаздела7) ТОГДА ""ДоходыРаздела7""
	|		ИНАЧЕ  
	|			ВЫБОР КОГДА КодДохода В (&МассивКодовДоходовАвторские) ТОГДА ""ДоходыРазделаАвторские""
	|			ИНАЧЕ  
	|				ВЫБОР КОГДА КодДохода В (&МассивКодовДоходовЛьготы) ТОГДА ""ДоходыРазделаЛьготы""
	|				ИНАЧЕ 
	|					ВЫБОР КОГДА КодДохода В (&МассивКодовДоходовНеОбл) ТОГДА ""ДоходыРазделаНеобл""
	|					ИНАЧЕ  ""ДоходыРаздела3""
	|					КОНЕЦ 
	|				КОНЕЦ 
	|			КОНЕЦ 
	|		КОНЕЦ 
	|	ИНАЧЕ ВЫБОР КОГДА КодДохода.СтавкаНалогообложенияРезидента <> &СтавкаНеОблагается ТОГДА ""НеДоход"" КОНЕЦ
	...
	//|	НДФЛСведенияОДоходах.ВидДвижения = Значение(ВидДвиженияНакопления.Расход) И //О.Т. Аверсон 21.02.2017
	|	НДФЛСведенияОДоходах.ПериодРегистрации <= &ДатаПодачиСведений И
	|	НДФЛСведенияОДоходах.ОбособленноеПодразделение В (&СписокОтчитывающихсяПодразделений) И
	|	НДФЛСведенияОДоходах.ФизЛицо в (" + СписокФизлицТекст + ")
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	СУММА(ВЫБОР
	|		КОГДА ГражданствоФизЛиц.Страна.УчастникЕАЭС
	|			ТОГДА НДФЛСведенияОДоходах.СуммаДохода
	|		ИНАЧЕ 0
	|	КОНЕЦ) КАК ОблагаемыйДоходЕАЭС,
	...
	|	НАЧАЛОПЕРИОДА(РегламентированныйПроизводственныйКалендарь.ДатаКалендаря, МЕСЯЦ) КАК Периоды
	|ИЗ
	|	РегистрСведений.РегламентированныйПроизводственныйКалендарь КАК РегламентированныйПроизводственныйКалендарь
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГражданствоФизЛиц КАК ГражданствоФизЛиц
	|		ПО (НАЧАЛОПЕРИОДА(РегламентированныйПроизводственныйКалендарь.ДатаКалендаря, МЕСЯЦ) >= НАЧАЛОПЕРИОДА(ГражданствоФизЛиц.Период, МЕСЯЦ))
	|ГДЕ
	|	ГражданствоФизЛиц.Период <= &КонецНП
	|	И РегламентированныйПроизводственныйКалендарь.ДатаКалендаря МЕЖДУ &НачалоНП И &КонецНП
	|
	|СГРУППИРОВАТЬ ПО
	|	ГражданствоФизЛиц.ФизЛицо,
	|	НАЧАЛОПЕРИОДА(РегламентированныйПроизводственныйКалендарь.ДатаКалендаря, МЕСЯЦ)) КАК Гражданство
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ГражданствоФизЛиц КАК ГражданствоФизЛиц
	|	ПО Гражданство.ФизЛицо = ГражданствоФизЛиц.ФизЛицо
	|		И Гражданство.Период = ГражданствоФизЛиц.Период
	|ПО НДФЛСведенияОДоходах.ФизЛицо = Гражданство.ФизЛицо
	|	И (НАЧАЛОПЕРИОДА(НДФЛСведенияОДоходах.Период, МЕСЯЦ) = Гражданство.Периоды)
	|
	|ГДЕ
	|	НДФЛСведенияОДоходах.Период МЕЖДУ &НачалоНП И &КонецНП И
	|	НДФЛСведенияОДоходах.ПериодРегистрации <= &ДатаПодачиСведений И
	...
	|
	|ГДЕ
	//ПСА 16.02.2021 СВФР-007186 +++ 
	//|	Период <= &ДатаПодачиСведений И
	//|   Период >= &НачалоНП И
	|	((Период <= &ДатаПодачиСведений И
    |	Период >= &НачалоНП  и ВидСтроки <> &РасчетыСБюджетомНачисление)
    |	или
    |	(МесяцНалоговогоПериода <= &ДатаПодачиСведений И
    |	МесяцНалоговогоПериода >= &НачалоНП  и ВидСтроки = &РасчетыСБюджетомНачисление)) И
   //+++ ПСА 16.02.2021 СВФР-007186
	//|	ВидСтроки = &РасчетыСБюджетомНачисление И
	|	ОбособленноеПодразделение В (&СписокОтчитывающихсяПодразделений) И
	|	ФизЛицо в (" + СписокФизлицТекст + ")
	|СГРУППИРОВАТЬ ПО 
	|	ФизЛицо, МесяцНалоговогоПериода, СтавкаНалогообложенияРезидента
	|	УПОРЯДОЧИТЬ ПО
	|	Период"; //Островская Татьяна 31.01.2014 № 17055
	...
	|	ВЫБОР КОГДА (Гражданство.ФизЛицо ЕСТЬ NULL) ИЛИ (Гражданство.Страна  = &ПустаяСтрана) 
	//+ СВЭЙ  21.03.19 КДЯ  СВФР-004094
	|				ИЛИ (НЕ ПаспортныеДанные.ФизЛицо ЕСТЬ NULL  
	|						И ( ПаспортныеДанные.ДокументВид.КодИМНС = ""09"" ИЛИ ПаспортныеДанные.ДокументВид.КодИМНС = ""9"" ) )
	//- СВЭЙ  21.03.19 КДЯ  СВФР-004094
	|			ТОГДА ""112"" ИНАЧЕ Гражданство.Страна.Код КОНЕЦ КАК КодСтраныГражданства,
	|	ВЫБОР КОГДА (Гражданство.ФизЛицо ЕСТЬ NULL) ИЛИ НЕ(Гражданство.НеЯвляетсяНалоговымРезидентомРФ) ТОГДА """" ИНАЧЕ Страны.Код КОНЕЦ КАК АдресКодСтраны,	
	|	ВЫБОР КОГДА (Гражданство.ФизЛицо ЕСТЬ NULL) ИЛИ НЕ(Гражданство.НеЯвляетсяНалоговымРезидентомРФ) ТОГДА АдресаРегистрации.Поле1 ИНАЧЕ """" КОНЕЦ КАК АдресИндекс,	
	|	ВЫБОР КОГДА (Гражданство.ФизЛицо ЕСТЬ NULL) ИЛИ НЕ(Гражданство.НеЯвляетсяНалоговымРезидентомРФ) ТОГДА РегионыПоРегистрации.Код ИНАЧЕ """" КОНЕЦ КАК АдресРегион,	
	|	ВЫБОР КОГДА (Гражданство.ФизЛицо ЕСТЬ NULL) ИЛИ НЕ(Гражданство.НеЯвляетсяНалоговымРезидентомРФ) ТОГДА АдресаРегистрации.Поле3 ИНАЧЕ """" КОНЕЦ КАК АдресРайон,	
	|	ВЫБОР КОГДА (Гражданство.ФизЛицо ЕСТЬ NULL) ИЛИ НЕ(Гражданство.НеЯвляетсяНалоговымРезидентомРФ) ТОГДА АдресаРегистрации.Поле4 ИНАЧЕ АдресаИностранцев.Представление КОНЕЦ КАК АдресГород,	
	|	ВЫБОР КОГДА (Гражданство.ФизЛицо ЕСТЬ NULL) ИЛИ НЕ(Гражданство.НеЯвляетсяНалоговымРезидентомРФ) ТОГДА АдресаРегистрации.Поле5 ИНАЧЕ """" КОНЕЦ КАК АдресНаселенныйПункт,	
	|	ВЫБОР КОГДА (Гражданство.ФизЛицо ЕСТЬ NULL) ИЛИ НЕ(Гражданство.НеЯвляетсяНалоговымРезидентомРФ) ТОГДА АдресаРегистрации.Поле6 ИНАЧЕ """" КОНЕЦ КАК АдресУлица,	
	|	ВЫБОР КОГДА (Гражданство.ФизЛицо ЕСТЬ NULL) ИЛИ НЕ(Гражданство.НеЯвляетсяНалоговымРезидентомРФ) ТОГДА АдресаРегистрации.Поле7 ИНАЧЕ """" КОНЕЦ КАК АдресДом,	
	|	ВЫБОР КОГДА (Гражданство.ФизЛицо ЕСТЬ NULL) ИЛИ НЕ(Гражданство.НеЯвляетсяНалоговымРезидентомРФ) ТОГДА АдресаРегистрации.Поле8 ИНАЧЕ """" КОНЕЦ КАК АдресКорпус,	
	|	ВЫБОР КОГДА (Гражданство.ФизЛицо ЕСТЬ NULL) ИЛИ НЕ(Гражданство.НеЯвляетсяНалоговымРезидентомРФ) ТОГДА АдресаРегистрации.Поле9 ИНАЧЕ """" КОНЕЦ КАК АдресКвартира,
	...
	Для каждого СтрокаТЧ из РаботникиОрганизации цикл
		...
		НовыйПроцент = ОбщегоНазначения.ОкруглитьЗначение2016(ОбработаноФизЛиц / ВсегоФизлиц * 100,ДатаПодачиСведений);//О.Т. Аверсон 17.03.2016 № 949
		...
		//Если СокрЛП(СтрокаТЧ.УНП) = "" Тогда
			...
			//- СВЭЙ  21.03.19 КДЯ  СВФР-004094
			//+ СВЭЙ  21.03.19 КДЯ  СВФР-004094
			Справка.АдрМЖ.Значение.Страна = ?( СокрЛП( СтрокаТЧ.КодДокумента ) = "09" ИЛИ СокрЛП( СтрокаТЧ.КодДокумента ) = "9",
										"112", СтрокаТЧ.КодСтраны );
			//- СВЭЙ  21.03.19 КДЯ  СВФР-004094
		...
		Пока ВыборкаДоходовПоВидамЗаГод.НайтиСледующий(СтруктураПоискаФизЛица) Цикл
			...
			ИначеЕсли ВыборкаДоходовПоВидамЗаГод.ВидСтрокиЗапроса = "ДоходыАвторские" Тогда
				 //раздел 3.1.1
				...
				//Островская Татьяна 30.01.2014 № 17055
				Справка.ВычетыАвторские.Значение["Доход" + ВыборкаДоходовПоВидамЗаГод.КодДохода] = ВыборкаДоходовПоВидамЗаГод.СуммаВычета;
				Справка.ВычетыАвторские.Значение["КодДохода" + ВыборкаДоходовПоВидамЗаГод.КодДохода] = ВыборкаДоходовПоВидамЗаГод.КодДохода;
				//Островская Татьяна 30.01.2014 № 17055
	...
	//	Если Справка.СтатусФЛ.Значение = 1 Тогда// только для резидентов
			...
			Пока ВыборкаДоходыПоМесяцамПоСтавке13ДляРаздела3_2.НайтиСледующий(СтруктураПоискаФизЛица) Цикл
				...
				Справка.ДоходМесЕАЭС.Значение["Доход" + Месяц(ВыборкаДоходыПоМесяцамПоСтавке13ДляРаздела3_2.Период)] = ВыборкаДоходыПоМесяцамПоСтавке13ДляРаздела3_2.ОблагаемыйДоходЕАЭС; //О.Т. Аверсон 01.04.2019 № 4172
			...
			Пока ВыборкаИмущественныеВычеты.НайтиСледующий(СтруктураПоискаФизЛица) Цикл
				Если ВыборкаИмущественныеВычеты.СумВычет <> 0 Тогда
			...
			Пока ВыборкаНачУдНалогЗаГодПоСтавкам.НайтиСледующий(СтруктураПоискаФизЛица) Цикл
				...
				КонецЕсли;
				//Островская Татьяна 29.01.2014 № 17055
				если ВыборкаНачУдНалогЗаГодПоСтавкам.НачисленоЗаТекущийГод <> 0 Тогда 
					НоваяСтрока = Справка.ВычетПодоходный.Значение.Добавить();
					НоваяСтрока.МесяцВычета = Месяц(ВыборкаНачУдНалогЗаГодПоСтавкам.Период);
					НоваяСтрока.СуммаВычета = ВыборкаНачУдНалогЗаГодПоСтавкам.НачисленоЗаТекущийГод;					
				КонецЕсли;
				//Островская Татьяна 29.01.2014 № 17055
...
Процедура ПечатьФайла(РежимПечати = 0, МестоВызова = "") Экспорт
	...
	//Островская Татьяна 29.01.2014 № 17055
	Макет2НДФЛ = ?(НалоговыйПериод>=2009,ПолучитьМакет("Приложение32014"),Макет2НДФЛ);
	//Островская Татьяна 29.01.2014 № 17055
	...
	//Островская Татьяна 29.01.2014 № 17055
	ОбластьР3Взносы = Макет2НДФЛ.ПолучитьОбласть("Р3Взносы");
	ОбластьР3Возврат = Макет2НДФЛ.ПолучитьОбласть("Р3Возврат");
	
	//О.Т. Аверсон 01.04.2019 № 4172
	ОбластьР3ВозвратСтрока = Макет2НДФЛ.ПолучитьОбласть("Р3ВозвратСтрока");
	ОбластьР3Возврат1 = Макет2НДФЛ.ПолучитьОбласть("Р3Возврат1");
	//О.Т. Аверсон 01.04.2019 № 4172
	ОбластьСуммаДохода = Макет2НДФЛ.ПолучитьОбласть("СуммаДохода");  
	ОбластьР3ШапкаПодоходный = Макет2НДФЛ.ПолучитьОбласть("Р3ШапкаПодоходный");
	ОбластьР3СтрокаПодоходный = Макет2НДФЛ.ПолучитьОбласть("Р3СтрокаПодоходный");
	Р3ШапкаПрофВыч = Макет2НДФЛ.ПолучитьОбласть("Р3ШапкаПрофВыч");
	СтрокаПрофВыч = Макет2НДФЛ.ПолучитьОбласть("СтрокаПрофВыч");
	Р3ШапкаВычСПолучДох = Макет2НДФЛ.ПолучитьОбласть("Р3ШапкаВычСПолучДох");

	СтрокаВычСПолучДох = Макет2НДФЛ.ПолучитьОбласть("СтрокаВычСПолучДох");
	...
	ОбластьР3ПрочееСтрока = Макет2НДФЛ.ПолучитьОбласть("СтрокаР3Прочее");
	ОбластьР3Прочее1 = Макет2НДФЛ.ПолучитьОбласть("Р3Прочее1");
	...
	ТекстДляИМНС.ДобавитьСтроку("0.5:"+НаборЗаписейНА.месВып.Значение);
...
//		ОбластьШапкаСправки.Параметры.Гражданство   = Справка.Гражд.Значение;
		...
		//Аверсон 27.04.2021 КВВ СВФР-007465 (
		ОбластьШапкаСправки.Параметры.КодСтраныИГ 			= "";
		ОбластьШапкаСправки.Параметры.ОбластьИГ 			= "";
		ОбластьШапкаСправки.Параметры.РайонИГ 				= "";
		ОбластьШапкаСправки.Параметры.НаселенныйПунктИГ 	= "";
		ОбластьШапкаСправки.Параметры.ТипУлицыИГ		 	= "";
		ОбластьШапкаСправки.Параметры.УлицаИГ			 	= "";
		ОбластьШапкаСправки.Параметры.ДомИГ				 	= "";
		ОбластьШапкаСправки.Параметры.КорпусИГ			 	= "";
		ОбластьШапкаСправки.Параметры.КвартираИГ		 	= "";
		
		ЗапросИГ = Новый Запрос;
		ЗапросИГ.Текст = "
		...
		|";
		ЗапросИГ.УстановитьПараметр("Объект",РаботникиОрганизации[НомерДокумента-1].Физлицо);
		ЗапросИГ.УстановитьПараметр("Тип",Перечисления.ТипыКонтактнойИнформации.Адрес);
		ЗапросИГ.УстановитьПараметр("Вид",Справочники.ВидыКонтактнойИнформации.ИнострАдресФизЛица);
		РезИГ = ЗапросИГ.Выполнить().Выгрузить();
		Если РезИГ.Количество() <> 0 Тогда
			Если ЗначениеЗаполнено(РезИГ[0].Поле1) Тогда
				СсылкаНаРодителя = Справочники.КлассификаторСтранМира.НайтиПоНаименованию(РезИГ[0].Поле1);
				ОбластьШапкаСправки.Параметры.КодСтраныИГ 			= ?(ЗначениеЗаполнено(СсылкаНаРодителя),СсылкаНаРодителя.Код,"");
			КонецЕсли;
			ОбластьШапкаСправки.Параметры.ОбластьИГ 			= ?(ЗначениеЗаполнено(РезИГ[0].Поле2),РезИГ[0].Поле2,"");
			ОбластьШапкаСправки.Параметры.РайонИГ 				= ?(ЗначениеЗаполнено(РезИГ[0].Поле3),РезИГ[0].Поле3,"");
			ОбластьШапкаСправки.Параметры.НаселенныйПунктИГ 	= ?(ЗначениеЗаполнено(РезИГ[0].Поле4),РезИГ[0].Поле4,"");
			ОбластьШапкаСправки.Параметры.ТипУлицыИГ		 	= ?(ЗначениеЗаполнено(РезИГ[0].Поле11),РезИГ[0].Поле11,"");
			ОбластьШапкаСправки.Параметры.УлицаИГ			 	= ?(ЗначениеЗаполнено(РезИГ[0].Поле6),РезИГ[0].Поле6,"");
			ОбластьШапкаСправки.Параметры.ДомИГ				 	= ?(ЗначениеЗаполнено(РезИГ[0].Поле7),РезИГ[0].Поле7,"");
			ОбластьШапкаСправки.Параметры.КорпусИГ			 	= ?(ЗначениеЗаполнено(РезИГ[0].Поле8),РезИГ[0].Поле8,"");
			ОбластьШапкаСправки.Параметры.КвартираИГ		 	= ?(ЗначениеЗаполнено(РезИГ[0].Поле9),РезИГ[0].Поле9,"");
		КонецЕсли;
		//Аверсон 27.04.2021 КВВ СВФР-007465 )
		...
		ТекстДляИМНС.ДобавитьСтроку("1.0:"+Формат(НомерСправки, "ЧГ=0"));
		ТекстДляИМНС.ДобавитьСтроку("1.1:"+ИНН_КПП_НА);
		ТекстДляИМНС.ДобавитьСтроку("1.2:"+СведенияОНА12);
		Если не ПустаяСтрока(Справка.ИННФЛ.Значение) тогда
		...
		Если ЗначениеЗаполнено(ИННФиз) тогда
		...
		Иначе
			СтрокаСообщения = "";
			Если ПустаяСтрока(Справка.АдрМЖ.Значение.Страна) тогда
				ТекстДляИМНС.ДобавитьСтроку("2.6.1:"+СокрЛП(Справка.АдрМЖ.Значение.Страна));
				СтрокаСообщения = СтрокаСообщения + " код страны";
			КонецЕсли;
			Если ПустаяСтрока(Справка.АдрМЖ.Значение.Индекс) тогда
				ТекстДляИМНС.ДобавитьСтроку("2.6.2:"+СокрЛП(Справка.АдрМЖ.Значение.Индекс));
				СтрокаСообщения = СтрокаСообщения + " индекс";
			КонецЕсли;
			Если ПустаяСтрока(КодСОАТО) тогда
				ТекстДляИМНС.ДобавитьСтроку("2.6.3:"+(Формат(Число(КодСОАТО),"ЧГ=0")));
				СтрокаСообщения = СтрокаСообщения + ", код СОАТО";
			КонецЕсли;
			Если ПустаяСтрока(Справка.АдрМЖ.Значение.Область) тогда
				ТекстДляИМНС.ДобавитьСтроку("2.6.4:"+СокрЛП(Справка.АдрМЖ.Значение.Область));
				СтрокаСообщения = СтрокаСообщения + ", область";
			КонецЕсли;
			Если ПустаяСтрока(Справка.АдрМЖ.Значение.Район) тогда
				ТекстДляИМНС.ДобавитьСтроку("2.6.5:"+СокрЛП(Справка.АдрМЖ.Значение.Район));
				СтрокаСообщения = СтрокаСообщения + ", район";
			КонецЕсли;
			Если ПустаяСтрока(Справка.АдрМЖ.Значение.Сельсовет) тогда
				ТекстДляИМНС.ДобавитьСтроку("2.6.6:"+СокрЛП(Справка.АдрМЖ.Значение.Сельсовет));
			КонецЕсли;
			Если ПустаяСтрока(Справка.АдрМЖ.Значение.ТипНаселенногоПункта) тогда
				ТекстДляИМНС.ДобавитьСтроку("2.6.7:"+СокрЛП(Справка.АдрМЖ.Значение.ТипНаселенногоПункта));
				СтрокаСообщения = СтрокаСообщения + ", тип нас. пункта";
			КонецЕсли;
			Если ПустаяСтрока(Справка.АдрМЖ.Значение.НаселенныйПункт) тогда
				ТекстДляИМНС.ДобавитьСтроку("2.6.8:"+СокрЛП(Справка.АдрМЖ.Значение.НаселенныйПункт));
				СтрокаСообщения = СтрокаСообщения + ", название нас. пункта";
			КонецЕсли;
			Если ПустаяСтрока(Справка.АдрМЖ.Значение.типУлицы) тогда
				ТекстДляИМНС.ДобавитьСтроку("2.6.9:"+СокрЛП(Справка.АдрМЖ.Значение.типУлицы));
				СтрокаСообщения = СтрокаСообщения + ", тип улицы";
			КонецЕсли;
			Если ПустаяСтрока(Справка.АдрМЖ.Значение.Улица) тогда
				ТекстДляИМНС.ДобавитьСтроку("2.6.10:"+СокрЛП(Справка.АдрМЖ.Значение.Улица));
				СтрокаСообщения = СтрокаСообщения + ", название улицы";
			КонецЕсли;
			Если ПустаяСтрока(Справка.АдрМЖ.Значение.Дом) тогда
				ТекстДляИМНС.ДобавитьСтроку("2.6.11:"+СокрЛП(Справка.АдрМЖ.Значение.Дом));
			КонецЕсли;
			Если ПустаяСтрока(Справка.АдрМЖ.Значение.Корпус) тогда
				ТекстДляИМНС.ДобавитьСтроку("2.6.12:"+СокрЛП(Справка.АдрМЖ.Значение.Корпус));
			КонецЕсли;
			Если ПустаяСтрока(Справка.АдрМЖ.Значение.Квартира) тогда
				ТекстДляИМНС.ДобавитьСтроку("2.6.13:"+СокрЛП(Справка.АдрМЖ.Значение.Квартира));
			КонецЕсли;
			...
			КонецЕсли;
		КонецЕсли;
		//ТекстФайла.ДобавитьСтроку("2.9:"+СокрЛП(СтрокаТЧ.КодИМНС));
		
		// Доходы по месяцам
		СовокупныйДоход = 0;
		Для М =1 По 12 Цикл
			ОбластьР3ДоходыПоМесяцам.Параметры["зн" + М] = СокрЛП(Строка(Формат((Справка.ДоходМес.Значение["Доход" + М]), "ЧДЦ=2")));
			МесяцДохода = ?(М<10,"0","")+СокрЛП(Строка(М));
			Если  Справка.ДоходМес.Значение["Доход" + М] <> 0 тогда
				ТекстДляИМНС.ДобавитьСтроку("3.1:"+МесяцДохода+";"+СокрЛП(Строка(Формат(Справка.ДоходМес.Значение["Доход" + М], "ЧДЦ=2"))));	
			КонецЕсли;
		...
		// Авторские
		Количество = 301;
		Если Справка.ДоходыАвторские.Значение.Доход301<>0 тогда
			ОбластьСтрАвторские.Параметры["Код"+Строка(Количество)] = "301";
			ОбластьСтрАвторские.Параметры["Авторские"+Строка(Количество)] = Справка.ДоходыАвторские.Значение.Доход301;
			Количество = Количество + 1;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:301"+";"+СокрЛП(Строка(Формат(Справка.ДоходыАвторские.Значение.Доход301, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ДоходыАвторские.Значение.Доход302<>0 тогда
			ОбластьСтрАвторские.Параметры["Код"+Строка(Количество)] = "302";
			ОбластьСтрАвторские.Параметры["Авторские"+Строка(Количество)] = Справка.ДоходыАвторские.Значение.Доход302;
			Количество = Количество + 1;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:302"+";"+СокрЛП(Строка(Формат(Справка.ДоходыАвторские.Значение.Доход302, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ДоходыАвторские.Значение.Доход303<>0 тогда
			ОбластьСтрАвторские.Параметры["Код"+Строка(Количество)] = "303";
			ОбластьСтрАвторские.Параметры["Авторские"+Строка(Количество)] = Справка.ДоходыАвторские.Значение.Доход303;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(ОбластьСтрАвторские);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:303"+";"+СокрЛП(Строка(Формат(Справка.ДоходыАвторские.Значение.Доход303, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ДоходыАвторские.Значение.Доход304<>0 тогда
			ОбластьСтрАвторские.Параметры["Код"+Строка(Количество)] = "304";
			ОбластьСтрАвторские.Параметры["Авторские"+Строка(Количество)] = Справка.ДоходыАвторские.Значение.Доход304;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(ОбластьСтрАвторские);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:304"+";"+СокрЛП(Строка(Формат(Справка.ДоходыАвторские.Значение.Доход304, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ДоходыАвторские.Значение.Доход305<>0 тогда
			ОбластьСтрАвторские.Параметры["Код"+Строка(Количество)] = "305";
			ОбластьСтрАвторские.Параметры["Авторские"+Строка(Количество)] = Справка.ДоходыАвторские.Значение.Доход305;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(ОбластьСтрАвторские);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:305"+";"+СокрЛП(Строка(Формат(Справка.ДоходыАвторские.Значение.Доход305, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ДоходыАвторские.Значение.Доход306<>0 тогда
			ОбластьСтрАвторские.Параметры["Код"+Строка(Количество)] = "306";
			ОбластьСтрАвторские.Параметры["Авторские"+Строка(Количество)] = Справка.ДоходыАвторские.Значение.Доход306;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(ОбластьСтрАвторские);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:306"+";"+СокрЛП(Строка(Формат(Справка.ДоходыАвторские.Значение.Доход306, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ДоходыАвторские.Значение.Доход307<>0 тогда
			ОбластьСтрАвторские.Параметры["Код"+Строка(Количество)] = "307";
			ОбластьСтрАвторские.Параметры["Авторские"+Строка(Количество)] = Справка.ДоходыАвторские.Значение.Доход307;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(ОбластьСтрАвторские);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:307"+";"+СокрЛП(Строка(Формат(Справка.ДоходыАвторские.Значение.Доход307, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ДоходыАвторские.Значение.Доход308<>0 тогда
			ОбластьСтрАвторские.Параметры["Код"+Строка(Количество)] = "308";
			ОбластьСтрАвторские.Параметры["Авторские"+Строка(Количество)] = Справка.ДоходыАвторские.Значение.Доход308;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(ОбластьСтрАвторские);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:308"+";"+СокрЛП(Строка(Формат(Справка.ДоходыАвторские.Значение.Доход308, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ДоходыАвторские.Значение.Доход309<>0 тогда
			ОбластьСтрАвторские.Параметры["Код"+Строка(Количество)] = "309";
			ОбластьСтрАвторские.Параметры["Авторские"+Строка(Количество)] = Справка.ДоходыАвторские.Значение.Доход309;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(ОбластьСтрАвторские);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:309"+";"+СокрЛП(Строка(Формат(Справка.ДоходыАвторские.Значение.Доход309, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ДоходыАвторские.Значение.Доход310<>0 тогда
			ОбластьСтрАвторские.Параметры["Код"+Строка(Количество)] = "310";
			ОбластьСтрАвторские.Параметры["Авторские"+Строка(Количество)] = Справка.ДоходыАвторские.Значение.Доход310;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(ОбластьСтрАвторские);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:310"+";"+СокрЛП(Строка(Формат(Справка.ДоходыАвторские.Значение.Доход310, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ДоходыАвторские.Значение.Доход311<>0 тогда
			ОбластьСтрАвторские.Параметры["Код"+Строка(Количество)] = "311";
			ОбластьСтрАвторские.Параметры["Авторские"+Строка(Количество)] = Справка.ДоходыАвторские.Значение.Доход311;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(ОбластьСтрАвторские);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:311"+";"+СокрЛП(Строка(Формат(Справка.ДоходыАвторские.Значение.Доход311, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ДоходыАвторские.Значение.Доход312<>0 тогда
			ОбластьСтрАвторские.Параметры["Код"+Строка(Количество)] = "312";
			ОбластьСтрАвторские.Параметры["Авторские"+Строка(Количество)] = Справка.ДоходыАвторские.Значение.Доход312;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(ОбластьСтрАвторские);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:312"+";"+СокрЛП(Строка(Формат(Справка.ДоходыАвторские.Значение.Доход312, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ДоходыАвторские.Значение.Доход313<>0 тогда
			ОбластьСтрАвторские.Параметры["Код"+Строка(Количество)] = "313";
			ОбластьСтрАвторские.Параметры["Авторские"+Строка(Количество)] = Справка.ДоходыАвторские.Значение.Доход313;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(ОбластьСтрАвторские);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:313"+";"+СокрЛП(Строка(Формат(Справка.ДоходыАвторские.Значение.Доход313, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ДоходыАвторские.Значение.Доход314<>0 тогда
			ОбластьСтрАвторские.Параметры["Код"+Строка(Количество)] = "314";
			ОбластьСтрАвторские.Параметры["Авторские"+Строка(Количество)] = Справка.ДоходыАвторские.Значение.Доход314;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(ОбластьСтрАвторские);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:314"+";"+СокрЛП(Строка(Формат(Справка.ДоходыАвторские.Значение.Доход314, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ДоходыАвторские.Значение.Доход315<>0 тогда
			ОбластьСтрАвторские.Параметры["Код"+Строка(Количество)] = "315";
			ОбластьСтрАвторские.Параметры["Авторские"+Строка(Количество)] = Справка.ДоходыАвторские.Значение.Доход315;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(ОбластьСтрАвторские);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:315"+";"+СокрЛП(Строка(Формат(Справка.ДоходыАвторские.Значение.Доход315, "ЧДЦ=2"))));
		КонецЕсли;
		...
		//О.Т. Аверсон 02.02.2017 № 198
		Документ2НДФЛ.Вывести(ОбластьР3Взносы);
		
		Документ2НДФЛ.Вывести(ОбластьР3Возврат);
		...
		Сч = 1;
		Для М =1 По 12 Цикл   
			Если Справка.ДоходМесЕАЭС.Значение["Доход" + М] <> 0 Тогда
				Если Сч > 3 Тогда
					Документ2НДФЛ.Вывести(ОбластьР3ВозвратСтрока);
					Сч = 1;
				КонецЕсли;
				
				ОбластьР3ВозвратСтрока.Параметры["Сумма" + Сч] = СокрЛП(Строка(Формат((Справка.ДоходМесЕАЭС.Значение["Доход" + М]), "ЧДЦ=2")));
				МесяцДохода = ?(М<10,"0","")+СокрЛП(Строка(М));
				ОбластьР3ВозвратСтрока.Параметры["Месяц" + Сч] = МесяцДохода;
				Если РаботникиОрганизации[0].Сотрудник.ВидДоговора = Перечисления.ВидыДоговоровСФизЛицами.Подряда Тогда 
					ОбластьР3ВозвратСтрока.Параметры["Код" + Сч] = "202";
				иначе
					ОбластьР3ВозвратСтрока.Параметры["Код" + Сч] = "201";
				КонецЕсли;                                             				
				
				
				Если  Справка.ДоходМес.Значение["Доход" + М] <> 0 тогда
					ТекстДляИМНС.ДобавитьСтроку("3.1:"+МесяцДохода+";"+СокрЛП(Строка(Формат(Справка.ДоходМесЕАЭС.Значение["Доход" + М], "ЧДЦ=2"))));	
				КонецЕсли;
				//СовокупныйДоход = Совокупныйдоход + Справка.ДоходМес.Значение["Доход" + М]; 
				Сч = Сч +1;
			КонецЕсли;
		...
		КонецЦикла;
		пока сч <=3 Цикл
			ОбластьР3ВозвратСтрока.Параметры["Сумма" + Сч] = "";
			ОбластьР3ВозвратСтрока.Параметры["Код" + Сч] = "";
			ОбластьР3ВозвратСтрока.Параметры["Месяц" + Сч] = "";
			сч = сч+1;
		КонецЦикла;
		Документ2НДФЛ.Вывести(ОбластьР3ВозвратСтрока);

		
		Документ2НДФЛ.Вывести(ОбластьР3Возврат1);
		//О.Т. Аверсон 01.04.2019 № 4172
		
        //Островская Татьяна 29.01.2014 № 17055
		ОбластьСуммаДохода.Параметры.СГДСумм 	= (Справка.СГДСумм.Значение);//СуммаВОтчет(Справка.СГДСумм.Значение);
		Документ2НДФЛ.Вывести(ОбластьСуммаДохода);
		//Островская Татьяна 29.01.2014 № 17055
		...
		Если НалоговыйПериод>=2009 тогда
			...
			ИндексВСтроке = 1;
			Справка.ДоходВидДр.Значение.Сортировать("МесДохода");
			Для каждого строкаЛьгот из Справка.ДоходВидДр.Значение цикл
				Если ИндексВСтроке = 3 тогда
				...
				ТекстДляИМНС.ДобавитьСтроку("4.2:"+СокрЛП(Строка(СтрокаЛьгот.КодДохода))+";"+МесяцДохода+";"+СокрЛП(Строка(Формат(СтрокаЛьгот.СуммаДохода,"ЧДЦ=2")))+";"+СокрЛП(Строка(Формат(СтрокаЛьгот.СуммаВычета,"ЧДЦ=2"))));
				...
				//Если ИндексВСтроке = 2 Тогда
				//	Документ2НДФЛ.Вывести(ОбластьСтрЛьготы);
				//	ОбластьСтрЛьготы.Параметры["Месяц"+Строка(ИндексВСтроке)] = "";
				//	ОбластьСтрЛьготы.Параметры["Код"+Строка(ИндексВСтроке)] = "";
				//	ОбластьСтрЛьготы.Параметры["Сумма"+Строка(ИндексВСтроке)] = "";
		...
		Иначе
			...
			Если Справка.Вычет.Значение.СуммаВычета600 <> 0 тогда
				...
				ТекстДляИМНС.ДобавитьСтроку("4.3:600"+";"+СокрЛП(Строка(Формат(Справка.Вычет.Значение.СуммаВычета600,"ЧДЦ=2"))));
							Строка43 = Истина;
			КонецЕсли;
			...
			Если Справка.Вычет.Значение.СуммаВычета610 <> 0 тогда
				...
				ТекстДляИМНС.ДобавитьСтроку("4.3:610"+";"+СокрЛП(Строка(Формат(Справка.Вычет.Значение.СуммаВычета610,"ЧДЦ=2"))));
							Строка43 = Истина;
			КонецЕсли;
			...
			Если Справка.Вычет.Значение.СуммаВычета611 <> 0 тогда
				...
				ТекстДляИМНС.ДобавитьСтроку("4.3:611"+";"+СокрЛП(Строка(Формат(Справка.Вычет.Значение.СуммаВычета611,"ЧДЦ=2"))));
							Строка43 = Истина;
			КонецЕсли;
			...
			Если Справка.Вычет.Значение.СуммаВычета620 <> 0 тогда
				...
				ТекстДляИМНС.ДобавитьСтроку("4.3:620"+";"+СокрЛП(Строка(Формат(Справка.Вычет.Значение.СуммаВычета620,"ЧДЦ=2"))));
							Строка43 = Истина;
			КонецЕсли;
		...
		//неОблагаетсяСНачГода = Справка.Вычет.Значение.СуммаВычета600 + Справка.Вычет.Значение.СуммаВычета610 + Справка.Вычет.Значение.СуммаВычета620 +
												//Справка.Вычет.Значение.СуммаВычета630 + Справка.Вычет.Значение.СуммаВычета640 + Справка.Вычет.Значение.СуммаВычета650;
		Если Строка41 или Строка42 тогда
			ТекстДляИМНС.ДобавитьСтроку("4.4:"+СокрЛП(Строка(Формат(ОбщаяСуммаЛьгот,"ЧДЦ=2"))));
		КонецЕсли;
			
		Если Строка43 тогда
			ТекстДляИМНС.ДобавитьСтроку("4.5:"+СокрЛП(Строка(Формат(ОбщаяСуммаВычетов,"ЧДЦ=2"))));
		КонецЕсли;
		...
		Документ2НДФЛ.Вывести(Р3ШапкаПрофВыч);
		// Авторские
		Количество = 301;
		Если Справка.ВычетыАвторские.Значение.Доход301<>0 тогда
			СтрокаПрофВыч.Параметры["Код"+Строка(Количество)] = "301";
			СтрокаПрофВыч.Параметры["Авторские"+Строка(Количество)] = Справка.ВычетыАвторские.Значение.Доход301;
			Количество = Количество + 1;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:301"+";"+СокрЛП(Строка(Формат(Справка.ВычетыАвторские.Значение.Доход301, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ВычетыАвторские.Значение.Доход302<>0 тогда
			СтрокаПрофВыч.Параметры["Код"+Строка(Количество)] = "302";
			СтрокаПрофВыч.Параметры["Авторские"+Строка(Количество)] = Справка.ВычетыАвторские.Значение.Доход302;
			Количество = Количество + 1;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:302"+";"+СокрЛП(Строка(Формат(Справка.ВычетыАвторские.Значение.Доход302, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ВычетыАвторские.Значение.Доход303<>0 тогда
			СтрокаПрофВыч.Параметры["Код"+Строка(Количество)] = "303";
			СтрокаПрофВыч.Параметры["Авторские"+Строка(Количество)] = Справка.ВычетыАвторские.Значение.Доход303;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(СтрокаПрофВыч);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:303"+";"+СокрЛП(Строка(Формат(Справка.ВычетыАвторские.Значение.Доход303, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ВычетыАвторские.Значение.Доход304<>0 тогда
			СтрокаПрофВыч.Параметры["Код"+Строка(Количество)] = "304";
			СтрокаПрофВыч.Параметры["Авторские"+Строка(Количество)] = Справка.ВычетыАвторские.Значение.Доход304;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(СтрокаПрофВыч);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:304"+";"+СокрЛП(Строка(Формат(Справка.ВычетыАвторские.Значение.Доход304, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ВычетыАвторские.Значение.Доход305<>0 тогда
			СтрокаПрофВыч.Параметры["Код"+Строка(Количество)] = "305";
			СтрокаПрофВыч.Параметры["Авторские"+Строка(Количество)] = Справка.ВычетыАвторские.Значение.Доход305;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(СтрокаПрофВыч);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:305"+";"+СокрЛП(Строка(Формат(Справка.ВычетыАвторские.Значение.Доход305, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ВычетыАвторские.Значение.Доход306<>0 тогда
			СтрокаПрофВыч.Параметры["Код"+Строка(Количество)] = "306";
			СтрокаПрофВыч.Параметры["Авторские"+Строка(Количество)] = Справка.ВычетыАвторские.Значение.Доход306;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(СтрокаПрофВыч);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:306"+";"+СокрЛП(Строка(Формат(Справка.ВычетыАвторские.Значение.Доход306, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ВычетыАвторские.Значение.Доход307<>0 тогда
			СтрокаПрофВыч.Параметры["Код"+Строка(Количество)] = "307";
			СтрокаПрофВыч.Параметры["Авторские"+Строка(Количество)] = Справка.ВычетыАвторские.Значение.Доход307;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(СтрокаПрофВыч);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:307"+";"+СокрЛП(Строка(Формат(Справка.ВычетыАвторские.Значение.Доход307, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ВычетыАвторские.Значение.Доход308<>0 тогда
			СтрокаПрофВыч.Параметры["Код"+Строка(Количество)] = "308";
			СтрокаПрофВыч.Параметры["Авторские"+Строка(Количество)] = Справка.ВычетыАвторские.Значение.Доход308;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(СтрокаПрофВыч);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:308"+";"+СокрЛП(Строка(Формат(Справка.ВычетыАвторские.Значение.Доход308, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ВычетыАвторские.Значение.Доход309<>0 тогда
			СтрокаПрофВыч.Параметры["Код"+Строка(Количество)] = "309";
			СтрокаПрофВыч.Параметры["Авторские"+Строка(Количество)] = Справка.ВычетыАвторские.Значение.Доход309;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(СтрокаПрофВыч);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:309"+";"+СокрЛП(Строка(Формат(Справка.ВычетыАвторские.Значение.Доход309, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ВычетыАвторские.Значение.Доход310<>0 тогда
			СтрокаПрофВыч.Параметры["Код"+Строка(Количество)] = "310";
			СтрокаПрофВыч.Параметры["Авторские"+Строка(Количество)] = Справка.ВычетыАвторские.Значение.Доход310;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(СтрокаПрофВыч);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:310"+";"+СокрЛП(Строка(Формат(Справка.ВычетыАвторские.Значение.Доход310, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ВычетыАвторские.Значение.Доход311<>0 тогда
			СтрокаПрофВыч.Параметры["Код"+Строка(Количество)] = "311";
			СтрокаПрофВыч.Параметры["Авторские"+Строка(Количество)] = Справка.ВычетыАвторские.Значение.Доход311;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(СтрокаПрофВыч);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:311"+";"+СокрЛП(Строка(Формат(Справка.ВычетыАвторские.Значение.Доход311, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ВычетыАвторские.Значение.Доход312<>0 тогда
			СтрокаПрофВыч.Параметры["Код"+Строка(Количество)] = "312";
			СтрокаПрофВыч.Параметры["Авторские"+Строка(Количество)] = Справка.ВычетыАвторские.Значение.Доход312;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(СтрокаПрофВыч);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:312"+";"+СокрЛП(Строка(Формат(Справка.ВычетыАвторские.Значение.Доход312, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ВычетыАвторские.Значение.Доход313<>0 тогда
			СтрокаПрофВыч.Параметры["Код"+Строка(Количество)] = "313";
			СтрокаПрофВыч.Параметры["Авторские"+Строка(Количество)] = Справка.ВычетыАвторские.Значение.Доход313;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(СтрокаПрофВыч);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:313"+";"+СокрЛП(Строка(Формат(Справка.ВычетыАвторские.Значение.Доход313, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ВычетыАвторские.Значение.Доход314<>0 тогда
			СтрокаПрофВыч.Параметры["Код"+Строка(Количество)] = "314";
			СтрокаПрофВыч.Параметры["Авторские"+Строка(Количество)] = Справка.ВычетыАвторские.Значение.Доход314;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(СтрокаПрофВыч);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:314"+";"+СокрЛП(Строка(Формат(Справка.ВычетыАвторские.Значение.Доход314, "ЧДЦ=2"))));
		КонецЕсли;
		
		Если Справка.ВычетыАвторские.Значение.Доход315<>0 тогда
			СтрокаПрофВыч.Параметры["Код"+Строка(Количество)] = "315";
			СтрокаПрофВыч.Параметры["Авторские"+Строка(Количество)] = Справка.ВычетыАвторские.Значение.Доход315;
			Количество = Количество + 1;
			Если Количество = 305 тогда
				Документ2НДФЛ.Вывести(СтрокаПрофВыч);
				Количество = 301;
			КонецЕсли;
			ТекстДляИМНС.ДобавитьСтроку("3.1.1:315"+";"+СокрЛП(Строка(Формат(Справка.ВычетыАвторские.Значение.Доход315, "ЧДЦ=2"))));
		КонецЕсли;
		...
		Строка42 = Ложь;
		Документ2НДФЛ.Вывести(СтрокаПрофВыч);
		Документ2НДФЛ.Вывести(Р3ШапкаВычСПолучДох);
		Документ2НДФЛ.Вывести(СтрокаВычСПолучДох);

		

		
		Строка43 = Ложь;
		ОбщаяСуммаВычетовПодоходный = 0;	
		Документ2НДФЛ.Вывести(ОбластьР3ШапкаПодоходный);
		//// Стандартные вычеты
		
		Индекс = 1;
		Пока Индекс<5 цикл
			ОбластьР3СтрокаПодоходный.Параметры["Месяц"+Строка(Индекс)] = "";
			ОбластьР3СтрокаПодоходный.Параметры["Сумма"+Строка(Индекс)] = "";
			Индекс = Индекс +1;
		КонецЦикла;
		
		
		ИндексВСтроке = 1;
		Для Каждого СтрокаВычеты из Справка.ВычетПодоходный.Значение Цикл
			Строка43 = Истина;	
			Если ИндексВСтроке = 5 тогда
				Документ2НДФЛ.Вывести(ОбластьР3СтрокаПодоходный);
				Индекс = 1;
				Пока Индекс<5 цикл
					ОбластьР3СтрокаПодоходный.Параметры["Месяц"+Строка(Индекс)] = "";
					ОбластьР3СтрокаПодоходный.Параметры["Сумма"+Строка(Индекс)] = "";
			...
			КонецЕсли;
			МесяцВычета= ?(СтрДлина(СокрЛП(СтрокаВычеты.МесяцВычета))=2,"","0")+СокрЛП(СтрокаВычеты.МесяцВычета); 
			ОбластьР3СтрокаПодоходный.Параметры["Месяц"+Строка(ИндексВСтроке)] = МесяцВычета;
			ОбластьР3СтрокаПодоходный.Параметры["Сумма"+Строка(ИндексВСтроке)] = СтрокаВычеты.СуммаВычета;
			
			ОбщаяСуммаВычетовПодоходный =  ОбщаяСуммаВычетовПодоходный + СтрокаВычеты.СуммаВычета;
			ИндексВСтроке = ИндексВСтроке + 1
		КонецЦикла;
		...
		//Островская Татьяна 30.01.2014 № 17055
		//ОбластьР3Прочее.Параметры.МатПомЛьгота = ОбщаяСуммаЛьгот;
		//ОбластьР3Прочее.Параметры.неОблагаетсяСНачГода = ОбщаяСуммаВычетов;//Островская Татьяна 30.01.2014 № 17055
		//ОбластьР3Прочее.Параметры.СГДСумм 	= (Справка.СГДСумм.Значение);//СуммаВОтчет(Справка.СГДСумм.Значение);
		//ОбластьР3Прочее.Параметры.ОблСумм 	= (Справка.СГДСумм.Значение - ОбщаяСуммаЛьгот - ОбщаяСуммаВычетов);
		...
		Индекс = 1;
		Пока Индекс<5 цикл
			ОбластьР3ПрочееСтрока.Параметры["Месяц"+Строка(Индекс)] = "";
			ОбластьР3ПрочееСтрока.Параметры["Сумма"+Строка(Индекс)] = "";
			Индекс = Индекс +1;
		КонецЦикла;
		
		//+ СВЭЙ  21.03.19 КДЯ  СВФР-004094
		Если Строка( Справка.Гражд.Значение ) <> "112" Тогда  
		//- СВЭЙ  21.03.19 КДЯ  СВФР-004094
			...
			ЗапросСтрана.УстановитьПараметр("Код", Строка( Справка.Гражд.Значение ) );
			РезультСтрана = ЗапросСтрана.Выполнить().Выбрать();
			Пока РезультСтрана.Следующий() Цикл 
				
				//+ СВЭЙ  21.03.19 КДЯ  СВФР-004094
				//Если РезультСтрана.УчастникЕАЭС и Строка(Справка.Гражд.Значение) <> "112" Тогда 
				//- СВЭЙ  21.03.19 КДЯ  СВФР-004094
				//+ СВЭЙ  21.03.19 КДЯ  СВФР-004094
				Если РезультСтрана.УчастникЕАЭС Тогда 
				//- СВЭЙ  21.03.19 КДЯ  СВФР-004094
					...
					Для Каждого СтрокаВычеты из Справка.ВычетПодоходный.Значение Цикл
						Строка43 = Истина;	
						Если ИндексВСтроке = 5 тогда
							Документ2НДФЛ.Вывести(ОбластьР3ПрочееСтрока);
							Индекс = 1;
							Пока Индекс<5 цикл
								ОбластьР3ПрочееСтрока.Параметры["Месяц"+Строка(Индекс)] = "";
								ОбластьР3ПрочееСтрока.Параметры["Сумма"+Строка(Индекс)] = "";
						...
						КонецЕсли;
						МесяцВычета= ?(СтрДлина(СокрЛП(СтрокаВычеты.МесяцВычета))=2,"","0")+СокрЛП(СтрокаВычеты.МесяцВычета); 
						ОбластьР3ПрочееСтрока.Параметры["Месяц"+Строка(ИндексВСтроке)] = МесяцВычета;
						ОбластьР3ПрочееСтрока.Параметры["Сумма"+Строка(ИндексВСтроке)] = СтрокаВычеты.СуммаВычета;
			...
			КонецЦикла; // РезультСтрана.Следующий()		
			
		//+ СВЭЙ  21.03.19 КДЯ  СВФР-004094
		КонецЕсли; // ( Строка( Справка.Гражд.Значение ) <> "112"  
		//- СВЭЙ  21.03.19 КДЯ  СВФР-004094
		...
		ОбластьР3Прочее1.Параметры.НИОблСумм = (Справка.НИОблСумм.Значение);
//		ОбластьР3Подвал.Параметры.НУОблСумм = СуммаВОтчет(Справка.НУОблСумм.Значение);
		Документ2НДФЛ.Вывести(ОбластьР3Прочее1);
		//Островская Татьяна 30.01.2014 № 17055
		// Подвал справки
		ОбластьПодвалСправки.Параметры.ДатаПодачиСведений = Формат(Дата,"ДФ=dd.MM.yyyy");
		ТекстДляИМНС.ДобавитьСтроку("5.1:"+СокрЛП(Строка(Формат(СовокупныйДоход,"ЧДЦ=2"))));
		ТекстДляИМНС.ДобавитьСтроку("5.2:"+СокрЛП(Строка(Формат(ОблСумм,"ЧДЦ=2"))));
		ТекстДляИМНС.ДобавитьСтроку("5.3:"+СокрЛП(Строка(Формат(Справка.НИОблСумм.Значение,"ЧДЦ=2"))));
		...
		Индекс = 1;
		Пока Индекс<3 цикл
			ОбластьСтрПрочие.Параметры["Месяц"+Строка(Индекс)] = "";
			ОбластьСтрПрочие.Параметры["Код"+Строка(Индекс)] = "";
			ОбластьСтрПрочие.Параметры["Сумма"+Строка(Индекс)] = "";
			Индекс = Индекс +1;
		КонецЦикла;
		...
		Для каждого строкаЛьгот из Справка.ДоходВидПр.Значение цикл
			...
			Если СтрокаЛьгот.СуммаДохода<>0 тогда // или СтрокаЛьгот.СуммаНалога<>0 тогда
				...
				МесяцДохода = ?(СтрДлина(СокрЛП(СтрокаЛьгот.МесДохода))=2,"","0")+СокрЛП(СтрокаЛьгот.МесДохода);
				ОбластьСтрПрочие.Параметры["Код"+Строка(ИндексВСтроке)] = СокрЛП(Строка(Формат(СтрокаЛьгот.КодДохода,"ЧДЦ=2")));
				...
				ТекстДляИМНС.ДобавитьСтроку("6.1:"+СокрЛП(Строка(Формат(СтрокаЛьгот.КодДохода,"ЧДЦ=2")))+";"+МесяцДохода+";"+СокрЛП(Строка(Формат(СтрокаЛьгот.СуммаДохода,"ЧДЦ=2")))+";"+СокрЛП(Строка(Формат(СуммаН,"ЧДЦ=2"))));
		...
		//ОбластьР3Подвал.Параметры.ИтогоСГДСумм = (СовокупныйДоход+ОбщийДоходПрочее);
		ОбластьР3Подвал.Параметры.ИтогоСГДСумм = Справка.СГДСумм.Значение + ОбщийДоходПрочее; //Аверсон 16.01.2020 КВВ СВФР-005370
		ОбластьР3Подвал.Параметры.ИтогоНИОблСумм = (Справка.НИОблСумм.Значение+ОбщийНалогПрочее);
		
		Документ2НДФЛ.Вывести(ОбластьР3Подвал);
		
		//		ОбластьПодвалСправки.Параметры.ГлБух = ФИОГлавногоБухгалтера;
		ОбластьПодвалСправки.Параметры.Руководитель = ПроцедурыУправленияПерсоналом.ИнициалыФамилияФизЛица(?(ЗначениеЗаполнено(Руководитель),Руководитель.ФизЛицо,""));  //Аверсон 16.01.2020 КВВ СВФР-005370     //Аверсон 30.01.2020 КВВ СВФР-005431
		Документ2НДФЛ.Вывести(ОбластьПодвалСправки);
		Документ2НДФЛ.ВывестиГоризонтальныйРазделительСтраниц();
		
		Если Строка61 тогда
			ТекстДляИМНС.ДобавитьСтроку("6.2:"+СокрЛП(Строка(Формат(ОбщийДоходПрочее,"ЧДЦ=2"))));
			ТекстДляИМНС.ДобавитьСтроку("6.3:"+СокрЛП(Строка(Формат(ОбщийНалогПрочее,"ЧДЦ=2"))));
		КонецЕсли;
		
		ТекстДляИМНС.ДобавитьСтроку("7.1:"+СокрЛП(Строка(Формат(СовокупныйДоход+ОбщийДоходПрочее,"ЧДЦ=2"))));
		ТекстДляИМНС.ДобавитьСтроку("7.2:"+СокрЛП(Строка(Формат(Справка.НИОблСумм.Значение+ОбщийНалогПрочее,"ЧДЦ=2"))));
...
//   нет
//
Процедура Автозаполнение(ФизЛицо = Неопределено,Подбор = Ложь,Сотрудник = Неопределено) Экспорт
	...
	СписокФизЛицТекст1 = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               |	НДФЛСведенияОДоходах.ФизЛицо,
				   |	&Сотрудник
	               |ИЗ
	               |	РегистрНакопления.НДФЛСведенияОДоходах КАК НДФЛСведенияОДоходах
				   ...
				   //|	НДФЛСведенияОДоходах.ФизЛицо.Наименование";
	Если ФизЛицо <> Неопределено тогда
		СписокФизЛицТекст1 = СписокФизЛицТекст1 + " И НДФЛСведенияОДоходах.ФизЛицо = &ФизЛицо";
	КонецЕсли;
	...
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ФизическиеЛица.Ссылка КАК ФизЛицо,
	|	РаботникиОрганизации.Сотрудник,//О.Т. Аверсон 01.04.2019 № 4172
	...
	//|	ВЫБОР КОГДА ПаспортныеДанные.ДокументКемВыдан ЕСТЬ NULL ТОГДА """" ИНАЧЕ ВЫРАЗИТЬ(ПаспортныеДанные.ДокументКемВыдан КАК СТРОКА(96)) КОНЕЦ КАК НаименованиеОргана,  //Аверсон 19.04.2021 КВВ СВФР-007465 (ДокументКемВыдан заменен на ДокументКемВыдан1.Наименование)
	|	ВЫБОР КОГДА ПаспортныеДанные.ДокументКемВыдан1 ЕСТЬ NULL ТОГДА """" ИНАЧЕ ВЫРАЗИТЬ(ПаспортныеДанные.ДокументКемВыдан1.Наименование КАК СТРОКА(100)) КОНЕЦ КАК НаименованиеОргана,  //Аверсон 19.04.2021 КВВ СВФР-007465 
	|	ВЫБОР КОГДА ПаспортныеДанные.ЛичныйНомер ЕСТЬ NULL ТОГДА """" ИНАЧЕ ПаспортныеДанные.ЛичныйНомер КОНЕЦ КАК ЛичныйНомер,
	|	ВЫБОР КОГДА (Гражданство.ФизЛицо ЕСТЬ NULL) ИЛИ (Гражданство.Страна  = &ПустаяСтрана) 
	|			ТОГДА ""112"" ИНАЧЕ Гражданство.Страна.Код КОНЕЦ КАК КодСтраны,
	...
	|	ВЫБОР КОГДА (Гражданство.ФизЛицо ЕСТЬ NULL) ИЛИ НЕ(Гражданство.НеЯвляетсяНалоговымРезидентомРФ) 
	//+ СВЭЙ  21.03.19 КДЯ  СВФР-004094
	|				ИЛИ ( НЕ ПаспортныеДанные.ФизЛицо ЕСТЬ NULL  
	|						И ( ПаспортныеДанные.ДокументВид.КодИМНС = ""09"" ИЛИ ПаспортныеДанные.ДокументВид.КодИМНС = ""9"" ) )
	//- СВЭЙ  21.03.19 КДЯ  СВФР-004094
	|			ТОГДА АдресаРегистрации.Поле11 ИНАЧЕ """" КОНЕЦ КАК ТипУлицы,	
	...
	//+ СВЭЙ  21.03.19 КДЯ  СВФР-004094
	//|	ВЫБОР КОГДА ФизическиеЛица.СтатусКатегория = &ПустойСтатус ТОГДА ""5"" ИНАЧЕ ФизическиеЛица.СтатусКатегория.КодИМНС КОНЕЦ КАК Статус,
	//- СВЭЙ  21.03.19 КДЯ  СВФР-004094
	//+ СВЭЙ  21.03.19 КДЯ  СВФР-004094
	|	ВЫБОР КОГДА НЕ ПаспортныеДанные.ФизЛицо ЕСТЬ NULL ТОГДА
	|				ВЫБОР КОГДА ПаспортныеДанные.ДокументВид.КодИМНС = """" ТОГДА ""5"" ИНАЧЕ ПаспортныеДанные.ДокументВид.КодИМНС КОНЕЦ
	|		  ИНАЧЕ	
	|				ВЫБОР КОГДА ФизическиеЛица.СтатусКатегория = &ПустойСтатус ТОГДА ""5"" ИНАЧЕ ФизическиеЛица.СтатусКатегория.КодИМНС КОНЕЦ
	|	КОНЕЦ КАК Статус,
	//- СВЭЙ  21.03.19 КДЯ  СВФР-004094
	...
	|ИЗ
	|	(" + СписокФизЛицТекст1 + ") КАК РаботникиОрганизации
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ФизическиеЛица КАК ФизическиеЛица
	|		ПО ФизическиеЛица.Ссылка = РаботникиОрганизации.ФизЛицо
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПаспортныеДанныеФизЛиц.СрезПоследних(&ДатаПодачиСведений, ФизЛицо в (" + СписокФизлицТекст + ")) КАК ПаспортныеДанные
	|		ПО ПаспортныеДанные.ФизЛицо = РаботникиОрганизации.ФизЛицо
