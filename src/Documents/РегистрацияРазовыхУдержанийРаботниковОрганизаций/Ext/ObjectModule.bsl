...
Функция СформироватьЗапросПоУдержания(ВыборкаПоШапкеДокумента)
	...
	|	СтрокиУдержания.Сотрудник, //О.Т. 17.09.2014 № 226
	|	СтрокиУдержания.ПодразделениеОрганизации
...
Процедура ОбработкаЗаполнения(Основание)//Островская Татьяна 21.03.2014
	ЗаполнитьДокумент(Основание);
КонецПроцедуры //Островская Татьяна 21.03.2014


Процедура ЗаполнитьДокумент(Основание)//Островская Татьяна 21.03.2014
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.РегистрацияРазовыхНачисленийРаботниковОрганизаций") Тогда
		...
		Начисление = ПланыВидовРасчета.УдержанияОрганизаций.ПолучитьФормуВыбора("ФормаВыбора").ОткрытьМодально();
		...
		ЗапросДляЗаполнения.УстановитьПараметр("Удержание",		Начисление);
		...
		|	РегистрацияРазовыхНачисленийРаботниковОрганизацийОсновныеНачисления.ДатаОкончания,
		|	РегистрацияРазовыхНачисленийРаботниковОрганизацийОсновныеНачисления.ПодразделениеОрганизации,
		...
		|	РегистрацияРазовыхНачисленийРаботниковОрганизацийОсновныеНачисления.Показатель1,
		|	РегистрацияРазовыхНачисленийРаботниковОрганизацийОсновныеНачисления.Показатель2,
		|	&Удержание КАК ВидРасчета,
		|	РегистрацияРазовыхНачисленийРаботниковОрганизацийОсновныеНачисления.НомерСтроки КАК НомерСтроки,
		|	NULL КАК НомерСтрокиДоп
		|ИЗ
		...
		|	РегистрацияРазовыхНачисленийРаботниковОрганизацийДополнительныеНачисления.ДатаНачала,
		|	РегистрацияРазовыхНачисленийРаботниковОрганизацийДополнительныеНачисления.ДатаОкончания,
		|	РегистрацияРазовыхНачисленийРаботниковОрганизацийДополнительныеНачисления.ПодразделениеОрганизации,
		...
		|	РегистрацияРазовыхНачисленийРаботниковОрганизацийДополнительныеНачисления.Показатель2,
		|	&Удержание,
		|	NULL,
		...
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки,
		|	НомерСтрокиДоп"; //О.Т. 12.07.2014
		
		Удержания.Загрузить(ЗапросДляЗаполнения.Выполнить().Выгрузить());
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.УдержанияТелефон") Тогда //О.Т. Аверсон 18.12.2017 № 1370
		ДокОснование = Основание.Ссылка;
		ОбщегоНазначения.ЗаполнитьШапкуДокументаПоОснованию(ЭтотОбъект,ДокОснование);
		ДатаНачала1 = НачалоМесяца(ДокОснование.Дата);
		ДатаОкончания1 = КонецМесяца(ДокОснование.Дата);
		Начисление = ПланыВидовРасчета.УдержанияОрганизаций.ПолучитьФормуВыбора("ФормаВыбора").ОткрытьМодально();
		ДокументОснование	= ДокОснование;
		ПериодРегистрации	= НачалоМесяца(ДокОснование.Дата);
		...
		"ВЫБРАТЬ
		|	ИСТИНА КАК Авторасчет,
		|	УдержанияТелефонУдержания.Сотрудник.ФизЛицо как ФизЛицо,
		|	УдержанияТелефонУдержания.Сотрудник как Сотрудник,
		|	УдержанияТелефонУдержания.НомерСтроки,
		|	УдержанияТелефонУдержания.СуммаУдержания как Результат,
		|	НАЧАЛОПЕРИОДА(УдержанияТелефонУдержания.Ссылка.Дата, МЕСЯЦ) КАК ДатаНачала,
		|	КОНЕЦПЕРИОДА(УдержанияТелефонУдержания.Ссылка.Дата, МЕСЯЦ) КАК ДатаОкончания,
		|	&Удержание как ВидРасчета
		|ИЗ
		|	Документ.УдержанияТелефон.Удержания КАК УдержанияТелефонУдержания
		|ГДЕ
		|	УдержанияТелефонУдержания.Ссылка = &ДокументСсылка";
...
//Аверсон ЕАЕ 17.06.2021 СВФР-007736
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	СтруктураПечатныхФорм.Вставить("Печать", "Печать");
...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если ИмяМакета = "Печать" Тогда
		Записать();
		ТабДокумент = ПечатьПечатнойФормы(ИмяМакета);
...
Функция ПечатьПечатнойФормы(ИмяМакета)
	...
	ЗапросШапка.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	                    |	РегистрацияРазовыхУдержанийРаботниковОрганизаций.Ссылка,
	                    |	РегистрацияРазовыхУдержанийРаботниковОрганизаций.Номер КАК НомерДокумента,
	                    |	РегистрацияРазовыхУдержанийРаботниковОрганизаций.Дата КАК ДатаДокумента
	                    |ИЗ
	...
	Если Не РезультатШапка.Пустой() Тогда
		...
		Если ВыборкаШапка.Следующий() Тогда
			...
			ЗапросТЧ.Текст = 
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	РегистрацияРазовыхУдержанийРаботниковОрганизацийУдержания.Сотрудник.Код КАК ТабНом,
			|	СУММА(РегистрацияРазовыхУдержанийРаботниковОрганизацийУдержания.Результат) КАК Результат,
			|	РегистрацияРазовыхУдержанийРаботниковОрганизацийУдержания.Сотрудник,
			|	РегистрацияРазовыхУдержанийРаботниковОрганизацийУдержания.Сотрудник.Наименование КАК СотрудникНаименование
			|ИЗ
			|	Документ.РегистрацияРазовыхУдержанийРаботниковОрганизаций.Удержания КАК РегистрацияРазовыхУдержанийРаботниковОрганизацийУдержания
			|ГДЕ
			...
			|
			|СГРУППИРОВАТЬ ПО
			|	РегистрацияРазовыхУдержанийРаботниковОрганизацийУдержания.Сотрудник.Код,
			|	РегистрацияРазовыхУдержанийРаботниковОрганизацийУдержания.Сотрудник
			|
			|УПОРЯДОЧИТЬ ПО
			...
			ВыборкаРезультат = ВыборкаОбщийИтог.Выбрать(ОбходРезультатаЗапроса.Прямой);
			Ном = 0;
			Пока ВыборкаРезультат.Следующий() Цикл
				Ном = Ном + 1;
				ОблСтрока.Параметры.Заполнить(ВыборкаРезультат);
