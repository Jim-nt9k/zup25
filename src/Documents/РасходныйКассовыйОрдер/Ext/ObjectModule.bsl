...
Функция ПечатьРКО()
	...
	ОбластьМакета.Параметры.СуммаПрописью				= РаботаСДиалогами.СформироватьСуммуПрописью(Шапка.Сумма, ?(ОтражатьВБухгалтерскомУчете,Константы.ВалютаРегламентированногоУчета.Получить(),Константы.ВалютаУправленческогоУчета.Получить()));
...
Процедура ЗаполнитьПоляВыдатьПоДокументу(СсылкаРаботник) Экспорт
	...
	Если Не Результат.Пустой() Тогда
		...
		Если Выборка.ДокументВид <> NULL Тогда
			...
			+ СокрЛП(Выборка.ДокументКемВыдан) + ", № подр. " + Выборка.ДокументКодПодразделения; //Островская Татьяна 26.03.2014 № 17191
		...
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыПоВедомостям Тогда
			...
			Пока РезультатЗапроса.Следующий() Цикл
				...
				Пока ВложеннаяВыборка.Следующий() Цикл
					...
					//заявка №2068 (база заявок Кристалла) Дмитрук О.Г.
...
Функция СформироватьЗапросПоРКОРеглВед()
	...
	Запрос.УстановитьПараметр("Ссылка",							Ссылка);
	Запрос.УстановитьПараметр("Организация",					Организация);
	Запрос.УстановитьПараметр("Период",							Дата);//О.Т. Аверсон 24.07.2018 № 2208
	Запрос.УстановитьПараметр("Выплачено",						Перечисления.ВыплаченностьЗарплаты.Выплачено);
	...
	//|	ВЫБОР
	//|		КОГДА ВложенныйЗапрос.СотрудникФизлицо ЕСТЬ NULL
	//|				ИЛИ ВложенныйЗапрос.ЕстьРаботающие = 0
	//|			ТОГДА ИСТИНА
	//|		ИНАЧЕ ЛОЖЬ
	//|	КОНЕЦ КАК Акционер
	...
	|	( ISNULL( ВложенныйЗапрос.ЕстьРаботающие, 0 ) = 0 И Подрядчики.Физлицо ЕСТЬ NULL 
	|		И НЕ ЗарплатаКВыплатеОрганизацийЗарплата.ПроводитьВзаиморасчеты ) КАК Акционер
	...
	|			СУММА(ВЫБОР
	|					КОГДА не РаботникиОрганизацийСрезПоследних.Регистратор ССЫЛКА Документ.УвольнениеИзОрганизаций
	|						ТОГДА 1
	|					ИНАЧЕ 0
	|				КОНЕЦ) КАК ЕстьРаботающие
	//(начало) закомментировано Кухарчуком Д.Я. 16.08.18, задача СВФР-002337
	//|		ИЗ
	...
	|		
	|		СГРУППИРОВАТЬ ПО
	|			РаботникиОрганизацийСрезПоследних.Сотрудник.Физлицо) КАК ВложенныйЗапрос //О.Т. Аверсон 25.07.2018 № 2208
	|		ПО ЗарплатаКВыплатеОрганизацийЗарплата.Физлицо = ВложенныйЗапрос.СотрудникФизлицо
	//(начало) добавлено Кухарчуком Д.Я. 16.08.18, задача СВФР-002337
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	...
	|		ПО ЗарплатаКВыплатеОрганизацийЗарплата.Физлицо = Подрядчики.Физлицо
	//(конец) добавлено Кухарчуком Д.Я. 16.08.18, задача СВФР-002337
	|ГДЕ
	|	РасходныйКассовыйОрдерВыплатаЗаработнойПлаты.Ссылка = &Ссылка
	...
	//(начало) добавлено Кухарчуком Д.Я. 16.08.18, задача СВФР-002337
	|	( ISNULL( ВложенныйЗапрос.ЕстьРаботающие, 0 ) = 0 И Подрядчики.Физлицо ЕСТЬ NULL 
	|		И НЕ ЗарплатаКВыплатеОрганизацийЗарплата.ПроводитьВзаиморасчеты ),
	//(конец) добавлено Кухарчуком Д.Я. 16.08.18, задача СВФР-002337
	|	ЗарплатаКВыплатеОрганизацийЗарплата.Ссылка
	|
	|ИМЕЮЩИЕ
...
Функция СформироватьЗапросПоРКОРеглРаб()
	...
	Запрос.УстановитьПараметр("парамУчетЗадолженностиПоМесяцам",ПроцедурыУправленияПерсоналом.ЗначениеУчетнойПолитикиПоПерсоналуОрганизации(глЗначениеПеременной("глУчетнаяПолитикаПоПерсоналуОрганизации"), Организация, "УчетЗадолженностиПоМесяцам"));
	Запрос.УстановитьПараметр("парамПустаяДата",				'00010101');
	//Аверсон 06.05.2021 КВВ СВФР-007549 (
	Запрос.УстановитьПараметр("Организация", Организация);
		...
		Если ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыПоВедомостям Тогда
			...
			Пока Результат.Следующий() Цикл
				Если Результат.СуммаВзаиморасчетов <> 0 Тогда   					 
					...
					Если ?(Результат.ХарактерВыплаты = Перечисления.ХарактерВыплатыЗарплаты.Дивиденды,не Результат.Акционер, Истина) Тогда //О.Т. Аверсон 31.08.2018 № 2398	
		...
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийРКО.ВыплатаЗаработнойПлатыРаботнику Тогда
			...
			Если Не Результат.Пустой() Тогда
				...
				Если Выборка.СуммаВзаиморасчетов <> 0 Тогда
					//Если ?(Результат.ХарактерВыплаты = Перечисления.ХарактерВыплатыЗарплаты.Дивиденды,не Результат.Акционер,истина) Тогда //О.Т. Аверсон 31.08.2018 № 2398
					  Если ?(Выборка.ХарактерВыплаты = Перечисления.ХарактерВыплатыЗарплаты.Дивиденды,НЕ Выборка.Акционер, Истина) Тогда //Аверсон 06.05.2021 КВВ СВФР-007549
						СтрокаДвижения = Движения.ВзаиморасчетыСРаботникамиОрганизаций.Добавить();
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	СуммаПоДокументу = ВыплатаЗаработнойПлаты.Итог("СуммаКВыплате");
	//заявка №2003
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ЗарплатаКВыплатеОрганизаций")	Тогда
		...
		ОбщегоНазначения.ЗаполнитьШапкуДокументаПоОснованию(ЭтотОбъект, Основание); //Островская Татьяна 11.06.2014 № 41
		...
		//Аверсон-софт, Плиско Ю.А. 10.11.2014   Начало:      (для Госпиталь)
...
Процедура ПриУстановкеНовогоНомера(СтандартнаяОбработка, Префикс)// О.Т. 02.12.2014 
