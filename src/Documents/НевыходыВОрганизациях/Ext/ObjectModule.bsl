...
Функция ПолучитьМассивВозможныхНачислений()
	...
	МассивНачислений.Добавить(ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.РождениеРебенка);//О.Т. Аверсон 16.08.2018 № 2307
	МассивНачислений.Добавить(ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.СмертьБлизкогоРодственника);//О.Т. Аверсон 16.08.2018 № 2307
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	ПроверкаПериодовНевыходов(); //Аверсон 22.09.2021 КВВ СВФР-008174
...
Процедура ОбработкаЗаполнения(Основание)
    ...
    ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ОтпускаОрганизаций") Тогда
        ...
        Запрос.УстановитьПараметр("ДокументОснование",				Основание);
        Запрос.УстановитьПараметр("ГоловнаяОрганизация",			ОбщегоНазначения.ГоловнаяОрганизация(Основание.Организация));
        //Островская Татьяна 06.06.2014 № 17422
        Запрос.Текст =
        "ВЫБРАТЬ
        |	ТЧРаботникиОрганизации.Сотрудник,
        |	ТЧРаботникиОрганизации.Сотрудник.Физлицо КАК Физлицо,
        ...
        |		КОГДА ТЧРаботникиОрганизации.ПричинаОтсутствия = ЗНАЧЕНИЕ(Перечисление.ПричиныОтсутствияНаРаботеВОрганизации.ОтпускБезСохраненияЗарплатыСемейноБытовыеПричины)
        |				ИЛИ ТЧРаботникиОрганизации.ПричинаОтсутствия = ЗНАЧЕНИЕ(Перечисление.ПричиныОтсутствияНаРаботеВОрганизации.ОтпускБезСохраненияЗарплатыУчебныеСборы)
        |				ИЛИ ТЧРаботникиОрганизации.ПричинаОтсутствия = ЗНАЧЕНИЕ(Перечисление.ПричиныОтсутствияНаРаботеВОрганизации.ОтпускУчебныйНеоплачиваемый)
        ...
        |	КОНЕЦ КАК ПодразделениеОрганизации,
        |	ИСТИНА КАК Авторасчет,
		|	&ДокументОснование КАК СторнируемыйДокумент //О.Т. Аверсон 11.06.2016 № 1217
        |ИЗ
        ...
        |ГДЕ
        |	ТЧРаботникиОрганизации.Ссылка = &ДокументОснование
        |	И ТЧРаботникиОрганизации.ПричинаОтсутствия В (ЗНАЧЕНИЕ(Перечисление.ПричиныОтсутствияНаРаботеВОрганизации.ОтпускБезСохраненияЗарплатыСемейноБытовыеПричины), ЗНАЧЕНИЕ(Перечисление.ПричиныОтсутствияНаРаботеВОрганизации.ОтпускУчебныйНеоплачиваемый), ЗНАЧЕНИЕ(Перечисление.ПричиныОтсутствияНаРаботеВОрганизации.ОтпускБезСохраненияЗарплатыУчебныеСборы))";
        //Островская Татьяна 06.06.2014 № 17422
        Начисления.Загрузить(Запрос.Выполнить().Выгрузить());
...
Процедура ПроверкаПериодовНевыходов()
	//Аверсон 22.09.2021 КВВ СВФР-008174 (
	З = Новый Запрос;
	З.Текст = "ВЫБРАТЬ 
	|ТЧ.Сотрудник,
	|ТЧ.ДатаНачала,
	|ТЧ.ДатаОкончания,
	|ТЧ.НомерСтроки
	|ПОМЕСТИТЬ ВтТЧТекДок
	|ИЗ Документ.НевыходыВОрганизациях.Начисления КАК ТЧ
	|ГДЕ ТЧ.Ссылка = &ТекДок
	|;
	|
	|ВЫБРАТЬ 
	|ТЧ2.Сотрудник,
	...
	|ТЧ2.НомерСтроки,
	|ТЧ2.Ссылка,
	|ВтТЧТекДок.Сотрудник КАК СотрудникТекДок,
	|ВтТЧТекДок.ДатаНачала КАК ДатаНачалаТекДок,
	|ВтТЧТекДок.ДатаОкончания КАК ДатаОкончанияТекДок,
	|ВтТЧТекДок.НомерСтроки КАК НомерСтрокиТекДок
	|ИЗ Документ.НевыходыВОрганизациях.Начисления КАК ТЧ2
	|ЛЕВОЕ СОЕДИНЕНИЕ  ВтТЧТекДок КАК ВтТЧТекДок
	|ПО ТЧ2.Сотрудник = ВтТЧТекДок.Сотрудник
	|ГДЕ НЕ ТЧ2.Ссылка = &ТекДок
	|И ТЧ2.Ссылка.Проведен = ИСТИНА
	|И НЕ (ТЧ2.ДатаНачала > ВтТЧТекДок.ДатаОкончания
	|ИЛИ ТЧ2.ДатаОкончания < ВтТЧТекДок.ДатаНачала)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ВтТЧТекДок.ДатаОкончания КАК ДатаОкончанияТекДок,
	|ВтТЧТекДок.НомерСтроки КАК НомерСтрокиТекДок
	|ИЗ Документ.НевыходыВОрганизациях.Начисления КАК ТЧ3
	|ЛЕВОЕ СОЕДИНЕНИЕ  ВтТЧТекДок КАК ВтТЧТекДок
	|ПО ТЧ3.Сотрудник = ВтТЧТекДок.Сотрудник
	|ГДЕ ТЧ3.Ссылка = &ТекДок
	|И ТЧ3.НомерСтроки <> ВтТЧТекДок.НомерСтроки
	|И НЕ (ТЧ3.ДатаНачала > ВтТЧТекДок.ДатаОкончания
	|ИЛИ ТЧ3.ДатаОкончания < ВтТЧТекДок.ДатаНачала)";
	З.УстановитьПараметр("ТекДок", Ссылка);
	РезЗ = З.Выполнить().Выгрузить();
	Если РезЗ.Количество() > 0 Тогда
		РезЗ.Сортировать("НомерСтрокиТекДок");
		Для Каждого СтрРезЗ Из РезЗ Цикл 
			Сообщить("Пересечение периодов по невыходам: строка """ + СокрЛП(СтрРезЗ.НомерСтрокиТекДок) + """ " + СокрЛП(СтрРезЗ.СотрудникТекДок) + " с " + ?(СтрРезЗ.Ссылка = Ссылка,"текущим ","") + "док. " + ?(СтрРезЗ.Ссылка = Ссылка,"",СокрЛП(Лев(СтрРезЗ.Ссылка,СтрДлина(СокрЛП(СтрРезЗ.Ссылка))-8)) + " ") + "строка """ + СокрЛП(СтрРезЗ.НомерСтроки) + """");
		КонецЦикла;
	КонецЕсли;
	//Аверсон 22.09.2021 КВВ СВФР-008174 ) 
КонецПроцедуры
