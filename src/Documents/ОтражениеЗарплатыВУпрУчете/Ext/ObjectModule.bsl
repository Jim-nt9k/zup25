Процедура Автозаполнение() Экспорт
	...
	"ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА Начисления.ВидРасчета.СпособОтраженияВУпрУчете <> &парамПустойСпособУУ
	...
	|		ИНАЧЕ &парамСпособПоУмолчаниюСтатьяЗатрат
	...
	|		ИНАЧЕ &парамСпособПоУмолчаниюОбъектСтроительства
	...
	|						И УчетЗаработкаРаботников.Период <= Начисления.ПериодДействияНачало
	|			ГДЕ
	...
	|	И ВЫБОР
	|			КОГДА Начисления.ВидРасчета.СпособОтраженияВУпрУчете <> &парамПустойСпособУУ
	|				ТОГДА Начисления.ВидРасчета.СпособОтраженияВУпрУчете
	|			КОГДА УчетЗаработкаРаботников.СпособОтраженияВУпрУчете <> &парамПустойСпособУУ
	|					И (НЕ УчетЗаработкаРаботников.СпособОтраженияВУпрУчете ЕСТЬ NULL )
	|				ТОГДА УчетЗаработкаРаботников.СпособОтраженияВУпрУчете
	|		КОНЕЦ <> &парамСпособНеОтражать
	|	И (НЕ Начисления.ПоВременнойСхемеМотивации)
	|	И (НЕ(Начисления.ВидРасчета.ЗачетОтработанногоВремени
	|				И (Начисления.Показатель1 = ЗНАЧЕНИЕ(Справочник.ПоказателиСхемМотивации.СдельнаяВыработка)
	|					ИЛИ Начисления.Показатель2 = ЗНАЧЕНИЕ(Справочник.ПоказателиСхемМотивации.СдельнаяВыработка)
	|					ИЛИ Начисления.Показатель3 = ЗНАЧЕНИЕ(Справочник.ПоказателиСхемМотивации.СдельнаяВыработка)
	|					ИЛИ Начисления.Показатель4 = ЗНАЧЕНИЕ(Справочник.ПоказателиСхемМотивации.СдельнаяВыработка))))
	|
	|СГРУППИРОВАТЬ ПО
	|	ВЫБОР
	|		КОГДА Начисления.ВидРасчета.СпособОтраженияВУпрУчете <> &парамПустойСпособУУ
	...
	Пока Выборка.Следующий() Цикл
		ЗаполнитьЗначенияСвойств(ОтражениеВУчете.Добавить(),Выборка);
	КонецЦикла;
