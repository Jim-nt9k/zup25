...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если  ИмяМакета = "Диаграмма" Тогда
		...
		|ГДЕ
		|	ОтсутствиеНаРаботеРаботники.ПричинаОтсутствия = ЗНАЧЕНИЕ(Перечисление.ПричиныОтсутствияНаРаботе.ОтпускЕжегодный)
		|	И ОтсутствиеНаРаботеРаботники.Ссылка = &Ссылка");
...
Функция СформироватьЗапросПоРаботники(Режим)
	...
	|	ТЧРаботники.НеНапоминатьОСобытии
	|ИЗ
...
Процедура ПроверитьЗаполнениеСтрокиРаботника(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	// ДатаОкончание
	НетДатыОкончания = НЕ ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.ДатаОкончания) И ВыборкаПоШапкеДокумента.СпособВводаДанных = Перечисления.СпособыВводаДанныхОВремени.ЗаДень;
	Если НетДатыОкончания Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не указана дата окончания отсутствия на работе!", Отказ, Заголовок);
	КонецЕсли;
...
Процедура ДобавитьСтрокуВДвиженияПоРегистрамСведений(ВыборкаПоШапкеДокумента, ВыборкаПоРаботники, СтруктураПараметров = "")
	...
	Если ЗначениеЗаполнено(ВыборкаПоРаботники.ДатаОкончания) Тогда
		...
		Иначе
			...
			Движение.НеНапоминатьОСобытии	= ВыборкаПоРаботники.НеНапоминатьОСобытии;
	...
	Если ВыборкаПоРаботники.ДатаОкончания <> '0001-01-01' И НЕ ВыборкаПоРаботники.НапомнитьПоЗавершении Тогда
