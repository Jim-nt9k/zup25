...
Функция СформироватьТаблицуНачислений(Начисления)
	...
	Пока Начисления.Следующий() Цикл
		...
		КонецЕсли;				
		Если Начисления.КурсВалюты5 = NULL Тогда
			Сообщить("Для расчетного периода не задан курс валюты " + Начисления.Валюта5 + "!", СтатусСообщения.Важное);
		КонецЕсли;		
		Если Начисления.КурсВалюты6 = NULL Тогда
			Сообщить("Для расчетного периода не задан курс валюты " + Начисления.Валюта6 + "!", СтатусСообщения.Важное);
		КонецЕсли;	
...
Функция Автозаполнение(Запрос = НеОпределено) Экспорт
	...
	Если Запрос = Неопределено Тогда
		...
		|ИЗ
		|	ВТСписокРаботников КАК РаботникиОрганизации
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			СписокСотрудников.Сотрудник КАК Сотрудник,
		|			СписокСотрудников.Физлицо КАК Физлицо
		|		ИЗ
	...
	Иначе
		...
		|		ПО СписокРаботников.Сотрудник.Физлицо = ДанныеРаботников.Физлицо
		|			И СписокРаботников.Подразделение = ДанныеРаботников.Подразделение";
		Запрос.Выполнить();
	...
	Пока ВыборкаПоФЛ.Следующий() Цикл
		...
		Пока ВыборкаПоНС.Следующий() Цикл
			...
			Если ВыборкаПоНС.Начисления = NULL Или (ВыборкаПоНС.ВидДвижения = "Удержания") Тогда
				...
				Если ПериодРегистрации >=Дата(2016,07,1) тогда //О.Т. Аверсон 22.06.2016 № 949
					ВременныйТабличныйДокумент.Область(НомерСтроки, КолонкаСуммаУдержано, НомерСтроки, КолонкаСуммаУдержано).Текст = Формат(ВыборкаПоНС.Удержания,"ЧЦ=15; ЧДЦ="+Строка(ЗначениеОкругления));
				иначе
			...
			Иначе
				...
				Если ПериодРегистрации >=Дата(2016,07,1) тогда //О.Т. Аверсон 22.06.2016 № 949
					ВременныйТабличныйДокумент.Область(НомерСтроки, КолонкаСуммаНачислено, НомерСтроки, КолонкаСуммаНачислено).Текст = Формат(ВыборкаПоНС.Начисления,"ЧЦ=15; ЧДЦ="+Строка(ЗначениеОкругления));
				иначе                  
		...
		Если Модифицированность  Тогда
			...
			Для Каждого СтрокаНачислений Из Строки Цикл
				...
				Если ПериодРегистрации >=Дата(2016,07,1) тогда //О.Т. Аверсон 22.06.2016 № 949
					ВременныйТабличныйДокумент.Область(НомерСтроки, КолонкаСуммаНачислено, НомерСтроки, КолонкаСуммаНачислено).Текст = Формат(СтрокаНачислений.Результат,"ЧЦ=15; ЧДЦ="+Строка(ЗначениеОкругления));
				иначе
			...
			Для Каждого СтрокаУдержано Из СтрокиУдержано Цикл
				...
				ВременныйТабличныйДокумент.Область(НомерСтроки, КолонкаПериодУдержано, НомерСтроки, КолонкаСуммаУдержано - 1).Объединить();
				Если ПериодРегистрации >=Дата(2016,07,1) тогда //О.Т. Аверсон 22.06.2016 № 949
					ВременныйТабличныйДокумент.Область(НомерСтроки, КолонкаСуммаУдержано, НомерСтроки, КолонкаСуммаУдержано).Текст = Формат(СтрокаУдержано.Результат,"ЧЦ=15; ЧДЦ="+Строка(ЗначениеОкругления));
				иначе
			...
			Для Каждого СтрокаЗайма Из СтрокиЗаймы Цикл
				...
				ВременныйТабличныйДокумент.Область(НомерСтроки, КолонкаПериодУдержано, НомерСтроки, КолонкаСуммаУдержано - 1).Объединить();
				Если ПериодРегистрации >=Дата(2016,07,1) тогда  //О.Т. Аверсон 22.06.2016 № 949
					ВременныйТабличныйДокумент.Область(НомерСтроки, КолонкаСуммаУдержано, НомерСтроки, КолонкаСуммаУдержано).Текст = Формат(СтрокаЗайма.ПогашеноЗайма,"ЧЦ=15; ЧДЦ="+Строка(ЗначениеОкругления));
				иначе
