...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если ИмяМакета = "ПечатьФайла" Тогда
		...
		Если ФорматФайла = Перечисления.ФорматФайлаПФР.Версия04 Тогда
			...
			ТаблицаФорматаЗаписи = ПроцедурыПерсонифицированогоУчета.ПолучитьФорматЗаписи(ТипЗаписи,ДокументФорматЗаписей);
			...
			Для Н=5 по СтрЧислоСтрок(ТекстФайла) цикл
				...
				СтрОшибка = "";
				Если НЕ ПроцедурыПерсонифицированогоУчета.РазложитьСтрокуВСтруктуруЗаписи(СтрокаФайла,ТипЗаписи,ТаблицаФорматаЗаписи,Запись,СтрОшибка) тогда
				...
				ОбластьЗаявление.Параметры.ДатаРождения 				= ПроцедурыПерсонифицированогоУчета.ДатаВОтчет(Запись.СтандартнаяДатаРождения); 
				...
				СерияНомер = Серия1 +?(Запись.ДокументВид = "ПАСПОРТ РБ", " ", ?(Запись.ДокументВид = "ПАСПОРТ", "-", "") ) + Серия2 +",  " + Запись.ДокументНомер;
				
				ОбластьЗаявление.Параметры.НаименованиеДокумента		= 	НаименованиеДокумента;
				ОбластьЗаявление.Параметры.СерияНомер			 		= 	СерияНомер;
				ОбластьЗаявление.Параметры.ДатаВыдачи 					= 	ПроцедурыПерсонифицированогоУчета.ДатаВОтчет(Запись.ДокументДатаВыдачи);
				...
				//Подвал
				ОбластьЗаявление.Параметры.ДатаЗаполнения				= 	"Дата заполнения " + Символы.ПС + ПроцедурыПерсонифицированогоУчета.ДатаВОтчет(Запись.ДатаЗаполнения);
				
				//Поля, присутствующие только в форме АДВ-3
				Если ВидАДВ = Перечисления.ВидыАДВ.АДВ3 тогда	
					ОбластьЗаявление.Параметры.ПолВСвидетельстве 							= Запись.ПолВСвидетельстве + "     (м/ж)"; 
					ОбластьЗаявление.Параметры.ДатаРожденияВСвидетельстве 					= ПроцедурыПерсонифицированогоУчета.ДатаВОтчет(Запись.СтандартнаяДатаРожденияВСвидетельстве); 
			...
			Если СтрокаДереваПачкаВходящихДокументов <> Неопределено Тогда
				Для Каждого СтрокаДереваЗаявление Из СтрокаДереваПачкаВходящихДокументов.Строки Цикл
					Если СтрокаДереваЗаявление.Имя = "ЗАЯВЛЕНИЕ_О_ДУБЛИКАТЕ" Или СтрокаДереваЗаявление.Имя = "ЗАЯВЛЕНИЕ_ОБ_ОБМЕНЕ" Тогда
						...
						//Данные, указанные в свидетельстве
						ОбластьЗаявление.Параметры.СтраховойНомерПФР = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваЗаявление, "СтраховойНомер");
						...
						Если СтрокаДереваФИОИзСтрахового <> Неопределено Тогда
							ОбластьЗаявление.Параметры.ФамилияВСвидетельстве 	= ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваФИОИзСтрахового, "Фамилия"); 
							ОбластьЗаявление.Параметры.ИмяВСвидетельстве 		= ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваФИОИзСтрахового, "Имя"); 
							ОбластьЗаявление.Параметры.ОтчествоВСвидетельстве 	= ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваФИОИзСтрахового, "Отчество"); 
						КонецЕсли;
						...
						Если СтрокаДереваИзменившиесяДанные <> Неопределено Тогда
							...
							Если СтрокаДереваФИО <> Неопределено Тогда
								ОбластьЗаявление.Параметры.Фамилия 				= ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваФИО, "Фамилия");
								ОбластьЗаявление.Параметры.Имя 					= ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваФИО, "Имя");
								ПризнакОтменыОтчества = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваЗаявление, "ПризнакОтменыОтчества");
								ОбластьЗаявление.Параметры.Отчество 			= ?(ПризнакОтменыОтчества = "ОТМЕНЕНО", "отменено", ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваФИО, "Отчество")); 
							КонецЕсли;
							Пол = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваИзменившиесяДанные, "Пол");
							...
							Если СтрокаДереваДатаРожденияОсобая = Неопределено Тогда
								ДатаРождения = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваИзменившиесяДанные, "ДатаРождения");
								Если ЗначениеЗаполнено(ДатаРождения) Тогда
									ОбластьЗаявление.Параметры.ДатаРождения	= ПроцедурыПерсонифицированогоУчета.ДатаXMLВОтчет(ДатаРождения); 
								Иначе
									ОбластьЗаявление.Параметры.ДатаРождения	= ""
								КонецЕсли;
							Иначе
								ОбластьЗаявление.Параметры.ДатаРождения = 
									ПроцедурыПерсонифицированогоУчета.ДатаВОтчет(Дата(ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваДатаРожденияОсобая,"Год"),
										ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваДатаРожденияОсобая,"Месяц"),
										ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваДатаРожденияОсобая,"День"))); 
							КонецЕсли;
							...
							Если СтрокаДереваМестоРождения <> Неопределено Тогда
								ТипМестаРождения = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваМестоРождения, "ТипМестаРождения");
								Если ТипМестаРождения = "ОСОБОЕ" Тогда
									ОбластьЗаявление.Параметры.МестоРожденияТип = "особое";
								КонецЕсли;
								ОбластьЗаявление.Параметры.МестоРожденияГород = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваМестоРождения, "ГородРождения");
								ОбластьЗаявление.Параметры.МестоРожденияРайон = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваМестоРождения, "РайонРождения");
								ОбластьЗаявление.Параметры.МестоРожденияОбласть = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваМестоРождения, "РегионРождения");
								ОбластьЗаявление.Параметры.МестоРожденияСтрана = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваМестоРождения, "СтранаРождения");
							КонецЕсли;
							
							ОбластьЗаявление.Параметры.Гражданство = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваИзменившиесяДанные, "Гражданство");
							...
							Если СтрокаДереваАдресРегистрации <> Неопределено Тогда
								ОбластьЗаявление.Параметры.АдресРегистрации = ПроцедурыПерсонифицированогоУчета.ПредставлениеАдресадляПФР(СтрокаДереваАдресРегистрации);
							КонецЕсли;
							...
							Если СтрокаДереваАдресФактический <> Неопределено Тогда
								ОбластьЗаявление.Параметры.АдресФактический = ПроцедурыПерсонифицированогоУчета.ПредставлениеАдресадляПФР(СтрокаДереваАдресФактический);
							КонецЕсли;
							
							ОбластьЗаявление.Параметры.Телефоны = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваИзменившиесяДанные, "Телефон");
						...
						Если СтрокаДереваУдостоверяющийДокумент <> Неопределено Тогда
							ТипУдостоверяющего = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваУдостоверяющийДокумент, "ТипУдостоверяющего");
							...
							Если СтрокаДереваДокумент <> Неопределено Тогда
								...
								НаименованиеДокумента = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваДокумент, "НаименованиеУдостоверяющего");
								Серия1 = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваДокумент, "СерияРимскиеЦифры");
								Серия2 = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваДокумент, "СерияРусскиеБуквы");
								СерияНомер = Серия1 +?(ТипУдостоверяющего = "ПАСПОРТ РБ", " ", ?(ТипУдостоверяющего = "ПАСПОРТ", "-", "") ) + Серия2 +",  " + ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваДокумент, "НомерУдостоверяющего");
								ОбластьЗаявление.Параметры.НаименованиеДокумента = НаименованиеДокумента;
								ОбластьЗаявление.Параметры.СерияНомер = СерияНомер;
								ОбластьЗаявление.Параметры.ДатаВыдачи = ПроцедурыПерсонифицированогоУчета.ДатаXMLВОтчет(ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваДокумент, "ДатаВыдачи"));
								ОбластьЗаявление.Параметры.КемВыдан = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваДокумент, "КемВыдан");
						...
						//Подвал
						ОбластьЗаявление.Параметры.ДатаЗаполнения = "Дата заполнения " + Символы.ПС + ПроцедурыПерсонифицированогоУчета.ДатаXMLВОтчет(ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваЗаявление, "ДатаЗаполнения"));
						
						
						//Поля, присутствующие только в форме АДВ-3
						Если СтрокаДереваЗаявление.Имя = "ЗАЯВЛЕНИЕ_О_ДУБЛИКАТЕ" Тогда 
							ПолИзСтрахового = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваЗаявление, "ПолИзСтрахового");
							...
							Если СтрокаДереваДатаРожденияОсобая = Неопределено Тогда
								ОбластьЗаявление.Параметры.ДатаРожденияВСвидетельстве = ПроцедурыПерсонифицированогоУчета.ДатаXMLВОтчет(ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваЗаявление, "ДатаРожденияИзСтрахового")); 
							Иначе
							...
							Если СтрокаДереваМестоРождения <> Неопределено Тогда
								...
								КонецЕсли;
								ОбластьЗаявление.Параметры.МестоРожденияГородВСвидетельстве = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваМестоРождения, "ГородРождения");
								ОбластьЗаявление.Параметры.МестоРожденияРайонВСвидетельстве = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваМестоРождения, "РайонРождения");
								ОбластьЗаявление.Параметры.МестоРожденияОбластьВСвидетельстве = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваМестоРождения, "РегионРождения");
								ОбластьЗаявление.Параметры.МестоРожденияСтранаВСвидетельстве = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваМестоРождения, "СтранаРождения");
							КонецЕсли;
							
							
							ОтметкаОПредставленииСведений = ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваЗаявление, "ОтметкаОпредставленииСведений");
				...
				Если СтрокаДереваОпись <> Неопределено Тогда
					...
					Если СтрокаДереваНомерПачки  <> Неопределено Тогда
						...
						Если СтрокаДереваОсновной <> Неопределено Тогда
							ИнформацияОПачке.Вставить("НомерПачкиРаботодателя", ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваНомерПачки, "Основной"));
						Иначе
							ИнформацияОПачке.Вставить("НомерПачкиРаботодателя", ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваНомерПачки, "ПоПодразделению"));
						КонецЕсли;
					КонецЕсли;
					ИнформацияОПачке.Вставить("ДатаСоставленияОписи",ПроцедурыПерсонифицированогоУчета.ДатаXMLВОтчет(ПроцедурыПерсонифицированогоУчета.ИзвлечьДанное(СтрокаДереваОпись, "ДатаСоставления"),Ложь));
					ПроцедурыПерсонифицированогоУчета.ВывестиОписьАДВ6(ДокументОписи,ИнформацияОПачке,ОтветственныеЛица);
					Возврат РаботаСДиалогами.НапечататьДокумент(ДокументОписи,1,Ложь,"Опись документов АДВ-6");
				КонецЕсли;
...
Процедура ЗаполнитьДанныеЗаявлений() Экспорт
	...
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.КлассификаторСтранМира.БЕЛАРУСЬ)
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ЕСТЬNULL(ГражданствоФизЛицСрезПоследних.Страна, ЗНАЧЕНИЕ(Справочник.КлассификаторСтранМира.БЕЛАРУСЬ)) = СтарыеДанныеОГражданстве.Гражданство
	|						ТОГДА ЗНАЧЕНИЕ(Справочник.КлассификаторСтранМира.ПустаяСсылка)
	|					ИНАЧЕ ЕСТЬNULL(ГражданствоФизЛицСрезПоследних.Страна, ЗНАЧЕНИЕ(Справочник.КлассификаторСтранМира.БЕЛАРУСЬ))
	|				КОНЕЦ
	|		КОГДА ДанныеАнкет.ФизЛицо ЕСТЬ НЕ NULL 
	|			ТОГДА ВЫБОР
	|					КОГДА ЕСТЬNULL(ГражданствоФизЛицСрезПоследних.Страна, ЗНАЧЕНИЕ(Справочник.КлассификаторСтранМира.БЕЛАРУСЬ)) = ДанныеАнкет.Гражданство
	|						ТОГДА ЗНАЧЕНИЕ(Справочник.КлассификаторСтранМира.ПустаяСсылка)
	|					ИНАЧЕ ЕСТЬNULL(ГражданствоФизЛицСрезПоследних.Страна, ЗНАЧЕНИЕ(Справочник.КлассификаторСтранМира.БЕЛАРУСЬ))
	|				КОНЕЦ
...
Функция СформироватьВыходнойФайл(Отказ) Экспорт
	...
	Если ВыборкаПоШапкеДокумента.ФорматФайла = Перечисления.ФорматФайлаПФР.Версия04 Тогда
		...
		ТекстФайла	= ТекстФайла + ПроцедурыПерсонифицированогоУчета.ПолучитьЗаписьОПачке(НомерПачки,ДатаЗаполнения,"","");
		Если Отказ Тогда
			Возврат "";
		КонецЕсли;
		...
		Иначе	
			ТипДокумента = "ДУБЛ";
		КонецЕсли;
		ТекстФайла	= ТекстФайла + ПроцедурыПерсонифицированогоУчета.ПолучитьЗаписьОТипеДокумента(ТипДокумента,КоличествоДокументов);
		
		//Получаем таблицу формата записи
		ДокументФорматЗаписей = ПолучитьМакет("ФорматЗаписей");
		ТаблицаФорматаЗаписи = ПроцедурыПерсонифицированогоУчета.ПолучитьФорматЗаписи(ТипДокумента,ДокументФорматЗаписей);
		...
		Пока ВыборкаПоРаботникиОрганизации.Следующий() Цикл
			...
			ИначеЕсли ЗначениеЗаполнено(ВыборкаПоРаботникиОрганизации.МестоРождения) тогда
				ПроцедурыПерсонифицированогоУчета.ВписатьВЗаписьМестоРождения(Запись, ВыборкаПоРаботникиОрганизации.МестоРождения);
			...
			Если ЗначениеЗаполнено(ВыборкаПоРаботникиОрганизации.ДокументВид) тогда
				ПроцедурыПерсонифицированогоУчета.ВписатьВЗаписьДокумент(Запись,ВыборкаПоРаботникиОрганизации);
			КонецЕсли;
			...
			СтрокавФайл	= ПроцедурыПерсонифицированогоУчета.ПолучитьСтрокуПоДаннымЗаписи(Запись,ТаблицаФорматаЗаписи);
	...
	ИначеЕсли ВыборкаПоШапкеДокумента.ФорматФайла = Перечисления.ФорматФайлаПФР.Версия07 Тогда
		...
		УзелПачкаВходящихДокументов = ПроцедурыПерсонифицированогоУчета.ЗаполнитьНаборЗаписейВходящаяОпись(УзелПФР, МакетФормата, ТипДокумента, ВыборкаПоШапкеДокумента, КоличествоДокументов, НомерПачки, НомерДокументаВПачке);
		
		Сокращение = "";
		ФорматАДВ23 = ПроцедурыПерсонифицированогоУчета.ЗагрузитьФорматНабораЗаписейдляПФР(МакетФормата, ТипДокумента);
		...
		Пока ВыборкаПоРаботникиОрганизации.Следующий()	Цикл
			...
			НаборЗаписейАДВ23 = ПроцедурыПерсонифицированогоУчета.СкопироватьСтруктуруДанных(ФорматАДВ23);
			...
			ПроцедурыПерсонифицированогоУчета.ЗаменитьБуквуЁ(НаборЗаписейФИОИзСтрахового.Фамилия, "Фамилия");
			НаборЗаписейФИОИзСтрахового.Имя = ВРег(ВыборкаПоРаботникиОрганизации.ИмяВСвидетельстве);
			ПроцедурыПерсонифицированогоУчета.ЗаменитьБуквуЁ(НаборЗаписейФИОИзСтрахового.Имя, "Имя");
			НаборЗаписейФИОИзСтрахового.Отчество = ВРег(ВыборкаПоРаботникиОрганизации.ОтчествоВСвидетельстве);
			ПроцедурыПерсонифицированогоУчета.ЗаменитьБуквуЁ(НаборЗаписейФИОИзСтрахового.Отчество, "Отчество");
			...
			Если ТипДокумента = "ЗАЯВЛЕНИЕ_О_ДУБЛИКАТЕ" Тогда
				...
				ПроцедурыПерсонифицированогоУчета.ЗаменитьБуквуЁ(НаборЗаписейМестоРожденияИзСтрахового.ГородРождения, "Город рождения");
				...
				ПроцедурыПерсонифицированогоУчета.ЗаменитьБуквуЁ(НаборЗаписейМестоРожденияИзСтрахового.РайонРождения, "Район рождения");
				НаборЗаписейМестоРожденияИзСтрахового.РегионРождения = МестоРожденияИзСтрахового.Область;
				ПроцедурыПерсонифицированогоУчета.ЗаменитьБуквуЁ(НаборЗаписейМестоРожденияИзСтрахового.РегионРождения, "Регион рождения");
				НаборЗаписейМестоРожденияИзСтрахового.СтранаРождения = МестоРожденияИзСтрахового.Страна;
				ПроцедурыПерсонифицированогоУчета.ЗаменитьБуквуЁ(НаборЗаписейМестоРожденияИзСтрахового.СтранаРождения, "Страна рождения");
			...
			ПроцедурыПерсонифицированогоУчета.ЗаменитьБуквуЁ(НаборЗаписейИзмФИО.Фамилия, "Фамилия");
			НаборЗаписейИзмФИО.Имя = ВРег(ВыборкаПоРаботникиОрганизации.Имя);
			ПроцедурыПерсонифицированогоУчета.ЗаменитьБуквуЁ(НаборЗаписейИзмФИО.Имя, "Имя");
			...
			Иначе
				...
				ПроцедурыПерсонифицированогоУчета.ЗаменитьБуквуЁ(НаборЗаписейИзмФИО.Отчество, "Отчество");
			КонецЕсли;
			//Пол
			Если ЗначениеЗаполнено(ВыборкаПоРаботникиОрганизации.Пол) тогда
			...
			ПроцедурыПерсонифицированогоУчета.ЗаменитьБуквуЁ(НаборЗаписейМестоРождения.ГородРождения, "Город рождения");
			...
			ПроцедурыПерсонифицированогоУчета.ЗаменитьБуквуЁ(НаборЗаписейМестоРождения.РайонРождения, "Район рождения");
			НаборЗаписейМестоРождения.РегионРождения = МестоРождения.Область;
			ПроцедурыПерсонифицированогоУчета.ЗаменитьБуквуЁ(НаборЗаписейМестоРождения.РегионРождения, "Регион рождения");
			НаборЗаписейМестоРождения.СтранаРождения = МестоРождения.Страна;
			ПроцедурыПерсонифицированогоУчета.ЗаменитьБуквуЁ(НаборЗаписейМестоРождения.СтранаРождения, "Страна рождения");
						
			// Гражданство
			НаборЗаписейИзменившиесяДанные.Гражданство.Значение = СокрЛП(ВыборкаПоРаботникиОрганизации.Гражданство);
			ПроцедурыПерсонифицированогоУчета.ЗаменитьБуквуЁ(НаборЗаписейИзменившиесяДанные.Гражданство.Значение, "Гражданство");
			...
			Иначе
				...
				ПроцедурыПерсонифицированогоУчета.ЗаполнитьАдрес(НаборЗаписейАдресРегистрации, ВыборкаПоРаботникиОрганизации.АдресРегистрации, 
			...
			Иначе
				...
				ПроцедурыПерсонифицированогоУчета.ЗаполнитьАдрес(НаборЗаписейАдресФактический, ВыборкаПоРаботникиОрганизации.АдресФактический, 
					СписокСтран, ФорматНеструктурированныйАдрес, ФорматИностранныйАдрес, ТекстОшибки);
			...
			ПроцедурыПерсонифицированогоУчета.ЗаменитьБуквуЁ(НаборЗаписейДокумент.КемВыдан, "Кем выдан");
		
			// ДатаЗаполнения
			НаборЗаписейАДВ23.ДатаЗаполнения.Значение = ДатаЗаполнения;
			ПроцедурыПерсонифицированогоУчета.ДобавитьИнформациюВДерево(ПроцедурыПерсонифицированогоУчета.ДобавитьУзелВДеревоXML(УзелПачкаВходящихДокументов, ТипДокумента,""), НаборЗаписейАДВ23);
		КонецЦикла;
...
Функция СформироватьЗапросПоШапке()
	...
	|	ВЫБОР
	|		КОГДА ЗаявлениеОбОбменеДубликатеСтраховогоСвидетельства.Организация.ГоловнаяОрганизация = &парамПустаяОрганизация
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
...
Функция	СформироватьЗапросПоРаботникиОрганизации(ВыборкаПоШапкеДокумента)
	...
	|	ЕСТЬNULL(ВЫБОР
	|			КОГДА РаботникиЗаявления.Гражданство = ЗНАЧЕНИЕ(Справочник.КлассификаторСтранМира.БЕЛАРУСЬ)
	|				ТОГДА """"
		...
		Иначе
			...
			Если ВыборкаПоСтрокамДокумента.ДокументВид <> Справочники.ДокументыУдостоверяющиеЛичность.ИМНС05
				...
				ИначеЕсли УказанВидДокумента 
					И НЕ ПроцедурыПерсонифицированогоУчета.ПроверитьСериюУдостоверенияЛичности(ВыборкаПоСтрокамДокумента.ДокументВид,ВыборкаПоСтрокамДокумента.ДокументСерия, СообщениеОбОшибке) Тогда 
				...
				ИначеЕсли УказанВидДокумента 
					И НЕ ПроцедурыПерсонифицированогоУчета.ПроверитьНомерУдостоверенияЛичности(ВыборкаПоСтрокамДокумента.ДокументВид,ВыборкаПоСтрокамДокумента.ДокументНомер, СообщениеОбОшибке) Тогда 
		...
		Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.АдресРегистрации) Тогда 
			...
			Если ЗаПределамиРФ <> Истина Тогда
				Если  РегламентированнаяОтчетность.АдресСоответствуетТребованиям(АдресРегистрацииСписком) Тогда
					ТекстОшибки = ПроцедурыПерсонифицированогоУчета.ПроверитьАдресПоКЛАДР(АдресРегистрацииСписком);
		...
		Если ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.АдресФактический) Тогда
			...
			Если ЗаПределамиРФ <> Истина Тогда
				Если РегламентированнаяОтчетность.АдресСоответствуетТребованиям(АдресФактическийСписком) Тогда
					ТекстОшибки = ПроцедурыПерсонифицированогоУчета.ПроверитьАдресПоКЛАДР(АдресФактическийСписком);
					Если НЕ ПустаяСтрока(ТекстОшибки) Тогда
