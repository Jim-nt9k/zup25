...
Процедура ДвиженияРегистров()
	...
	//Если НЕ ЗначениеЗаполнено(СпособОтраженияВБухучете) И НЕ ЗначениеЗаполнено(ОтнесениеРасходовКДеятельностиЕНВД) Тогда
	...
	//РБ
	//Островская Татьяна 23.05.2013 № 16496
	если ЗначениеЗаполнено(ВидРасчета.КодДоходаЕСН) Тогда 
		КодДоходаЕСНПоДоговору = ВидРасчета.КодДоходаЕСН;	
	иначе	
		КодДоходаЕСНПоДоговору = Справочники.ДоходыЕСН.ОблагаетсяФСЗНиСТРАХ;
	КонецЕсли;
	//Островская Татьяна 23.05.2013 № 16496
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если Организация.ИНН = "806000046" Тогда
			СпрСотр = Справочники.СотрудникиОрганизаций.НайтиПоКоду(Сотрудник.Код).ПолучитьОбъект();
			СпрСотр.СчетУчета = ПланыСчетов.Хозрасчетный.НайтиПоКоду(?(ЗначениеЗаполнено(Сотрудник.КодIBS),"70.03","70.05"));
			Попытка
				СпрСотр.Записать();
			Исключение
		...
		КонецЕсли;           
		//Аверсон 28.02.2019 КВВ СВФР-004035 )
		Если не ЗначениеЗаполнено(ДатаСоздания) Тогда //записывает дату создания в базе
