	...
	Если ВыборкаПоСтрокамДокумента.ВидСтрокиЗапроса = "РабочиеМеста" Тогда
		...
		Если ВыборкаПоШапкеДокумента.РежимНабораПерсонала = Перечисления.ВидыОрганизационнойСтруктурыПредприятия.ПоСтруктуреЮридическихЛиц Тогда
			Если ВыборкаПоСтрокамДокумента.ЭтоУправленческоеПодразделение Тогда
				ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "недопустимо указывать управленческие подразделения при текущем режиме кадрового планирования!", Отказ, Заголовок);
			КонецЕсли;
		Иначе
			Если Не ВыборкаПоСтрокамДокумента.ЭтоУправленческоеПодразделение Тогда
				ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "недопустимо указывать подразделения организаций при текущем режиме кадрового планирования!", Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;
