...
Функция Автозаполнение(ВременнаяТаблица = Неопределено, Реквизиты = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ПарамСчитатьПоДолжностнымОкладам",ПоДолжностномуОкладу);//О.Т. Аверсон 11.07.2017 № 808
	...
	|			ТОГДА ТарифныеСтавкиЗамещающего.Ставка
	|			КОГДА &ПарамСчитатьПоДолжностнымОкладам = 3
	|				ТОГДА ТарифныеСтавкиЗамещающего.ТарифнаяСПовышениемПоИнструкции //О.Т. Аверсон 26.10.2017 № 1158
	|		ИНАЧЕ ТарифныеСтавкиЗамещающего.ТарифнаяСтавка
	|	КОНЕЦ,0) КАК ОкладЗамещающего,
	|	ЕСТЬNULL(ВЫБОР
	|		КОГДА &ПарамСчитатьПоДолжностнымОкладам = 1
	|			ТОГДА ТарифныеСтавкиЗамещаемого.Ставка
	|			КОГДА &ПарамСчитатьПоДолжностнымОкладам = 3
	|				ТОГДА ТарифныеСтавкиЗамещаемого.ТарифнаяСПовышениемПоИнструкции //О.Т. Аверсон 26.10.2017 № 1158
	|		ИНАЧЕ ТарифныеСтавкиЗамещаемого.ТарифнаяСтавка
	|	КОНЕЦ,0) КАК ОкладЗамещаемого
	|ПОМЕСТИТЬ ВТОклады
	|ИЗ
	|	ВТСотрудники КАК Сотрудники
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТарифныеСтавкиРаботниковОрганизации.СрезПоследних(&парамПериодРегистрации, ) КАК ТарифныеСтавкиЗамещающего
	|		ПО Сотрудники.Сотрудник = ТарифныеСтавкиЗамещающего.Сотрудник
	|			И (ТарифныеСтавкиЗамещающего.Организация = &ПарамОрганизация)
	...
	|	Оклады.Сотрудник,
	|	Оклады.Подразделение КАК ПодразделениеОрганизации,
	|	Оклады.ОкладЗамещаемого КАК Показатель2,
	|	Оклады.ОкладЗамещающего КАК Показатель1,
	|	ВЫБОР КОГДА (Оклады.ОкладЗамещающего - Оклады.ОкладЗамещаемого) > 0 ТОГДА (Оклады.ОкладЗамещающего - Оклады.ОкладЗамещаемого) ИНАЧЕ 0 КОНЕЦ КАК РазницаВОкладах
	|ИЗ
	|	ВТОклады КАК Оклады
	|ГДЕ
...
Процедура АвтозаполнениеПоСтроке(Строка = Неопределено) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("ЗамещающийСотрудник", Строка.Сотрудник);
	...
	////Аверсон 26.03.2020 КВВ СВФР-005727 (
	//ЗанимаемыхСтавокЗамещающего = 1;
	//ЗанимаемыхСтавокЗамещаемого = 1;
	//Если Организация.ИНН = "100010315" И ВидыРасчетаДляЗамещения = Перечисления.ВидыРасчетаДляЗамещения.Замены Тогда
	//	СтавкаПоСотр = ?(СчитатьПоДолжностнымОкладам,Строка.Сотрудник,Строка.ЗамещаемыйСотрудник);
	//	З = Новый Запрос;
	//	З.Текст = "ВЫБРАТЬ
	//	|РаботникиОрганизацийСрезПоследних.ЗанимаемыхСтавок
	//	|ИЗ
	//	|РегистрСведений.РаботникиОрганизаций.СрезПоследних(&ПарамПериодРегистрации) КАК РаботникиОрганизацийСрезПоследних
	//	|ГДЕ РаботникиОрганизацийСрезПоследних.Сотрудник = &Сотрудник";
	//	З.УстановитьПараметр("Сотрудник", СтавкаПоСотр); 
	//	З.УстановитьПараметр("ПарамПериодРегистрации", КонецМесяца(ПериодРегистрации));
	//	РезЗ = З.Выполнить().Выбрать();
	//	Пока РезЗ.Следующий() Цикл
	//		Если СчитатьПоДолжностнымОкладам Тогда
	//			ЗанимаемыхСтавокЗамещающего = РезЗ.ЗанимаемыхСтавок; 
	//		Иначе
	//			ЗанимаемыхСтавокЗамещаемого = РезЗ.ЗанимаемыхСтавок; 
	//		КонецЕсли;
	//	КонецЦикла;
	//КонецЕсли;
	//Запрос.УстановитьПараметр("ЗанимаемыхСтавокЗамещающего",ЗанимаемыхСтавокЗамещающего);
	//Запрос.УстановитьПараметр("ЗанимаемыхСтавокЗамещаемого",ЗанимаемыхСтавокЗамещаемого);
	////Аверсон 26.03.2020 КВВ СВФР-005727 )
	...
	|				ТОГДА ТарифныеСтавкиЗамещающего.ТарифнаяСПовышениемПоИнструкции //О.Т. Аверсон 26.10.2017 № 1158
	|			ИНАЧЕ ТарифныеСтавкиЗамещающего.ТарифнаяСтавка
//	|		КОНЕЦ * &ЗанимаемыхСтавокЗамещающего, 0) КАК ОкладЗамещающего  //Аверсон 26.03.2020 КВВ СВФР-005727 (* &ЗанимаемыхСтавокЗамещающего)
	|		КОНЕЦ, 0) КАК ОкладЗамещающего  
	|ПОМЕСТИТЬ ВТЗамещающий
	|ИЗ
	|	РегистрСведений.ТарифныеСтавкиРаботниковОрганизации.СрезПоследних(&парамПериодРегистрации, ) КАК ТарифныеСтавкиЗамещающего
	|ГДЕ
	|	ТарифныеСтавкиЗамещающего.Сотрудник = &ЗамещающийСотрудник";
	...
	|				ТОГДА ТарифныеСтавкиЗамещаемого.ТарифнаяСПовышениемПоИнструкции //О.Т. Аверсон 26.10.2017 № 1158
	|			ИНАЧЕ ТарифныеСтавкиЗамещаемого.ТарифнаяСтавка
	//|		КОНЕЦ * &ЗанимаемыхСтавокЗамещаемого, 0) КАК ОкладЗамещаемого  //Аверсон 26.03.2020 КВВ СВФР-005727 (* &ЗанимаемыхСтавокЗамещаемого)
	|		КОНЕЦ, 0) КАК ОкладЗамещаемого  
	|ПОМЕСТИТЬ ВТЗамещаемый
	|ИЗ
	|	РегистрСведений.ТарифныеСтавкиРаботниковОрганизации.СрезПоследних(&парамПериодРегистрации, ) КАК ТарифныеСтавкиЗамещаемого
	|ГДЕ
	|	ТарифныеСтавкиЗамещаемого.Сотрудник = &ЗамещаемыйСотрудник";

	Запрос.Выполнить();
	                                     	
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ЕСТЬNULL(Замещаемый.ОкладЗамещаемого,0) КАК Показатель2,
	|	ЕСТЬNULL(Замещающий.ОкладЗамещающего,0) КАК Показатель1,
	|	ВЫБОР КОГДА ЕСТЬNULL(Замещающий.ОкладЗамещающего,0) - ЕСТЬNULL(Замещаемый.ОкладЗамещаемого,0) > 0 ТОГДА ЕСТЬNULL(Замещающий.ОкладЗамещающего,0) - ЕСТЬNULL(Замещаемый.ОкладЗамещаемого,0) ИНАЧЕ 0 КОНЕЦ  КАК РазницаВОкладах
	|ИЗ
	|	ВТЗамещающий КАК Замещающий
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТЗамещаемый КАК Замещаемый
	|		ПО (ИСТИНА)";
	...
	//Аверсон ЕАЕ 06.02.2024 СВФР-012141
	//БобруйскСельмаш
	//Аверсон ЕАЕ 21.05.2024 СВФР-012907
	//Климовичи
	Если Организация.ИНН = "790056159" ИЛИ Организация.ИНН = "700103211" Тогда
		Если ЗначениеЗаполнено(Строка.ВидРасчета) Тогда
			Если ПоДолжностномуОкладу = 1 ИЛИ ПоДолжностномуОкладу = 2 Тогда
				ВР = Строка.ВидРасчета;
				СР = ВР.СпособРасчета;
				Если СР = Перечисления.СпособыРасчетаОплатыТруда.ДоплатаЗаСовмещениеПоТарифу Тогда
					Строка.Показатель1 = ПроцентОплаты;
					СведенияОЗамещаемомСотруднике = РегистрыСведений.РаботникиОрганизаций.ПолучитьПоследнее(КонецМесяца(ПериодРегистрации),Новый Структура("Сотрудник",Строка.ЗамещаемыйСотрудник));
					
					График = СведенияОЗамещаемомСотруднике.ГрафикРаботы;
					...
					Строка.Показатель2 = Окр(Строка.Показатель2 * СведенияОЗамещаемомСотруднике.ПовышенныйКоэффициентЕТС / РазмерСреднемесячнаяНормаЧасов,ТочнОкр);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура РассчитатьНачисления() Экспорт
	...
	//Островская Татьяна 14.01.2014 № 16993
	ВыборкаПоНачислениям = СформироватьЗапросПоНачисления(ВыборкаПоШапкеДокумента).Выбрать();
	//ВыборкаПоНачислениям = ВыборкаПоНачислениям1.Выбрать();
	//ОбщаяСуммаНачислений = 0;
	//Пока ВыборкаПоНачислениям.Следующий() Цикл
	//	если не(ВыборкаПоНачислениям.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.НайтиПоНаименованию("Ночные") или
	//		ВыборкаПоНачислениям.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.НайтиПоНаименованию("Ночные по экстр. помощи") или
	//		ВыборкаПоНачислениям.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.НайтиПоНаименованию("Доплата за ночные часы")) Тогда 
	//		ОбщаяСуммаНачислений = ОбщаяСуммаНачислений + ВыборкаПоНачислениям.Результат;
	//	КонецЕсли;
	//КонецЦикла;
	//ВыборкаПоНачислениям = ВыборкаПоНачислениям1.Выбрать();
	////Островская Татьяна 14.01.2014 № 16993
	Пока ВыборкаПоНачислениям.Следующий() Цикл
		...
		Если НЕ Отказ Тогда
			...
			ДобавитьСтрокуОсновныхНачислений(ВыборкаПоНачислениям, ВыборкаПоШапкеДокумента, НаборОсновныеНачисления,Неопределено);//Островская Татьяна 14.01.2014 № 16993
...
Процедура ЗаполнитьПоПерерассчитываемомуДокументу(ИсходныйДокумент, Знач Сотрудники = Неопределено) Экспорт
	...
	Если Сотрудники = "ПоДаннымПерерасчета" Тогда
		...
		|ИЗ
		|	Документ.ОплатаЗамещенияСотрудников.ОсновныеНачисления КАК Начисления
		|ГДЕ
		...
		|ИЗ
		|	Документ.ОплатаЗамещенияСотрудников.ОсновныеНачисления КАК СотрудникиОрганизаций
		|ГДЕ
...
Функция СформироватьЗапросПоШапке()
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ОплатаЗамещенияСотрудников.Дата,
	|	ОплатаЗамещенияСотрудников.ПериодРегистрации,
	|	ОплатаЗамещенияСотрудников.Организация,
	|	ОплатаЗамещенияСотрудников.ПерерассчитываемыйДокумент КАК ПерерассчитываемыйДокумент,
	|	ОплатаЗамещенияСотрудников.ПерерассчитываемыйДокумент.ПериодРегистрации КАК ПериодПерерасчета,
	|	ОплатаЗамещенияСотрудников.ПерерассчитываемыйДокумент.Организация КАК ОрганизацияПерерасчета,
	|	ВЫБОР
	|		КОГДА ОплатаЗамещенияСотрудников.Организация.ГоловнаяОрганизация = &парамПустаяОрганизация
	|			ТОГДА ОплатаЗамещенияСотрудников.Организация
	|		ИНАЧЕ ОплатаЗамещенияСотрудников.Организация.ГоловнаяОрганизация
	|	КОНЕЦ КАК ГоловнаяОрганизация,
	|	ОплатаЗамещенияСотрудников.Организация КАК ОбособленноеПодразделение,
	|	ОплатаЗамещенияСотрудников.Ссылка
	|ИЗ
	|	Документ.ОплатаЗамещенияСотрудников КАК ОплатаЗамещенияСотрудников
	|ГДЕ
	|	ОплатаЗамещенияСотрудников.Ссылка = &ДокументСсылка";
...
Функция СформироватьЗапросПоНачисления(ВыборкаПоШапкеДокумента)
	...
	|ИЗ
	|	Документ.ОплатаЗамещенияСотрудников.ОсновныеНачисления КАК СтрокиДокумента
	|ГДЕ
...
//	Нет.
//
Процедура ДобавитьСтрокуОсновныхНачислений(ВыборкаПоСтрокамДокумента, ВыборкаПоШапкеДокумента, НаборЗаписей,ОбщаяСуммаНачислений)
	//Островская Татьяна 14.01.2014 № 16993
	если (ВидыРасчетаДляЗамещения = Перечисления.ВидыРасчетаДляЗамещения.ЭкстренныйВызов или ВидыРасчетаДляЗамещения = Перечисления.ВидыРасчетаДляЗамещения.ДежурствоНаДому)
		и ОбщаяСуммаНачислений <> Неопределено Тогда 
		если ВыборкаПоСтрокамДокумента.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.НайтиПоНаименованию("Экстренный вызов") или
			ВыборкаПоСтрокамДокумента.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.НайтиПоНаименованию("Дежурство на дому") или
			ВыборкаПоСтрокамДокумента.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.НайтиПоНаименованию("Ночные") или
			ВыборкаПоСтрокамДокумента.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.НайтиПоНаименованию("Ночные по экстр. помощи") или
			ВыборкаПоСтрокамДокумента.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.НайтиПоНаименованию("Доплата за ночные часы") Тогда 
			...
			// Ресурсы
			//Островская Татьяна 14.01.2014 № 16993
			если ВыборкаПоСтрокамДокумента.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.НайтиПоНаименованию("Ночные") или
				ВыборкаПоСтрокамДокумента.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.НайтиПоНаименованию("Ночные по экстр. помощи") или
				ВыборкаПоСтрокамДокумента.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.НайтиПоНаименованию("Доплата за ночные часы") Тогда
				Движение.Результат						= ВыборкаПоСтрокамДокумента.Результат;
			иначе
				
				Движение.Результат						= ОбщаяСуммаНачислений;
			КонецЕсли;
			//Островская Татьяна 14.01.2014 № 16993
	...
	иначе
		...
		Если ВидыРасчетаДляЗамещения = Перечисления.ВидыРасчетаДляЗамещения.СверхУстановленнойНормы и
			ВыборкаПоСтрокамДокумента.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.НайтиПоНаименованию("Сверх установленной нормы") Тогда //О.Т. Аверсон 14.02.2017 № 312
			
			Движение.ОтработаноЧасов   = ОплаченоДнейЧасовШапка;
		КонецЕсли;
...
Процедура СформироватьДоходыПоКодамНДФЛ(ВыборкаПоШапкеДокумента, НаборЗаписей)
	...
	|		Основные.ВидРасчета
	|	ИЗ
	|		Документ.ОплатаЗамещенияСотрудников.ОсновныеНачисления КАК Основные
	|	ГДЕ
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Заголовок = ОбщегоНазначения.ПредставлениеДокументаПриПроведении(Ссылка);
	//Островская Татьяна 18.11.2013 № 16909
	если не ЗначениеЗаполнено(ЗаменыНаВремя) Тогда 
		Сообщить("Не заполнено поле: Замены на время (основание)!");
		Отказ = Истина;
	КонецЕсли;
	...
	Если ВыборкаПоШапкеДокумента.Следующий() Тогда
		...
		Если НЕ Отказ Тогда
			...
			// получим реквизиты табличной части
			//Островская Татьяна 14.01.2014 № 16993
			ВыборкаПоНачислениям1 = СформироватьЗапросПоНачисления(ВыборкаПоШапкеДокумента);
			ВыборкаПоНачислениям = ВыборкаПоНачислениям1.Выбрать();
			ОбщаяСуммаНачислений = 0;
			Пока ВыборкаПоНачислениям.Следующий() Цикл
				если не(ВыборкаПоНачислениям.ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.НайтиПоНаименованию("Ночные") или
			...
			Пока ВыборкаПоНачислениям.Следующий() Цикл 
				...
				Если НЕ Отказ Тогда
					// Заполним записи в наборах записей регистров
					ДобавитьСтрокуОсновныхНачислений(ВыборкаПоНачислениям, ВыборкаПоШапкеДокумента, Движения.ОсновныеНачисленияРаботниковОрганизаций,ОбщаяСуммаНачислений); //Островская Татьяна 14.01.2014 № 16993
...
Функция ДоработатьСведенияОВидеРасчетаСхемыМотивации(СведенияОВидеРасчета,ВидРасчета,Режим) Экспорт //Островская Татьяна 18.04.2013 № 16410
	...
	Если СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.Процентом 
		или СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПроцентомОтТарифаИБазы или СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПроцентОтТарифнойСтавки1Разряда 
		или СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПроцентОтТарифнойСтавки1РазрядаПоРаботнику 
		или СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ИндексацияЗП Тогда //О.Т. Аверсон 04.11.2016 № 1501
	...
	ИначеЕсли СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ОтСтажаПроцентом
		...
		СведенияОВидеРасчета.Вставить("Показатель2Наименование", ?(Режим = "ПлановыеНачисления","Стаж","Процент оплаты"));
		СведенияОВидеРасчета.Вставить("Показатель2Видимость", Истина);
		СведенияОВидеРасчета.Вставить("Показатель2Точность", 2);
		СведенияОВидеРасчета.Вставить("Показатель2НаименованиеВидимость", Истина);
	...
	ИначеЕсли СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ОтСтажаПоСумме Тогда
		СведенияОВидеРасчета.Вставить("Показатель2Наименование", "От стажа по сумме");
		СведенияОВидеРасчета.Вставить("Показатель2Видимость", Истина);
		СведенияОВидеРасчета.Вставить("Показатель2Точность", 2);
		СведенияОВидеРасчета.Вставить("Показатель2НаименованиеВидимость", Истина);
		СведенияОВидеРасчета.Вставить("Валюта2Видимость", Истина);
	...
	ИначеЕсли  СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.Разъездные Тогда
		СведенияОВидеРасчета.Вставить("Показатель2Наименование", "Кол-во (дни)");
		СведенияОВидеРасчета.Вставить("Показатель2Видимость", Истина);
	...
	ИначеЕсли  СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.Питание Тогда
		Попытка
			СведенияОВидеРасчета.Вставить("Показатель2Видимость", Истина);
		Исключение
			СведенияОВидеРасчета.Вставить("Показатель2Наименование", "Кол-во");
			СведенияОВидеРасчета.Вставить("Показатель2Видимость", Истина);
			СведенияОВидеРасчета.Вставить("Показатель2Точность", 0);
			СведенияОВидеРасчета.Вставить("Показатель2НаименованиеВидимость", Истина);
			СведенияОВидеРасчета.Вставить("Валюта2Видимость", Ложь);
		...
		КонецПопытки;
		СведенияОВидеРасчета["Показатель2Наименование"] = "Кол-во";
		...
		СведенияОВидеРасчета["Валюта2Видимость"] = Истина;
		СведенияОВидеРасчета.Вставить("КоличествоПоказателей",2);
		СведенияОВидеРасчета.Вставить("Показатель2ВозможностьИзменения", Истина);
	ИначеЕсли  СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ДетскийДень Тогда
		...
		КонецПопытки;
		СведенияОВидеРасчета["Показатель2Наименование"] = "";
	...
	ИначеЕсли СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПособиеНаРебенкаДо3Лет Тогда
		СведенияОВидеРасчета.Вставить("Показатель2Наименование", "Размер пособия");
		СведенияОВидеРасчета.Вставить("Показатель2Видимость", Истина);
		СведенияОВидеРасчета.Вставить("Показатель2Точность", 2);
		СведенияОВидеРасчета.Вставить("Показатель2НаименованиеВидимость", Истина);
		СведенияОВидеРасчета.Вставить("Валюта2Видимость", Ложь);
		СведенияОВидеРасчета["Показатель2Наименование"] = "Размер пособия";
		СведенияОВидеРасчета["Показатель2Видимость"] = Истина;
		СведенияОВидеРасчета["Показатель2НаименованиеВидимость"] = Истина;	
		СведенияОВидеРасчета["Показатель2Точность"] = 2;//Островская Татьяна 12.03.2013 № 16313
		СведенияОВидеРасчета["Валюта2Видимость"] = Истина;
		СведенияОВидеРасчета.Вставить("КоличествоПоказателей",2);
		СведенияОВидеРасчета.Вставить("Показатель2ВозможностьИзменения", Истина);
	...
	ИначеЕсли СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ДоплатаЗаВечерниеЧасы
		...
		СведенияОВидеРасчета.Вставить("Показатель2Наименование", ?(Режим = "ПлановыеНачисления","Процент доплаты","Тарифная ставка"));
		СведенияОВидеРасчета.Вставить("Показатель2Видимость", Истина);
		СведенияОВидеРасчета.Вставить("Показатель2НаименованиеВидимость", Истина);
		СведенияОВидеРасчета.Вставить("Показатель2Точность", 2);
		СведенияОВидеРасчета.Вставить("Валюта2Видимость", Ложь);
	...
	ИначеЕсли СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ОтСтажаПоДневнойТарифнойСтавке
		...
		СведенияОВидеРасчета.Вставить("Показатель2Наименование", ?(Режим = "ПлановыеНачисления","Стаж","Тарифная ставка"));
		СведенияОВидеРасчета.Вставить("Показатель2Видимость", Истина);
		СведенияОВидеРасчета.Вставить("Показатель2НаименованиеВидимость", Истина);
		СведенияОВидеРасчета.Вставить("Валюта2Видимость", Ложь);
		СведенияОВидеРасчета.Вставить("КоличествоПоказателей",2);
	...
	ИначеЕсли СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПоСреднемуЗаработкуФСС Тогда
		СведенияОВидеРасчета.Вставить("Показатель2Наименование", "Процент среднего заработка");
		СведенияОВидеРасчета.Вставить("Показатель2Видимость", Истина);
		СведенияОВидеРасчета.Вставить("Показатель2НаименованиеВидимость", Истина);
		СведенияОВидеРасчета.Вставить("Валюта2Видимость", Ложь);
		СведенияОВидеРасчета.Вставить("КоличествоПоказателей",2);
	...
	ИначеЕсли  СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.СдельныйЗаработок Тогда
		СведенияОВидеРасчета.Вставить("Показатель2Наименование", "Тарифная ставка");
		СведенияОВидеРасчета.Вставить("КоличествоПоказателей",2);
		СведенияОВидеРасчета.Вставить("Показатель2ВозможностьИзменения", Истина);
		СведенияОВидеРасчета.Вставить("Показатель2Точность", 2);
	ИначеЕсли  СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ФиксированнойСуммой Или
		...
		//заявка №1676
		СведенияОВидеРасчета.Вставить("Показатель2Наименование", "Сумма");
		СведенияОВидеРасчета.Вставить("Показатель2Видимость", Истина);
		СведенияОВидеРасчета.Вставить("Показатель2НаименованиеВидимость", Истина);
		СведенияОВидеРасчета.Вставить("Валюта2Видимость", Истина);
		СведенияОВидеРасчета.Вставить("КоличествоПоказателей",2);
	...
	ИначеЕсли  СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.РазницаВОкладах  Тогда
		
		
	ИначеЕсли  СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПрямойСдельныйЗаработок Тогда
		СведенияОВидеРасчета.Вставить("Показатель2Наименование", "Сдельный заработок");
		СведенияОВидеРасчета.Вставить("Показатель2Видимость", Истина);
		СведенияОВидеРасчета.Вставить("Показатель2НаименованиеВидимость", Истина);
		СведенияОВидеРасчета.Вставить("Валюта2Видимость", Истина);
		СведенияОВидеРасчета.Вставить("КоличествоПоказателей",2);
	...
	ИначеЕсли  СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ИсполнительныйЛистПроцентом 
		...
		СведенияОВидеРасчета.Вставить("Показатель2Наименование", "Процент удержаний");
		СведенияОВидеРасчета.Вставить("Показатель2Видимость", Истина);
		СведенияОВидеРасчета.Вставить("Показатель2НаименованиеВидимость", Истина);
		СведенияОВидеРасчета.Вставить("Валюта2Видимость", Истина);
		СведенияОВидеРасчета.Вставить("КоличествоПоказателей",2);
	...
	ИначеЕсли  СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ИсполнительныйЛистФиксСуммойДоПредела
		...
		СведенияОВидеРасчета.Вставить("Показатель2Наименование", "Фиксированная сумма");
		СведенияОВидеРасчета.Вставить("Показатель2Видимость", Истина);
		СведенияОВидеРасчета.Вставить("Показатель2НаименованиеВидимость", Истина);
		СведенияОВидеРасчета.Вставить("Валюта2Видимость", Истина);
		СведенияОВидеРасчета.Вставить("КоличествоПоказателей",2);
	...
	ИначеЕсли  СтруктураВидаРасчета.СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПочтовыйСбор Тогда
		СведенияОВидеРасчета.Вставить("Показатель2Наименование", "Процент сбора");
		СведенияОВидеРасчета.Вставить("Показатель2Видимость", Истина);
		СведенияОВидеРасчета.Вставить("Показатель2НаименованиеВидимость", Истина);
		СведенияОВидеРасчета.Вставить("Валюта2Видимость", Истина);
		СведенияОВидеРасчета.Вставить("КоличествоПоказателей",2);
	...
	иначе
		Попытка
			ЕстьЗначение = СведенияОВидеРасчета.Показатель2Наименование;
		Исключение
			ЕстьЗначение = Неопределено;
			
		КонецПопытки;
		если не ЕстьЗначение = Неопределено Тогда 
			если не ЗначениеЗаполнено(СведенияОВидеРасчета.Показатель2Наименование) Тогда 
				СведенияОВидеРасчета.Вставить("Показатель2Наименование", " ");
				СведенияОВидеРасчета.Вставить("Показатель2Видимость", Истина);
				СведенияОВидеРасчета.Вставить("Показатель2НаименованиеВидимость", Истина);
				СведенияОВидеРасчета.Вставить("Валюта2Видимость", Истина);
...
КонецФункции //Островская Татьяна 18.04.2013 № 16410
