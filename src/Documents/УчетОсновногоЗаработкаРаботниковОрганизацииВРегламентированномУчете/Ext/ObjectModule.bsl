		...
		Если НЕ Отказ Тогда
			...
			Пока ВыборкаСтрокЗапроса.Следующий() Цикл
				...
				Если НЕ Отказ Тогда
					...
					//КонецЕсли;
					Если ВРЕГ(ВыборкаСтрокЗапроса.СпособОтраженияВБухучете) = "ДНПС" и ВыборкаПоШапкеДокумента.Организация.ИНН = "806000046" Тогда
						ОбъектФ = ВыборкаСтрокЗапроса.Сотрудник.Физлицо.ПолучитьОбъект();
						ОбъектФ.ДНПС = Истина;
						ОбъектФ.записать();
					КонецЕсли;
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ПриемНаРаботуВОрганизацию") ИЛИ ТипЗнч(Основание) = Тип("ДокументСсылка.КадровоеПеремещениеОрганизаций") Тогда //Островская Татьяна 10.06.2014 № 17427 
