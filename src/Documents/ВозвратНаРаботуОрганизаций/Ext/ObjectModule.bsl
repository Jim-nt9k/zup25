...
Функция СформироватьЗапросПоШапке(Режим)
	...
	|	ВЫБОР
	|		КОГДА ВозвратНаРаботуОрганизаций.Организация.ГоловнаяОрганизация = &ПустаяОрганизация
...
Процедура ПроверитьЗаполнениеСтрокиРаботникаОрганизации(ВыборкаПоШапкеДокумента, ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	Если ВыборкаПоСтрокамДокумента.КонфликтноеСостояние <> NULL Тогда
		...
		Если Организация.ИНН = "700067572" Тогда
			Если Лев(ВыборкаПоСтрокамДокумента.КонфликтныйДокумент,19)  = "Отпуска организаций" 
				ИЛИ Найти(Нрег(ВыборкаПоСтрокамДокумента.КонфликтныйДокумент),"отпуск по уходу за ребенком")> 0 //Аверсон 18.11.2021 КВВ СВФР-008392
				или Лев(ВыборкаПоСтрокамДокумента.КонфликтныйДокумент,24) = "Командировки организаций" Тогда //ПСА 12.02.2021 СВФР-007197+++
				ВыводСообщения = Ложь;	
			КонецЕсли;
		КонецЕсли;	
		...
		//+++ПСА 08.02.2021 СВФР-007152
			...
			//Аверсон ЕАЕ 11.11.2024 СВФР-013935
			//Если есть конфликтное состояние Прогулы, возврат может проводиться
			//ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + СтрокаСообщениеОбОшибке, Отказ, Заголовок);
			Если ВыборкаПоСтрокамДокумента.КонфликтноеСостояние = Перечисления.СостоянияРаботникаОрганизации.Прогулы 
				
				//Аверсон ЕАЕ 13.10.2025 СВФР-017010
				ИЛИ ВыборкаПоСтрокамДокумента.КонфликтноеСостояние = Перечисления.СостоянияРаботникаОрганизации.ОтпускЕжегодный Тогда
				//Аверсон СВФР-017010
...
Процедура ДобавитьСтрокуВДвиженияПоРегистрамСведений(ВыборкаПоШапкеДокумента, ВыборкаПоРаботникиОрганизации,  
	...
	Если СтруктураПроведенияПоРегистрамСведений.Свойство(ИмяРегистра) Тогда
		...
		// Измерения
		Движение.Сотрудник		= ВыборкаПоРаботникиОрганизации.Сотрудник;
		Движение.ДатаОкончания	= ВыборкаПоРаботникиОрганизации.ДатаВозврата-86400;  //Наркевич 23.04.2014 период - 1 день 
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ОтсутствиеНаРаботеОрганизаций") Тогда	
		ОбщегоНазначения.ЗаполнитьШапкуДокументаПоОснованию(ЭтотОбъект, Основание);
		...
		Если Организация.ИНН = "100194961" 
			...
			ТЗТЧ = Основание.РаботникиОрганизации.Выгрузить();  //Аверсон 02.12.2022 КВВ СВФР-009868
			
			Для Каждого ТекСтрока Из Основание.РаботникиОрганизации Цикл
				Если ЗначениеЗаполнено(ТекСтрока.ДатаОкончания) 
					И (ТекСтрока.ПричинаОтсутствия = Перечисления.СостоянияРаботникаОрганизации.Заболевание 
					ИЛИ ТекСтрока.ПричинаОтсутствия = Перечисления.СостоянияРаботникаОрганизации.ОтсутствиеПоИнициативеНанимателя) Тогда //Аверсон 03.05.2023 КВВ СВФР-010777
					
					//Аверсон 02.12.2022 КВВ СВФР-009868 (
					Отбор = Новый Структура;
					Отбор.Вставить("Сотрудник", ТекСтрока.Сотрудник);
					//Отбор.Вставить("ПричинаОтсутствия", Перечисления.СостоянияРаботникаОрганизации.Заболевание);
					Отбор.Вставить("ПричинаОтсутствия", ?(ТекСтрока.ПричинаОтсутствия = Перечисления.СостоянияРаботникаОрганизации.Заболевание,Перечисления.СостоянияРаботникаОрганизации.Заболевание,Перечисления.СостоянияРаботникаОрганизации.ОтсутствиеПоИнициативеНанимателя));//Аверсон 03.05.2023 КВВ СВФР-010777
					Отбор.Вставить("ДатаНачала", НачалоДня(ТекСтрока.ДатаОкончания + 86400));
					СтрокиПоОтбору = ТЗТЧ.НайтиСтроки(Отбор);
				...
				Если ЗначениеЗаполнено(ТекСтрока.ДатаОкончания) И ТекСтрока.ПричинаОтсутствия = Перечисления.СостоянияРаботникаОрганизации.Простой Тогда
					Отбор = Новый Структура;
					Отбор.Вставить("Сотрудник", ТекСтрока.Сотрудник);
					Отбор.Вставить("ПричинаОтсутствия", Перечисления.СостоянияРаботникаОрганизации.Простой);
					Отбор.Вставить("ДатаНачала", НачалоДня(ТекСтрока.ДатаОкончания + 86400));
					СтрокиПоОтбору = ТЗТЧ.НайтиСтроки(Отбор);
					Если СтрокиПоОтбору.Количество() = 0 Тогда
						НоваяСтрока = РаботникиОрганизации.Добавить();
		...
		Комментарий = "Создано автоматически из док." + СокрЛП(Основание);
		//Аверсон 24.11.2022 КВВ СВФР-009868 )
