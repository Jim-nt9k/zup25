...
#Если Клиент Тогда
	...
	Функция ПечатьКомандировочноеУдостоверение(ИмяМакета)//О.Т. 11.02.2015 № 270
		...
		//Аверсон ЕАЕ 29.01.2026 СВФР-017825
		//Макет = Документы.КомандировкиОрганизаций.ПолучитьМакет(ИмяМакета);
		Если ИмяМакета = "КомандировочноеУдостоверение" И Организация.ИНН = "100194961" Тогда
			Макет = Документы.КомандировкиОрганизаций.ПолучитьМакет("КомандировочноеУдостоверение_БольшеСтрокОВыбытииПрибытии");	
		Иначе
			Макет = Документы.КомандировкиОрганизаций.ПолучитьМакет(ИмяМакета);
		КонецЕсли;
		//Аверсон СВФР-017825
