...
// ПСА 11-09-2020 СВФР-006182 +++
Процедура ЗаполнитьДокументПоПартионныеДенежниеПереводы(Основание)
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Основание",Основание);
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПартионныеДенежниеПереводыДенежныеПереводы.Ссылка КАК Ведомость,
	|	ПартионныеДенежниеПереводыДенежныеПереводы.Ссылка.Организация,
	|	ПартионныеДенежниеПереводыДенежныеПереводы.Ссылка.СчетОрганизации,
	|	ПартионныеДенежниеПереводыДенежныеПереводы.Ссылка.Ответственный,
	|	СУММА(ПартионныеДенежниеПереводыДенежныеПереводы.СуммаПеревода + ПартионныеДенежниеПереводыДенежныеПереводы.ПочтовыйСбор) КАК СуммаКВыплате
	|ИЗ
	|	Документ.ПартионныеДенежниеПереводы.ДенежныеПереводы КАК ПартионныеДенежниеПереводыДенежныеПереводы
	|ГДЕ
	|	ПартионныеДенежниеПереводыДенежныеПереводы.Ссылка = &Основание
	|
	|СГРУППИРОВАТЬ ПО
	|	ПартионныеДенежниеПереводыДенежныеПереводы.Ссылка,
	|	ПартионныеДенежниеПереводыДенежныеПереводы.Ссылка.Организация,
	|	ПартионныеДенежниеПереводыДенежныеПереводы.Ссылка.СчетОрганизации,
	|	ПартионныеДенежниеПереводыДенежныеПереводы.Ссылка.Ответственный";
	Выборка = Запрос.Выполнить().Выбрать();
...
Функция СформироватьЗапросПоДокПлатежноеПоручение()
	...
	Если ПроцедурыУправленияПерсоналом.ЗначениеУчетнойПолитикиПоПерсоналуОрганизации(глЗначениеПеременной("глУчетнаяПолитикаПоПерсоналуОрганизации"), Организация, "УчетЗадолженностиПоМесяцам") Тогда
		...
		|	ЗарплатаКВыплатеОрганизацииЗарплата.ПроводитьВзаиморасчеты КАК ПроводитьВзаиморасчеты,
		...
		//(начало) закомментировано Кухарчуком Д.Я. 10.08.18, Задача СВФР-002303
		//|	,ВЫБОР
		//|		КОГДА РаботникиОрганизацийСрезПоследних.Сотрудник ЕСТЬ NULL 
		//|				ИЛИ РаботникиОрганизацийСрезПоследних.Регистратор ССЫЛКА Документ.УвольнениеИзОрганизаций
		//|			ТОГДА ИСТИНА
		//|		ИНАЧЕ ЛОЖЬ
		//|	КОНЕЦ КАК Акционер
		//(конец) закомментировано Кухарчуком Д.Я. 10.08.18, Задача СВФР-002303
		//(начало) добавлено Кухарчуком Д.Я. 10.08.18, Задача СВФР-002303
		|	ПОМЕСТИТЬ ВтВыплаты
		//(конец) добавлено Кухарчуком Д.Я. 10.08.18, Задача СВФР-002303
		|ИЗ
		...
		//|		ПО ЗарплатаКВыплатеОрганизацииЗарплата.Физлицо = РаботникиОрганизацийСрезПоследних.Сотрудник.Физлицо //О.Т. Аверсон 24.07.2018 № 2208
		//(конец) закомментировано Кухарчуком Д.Я. 10.08.18, Задача СВФР-002303
		|ГДЕ
		...
		|	ЗарплатаКВыплатеОрганизацииЗарплата.ПроводитьВзаиморасчеты,
		...
		|	Выплаты.ХарактерВыплаты,
		|	Выплаты.СуммаВзаиморасчетов,
		|	Выплаты.ПериодВзаиморасчетов, //О.Т. Аверсон 03.10.2018 № 2558 нужна для вывода в регистр взаиморасчеты
		//(начало) закомментировано Кухарчуком Д.Я. 16.08.18, задача СВФР-002337
		//|	МИНИМУМ( ВЫБОР
		//|		КОГДА РаботникиОрганизацийСрезПоследних.Сотрудник ЕСТЬ NULL 
		//|				ИЛИ РаботникиОрганизацийСрезПоследних.Регистратор ССЫЛКА Документ.УвольнениеИзОрганизаций
		//|			ТОГДА ИСТИНА
		//|		ИНАЧЕ ЛОЖЬ
		//|	КОНЕЦ ) КАК Акционер
		//(конец) закомментировано Кухарчуком Д.Я. 16.08.18, задача СВФР-002337
		//(начало) добавлено Кухарчуком Д.Я. 16.08.18, задача СВФР-002337
		|	МИНИМУМ( 
		|		( РаботникиОрганизацийСрезПоследних.Сотрудник ЕСТЬ NULL 
		|			И Подрядчики.Физлицо ЕСТЬ NULL
		|		ИЛИ 
		|		НЕ РаботникиОрганизацийСрезПоследних.Сотрудник ЕСТЬ NULL 
		|			И РаботникиОрганизацийСрезПоследних.Регистратор ССЫЛКА Документ.УвольнениеИзОрганизаций
		|		) И НЕ Выплаты.ПроводитьВзаиморасчеты
		|	 ) КАК Акционер
		//(конец) добавлено Кухарчуком Д.Я. 16.08.18, задача СВФР-002337
		|ИЗ
		|	ВтВыплаты КАК Выплаты
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций.СрезПоследних( &Период,
		|               Сотрудник.Физлицо В (ВЫБРАТЬ Выплаты.Физлицо ИЗ ВтВыплаты КАК Выплаты)
		|				) КАК РаботникиОрганизацийСрезПоследних
		|		ПО Выплаты.Физлицо = РаботникиОрганизацийСрезПоследних.Сотрудник.Физлицо
		//(начало) добавлено Кухарчуком Д.Я. 16.08.18, задача СВФР-002337
		...
		|		ГДЕ
		//(начало) закомментировано Кухарчуком Д.Я. 29.08.18, задача СВФР-002367
		//|			ДоговорНаВыполнениеРаботСФизЛицом.ДатаНачала <= КОНЕЦПЕРИОДА( &Период, МЕСЯЦ )
		//|			И ДоговорНаВыполнениеРаботСФизЛицом.ДатаОкончания >= НАЧАЛОПЕРИОДА( &Период, МЕСЯЦ )
		//(начало) закомментировано Кухарчуком Д.Я. 29.08.18, задача СВФР-002367
		//(начало) добавлено Кухарчуком Д.Я. 29.08.18, задача СВФР-002367
		|			ДоговорНаВыполнениеРаботСФизЛицом.ДатаНачала <= КОНЕЦПЕРИОДА( &ВерхняяГраницаПериодаДоговорников, МЕСЯЦ )
		|			И ДоговорНаВыполнениеРаботСФизЛицом.ДатаОкончания >= НАЧАЛОПЕРИОДА( &НижняяГраницаПериодаДоговорников, МЕСЯЦ )
		//(начало) добавлено Кухарчуком Д.Я. 29.08.18, задача СВФР-002367
		|			И ДоговорНаВыполнениеРаботСФизЛицом.Проведен
		|			И ДоговорНаВыполнениеРаботСФизЛицом.Организация = &Организация
		|           И ДоговорНаВыполнениеРаботСФизЛицом.Физлицо В (ВЫБРАТЬ Выплаты.Физлицо ИЗ ВтВыплаты КАК Выплаты)
		|		СГРУППИРОВАТЬ ПО
		|			ДоговорНаВыполнениеРаботСФизЛицом.ФизЛицо) КАК Подрядчики  // добавлено Кухарчуком Д.Я. 16.08.18, задача СВФР-002337
		|		ПО Выплаты.Физлицо = Подрядчики.Физлицо
		//(конец) добавлено Кухарчуком Д.Я. 16.08.18, задача СВФР-002337
		|СГРУППИРОВАТЬ ПО
		|	Выплаты.Период,
		|	Выплаты.Организация,
	...
	//(начало) добавлено Кухарчуком Д.Я. 29.08.18, задача СВФР-002367
	НижняяГраницаПериодаДоговорников = Неопределено;
	ВерхняяГраницаПериодаДоговорников = Неопределено;
	Для каждого СтрТЧ Из ПеречислениеЗаработнойПлаты Цикл
		Если НЕ ЗначениеЗаполнено( СтрТЧ.Ведомость ) ИЛИ ТипЗнч( СтрТЧ.Ведомость ) <> Тип( "ДокументСсылка.ЗарплатаКВыплатеОрганизаций" ) Тогда
			Продолжить;	
		КонецЕсли;
		Если НижняяГраницаПериодаДоговорников = Неопределено Тогда
			НижняяГраницаПериодаДоговорников = НачалоМесяца( СтрТЧ.Ведомость.ПериодРегистрации );
		Иначе	
			НижняяГраницаПериодаДоговорников = Мин( НачалоМесяца( СтрТЧ.Ведомость.ПериодРегистрации ), НижняяГраницаПериодаДоговорников );
		КонецЕсли; 
		Если ВерхняяГраницаПериодаДоговорников = Неопределено Тогда
			ВерхняяГраницаПериодаДоговорников = КонецМесяца( СтрТЧ.Ведомость.ПериодРегистрации );
		Иначе	
			ВерхняяГраницаПериодаДоговорников = Макс( КонецМесяца( СтрТЧ.Ведомость.ПериодРегистрации ), ВерхняяГраницаПериодаДоговорников );
		КонецЕсли; 
	КонецЦикла;
	НижняяГраницаПериодаДоговорников = ?( НижняяГраницаПериодаДоговорников = Неопределено, НачалоМесяца( Дата ), НижняяГраницаПериодаДоговорников );
	ВерхняяГраницаПериодаДоговорников = ?( ВерхняяГраницаПериодаДоговорников = Неопределено, КонецМесяца( Дата ), ВерхняяГраницаПериодаДоговорников );
	Запрос.УстановитьПараметр("НижняяГраницаПериодаДоговорников",	НижняяГраницаПериодаДоговорников);
	Запрос.УстановитьПараметр("ВерхняяГраницаПериодаДоговорников",	ВерхняяГраницаПериодаДоговорников);
	//(конец) добавлено Кухарчуком Д.Я. 29.08.18, задача СВФР-002367
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, Отказ, Заголовок)
	...
	Пока Выборка.Следующий() Цикл
		Если ?(Выборка.ХарактерВыплаты = Перечисления.ХарактерВыплатыЗарплаты.Дивиденды,не Выборка.Акционер,Истина) Тогда //О.Т. Аверсон 31.08.2018 № 2398
			Если  Выборка.ХарактерВыплаты = Перечисления.ХарактерВыплатыЗарплаты.Командировочные и не Выборка.Организация.ИНН = "100765738" Тогда
			Иначе
		...
		Пока Выборка.СледующийПоЗначениюПоля("СтавкаНалогообложенияРезидента") Цикл
			...
			Пока Выборка.СледующийПоЗначениюПоля("МесяцНалоговогоПериода") И ОсталосьРаспределить > 0 Цикл
				...
				Иначе
					...
					Результат = ОбщегоНазначения.РаспределитьПропорционально(ПогашаемаяСумма,ВременнаяТаблица.ВыгрузитьКолонку("СальдоПоОКАТО"),,Дата);//О.Т. Аверсон 15.03.2016 № 949
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПартионныеДенежниеПереводы") Тогда
		ЗаполнитьДокументПоПартионныеДенежниеПереводы(Основание);
	КонецЕсли;
	// +++ПСА 11-09-2020 СВФР-006182		
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//заявка №2003 (база заявок Кристалла) Дмитрук О.Г.
	СуммаПоДокументу = ПеречислениеЗаработнойПлаты.Итог("СуммаКВыплате");
	//заявка №2003
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	// ПСА 11-09-2020 СВФР-006182 +++
	ЭтоПартионныйДенежныйПеревод = Ложь;
	Для Каждого СтрокаВедомость Из ПеречислениеЗаработнойПлаты Цикл
		Если ТипЗнч(СтрокаВедомость.Ведомость) = Тип("ДокументСсылка.ПартионныеДенежниеПереводы") Тогда
			ЭтоПартионныйДенежныйПеревод = Истина;
		КонецЕсли;	
	КонецЦикла;	
	
	Если ЭтоПартионныйДенежныйПеревод Тогда
		ДвижениеПоСоответствиеПлатежнногоПорученияСПартионнымДенежнымПереводом();
	Иначе	
...
//толик
Процедура СформироватьФайлЭлектронногоВидаПлатежки() Экспорт
	
	ТекстВопроса = "Для формирования файла необходимо записать документ. Продолжить?";
	Если НЕ РаботаСДиалогами.ЗаписатьДокументПередВыполнениемДействия(ЭтотОбъект, , ТекстВопроса) Тогда
		Возврат;
	КонецЕсли; 
	...
	ТекстДокумент.ВставитьСтроку(мНомерСтроки, "12" + Строка(Номер));
	мНомерСтроки = мНомерСтроки + 1;
	
	ТекстДокумент.ВставитьСтроку(мНомерСтроки, "41" + ?(ВидПлатежа="Обычный", "0", "1"));
	мНомерСтроки = мНомерСтроки + 1;
	
	ТекстДокумент.ВставитьСтроку(мНомерСтроки, "51" + Строка(Валюта.Наименование));
	мНомерСтроки = мНомерСтроки + 1;
	
	ТекстДокумент.ВставитьСтроку(мНомерСтроки, "52" + Строка(ПеречислениеЗаработнойПлаты.Итог("СуммаКВыплате")));
	мНомерСтроки = мНомерСтроки + 1;
	
	ТекстДокумент.ВставитьСтроку(мНомерСтроки, "61" + Строка(СчетОрганизации.НомерСчета));
	мНомерСтроки = мНомерСтроки + 1;
	
	мКаталог = ПрочиеПроцедуры.ОпределитьПутьКФайламБанка(Организация);
	ТекстДокумент.Записать(мКаталог+"PP"+Прав(СокрЛП(ЭтотОбъект.Номер), 6)+".txt", КодировкаТекста.OEM);
	
КонецПроцедуры
//енд

// ПСА 11-09-2020 СВФР-006182 +++
Процедура ДвижениеПоСоответствиеПлатежнногоПорученияСПартионнымДенежнымПереводом()
	Движения.СоответствиеПлатежнногоПорученияСПартионнымДенежнымПереводом.Очистить();
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|ГДЕ
		|	ПлатежноеПоручениеИсходящееПеречислениеЗаработнойПлаты.Ведомость ССЫЛКА Документ.ПартионныеДенежниеПереводы
		|	И ПлатежноеПоручениеИсходящееПеречислениеЗаработнойПлаты.Ссылка = &Ссылка";
	...
	Пока Выборка.Следующий() Цикл
		Движение = Движения.СоответствиеПлатежнногоПорученияСПартионнымДенежнымПереводом.Добавить();
		Движение.ПлатежноеПоручениеИсходящее = Ссылка;
		Движение.ПартионныйДенежныйПеревод   = Выборка.Ведомость;
	КонецЦикла;
