...
Процедура ПечатьСМ(ТаблицаВидовРасчета, ВидОрганизационнойСтруктурыПредприятия) Экспорт
	...
	Для Каждого СтрокаВидаРасчета Из ТаблицаВидовРасчета Цикл
		...
		Если ВидОрганизационнойСтруктурыПредприятия = Перечисления.ВидыОрганизационнойСтруктурыПредприятия.ПоЦентрамОтветственности Тогда
			Область.Параметры.Описание = ВидРасчета.Комментарий;
		КонецЕсли;
		ТекущийРаздел.Вывести(Область);
		...
		ИначеЕсли  СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПособиеНаРебенкаДо3Лет Тогда
			...
			// Формирование строки значения и вида показателя.
			ВыводПоказателя(Макет, "Пособие по уходу на ребенком до 3х лет", СтрокаЗначения, ТекущийРаздел);
			
		ИначеЕсли  СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПособиеМатериОдиночкеДо1_5лет Тогда
			
			// Формирование строки значения и вида показателя.
			ВыводПоказателя(Макет, "Пособие матери одиночке до 1.5 лет", СтрокаЗначения, ТекущийРаздел);
			
        ИначеЕсли  СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПособиеМатериОдиночкеДо3Лет Тогда			
			// Формирование строки значения и вида показателя.
			ВыводПоказателя(Макет, "Пособие матери одиночке до 3х лет", СтрокаЗначения, ТекущийРаздел);
			
        ИначеЕсли  СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПособиеНаРебенкаВИЧ Тогда
			
			// Формирование строки значения и вида показателя.
			ВыводПоказателя(Макет, "Пособие на ребенка ВИЧ", СтрокаЗначения, ТекущийРаздел);

		ИначеЕсли  СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПособиеНаРебенкаДо18ЛетНадб40 Тогда
			
			// Формирование строки значения и вида показателя.
			ВыводПоказателя(Макет, "Пособие на ребенка до 18 лет с надбавкой 40%", СтрокаЗначения, ТекущийРаздел);
			
		ИначеЕсли  СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПособиеНаРебенкаИнвалида Тогда
			
			// Формирование строки значения и вида показателя.
			ВыводПоказателя(Макет, "Пособие на ребенка инвалида", СтрокаЗначения, ТекущийРаздел);
			
		ИначеЕсли  СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПособиеНаРебенкаСНадбавкой40До3Лет Тогда
			
			// Формирование строки значения и вида показателя.
			ВыводПоказателя(Макет, "Пособие на ребенка до 3 лет с надбавкой 40%", СтрокаЗначения, ТекущийРаздел);

		ИначеЕсли  СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПособиеНаРебенкаСтарше3Лет Тогда
			
			// Формирование строки значения и вида показателя.
			ВыводПоказателя(Макет, "Пособие на ребенка старще 3х лет", СтрокаЗначения, ТекущийРаздел);
			
		ИначеЕсли  СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПособиеНеРабМатериНадб40До3Лет Тогда
			
			// Формирование строки значения и вида показателя.
			ВыводПоказателя(Макет, "Пособие не работающей матери на ребенка до 3х лет с надбавкой 40%", СтрокаЗначения, ТекущийРаздел);
			
		ИначеЕсли  СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПособиеНеРабМатериОдиночкеДо1_5Лет Тогда
			
			// Формирование строки значения и вида показателя.
			ВыводПоказателя(Макет, "Пособие не работающей матери одиночке на ребенка до 1.5 лет ", СтрокаЗначения, ТекущийРаздел);

        ИначеЕсли  СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПособиеНеРабМатериОдиночкеДо3Лет Тогда
			
			// Формирование строки значения и вида показателя.
			ВыводПоказателя(Макет, "Пособие не работающей матери одиночке на ребенка до 3х лет ", СтрокаЗначения, ТекущийРаздел);
			
		ИначеЕсли  СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ПособиеНеРаботающейМатериДо3Лет Тогда
			
			// Формирование строки значения и вида показателя.
			ВыводПоказателя(Макет, "Пособие не работающей матери на ребенка до 3х лет ", СтрокаЗначения, ТекущийРаздел);
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ИсполнительныйЛистПроцентом 
			...
			Или СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.УдержаниеПоИсполнительномуЛистуБезПНИПФ //О.Т. Аверсон 24.05.2017 № 665
			Или СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.УдержаниеПоИсполнительномуЛистуБезПНИПФДоПредела Тогда
		...
		ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ДоплатаЗаНочныеЧасы
			или СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ДоплатаЗаНочныеЧасыТариф
			или СпособРасчета = Перечисления.СпособыРасчетаОплатыТруда.ДоплатаЗаНочныеЧасыТарифИнстр Тогда  //О.Т. Аверсон 03.04.2016 № 981
	...
	РаботаСДиалогами.НапечататьДокумент(ПечатныйДокумент,,, "Данные по схеме мотивации.");
