...
//
Функция СформироватьЗапрос(Организации = Неопределено, ДатаНачала, ДатаОкончания, МаксимальноеКоличествоСобытий = 0,Сотрудник = Неопределено) Экспорт
	...
	Если не Сотрудник = Неопределено Тогда 
		СписокСотрудниковТекст = СписокСотрудниковТекст +"
	|		И РаботникиОсновноеМесто.Сотрудник = &Сотрудник";
	КонецЕсли;
	СписокСотрудниковТекст = СписокСотрудниковТекст +"
	|
	|СГРУППИРОВАТЬ ПО
	...
	Если не Сотрудник = Неопределено Тогда 
		СписокСотрудниковТекст = СписокСотрудниковТекст +"
	|		И РаботникиСовместительство.Сотрудник = &Сотрудник";
	КонецЕсли;
	СписокСотрудниковТекст = СписокСотрудниковТекст +"
	...
	Если не Сотрудник = Неопределено Тогда 
		СписокСотрудниковТекст = СписокСотрудниковТекст +"
		|		ГДЕ СотрудникиДУ.Ссылка = &Сотрудник";
	КонецЕсли;
	СписокСотрудниковТекст = СписокСотрудниковТекст +"
	...
	|	НамеченныеИзменения.Регистратор
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|		НамеченныеСобытияПоПерсоналу.Регистратор
	|	ИЗ
	...
	|	ГДЕ
	|		НамеченныеСобытияПоПерсоналу.ПланируемоеСобытие В (ЗНАЧЕНИЕ(Перечисление.НамеченныеСобытияПоПерсоналу.Командировка), ЗНАЧЕНИЕ(Перечисление.НамеченныеСобытияПоПерсоналу.ОтпускБезСохраненияЗарплатыСемейноБытовыеПричины), ЗНАЧЕНИЕ(Перечисление.НамеченныеСобытияПоПерсоналу.ОтпускБезСохраненияЗарплатыУчебныеСборы), ЗНАЧЕНИЕ(Перечисление.НамеченныеСобытияПоПерсоналу.ОтпускЕжегодный), ЗНАЧЕНИЕ(Перечисление.НамеченныеСобытияПоПерсоналу.ОтпускЕжегодныйЧасть), ЗНАЧЕНИЕ(Перечисление.НамеченныеСобытияПоПерсоналу.ОтпускПоУходуЗаРебенком), ЗНАЧЕНИЕ(Перечисление.НамеченныеСобытияПоПерсоналу.ОтпускУчебный), ЗНАЧЕНИЕ(Перечисление.НамеченныеСобытияПоПерсоналу.ОтпускУчебныйНеоплачиваемый))
	|		И НамеченныеСобытияПоПерсоналу.ФизЛицо В (ВЫБРАТЬ РАЗЛИЧНЫЕ Физлицо ИЗ ВТСписокФизическихЛиц)";
	Если ЗначениеЗаполнено(ДатаНачала) Тогда
	Запрос.Текст = Запрос.Текст + "
	|		И НамеченныеСобытияПоПерсоналу.ДатаИзменения >= &ДатаНачала";
	...
	|		НамеченныеСобытияПоПерсоналу.Регистратор) КАК НамеченныеИзменения
	...
	|					И НамеченныеИзменения.ПланируемоеСобытие <> ЗНАЧЕНИЕ(Перечисление.НамеченныеСобытияПоПерсоналу.ОтпускЕжегодныйЧасть)
	|				ТОГДА ИСТИНА
	...
	|					И НамеченныеИзменения.ПланируемоеСобытие <> ЗНАЧЕНИЕ(Перечисление.НамеченныеСобытияПоПерсоналу.ОтпускУчебныйНеоплачиваемый)
	|				ТОГДА ИСТИНА
	...
	|					И НамеченныеИзменения.ПланируемоеСобытие <> ЗНАЧЕНИЕ(Перечисление.НамеченныеСобытияПоПерсоналу.ОтпускБезСохраненияЗарплатыСемейноБытовыеПричины)
	|				ТОГДА ИСТИНА
	...
	|					И НамеченныеИзменения.ПланируемоеСобытие <> ЗНАЧЕНИЕ(Перечисление.НамеченныеСобытияПоПерсоналу.ОтпускБезСохраненияЗарплатыУчебныеСборы)
	|				ТОГДА ИСТИНА
	...
	|	ГДЕ
	|		НамеченныеСобытияПоПерсоналу.ПланируемоеСобытие = ЗНАЧЕНИЕ(Перечисление.НамеченныеСобытияПоПерсоналу.Перемещение)
	|		И не НамеченныеСобытияПоПерсоналу.НеНапоминатьОСобытии
	...
	|					И НамеченныеИзменения.ПланируемоеСобытие <> ЗНАЧЕНИЕ(Перечисление.НамеченныеСобытияПоПерсоналу.ОтпускЕжегодный)
	|				ТОГДА ИСТИНА
	|			КОГДА СостояниеРаботниковОрганизации.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.ОтпускЕжегодныйЧасть)
	|					И НамеченныеИзменения.ПланируемоеСобытие <> ЗНАЧЕНИЕ(Перечисление.НамеченныеСобытияПоПерсоналу.ОтпускЕжегодныйЧасть)
	|				ТОГДА ИСТИНА
	|			КОГДА СостояниеРаботниковОрганизации.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.ОтпускУчебный)
	|					И НамеченныеИзменения.ПланируемоеСобытие <> ЗНАЧЕНИЕ(Перечисление.НамеченныеСобытияПоПерсоналу.ОтпускУчебный)
	|				ТОГДА ИСТИНА
	|			КОГДА СостояниеРаботниковОрганизации.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.ОтпускУчебныйНеоплачиваемый)
	|					И НамеченныеИзменения.ПланируемоеСобытие <> ЗНАЧЕНИЕ(Перечисление.НамеченныеСобытияПоПерсоналу.ОтпускУчебныйНеоплачиваемый)
	|				ТОГДА ИСТИНА
	...
	|					И НамеченныеИзменения.ПланируемоеСобытие <> ЗНАЧЕНИЕ(Перечисление.НамеченныеСобытияПоПерсоналу.ОтпускПоУходуЗаРебенком)
	|				ТОГДА ИСТИНА
	|			КОГДА СостояниеРаботниковОрганизации.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.ОтпускБезСохраненияЗарплатыСемейноБытовыеПричины)
	|					И НамеченныеИзменения.ПланируемоеСобытие <> ЗНАЧЕНИЕ(Перечисление.НамеченныеСобытияПоПерсоналу.ОтпускБезСохраненияЗарплатыСемейноБытовыеПричины)
	|				ТОГДА ИСТИНА
	|			КОГДА СостояниеРаботниковОрганизации.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияРаботникаОрганизации.ОтпускБезСохраненияЗарплатыУчебныеСборы)
	|					И НамеченныеИзменения.ПланируемоеСобытие <> ЗНАЧЕНИЕ(Перечисление.НамеченныеСобытияПоПерсоналу.ОтпускБезСохраненияЗарплатыУчебныеСборы)
	|				ТОГДА ИСТИНА
