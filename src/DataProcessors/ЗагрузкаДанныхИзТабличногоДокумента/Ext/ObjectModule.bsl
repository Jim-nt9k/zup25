...
//
Функция ПоучитьНастройкуПоУмолчанию(СпискокСохраненныхНастроек) Экспорт
	
	МетаданныеИсточника = ПолучитьМетаданныеИсточника();
	Если МетаданныеИсточника = Неопределено или Не ТипЗнч(СпискокСохраненныхНастроек) = Тип("ТаблицаЗначений") Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Для каждого СтрокаСписка Из СпискокСохраненныхНастроек Цикл
