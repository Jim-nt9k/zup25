...
Функция ПолучитьСписокНеРазобранныхПисем(ПолеДляУпорядочивания) Экспорт
	...
	|	И ЭлектронноеПисьмо.УчетнаяЗапись.НазначениеУчетнойЗаписи = ЗНАЧЕНИЕ(Перечисление.НазначенияУчетнойЗаписи.ПерепискаСКандидатами)
	|
	|УПОРЯДОЧИТЬ ПО
...
Функция ПолучитьСписокСобытийПоЗаявке(ЗаявкаСоискателя) Экспорт
	...
	|	И Событие.Проведен
	|	И Событие.ВидОбъекта = ЗНАЧЕНИЕ(Перечисление.ВидыОбъектовСобытия.ЗаявкаКандидата)
