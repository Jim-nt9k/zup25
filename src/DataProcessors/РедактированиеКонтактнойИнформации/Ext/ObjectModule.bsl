...
Функция Записать() Экспорт
	...
	Если ТипЗнч(мФормаВладелец) = Тип("Форма") И Справочники.ТипВсеСсылки().СодержитТип(ТипЗнч(мФормаВладелец.ЭтотОбъект)) Тогда
		
		Если Не мФормаВладелец.ЭтотОбъект.Заблокирован() Тогда
			Попытка
				мФормаВладелец.ЭтотОбъект.Заблокировать();
			Исключение
				РаботаСДиалогами.СообщитьОбОшибке(ОписаниеОшибки(),, "Не удалось записать данные о контактной информации.");
				Возврат Ложь;
			КонецПопытки;
		КонецЕсли; 
