		...
		Для каждого ЭлементСписка Из СписокПолей Цикл
			...
			Иначе
				...
				Если СтрЧислоВхождений(ВРег(ЭлементСписка.Значение), ВРег("телефон")) > 0
					...
					ИЛИ СтрЧислоВхождений(ВРег(ЭлементСписка.Значение), ВРег("пейджер")) > 0
					ИЛИ (СтрЧислоВхождений(ВРег(ЭлементСписка.Значение), ВРег("Адрес")) > 0 И СтрЧислоВхождений(ВРег(ЭлементСписка.Значение), ВРег("эл")) > 0 И СтрЧислоВхождений(ВРег(ЭлементСписка.Значение), ВРег("почты")) > 0) Тогда
...
Функция ЗаполнитьТаблицуДанныхФайла()
	...
	ТаблицаДанныхФайла.Индексы.Добавить("НомерСтрокиФайла");
...
Функция ПреобразоватьВСтрокуДаты(ЗначениеФайла) Экспорт
	...
	Иначе	//Если НаименованиеПрограммы = "MSOutlook" Тогда
...
Процедура ЗаписатьВыгружаемуюИнформацию() Экспорт
	...
	мФормаПрогрессора.Значение = 0;
	мФормаПрогрессора.ЭлементыФормы.Индикатор.МаксимальноеЗначение = СписокПолей.Количество();
	Для каждого ЭлементСписка Из СписокПолей Цикл
		...
		Для каждого Поле Из СписокПолей Цикл
			...
			Пока КонечнаяВыборка.Следующий() Цикл
				Если КонечнаяВыборка[Поле.Представление] <> Null Тогда
					...
					Если Поле.Представление = "ДатаРождения" Тогда
						ЗначениеОбъекта = СокрЛП(Строка(Формат(КонечнаяВыборка[Поле.Представление], "ДФ=dd.MM.yyyy")));
					Иначе
...
// 
Процедура ЗаписатьЗагружаемуюИнформацию(КомментироватьПроцессЗагрузки = Истина, РодительДляЗагрузки = Неопределено) Экспорт
		...
		Для каждого СтрокаПолей Из НастройкаПолей Цикл
			...
			Если СтрокаПолей.ТипРеквизита1С = "Строка" Тогда
				...
				Если СтрЧислоВхождений(СтрокаПолей.ИндПоля, ";") = 0 Тогда
					...
					Если НЕ ЗначениеЗаполнено(ЗначениеИзФайла) Тогда
						Продолжить;
					КонецЕсли; 
					...
					Иначе
						...
						Если ПреобразованноеЗначениеФайла = Неопределено Тогда
							Сообщить("Установлено несоответствие типов полей. 1С - " + СокрЛП(СтрокаПолей.НазначениеРеквизита1С) + ", файл - " + СокрЛП(СтрокаПолей.ИмяРеквизита) + ", " + СтрокаТаблицы.ПредставлениеОбъекта1С);
							Продолжить;
						КонецЕсли;
						
						ИмяРеквизита = ПолучитьИмяРеквизита(СтрокаПолей.НазначениеРеквизита1С);
						
						Если ТипЗнч(Объект1С[ИмяРеквизита]) = Тип("Строка") И НЕ ПустаяСтрока(Объект1С[ИмяРеквизита]) Тогда
			...
			ИначеЕсли СтрокаПолей.ТипРеквизита1С = "ВидКИ" Тогда
				...
				Если СтрЧислоВхождений(СтрокаПолей.ИндПоля, ";") = 0 Тогда
					...
					Если ТипКонтактнойИнформации = Перечисления.ТипыКонтактнойИнформации.Телефон Тогда
						...
						СтруктураПолейТелефона = УправлениеКонтактнойИнформацией.РазложитьТелефонПоПолям(СтрокаФайла[СтрокаПолей.ИндПоля]);

						ЗаписьРегистраКИ.Поле3 = СтруктураПолейТелефона.СамТелефон;
						ЗаписьРегистраКИ.Поле1 = СтруктураПолейТелефона.КодСтраны;
						ЗаписьРегистраКИ.Поле2 = СтруктураПолейТелефона.КодГорода;
						
						УправлениеКонтактнойИнформацией.СформироватьПредставлениеТелефона(ЗаписьРегистраКИ);
						
					КонецЕсли; 

					Попытка
						ЗаписьРегистраКИ.Записать(Истина);
						Если КомментироватьПроцессЗагрузки Тогда
	...
	Если ОтменитьТранзакцию Тогда
		ОтменитьТранзакцию();
		Предупреждение("Загрузка отменена!");
	Иначе
		ЗафиксироватьТранзакцию();
		Предупреждение("Загрузка завершена успешно!");
	КонецЕсли; 
