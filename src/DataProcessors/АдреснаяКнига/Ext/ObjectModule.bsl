...
Процедура ДобавитьЭлементы(ЭлементыФормы, СписокДанныхТекущейСтроки, ЭлементФормы, ЭлементУправленияАдрес, СтандартнаяОбработка = Неопределено, ИдентификаторСправочника = Неопределено, Множественный = Ложь) Экспорт
	...
	Если НЕ Множественный Тогда
		...
		Если ВыбранныеСтроки.Количество() = 1 И ЗначениеЗаполнено(ВыбранныеСтроки[0]) Тогда
			...
			Запрос.Текст = "
			|ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	" + ?((Метаданные.Справочники[ИдентификаторСправочника].Иерархический И Метаданные.Справочники[ИдентификаторСправочника].ВидИерархии = Метаданные.СвойстваОбъектов.ВидИерархии.ИерархияГруппИЭлементов), "ТаблицаСправочника.ЭтоГруппа", "ЛОЖЬ") + " КАК ЭтоГруппа
			|ИЗ
			|	Справочник." + ИдентификаторСправочника + " КАК ТаблицаСправочника
			|ГДЕ
			|	ТаблицаСправочника.Ссылка = &СсылкаСправочника
			|";
			
			Запрос.УстановитьПараметр("СсылкаСправочника", ВыбранныеСтроки[0]);
	...
	Иначе
		...
		Если Метаданные.Справочники[ИдентификаторСправочника].Иерархический И Метаданные.Справочники[ИдентификаторСправочника].ВидИерархии = Метаданные.СвойстваОбъектов.ВидИерархии.ИерархияГруппИЭлементов Тогда
...
Процедура ПроизвестиПоиск(ТекстДляПоиска) Экспорт
	...
	Если (СтрЧислоВхождений(ТекстДляПоиска, "<") = 1 ИЛИ СтрЧислоВхождений(ТекстДляПоиска, "[") = 1)
	   И (СтрЧислоВхождений(ТекстДляПоиска, ">") = 1 ИЛИ СтрЧислоВхождений(ТекстДляПоиска, "]") = 1)
	   И СтрЧислоВхождений(ТекстДляПоиска, "@") = 1 Тогда
		НачалоАдреса  = Макс(Найти(ТекстДляПоиска, "<"), Найти(ТекстДляПоиска, "["));
		КонецАдреса   = Макс(Найти(ТекстДляПоиска, ">"), Найти(ТекстДляПоиска, "]"));
