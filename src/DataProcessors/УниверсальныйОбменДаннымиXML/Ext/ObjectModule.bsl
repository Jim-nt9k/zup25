...
Функция НайтиОбъектПоГлобальномуНомеру(Нпп)

	Возврат ЗагруженныеГлобальныеОбъекты[Нпп];
...
Функция СоздатьНовыйОбъект(Тип, СвойстваПоиска, Объект = Неопределено, 
	...
	НоваяСсылка = Неопределено, Нпп = 0, ГНпп = 0)
	...
	Если ЗаписыватьОбъектСразуПослеСоздания Тогда
		...
		Иначе
			...
			ДополнитьСтекНеЗаписанныхОбъектов(Нпп, ГНпп, Объект, НоваяСсылка, Тип);
...
Функция НайтиИлиСоздатьОбъектПоСвойству(СтруктураСвойств, ТипОбъекта, СвойстваПоиска, СвойстваПоискаНеЗамещать,
	...
	НоваяСсылкаУникальногоИдентификатора = Неопределено, Нпп = 0, ГНпп = 0)
	...
	Если Не ОбъектНайденИлиСоздан 
		...
		Объект = СоздатьНовыйОбъект(ТипОбъекта, СвойстваПоиска, НайденныйИлиСозданныйОбъект, 
			НЕ РежимПоискаОсновногоОбъекта,,НоваяСсылкаУникальногоИдентификатора, Нпп, ГНпп);
...
Процедура ДобавитьСсылкуВСписокЗагруженныхОбъектов(ГНппСсылки, НппСсылки, СсылкаНаОбъект)
	...
	Если ГНппСсылки <> 0 Тогда
		...
		ЗагруженныеГлобальныеОбъекты[ГНппСсылки] = СсылкаНаОбъект;
...
Функция ОбработатьПоискОбъектаПоСтруктуре(НомерОбъекта, ТипОбъекта, СозданныйОбъект, 
	...
	Иначе
		...
		Иначе
			СсылкаНаОбъект = НайтиОбъектПоНомеру(НомерОбъекта);
		КонецЕсли;
...
Процедура ПрочитатьИнформациюОСвойствахПоиска(ТипОбъекта, СвойстваПоиска, СвойстваПоискаНеЗамещать)
	...
	ПрочитатьСвойстваПоискаИзФайла(СвойстваПоиска, СвойстваПоискаНеЗамещать, ИнформацияОТипах);	
...
Функция НайтиОбъектПоСсылке(ТипОбъекта, 
	...
	Если СвойствоИмяПредопределенного <> Неопределено Тогда
		...
		СсылкаНаОбъект = НайтиИлиСоздатьОбъектПоСвойству(СтруктураСвойств, ТипОбъекта, СвойстваПоиска, СвойстваПоискаНеЗамещать,
			...
			НЕ НеСоздаватьОбъектЕслиНеНайден, СозданныйОбъект, РежимПоискаОсновногоОбъекта, СвойстваОбъектаМодифицированы, , НппСсылки, НППГлобальнойСсылки);
	...
	ИначеЕсли (СвойствоУникальныйИдентификатор <> Неопределено) Тогда
		...
		СсылкаНаОбъект = НайтиИлиСоздатьОбъектПоСвойству(СтруктураСвойств, ТипОбъекта, СвойстваПоиска, СвойстваПоискаНеЗамещать,
			...
			НЕ НеСоздаватьОбъектЕслиНеНайден И НЕ НужноПродолжитьПоискЕслиЭлементПоGUIDНеНайден, СозданныйОбъект, 
			РежимПоискаОсновногоОбъекта, СвойстваОбъектаМодифицированы, ИзвестнаяСсылкаУникальногоИдентификатора, НппСсылки, НППГлобальнойСсылки);
			
		Если Не НужноПродолжитьПоискЕслиЭлементПоGUIDНеНайден Тогда

			Если Не РежимПоискаОсновногоОбъекта Тогда	
				ДобавитьСсылкуВСписокЗагруженныхОбъектов(НППГлобальнойСсылки, НппСсылки, СсылкаНаОбъект);
			Конецесли;
	...
	Пока НомерВариантаПоиска <= 10 Цикл
		...
		Если Не ПустаяСтрока(АлгоритмПоиска) Тогда
			...
			Если ПрекратитьПоиск = Истина 
				...
				Если РежимПоискаОсновногоОбъекта Тогда	
					ОбработатьУстановкуСвойствПоискаУОбъекта(УстанавливатьУОбъектаВсеСвойстваПоиска, СвойстваПоиска, 
		...
		Если СсылкаНаОбъект <> Неопределено
			...
			ИначеЕсли ИмяТипаОбъекта = "Документ" 
	...
	Если НЕ НеСоздаватьОбъектЕслиНеНайден
		И (СсылкаНаОбъект = Неопределено
			ИЛИ СсылкаНаОбъект.Пустая())
		И СозданныйОбъект = Неопределено Тогда 
		
		СсылкаНаОбъект = СоздатьНовыйОбъект(ТипОбъекта, СвойстваПоиска, СозданныйОбъект, НЕ РежимПоискаОсновногоОбъекта, , , НппСсылки, НППГлобальнойСсылки);
		ОбъектНайден = Ложь;
		СвойстваОбъектаМодифицированы = Истина;
...
//
Процедура УстановитьСвойстваЗаписи(Запись, ИнформацияОТипах, ПараметрыОбъекта, ИмяВетки, НомерЗаписи)
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если ИмяУзла = "Свойство"
			...
			Если ЭтоПараметр Тогда
				ДобавитьСложныйПараметрПриНеобходимости(ПараметрыОбъекта, ИмяВетки, НомерЗаписи, Имя, Значение);
			Иначе
				Запись[Имя] = Значение;
			КонецЕсли;
...
// 
Процедура ЗагрузитьТабличнуюЧасть(Объект, Имя, Очистить, ОбщаяИнформацияОтипеДокумента, НужноЗаписатьОбъект, ПараметрыОбъекта)
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если ИмяУзла = "Запись" Тогда
			...
			Если Запись = Неопределено Тогда
				одПропустить(ФайлОбмена);
			Иначе
				УстановитьСвойстваЗаписи(Запись, ИнформацияОТИпах, ПараметрыОбъекта, ИмяТабличнойЧасти, НомерЗаписи);
			КонецЕсли;
...
// 
Процедура ЗагрузитьДвижения(Объект, Имя, Очистить, ОбщаяИнформацияОтипеДокумента, НужноЗаписатьОбъект, ПараметрыОбъекта)
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если      ИмяУзла = "Запись" Тогда
			...
			УстановитьСвойстваЗаписи(Запись, ИнформацияОТИпах, ПараметрыОбъекта, ИмяДвижений, НомерЗаписи);
...
Процедура ДополнитьСтекНеЗаписанныхОбъектов(Нпп, ГНпп, Объект, ИзвеснаяСсылка, ТипОбъекта)
	...
	мГлобальныйСтекНеЗаписанныхОбъектов.Вставить(НомерДляСтека, Новый Структура("Объект, ИзвеснаяСсылка, ТипОбъекта", Объект, ИзвеснаяСсылка, ТипОбъекта));
	...
	Пока ФайлОбмена.Прочитать() Цикл
			...
			Если Объект = Неопределено Тогда
				...
				Если Объект = Неопределено Тогда
					...
					Иначе
						...
						СоздатьНовыйОбъект(ТипОбъекта, СвойстваПоиска, Объект, Ложь, НаборЗаписей, , НппСсылки, НППГлобальнойСсылки);
		...
		ИначеЕсли ИмяУзла = "Ссылка" Тогда
			...
			Ссылка = НайтиОбъектПоСсылке(ТипОбъекта, СвойстваПоиска, СвойстваПоискаНеЗамещать, ОбъектНайден, 
				...
				НппСсылки, ИзвестнаяСсылкаУникальногоИдентификатора);
		...
		ИначеЕсли ИмяУзла = "ТабличнаяЧасть"
			...
			Если ИмяУзла = "ТабличнаяЧасть" Тогда
				...
				ЗагрузитьТабличнуюЧасть(Объект, Имя, Не НеОчищать, ИнформацияОТипах, НужноЗаписатьОбъект, ПараметрыОбъекта);
		...
		ИначеЕсли (ИмяУзла = "Объект") И (ФайлОбмена.ТипУзла = одТипУзлаXML_КонецЭлемента) Тогда
			...
			ИначеЕсли ИмяТипаОбъекта <> "Перечисление" Тогда
				...
				Если ИмяТипаОбъекта = "РегистрСведений" Тогда
					...
					Если НЕ ЗаписыватьВИнформационнуюБазуТолькоИзмененныеОбъекты Тогда
						НужноЗаписатьОбъект = Истина;
					Иначе
					...
					Если ЗаписыватьРегистрыНаборамиЗаписей Тогда
						...
						Если ЗаписыватьВИнформационнуюБазуТолькоИзмененныеОбъекты
							И НЕ НужноЗаписатьОбъект Тогда
						...
						Если ЗаписыватьВИнформационнуюБазуТолькоИзмененныеОбъекты
							...
							Если ВременныйНаборЗаписей.Количество() <> 1 Тогда
								НужноЗаписатьОбъект = Истина;
							Иначе
		...
		ИначеЕсли ИмяУзла = "Типы" Тогда
			...
			Если Объект = Неопределено Тогда
				...
				ОбъектНайден = Ложь;
				Ссылка       = СоздатьНовыйОбъект(ТипОбъекта, СвойстваПоиска, Объект, , , , НппСсылки, НППГлобальнойСсылки);
...
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
	...
	Если СтандартнаяОбработка Тогда
        ...
        ВыгрузитьСвойства(Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО, ПКО.Свойства, , ВыборкаДляВыгрузкиДанных, , 
			ПКО.НеВыгружатьОбъектыСвойствПоСсылкам ИЛИ ТолькоПолучитьУзелСсылки);
...
Функция ПолучитьВыборкуДляВыгрузкиСОграничениями(Правило, ВыборкаДляПодстановкиВПКО = Неопределено)
	...
	ПостроительОтчета.Текст = "ВЫБРАТЬ " + СтрокаРазрешения + " " + ПоляВыборки + " ИЗ " + ИмяМетаданных + " КАК Объект "+ "{ГДЕ Объект.Ссылка.* КАК " + СтрЗаменить(ИмяМетаданных, ".", "_") + "}";
...
Процедура ВыгрузитьДанныеПоПравилу(Правило)
	...
	Если Правило.СпособОтбораДанных = "СтандартнаяВыборка" И Правило.ИспользоватьОтбор Тогда
		...
		Выборка = ПолучитьВыборкуДляВыгрузкиСОграничениями(Правило, ВыборкаДляПКО);
		
		Пока Выборка.Следующий() Цикл
			
			ВыгрузкаОбъектаВыборки(Выборка.Ссылка, Правило, , ИсходящиеДанные, ВыборкаДляПКО);
...
Процедура ЗаполнитьСписокТиповДоспутныхДляУдаления(ТаблицаДанных) Экспорт
...
Процедура ЗагрузитьИнформациюОТипахДанных()
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если ИмяУзла = "ТипДанных" Тогда
			...
			мСоответвиеТиповДанныхДляЗагрузки.Вставить(Тип(ИмяТипа), СоотвествиеТипа);
...
Процедура ЗагрузитьПравилаОбмена(Источник="", ТипИсточника="XMLФайл") Экспорт
	...
	Если Конвертация.Свойство("ПослеЗагрузкиПравилОбмена", ТекстСобытияПослеЗагрузкиПравилОбмена) Тогда
		
		Если Не ПустаяСтрока(ТекстСобытияПослеЗагрузкиПравилОбмена) Тогда
...
Процедура ОткрытьДанныеПротоколовОбменаПриНеобходимости()
	...
	Если Не ПустаяСтрока(ИмяФайлаПротоколаОбмена) Тогда
		ЗапуститьПриложение(ИмяФайлаПротоколаОбмена);
	КонецЕсли;
	...
	Если НепосредственноеЧтениеВИБПриемнике Тогда
		...
		Если Не ПустаяСтрока(ИмяПротоколаЗагрузки) Тогда
			ЗапуститьПриложение(ИмяПротоколаЗагрузки);
		КонецЕсли;
...
Процедура ИнициализацияТаблицыПравилКонвертации()
	...
	Колонки.Добавить("ВыгруженныеТолькоСсылки",				одОписаниеТипа("Соответствие"));
...
Процедура ИнициализацияМенеджеров()
	...
	Для каждого ОбъектМД Из Метаданные.БизнесПроцессы Цикл
		...
		ТипСсылки        = Тип(ТипСсылкиСтрокой);
		Структура = Новый Структура("Имя,ИмяТипа,ТипСсылкиСтрокой,Менеджер,ОбъектМД,ПКО,ПустаяСсылка,ВозможенПоискПоПредопределенным", Имя, 
			ИмяТипа, ТипСсылкиСтрокой, Менеджер, ОбъектМД, , Неопределено, Ложь);		
		Менеджеры.Вставить(ТипСсылки, Структура);
